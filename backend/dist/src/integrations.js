...
const ref = (prefix, input) => `${prefix}_${createHash("sha256").update(input).digest("hex").slice(0, 16)}`;
const configured = (key) => Boolean(process.env[key]);
function modeFor(required) { return required.every(configured) ? "production" : "sandbox"; }
...
export class IntegrationGateway {
    ...
    health() {
        const checked = now();
        const row = (key, provider, mode) => ({ key, provider, mode, status: mode === "production" ? "ready" : "configuration_required", lastCheckedAt: checked });
        return [
    ...
    async sendOtp(phone, purpose) { return { provider: "otp-sandbox", challengeRef: ref("otp", `${phone}:${purpose}:${Date.now()}`), accepted: true }; }
    async deliver(channel, event) {
        if (this.options.failChannels?.has(channel))
            return { channel, delivered: false, errorCode: "SANDBOX_PROVIDER_UNAVAILABLE" };
        return { channel, delivered: true, providerMessageId: ref(channel, `${event.id}:${event.attempts}`) };
    }
    async createPaymentOrder(amount, receipt) { return { provider: "razorpay-sandbox", orderId: ref("order", `${receipt}:${amount}`), amount, currency: "INR", status: "created" }; }
    async createPayout(amount, providerId) { return { provider: "razorpayx-sandbox", payoutId: ref("payout", `${providerId}:${amount}:${Date.now()}`), amount, currency: "INR", status: "queued" }; }
    async quoteRoute(origin, destination) { const seed = parseInt(createHash("sha256").update(`${origin}:${destination}`).digest("hex").slice(0, 4), 16); const distanceKm = Number((3 + (seed % 220) / 10).toFixed(1)); return { provider: "maps-sandbox", routeId: ref("route", `${origin}:${destination}`), distanceKm, durationMinutes: Math.round(distanceKm * 3.2 + 18), trackingReady: true }; }
}
export function buildIntegrationGateway(options) { return new IntegrationGateway(options); }
export async function processNotificationOutbox(repository, gateway, limit = 25) {
    ...
    for (const event of due) {
        if (event.attempts >= 5) {
            results.push({ notificationId: event.id, status: "dead_letter", deliveredChannels: [] });
            continue;
        }
        const prior = Array.isArray(event.payload.deliveredChannels) ? event.payload.deliveredChannels.filter((x) => typeof x === "string" && event.channels.includes(x)) : [];
