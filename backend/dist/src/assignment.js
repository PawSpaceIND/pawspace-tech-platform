export async function decideAssignment(repository, input) {
    const candidates = await repository.listEligibleProviders(input.cityId, input.zoneId, input.serviceCode);
    if (!candidates.length)
        return { provider: null, mode: null, explanation: ["No live provider matches the city, zone and required skill"], alternatives: [] };
    ...
    if (!selected)
        return { provider: null, mode: null, explanation: ["No provider passed assignment scoring"], alternatives: [] };
