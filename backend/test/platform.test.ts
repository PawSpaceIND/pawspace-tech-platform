...
test("notification outbox records partial delivery",async()=>{const repository=new MemoryRepository();const event=await enqueueNotification(repository,{eventType:"booking.confirmed",customerId:"cus_10428",bookingId:"book_1",channels:["push","whatsapp"],templateCode:"booking_confirmation",payload:{}});assert.equal(event.status,"pending");const processed=await processNotification(repository,event.id,["push"]);assert.equal(processed?.status,"partially_sent");assert.equal(processed?.attempts,1);});
