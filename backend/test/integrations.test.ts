...
test("unconfigured communications fail closed instead of reporting delivery",async()=>{const repository=new MemoryRepository();const event=await enqueueNotification(repository,{eventType:"booking.confirmed",customerId:"cus_10428",channels:["whatsapp","sms","email"],templateCode:"booking_confirmation",payload:{}});const result=await processNotificationOutbox(repository,buildIntegrationGateway());assert.equal(result.evaluated,1);assert.deepEqual(result.results[0]?.deliveredChannels,[]);assert.deepEqual(result.results[0]?.failures.map(x=>x.errorCode),["PROVIDER_NOT_CONFIGURED","PROVIDER_NOT_CONFIGURED","PROVIDER_NOT_CONFIGURED"]);const saved=(await repository.listNotifications()).find(x=>x.id===event.id);assert.equal(saved?.status,"failed");assert.equal(saved?.attempts,1);});
...
test("voice delivery is disabled by default",async()=>{delete process.env.VOICE_TELEPHONY_ENABLED;delete process.env.VOICE_PROVIDER_API_KEY;const event={id:"notify_voice",eventType:"test",channels:["voice"] as const,templateCode:"test",payload:{},status:"pending" as const,attempts:0,nextAttemptAt:new Date().toISOString(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};const result=await buildIntegrationGateway().deliver("voice",event);assert.equal(result.delivered,false);assert.equal(result.errorCode,"VOICE_DISABLED");});
