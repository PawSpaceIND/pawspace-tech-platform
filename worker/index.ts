...
import {runAutonomousBookingExceptionSweep} from "../lib/autonomous-booking-engine";
...
const worker = {
  ...
  async scheduled(controller:ScheduledControllerLike,env:Env,ctx:ExecutionContext){
    ctx.waitUntil((async()=>{
      ...
      const [cleanup,scheduler,outboxDispatch,voiceRecovery,whatsappRecovery,whatsappOutbox,razorpayOrderOutbox,settlementRecon,subscriptionMaintenance,marketingConnector,eliteRuntime,diamondCrm,autonomousBookingExceptions]=await Promise.allSettled([
        ...
        runAutonomousBookingExceptionSweep(env.DB,{asOf:controller.scheduledTime,actorId:"system:scheduled-worker"}),
      ]);
      const errors:string[]=[];
      ...
      if(autonomousBookingExceptions.status==="rejected")errors.push(`autonomous booking exception sweep: ${autonomousBookingExceptions.reason instanceof Error?autonomousBookingExceptions.reason.message:String(autonomousBookingExceptions.reason)}`);
