...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if(url.pathname==="/healthz")return Response.json(
      {status:"ok",service:"pawspace-tech-platform"},
  ...
  async scheduled(controller:ScheduledControllerLike,env:Env,ctx:ExecutionContext){
    ctx.waitUntil((async()=>{const [cleanup,scheduler,whatsappRecovery,settlementRecon,subscriptionMaintenance]=await Promise.allSettled([cleanupExpiredReservationLeases(env.DB,controller.scheduledTime),runBackgroundScheduler(env.DB,{actorId:"system:scheduled-worker",asOf:controller.scheduledTime,cron:controller.cron}),processDueWhatsAppNoResponseSequences(env.DB,{now:controller.scheduledTime,actorEmail:"system:scheduled-worker"}),runRazorpaySettlementReconciliationSweep(env.DB,env as unknown as Record<string,unknown>,{asOf:controller.scheduledTime}),runSubscriptionScheduledMaintenance(env.DB,env as unknown as Record<string,unknown>,{asOf:controller.scheduledTime,billingSweep:(db,input)=>runSubscriptionBillingSweep(db,input)})]);const errors:string[]=[];if(cleanup.status==="rejected")errors.push("reservation_cleanup");if(scheduler.status==="rejected")errors.push("background_scheduler");else if(Array.isArray(scheduler.value.errors)&&scheduler.value.errors.length)errors.push("background_scheduler_job");if(whatsappRecovery.status==="rejected")errors.push("whatsapp_recovery");if(settlementRecon.status==="rejected")errors.push("razorpay_settlement_reconciliation");if(subscriptionMaintenance.status==="rejected")errors.push("subscription_maintenance");else if(Number(subscriptionMaintenance.value.errors||0)>0)errors.push("subscription_maintenance_job");if(errors.length)throw new Error(`Background scheduler partial failure [${errors.join(",")}]`);})());
  },
};
