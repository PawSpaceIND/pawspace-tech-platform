...
const worker = {
  ...
  async scheduled(controller:ScheduledControllerLike,env:Env,ctx:ExecutionContext){
    ctx.waitUntil((async()=>{
      const [cleanup,scheduler,outboxDispatch,voiceRecovery,whatsappRecovery,settlementRecon,subscriptionMaintenance]=await Promise.allSettled([
      ...
      if(outboxDispatch.status==="rejected")errors.push(`communication outbox dispatcher: ${outboxDispatch.reason instanceof Error?outboxDispatch.reason.message:String(outboxDispatch.reason)}`);
