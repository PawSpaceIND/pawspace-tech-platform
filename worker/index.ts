...
interface Env {ASSETS: Fetcher;DB: D1Database;FOUNDER_EMAIL?: string;AI?: unknown;IMAGES:{input(stream:ReadableStream):{transform(options:Record<string,unknown>):{output(options:{format:string;quality:number}):Promise<{response():Response}>}}};[key:string]:unknown;}
...
const worker={
 async fetch(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{
  ...
  if(url.pathname.startsWith("/api/")){
   ...
   const isMetaWebhook=url.pathname==="/api/whatsapp/meta-webhook",isEmailWebhook=url.pathname==="/api/email-provider-webhook",isProviderWebhook=isMetaWebhook||isEmailWebhook,eliteRequest=isMetaWebhook?request.clone():null;
   ...
   const inspectionRequest=request.clone(),sessionAccess=await authorizePlatformSessionRequest(inspectionRequest,env.DB);
   if(sessionAccess instanceof Response)return sessionAccess;
   const access=isProviderWebhook?{actor:{email:isMetaWebhook?"meta-webhook@provider":"email-webhook@provider",roleCode:"provider_webhook",permissions:[],preview:false},permission:null}:sessionAccess??await authorizeApiRequest(inspectionRequest,env);
   ...
   ctx.waitUntil(auditApiResponse(env,access.actor,access.permission,inspectionRequest,response.clone()));return secureApiResponse(response);
  }
 ...
 async scheduled(controller:ScheduledControllerLike,env:Env,ctx:ExecutionContext){
  ctx.waitUntil((async()=>{
   const templateSyncTask=syncSubmittedMetaTemplateStatuses(env.DB,env as unknown as Record<string,unknown>,{actorId:"system:scheduled-worker",limit:50}).then(result=>{if(result.failed&&result.processed)throw new Error(`whatsapp template sync: ${result.failed} verification exception(s)`);return result;});
   ...
   const marketingTask=marketingHour>=6?runMarketingConnectorScheduler(env.DB,{asOf:controller.scheduledTime,runtime:env as unknown as Record<string,unknown>}).then(result=>{const failedSync=Array.isArray(result.sync)?result.sync.filter(item=>String((item as Record<string,unknown>).status)==="failed"):[];const offline=result.offlineConversions as Record<string,unknown>;if(failedSync.length||String(offline?.status||"")==="failed")throw new Error(`provider sync/upload failure: ${JSON.stringify({failedSync,offline})}`);return result;}):Promise.resolve({status:"not_due_before_06_ist"});
   const[templateSync,cleanup,scheduler,outboxDispatch,voiceRecovery,whatsappRecovery,whatsappOutbox,razorpayOrderOutbox,settlementRecon,subscriptionMaintenance,marketingConnector,eliteRuntime,diamondCrm,voiceCarrierUat]=await Promise.allSettled([
    templateSyncTask,
   ...
   const errors:string[]=[];
   if(templateSync.status==="rejected")errors.push(`whatsapp template sync: ${templateSync.reason instanceof Error?templateSync.reason.message:String(templateSync.reason)}`);
