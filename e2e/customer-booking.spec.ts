...
test("grooming truth test mounts Razorpay iframe from payment_pending",async({page})=>{
 ...
 await page.getByLabel("Customer Name",{exact:true}).fill("Razorpay Browser Customer");await page.getByLabel("Customer Phone Number",{exact:true}).fill("9000000915");await expect(page.getByRole("button",{name:"Confirm booking",exact:true})).toBeEnabled();await page.getByLabel("Alternative Phone Number",{exact:true}).fill("9000000916");await page.getByLabel("Special instructions to groomer",{exact:true}).fill("Please ring the bell once.");await page.getByRole("button",{name:/^Pay online/}).click();
