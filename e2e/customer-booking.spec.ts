...
test("customer: sandbox sign-in -> grooming checkout -> persisted booking", async ({ page }) => {
  ...
  await confirm.click();await expect(page.getByRole("status")).toContainText("Case reference: UI-REVIEW-CASE");await expect(page.getByRole("status")).toContainText("no refund is promised");await expect(page.getByRole("region",{name:"Booking details",exact:true})).toContainText("assigned");
  ...
  await expect(page.getByRole("region",{name:"Booking details",exact:true})).toContainText("cancelled");await expect(page.getByRole("status")).toContainText("Booking cancelled");await expect(cancelForm).toHaveCount(0);
