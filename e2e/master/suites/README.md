...
| `60-transactions-audit` | Runs last, read-only: for every booking the run saved, staging D1 shows a complete, consistent money trail (booking payment, split / taxi-fee schedule, Razorpay capture events counted once, webhooks processed, reconciliation, balanced collection ledger, refunds reversed, timeline, invoices); staging-wide 7-day invariants (captures without evidence, duplicate/missing collections, open over-collection exceptions, orphan refunds, unreversed refunds, stuck capture effects, unbalanced journals, invoices for unpaid bookings, stuck webhooks) — rows from before the live deploy are PARTIAL; as the founder every /team/finance page loads, the Boarding/Sitting/Taxi workspaces and the Booking Command Center Payments tab show this run's bookings as D1 holds them (the Finance role is refused by MFA, as expected). |
