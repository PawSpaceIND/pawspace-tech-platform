...
const ORIGIN = "https://pawspace-checkout-674-34519217683-1.karthik-fce.workers.dev";
const WORKER = "pawspace-checkout-674-34519217683-1";
const CANDIDATE = "59885ffed9018d7663f3e1b718776fe5ef806117";
      ...
      try {
        ...
        if (!netbankingSelected) {
          const methodChoices = [
            ...
            frame.getByRole("radio", { name: /Netbanking/i }).first(),
          ];
          for (const netbanking of methodChoices) {
            if (!await netbanking.isVisible().catch(() => false)) continue;
            await netbanking.click({ timeout: 8_000 });
