...
const ORIGIN = "https://pawspace-checkout-674-34517081246-1.karthik-fce.workers.dev";
const WORKER = "pawspace-checkout-674-34517081246-1";
const CANDIDATE = "92c760c151b01c283f8961e9ae51d838c66fb7e4";
...
async function submitNetbankingSuccess(page: Page) {
  ...
  while (Date.now() < deadline) {
    ...
    for (const frame of frames) {
      try {
        ...
        if (!netbankingSelected) {
          const netbanking = frame.getByRole("radio", { name: /Netbanking/i }).first();
          if (await netbanking.isVisible().catch(() => false)) {
            await netbanking.click(); netbankingSelected = true; await page.waitForTimeout(700); continue;
          }
        }
        if (netbankingSelected && !bankSelected) {
          ...
          const bankChoices = [
            frame.getByRole("img", { name: /^SBIN$/i }).first(),
            frame.getByRole("img", { name: /State Bank/i }).first(),
            frame.getByText(/State Bank of India/i).first(),
            frame.getByText(/^SBI$/i).first(),
            frame.getByText(/^SBIN$/i).first(),
            frame.getByRole("button", { name: /State Bank|SBI|SBIN/i }).first(),
          ];
    ...
    for (const providerPage of page.context().pages()) {
      for (const surface of [providerPage, ...providerPage.frames()]) {
        try {
          const successControls = [
            surface.getByRole("button", { name: /^Success$/i }).first(),
            surface.getByText(/^Success$/i).first(),
            surface.locator('input[type="submit"][value="Success" i],input[type="button"][value="Success" i]').first(),
          ];
          for (const success of successControls) {
