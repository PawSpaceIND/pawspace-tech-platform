...
test("confirmed bookings disable generic reassignment and explain service recovery", async ({ page }) => {
  ...
  await expect(page.getByText(/Booking BOOKING-001 · a booked slot is never moved by reassignment/)).toBeVisible();
  // Visible, not a tooltip: mobile-chromium runs this same journey and cannot hover.
  await expect(page.getByText(/the booking, slot and payment are preserved/)).toBeVisible();
});
for (const outcome of ["assigned","awaiting_acceptance","ops_escalation","conflict","notifications"]) test(`employee recovery handles ${outcome} without a false confirmation`, async ({page})=>{
 ...
 await page.goto("/team/scheduling");const submit=page.getByRole("button",{name:"Find replacement",exact:true});if(outcome==="notifications")await page.getByRole("button",{name:"Retry notifications",exact:true}).click();else{await page.getByRole("button",{name:"Recover groomer",exact:true}).click();await expect(submit).toBeDisabled();await page.getByLabel("Recovery reason",{exact:true}).fill("Original groomer reported illness");if(outcome==="assigned")await page.screenshot({path:test.info().outputPath("employee-recovery-form.png"),fullPage:true});await submit.click();}
