...
for (const outcome of ["assigned","awaiting_acceptance","ops_escalation","conflict","notifications"]) test(`employee recovery handles ${outcome} without a false confirmation`, async ({page})=>{
 ...
 if(outcome==="notifications")await expect(page.getByRole("status").filter({hasText:/\S/})).toContainText("Notification retry finished: 1 queued. Delivery is not yet confirmed.");
 if(outcome==="assigned")await expect(page.getByRole("status").filter({hasText:/\S/})).toContainText("reassigned to Replacement groomer");
 if(outcome==="awaiting_acceptance")await expect(page.getByRole("status").filter({hasText:/\S/})).toContainText("partner acceptance is still pending");
 if(outcome==="ops_escalation")await expect(page.getByRole("status").filter({hasText:/\S/})).toContainText("Operations follow-up is required (case CASE-REAL)");
...
for(const outcome of ["full_time","commission","cancel","conflict","incomplete"])test(`staff manages a waiting request with an honest ${outcome} outcome`,async({page})=>{
 ...
 if(outcome==="conflict"||outcome==="incomplete"){
  await expect(form.getByRole("textbox",{name:"Reason",exact:true})).toHaveValue("Reviewed the customer request");await expect(submit).toBeDisabled();await expect(page.getByRole("alert")).toBeVisible();await expect(page.getByRole("status").filter({hasText:/\S/})).toHaveCount(0);await page.getByRole("button",{name:"Refresh schedule",exact:true}).click();await expect(page.getByRole("button",{name:"Manage request",exact:true})).toBeVisible();
 }else await expect(page.getByRole("status").filter({hasText:/\S/})).toContainText(outcome==="cancel"?"Request WAITING-GROUP cancelled.":outcome==="commission"?"Partner acceptance and customer booking confirmation are still pending.":"Customer booking confirmation is still pending.");
});
