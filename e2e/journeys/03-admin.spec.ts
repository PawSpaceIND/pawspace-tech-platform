...
for (const outcome of ["assigned","awaiting_acceptance","ops_escalation","conflict","notifications"]) test(`employee recovery handles ${outcome} without a false confirmation`, async ({page})=>{
 ...
 if(outcome==="notifications")await expect(page.getByRole("status").filter({hasText:"Notification retry finished: 1 queued. Delivery is not yet confirmed."})).toBeVisible();
 if(outcome==="assigned")await expect(page.getByRole("status").filter({hasText:"reassigned to Replacement groomer"})).toBeVisible();
 if(outcome==="awaiting_acceptance")await expect(page.getByRole("status").filter({hasText:"partner acceptance is still pending"})).toBeVisible();
