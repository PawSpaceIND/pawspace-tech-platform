...
async function revalidateTarget() {
  const main = await fetch("https://api.github.com/repos/PawSpaceIND/pawspace-tech-platform/branches/main", {
    headers: { authorization: `Bearer ${GH_TOKEN}`, accept: "application/vnd.github+json" },
    redirect: "manual",
    signal: AbortSignal.timeout(20_000),
  });
  expect(main.status).toBe(200);
  const mainBody = await main.json() as Json;
  expect(mainBody.name).toBe("main");
  expect(mainBody.commit?.sha).toBe(CANDIDATE);
...
test("current engineering-closure main Worker proves Razorpay Test capture and provider-origin webhook", async ({ page }, testInfo) => {
  ...
  const verified = await pagePost(page, "/api/customer-otp", {
    ...
    name: "Engineering Closure Provider Proof",
    cityId: "blr",
