...
// A lifecycle action costs 36-70 sequential D1 round trips (gateway included; complete is the largest), 18-27 s
// at staging's measured 0.33-0.38 s per round trip, which the shared 20 s abort would cut off. Actions get 60 s.
// boundedFetch's timeout text promises that replay-safe actions reuse their idempotency key; these do not (every
// tap is a fresh key), so the trainer is told the truth instead: it may have been saved, refresh before retrying.
// Once a status step (accept, on_the_way, arrive, start, complete) is saved, the server's state guard refuses a repeat.
const actionTimeout="The session update took too long to confirm and may still have been saved. Refresh your sessions to see its current state before trying again.";
export async function trainingSessionAction(input:{sessionId:string;action:string;reason?:string;report?:TrainingReport;newStart?:string;newEnd?:string;newProviderId?:string;ownerHandoverMinutes?:number}){const location=input.action==="arrive"?await getDeviceLocation():{};let response:Response;try{response=await boundedFetch("/api/training-sessions",{method:"POST",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({...input,...location,idempotencyKey:`training:${input.sessionId}:${input.action}:${crypto.randomUUID()}`})},60_000);}catch(error){if(error instanceof Error&&error.message.startsWith("Network request timed out"))throw new Error(actionTimeout);throw error;}return body<Record<string,unknown>>(response);}
export type TrainingEvidencePurpose="before_service"|"after_service"|"training_homework";
export type TrainingEvidenceAsset={id:string;ref:string;purpose:TrainingEvidencePurpose;proofReady:boolean;scan_status:string;access_status:string;review_status?:string;retention_status?:string;/** What the upload boundary actually verified about the bytes; null when unknown, never assumed true. */objectStored?:boolean|null};
