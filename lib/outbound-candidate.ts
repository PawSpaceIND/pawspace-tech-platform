...
export async function evaluateOutboundCustomer(db: Db, customer: Row, asOf: number) {
  ...
  const preferences = await first(
    db,
    "SELECT marketing_consent,service_consent,opt_out FROM customer_contact_preferences WHERE customer_id=?",
    [customerId],
    degradation,
    "contact_preferences",
  ) || {};
  const lead = await first(
    db,
    "SELECT * FROM lead_work_items WHERE customer_id=? AND status NOT IN ('closed','merged') AND converted_booking_id IS NULL ORDER BY updated_at DESC LIMIT 1",
    [customerId],
    degradation,
    "open_lead",
  );
  ...
  if (lead) {
    try {
      const scored = await scoreLead(db, text(lead.id));
      leadScore = Number(scored?.totalScore || 0);
    } catch (error) {
      degradation.note("lead_score", error, null);
    }
  }

  const bookings = await all(
    db,
    "SELECT service_code,status,scheduled_end,total_amount FROM canonical_bookings WHERE customer_id=? AND status NOT IN ('cancelled','refunded','draft') ORDER BY scheduled_end DESC",
    [customerId],
    degradation,
    "service_history",
  );
  ...
  const pet = await first(
    db,
    "SELECT p.id,p.species,p.breed,b.date_of_birth FROM canonical_pets p LEFT JOIN pet_birthdays b ON b.pet_id=p.id WHERE p.customer_id=? ORDER BY p.created_at LIMIT 1",
    [customerId],
    degradation,
    "pet_profile",
  );
  const subscription = await first(
    db,
    "SELECT * FROM customer_grooming_subscriptions WHERE customer_id=? AND status IN ('active','paused') ORDER BY expires_at LIMIT 1",
    [customerId],
    degradation,
  ...
  const recovery = await first(
    db,
    "SELECT * FROM payment_recovery_entitlements WHERE customer_id=? AND status='active' AND expires_at>? ORDER BY expires_at LIMIT 1",
    [customerId, asOf],
    degradation,
    "payment_recovery",
  );
  const control = await first(
    db,
    "SELECT control_type,scope,expires_at FROM outbound_contact_controls WHERE customer_id=? AND (expires_at IS NULL OR expires_at>?) ORDER BY CASE control_type WHEN 'dnd' THEN 1 WHEN 'wrong_number' THEN 2 ELSE 3 END,created_at DESC LIMIT 1",
    [customerId, asOf],
    degradation,
    "outbound_contact_controls",
  );

  // Routing must fail closed if any source required to choose a lane could not be read. A missing
  // subscription, DND control, booking history or score must never be interpreted as a confident zero.
  if (degradation.degraded()) {
    return { lane: "hold" as const, scored: Boolean(lead), degradedReads: degradation.entries() };
  }

  let next: OutboundNextBestRecommendation | null = null;
  if (pet) {
    const ageMonths = pet.date_of_birth
      ? Math.max(0, Math.floor((asOf - Date.parse(text(pet.date_of_birth))) / (DAY * 30.4375)))
      : null;
    const serviceHistory: ServiceHistoryFact[] = [...historyMap].map(([serviceCode, value]) => ({
      serviceCode,
      completedCount: value.count,
      lastCompletedAt: value.last || null,
    }));
    ...
    next = evaluateOutboundNextBestService({
      pet: {
        ...
        breed: text(pet.breed) || null,
        ageMonths,
      },
      serviceHistory,
      statedIntent: statedIntent ? [statedIntent] : [],
      activeEntitlements: subscription
        ? {
            grooming: Math.max(
              0,
              Number(subscription.total_sessions || 0)
                - Number(subscription.sessions_reserved || 0)
                - Number(subscription.sessions_consumed || 0),
            ),
          }
        : {},
    })[0] || null;
  ...
  const callbackRequested = Boolean(
    lead
      && (/callback|call me/i.test(text(lead.last_outcome))
        || (Number(lead.next_action_at || 0) > asOf
          && Number(lead.next_action_at || 0) < asOf + 7 * DAY)),
  );
  ...
  if (callbackRequested && lead) {
    ...
    const leadAge = Math.floor(
      (asOf - Math.max(Number(lead.updated_at || 0), Number(lead.assigned_at || 0))) / DAY,
    );
    ...
    targetOffer = next.offerCode === "grooming_subscription"
      ? "Grooming subscription"
      : `Next best service · ${next.targetService}`;
  }

  const hardSuppression = Boolean(
    control && ["dnd", "wrong_number"].includes(text(control.control_type)),
  );
  const coolingUntil = control
    && text(control.control_type) === "cooling_period"
    && isMarketingLifecycle(lifecycle)
    ? Number(control.expires_at || 0)
    : null;
  const customerScore = Math.round(
    (Math.min(1, ltv / 20_000) * 0.55 + Math.max(0, 1 - recency / 180) * 0.45) * 100,
  );
  const decision = decideOutboundRoute({
    ...
    serviceConsent: preferences.service_consent == null
      ? true
      : Boolean(Number(preferences.service_consent)),
  ...
  if (decision.lane === "hold") return { lane: "hold", scored: Boolean(lead), degradedReads: [] };
  ...
  await db.prepare("INSERT INTO outbound_routing_queue (id,source_key,customer_id,lead_id,source_type,lane,priority_score,high_intent,lifecycle_code,target_offer,next_best_service,expected_revenue,ltv,callback_at,status,context_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(source_key) DO UPDATE SET lane=excluded.lane,priority_score=MAX(outbound_routing_queue.priority_score,excluded.priority_score),target_offer=excluded.target_offer,next_best_service=excluded.next_best_service,ltv=excluded.ltv,callback_at=COALESCE(excluded.callback_at,outbound_routing_queue.callback_at),context_json=excluded.context_json,updated_at=excluded.updated_at")
    .bind(
      ...
      sourceKey,
      customerId,
      lead ? text(lead.id) : null,
      lifecycle,
      decision.lane,
      decision.priorityScore,
      decision.highIntent ? 1 : 0,
      lifecycle,
      targetOffer,
      next?.targetService || null,
      next?.expectedRevenue ?? null,
      ltv,
      callbackAt,
      decision.lane === "suppressed" ? "suppressed" : "queued",
      JSON.stringify({
        leadScore,
        decisionReasons: decision.reasons,
        nextBestReason: next?.explanation || null,
        journeyCode: next?.journeyCode || null,
        journeyStep: next?.journeyStep || null,
      }),
      asOf,
      asOf,
    )
    .run();
  return { lane: decision.lane, scored: Boolean(lead), degradedReads: [] };
}
