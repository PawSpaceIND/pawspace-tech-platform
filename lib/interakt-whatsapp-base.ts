...
export async function dispatchInteraktWhatsApp(db: Db, env: Env, input: { messageId: string; recipient?: string | null; timeoutMs?: number; fetcher?: Fetcher }) {
  ...
  if (!await whatsappConsent(db, owner.customerId) || !await centralConsentAllows(db, owner.customerId, "whatsapp")) return { status: "consent_refused", provider: "interakt", externalDelivery: false, productionDelivery: false };
  ...
  if (!await whatsappConsent(db, owner.customerId) || !await centralConsentAllows(db, owner.customerId, "whatsapp")) {
    await db.batch([
  ...
  const frequency = await reserveCommunicationFrequency(db,{customerId:owner.customerId,channel:"whatsapp",messageId:input.messageId});
  if(!frequency.allowed){await db.prepare("UPDATE communication_outbox SET status='suppressed',locked_at=NULL,last_error='frequency_cap_whatsapp',updated_at=? WHERE message_id=?").bind(Date.now(),input.messageId).run();return {status:"frequency_capped",provider:"interakt",nextEligibleAt:frequency.releaseAt,externalDelivery:false,productionDelivery:false};}
