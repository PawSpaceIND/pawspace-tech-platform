...
export async function resolveGovernedServiceAddress(db:Db,input:{customerId:string;serviceCode:string;serviceAddress?:string;servicePincode?:string}) : Promise<GovernedServiceAddress>{
  ...
  if(!geo){
    ...
    if(suppliedAddress||fixture){
      ...
      const line1=fixture&&!suppliedAddress?String(row.line1):suppliedAddress;
      await db.prepare("INSERT INTO customer_addresses (id,customer_id,label,line1,line2,area,city,postal_code,is_default,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,?,?) ON CONFLICT(id) DO UPDATE SET line1=excluded.line1,city=excluded.city,postal_code=excluded.postal_code,is_default=1,updated_at=excluded.updated_at WHERE customer_addresses.customer_id=excluded.customer_id").bind(id,input.customerId,"Service address",line1,null,resolved.assignment.area,resolved.assignment.city,validated.pincode,now,now).run();
    }
