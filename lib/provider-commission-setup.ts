...
export async function draftProviderCommercialTerms(db:Db,input:ProviderTermsProposal){
 ...
 for(const line of lines){
  ...
  drafts.push({termId:saved.id,serviceCode:line.serviceCode,engagementModel:model,providerSharePct:share,pawspaceCommissionPercent:model==="direct_employee"?null:line.percent,version:saved.version,preview:commissionPreview({engagement:model==="funeral_exempt"?"funeral_vendor":chosen,pawspaceCommissionPercent:line.percent,gstPolicy})});
 }
...
/** The CHECKER step: a second person activates every draft for the provider, with an approval reference. */
export async function activateProviderCommercialTerms(db:Db,input:{providerId:string;approvalReference:string;actorId:string}){
 await ensureCommercialTermsTables(db);
 ...
 const engagement=engagementOfModel(text(drafts[0].engagement_model))??"commission",recorded=await recordProviderEngagement(db,{providerId,engagement,actorId});
...
export async function providerCommercialTermsView(db:Db,input:{providerId:string}){
 ...
 const first=(rows:Row[],code:string,status?:string)=>rows.find(r=>text(r.service_code)===code&&(!status||text(r.status)===status));
 const codes=[...new Set([...offered,...terms.map(r=>text(r.service_code))])].sort();
 const services=codes.map(code=>{const active=first(terms,code,"active"),draft=first(terms,code,"draft"),standard=first(defaults,code);return{serviceCode:code,active:active?termSummary(active):null,draft:draft?termSummary(draft):null,serviceDefault:standard?termSummary(standard):null};});
 const models=[...terms.filter(t=>text(t.status)==="draft"),...terms.filter(t=>text(t.status)==="active")].map(t=>engagementOfModel(text(t.engagement_model))).filter(Boolean) as ProviderEngagement[];
 const engagement:ProviderEngagement=models[0]??(text(capacity[0]?.provider_model)==="full_time"?"full_time":"commission");
...
export async function commercialTermsOverview(db:Db){
 ...
 for(const row of terms){const id=text(row.provider_id),entry=providers.get(id)??{providerId:id,engagement:null,terms:[]};const summary=termSummary(row);if(!entry.terms.some(t=>t.serviceCode===summary.serviceCode&&t.status===summary.status))entry.terms.push(summary);entry.engagement=entry.engagement??summary.engagement;providers.set(id,entry);}
 const serviceDefaults:ReturnType<typeof termSummary>[]=[];for(const row of defaults)if(!serviceDefaults.some(t=>t.serviceCode===text(row.service_code)))serviceDefaults.push(termSummary(row));/* newest first, so the first per service is the one in force */
