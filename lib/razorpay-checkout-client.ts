...
export async function createCustomerPaymentOrder(bookingId: string) {
  ...
  if (data.environment !== "sandbox") throw new Error("Grooming certification is restricted to Razorpay Test Mode");
...
export async function openGroomingRazorpayTestCheckout(input: {
  bookingId: string;
  customerName?: string;
  ...
  return new Promise<{ outcome: "captured" | "capture_pending" | "cancelled" | "failed"; order: CheckoutOrder; truth?: PaymentTruth; error?: string }>(resolve => {
    ...
    const checkout = new window.Razorpay!({
      ...
      name: "PawSpace",
      description: input.description || "Grooming service",
