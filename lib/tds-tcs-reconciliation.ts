...
// deposit and return records, and (d) the source payout population. It never writes.

import{TDS_RATES,monthlySalaryTds}from"./tds-governance";
import{tcsRateS52For}from"./tcs-governance";
...
export async function reconcileTds(db:Db,input:{period:string}){
 ...
 for(const r of rows){
  ...
  if(section==="192"){expected=monthlySalaryTds(base);expectedRatePct=null;}
...
export async function reconcileTcs(db:Db,input:{period:string}){
 const period=input.period;
 const rows=await safeAll(db,"SELECT supplier_id,booking_id,source_ref,supply_type,net_taxable_value,cgst_tcs,sgst_tcs,igst_tcs,tcs_total,rate_pct FROM tcs_collections WHERE period=?",[period]);
 const findings:ReconciliationFinding[]=[];
 let recordedTcs=0,verifiedRows=0;
 for(const r of rows){
  const sourceAt=await safeFirst(db,"SELECT computed_at FROM provider_payout_computations WHERE booking_id=?",[text(r.source_ref)||text(r.booking_id)]);
  const effectiveAt=sourceAt?num(sourceAt.computed_at):Date.parse(`${period}-01T00:00:00+05:30`);
  const rate=tcsRateS52For(effectiveAt),net=round2(num(r.net_taxable_value)),recorded=round2(num(r.tcs_total)),componentSum=round2(num(r.cgst_tcs)+num(r.sgst_tcs)+num(r.igst_tcs)),ratePct=num(r.rate_pct);
  recordedTcs+=recorded;
  const expected=round2(net*rate.total),expectedRatePct=rate.total*100;
  if(Math.abs(ratePct-expectedRatePct)>TOLERANCE)findings.push({severity:"critical",code:"rate_mismatch",detail:`${text(r.supplier_id)}/${text(r.booking_id)}: recorded rate ${ratePct}% ≠ statutory ${expectedRatePct}%`,deducteeId:text(r.supplier_id),expected:expectedRatePct,recorded:ratePct});
...
export async function reconcilePartnerPayoutTax(db:Db,input:{period:string}){
 ...
 return{period:input.period,tds,tcs,reconciled:tds.summary.reconciled&&tcs.summary.reconciled,productionReady:false,liveFilingEnabled:false};
}
