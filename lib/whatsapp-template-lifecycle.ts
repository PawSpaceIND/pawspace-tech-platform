...
export async function submitWhatsAppTemplate(db: D1Database, input: { templateKey: string; actorEmail: string; reason: string }) {
  ...
  const results = await db.batch([
    ...
    db.prepare("UPDATE whatsapp_uat_templates SET status='submitted',updated_by=?,updated_at=? WHERE template_key=? AND status IN ('draft','rejected') AND EXISTS (SELECT 1 FROM whatsapp_template_lifecycle WHERE template_key=?)").bind(input.actorEmail, now, key, key),
  ]);
  if (Number(results[0]?.meta?.changes || 0) !== 1 || Number(results[1]?.meta?.changes || 0) !== 1) responseError("Template state or lifecycle companion changed before submission; refresh and retry", 409);
...
export async function reconcileWhatsAppTemplate(db: D1Database, input: { templateKey: string; actorEmail: string; outcome: "approved" | "rejected"; metaReference: string; reason: string }) {
  ...
  const results = await db.batch([
    ...
    db.prepare("UPDATE whatsapp_uat_templates SET status=?,updated_by=?,updated_at=? WHERE template_key=? AND status='submitted' AND EXISTS (SELECT 1 FROM whatsapp_template_lifecycle WHERE template_key=?)").bind(input.outcome, input.actorEmail, now, key, key),
  ]);
  if (Number(results[0]?.meta?.changes || 0) !== 1 || Number(results[1]?.meta?.changes || 0) !== 1) responseError("Template state or lifecycle companion changed before reconciliation; refresh and retry", 409);
...
export async function pauseWhatsAppTemplate(db: D1Database, input: { templateKey: string; actorEmail: string; reason: string }) {
  ...
  const results = await db.batch([
    ...
    db.prepare("UPDATE whatsapp_uat_templates SET status='paused',updated_by=?,updated_at=? WHERE template_key=? AND status='approved' AND EXISTS (SELECT 1 FROM whatsapp_template_lifecycle WHERE template_key=?)").bind(input.actorEmail, now, key, key),
  ]);
  if (Number(results[0]?.meta?.changes || 0) !== 1 || Number(results[1]?.meta?.changes || 0) !== 1) responseError("Template state or lifecycle companion changed before pause; refresh and retry", 409);
