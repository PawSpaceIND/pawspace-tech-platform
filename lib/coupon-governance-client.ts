...
async function payload(response:Response){const body=await response.json().catch(()=>({})) as Record<string,unknown>;if(!response.ok)throw new Error(String(body.error||(body.data as {error?:string}|undefined)?.error||"Coupon request failed"));return body.data;}
