...
export type WalkingBookingInput={ownerCare?:WalkingOwnerCare;idempotencyKey:string;groupId:string;walkingQuoteId:string;customer:{id:string;name:string;primaryPhone:string;secondaryPhone?:string;email?:string};pets:Array<{sourceId:string;name:string;species?:"dog"|"cat"|"other";breed?:string;vaccinationStatus?:string}>;cityId:string;zoneId:string;packageCode:string;packageName:string;walkCount:number;weekdays:number[];scheduledStart:string;scheduledEnd:string;provider:{id:string;name:string;model:"full_time"|"commission"};totalAmount:number;amountDueNow:number;payment:{method:string;mode:"pay_after_service";detail:string}};
export type WalkingBookingResult={totalAmount:number;amountDueNow:number;packageName:string;perWalkAmount:number|null;bookingId:string;customerId:string;petIds:string[];scheduleGroupId:string;workOrderId:string;paymentId:string;status:string;sessions:Array<{id:string;occurrenceNumber:number;scheduledStart:string;scheduledEnd:string;status:string}>;duplicatePrevented:boolean;liveMoney:false};
...
export async function reserveWalkingSchedule(input:{clientRequestId:string;customerId:string;petIds:string[];cityId:string;zoneId:string;serviceAddress?:string;servicePincode?:string;scheduledStart:string;scheduledEnd:string;walkCount:number;weekdays?:number[]}):Promise<WalkingScheduleReservation>{
  const response=await fetch("/api/uat-scheduling",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({clientRequestId:input.clientRequestId,customerId:input.customerId,petIds:input.petIds,serviceAddress:input.serviceAddress,servicePincode:input.servicePincode,serviceCode:"dog_walking",cityId:input.cityId,zoneId:input.zoneId,scheduledStart:input.scheduledStart,scheduledEnd:input.scheduledEnd,occurrences:input.walkCount,weekdays:input.weekdays})});
