...
import{verifyCanonicalInteraktWebhook}from"./interakt-webhook-auth";
...
export async function recordInteraktDeliveryWebhookAtomic(db:D1Database,env:Env,input:{rawBody:string;headers:Headers}){
 const verified=await verifyCanonicalInteraktWebhook(input.rawBody,input.headers,env);if(!verified.ok)return{accepted:false,status:verified.status,reason:verified.reason,externalDelivery:false};
