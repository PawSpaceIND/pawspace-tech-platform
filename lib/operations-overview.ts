 ...
 *
 * Everything here is computed from canonical tables. Revenue recognition matches P&L/analytics,
...
export async function buildOperationsOverview(db: Db, input: { asOf?: number; zoneId?: string } = {}) {
  ...
  if (await tableExists(db, "provider_capacity_profiles")) {
    const providerWhere = `effective_from<=? AND (effective_to IS NULL OR effective_to>=?)${zoneId ? " AND zones_json LIKE ?" : ""}`;
    const providerBinds: unknown[] = [day, day, ...(zoneId ? [`%\"${zoneId}\"%`] : [])];
    const row = await db.prepare(
      `SELECT COUNT(*) total, SUM(CASE WHEN status='active' AND live=1 THEN 1 ELSE 0 END) active
       FROM provider_capacity_profiles WHERE ${providerWhere}`)
      .bind(...providerBinds).first<Row>();
  ...
  let zones: string[] = [];
  if (await tableExists(db, "provider_capacity_profiles")) {
    const rows = await db.prepare("SELECT DISTINCT zones_json FROM provider_capacity_profiles WHERE status='active' AND live=1 AND effective_from<=? AND (effective_to IS NULL OR effective_to>=?)").bind(day,day).all<Row>();
  ...
  const providerRows = await tableExists(db, "provider_capacity_profiles")
    ? (await db.prepare(
        `SELECT id,name,city_id,zones_json,status,live FROM provider_capacity_profiles
         WHERE status='active' AND live=1 AND effective_from<=? AND (effective_to IS NULL OR effective_to>=?)${zoneId ? " AND zones_json LIKE ?" : ""} ORDER BY name LIMIT ${CAPACITY_BOARD_LIMIT}`)
        .bind(day,day,...(zoneId ? [`%\"${zoneId}\"%`] : [])).all<Row>()).results
    : [];
  ...
  const capacity = providerRows.map(provider => {
    ...
    const slots = OVERVIEW_SLOTS.map(slot => {
      ...
      return {
        ...
        bookingId: text(first.id),
        label: text(first.package_name) || text(first.service_code),
        concurrentBookings: slotBookings.length,
      };
    });
  ...
  return {
    ...
    sourceStatus: {
      ...
      providers: providersTotal === null ? "not_connected" : "provider_capacity_profiles_effective_today",
