...
export const phase2ToolSchemas:Record<Phase2ToolCode,AtlasToolDefinition<Phase2ToolCode>>={
 "ops.voice.dispatch":schema("ops.voice.dispatch",["ops"],"high","within_envelope",["communications.call","customers.manage"],{bookingId:{type:"string"},useCase:{type:"string"}},["bookingId","useCase"],"allowlisted_https"),
 ...
 "ops.provisioning.execute":schema("ops.provisioning.execute",["ops"],"high","within_envelope",["providers.manage"],{providerId:{type:"string"},cityId:{type:"string"},vehicleClass:{type:"string"},label:{type:"string"},registrationNumber:{type:"string"}},["providerId","cityId","vehicleClass","label","registrationNumber"]),
 ...
 "vet.prescription.digitize":schema("vet.prescription.digitize",["healthcare"],"medium","within_envelope",["bookings.manage"],{appointmentId:{type:"string"},providerId:{type:"string"},sourceType:{type:"string"},clinicalNotes:{type:"string"},sourceMediaRef:{type:"string"}},["appointmentId","providerId","sourceType","clinicalNotes"]),
 "finance.vet_payout.calculate":schema("finance.vet_payout.calculate",["finance","healthcare"],"high","within_envelope",["finance.manage"],{appointmentId:{type:"string"},providerId:{type:"string"}},["appointmentId","providerId"]),
};
