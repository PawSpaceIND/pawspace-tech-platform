...
export async function createGroundedAiRuntimeProvider(db:D1Database,actor:AuthenticatedActor,channel:AiToolChannel,options:{dispatchItemId?:string|null}={}):Promise<AiResponseProvider>{
 const connection=await aiProviderConnection(channel),providerTimeoutMs=channel==="chat"?Math.min(connection.timeoutMs,CHAT_PROVIDER_TIMEOUT_MS):connection.timeoutMs,deadlineMs=channel==="chat"?Math.min(connection.timeoutMs,CHAT_ORCHESTRATOR_DEADLINE_MS):connection.timeoutMs;
 return{status:connection.connected?"connected":"not_connected",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,deadlineMs,async generate(input:AiProviderInput){if(requiresImmediateHumanHandoff(input.inputText))return{text:"",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,latencyMs:0,unsupported:true,highImpactAction:true};const grounded=await buildGroundedAiTurnContext(db,{actor,threadId:input.threadId,customerId:input.customerId,intent:input.intent.intent as AiToolIntent,channel,query:input.inputText,canonicalContext:input.context});const systemPrompt=await buildRuntimeSystemPrompt(db,{customerId:input.customerId,channel,dispatchItemId:options.dispatchItemId});const result=await requestAiDraft({systemPrompt,userPrompt:JSON.stringify({channel,customerMessage:input.inputText,intent:input.intent,canonicalContext:grounded.context}),channel,intent:input.intent.intent,maxTokens:channel==="voice"?450:1200,timeoutMs:providerTimeoutMs});if(!result.connected)return{text:"",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,latencyMs:0,failure:result.failure};const envelope=parseGroundedActionEnvelope(result.text);return{text:envelope?.reply||result.text,provider:result.providerRef,modelRef:result.modelRef,latencyMs:result.latencyMs,referencedCustomerIds:[input.customerId],groundingRefs:grounded.groundingRefs,highImpactAction:false,actionRequests:envelope?.actions};}};
}
