...
export async function createGroundedAiRuntimeProvider(db:D1Database,actor:AuthenticatedActor,channel:AiToolChannel,options:{dispatchItemId?:string|null;fastVoice?:boolean;onTiming?:(stage:string)=>void}={}):Promise<AiResponseProvider>{const connection=await aiProviderConnection(channel);return{status:connection.connected?"connected":"not_connected",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,deadlineMs:connection.timeoutMs,async generate(input:AiProviderInput){if(requiresImmediateHumanHandoff(input.inputText))return{text:"",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,latencyMs:0,unsupported:true,highImpactAction:true};options.onTiming?.("groundingStarted");const grounded=await buildGroundedAiTurnContext(db,{actor,threadId:input.threadId,customerId:input.customerId,intent:input.intent.intent as AiToolIntent,channel,query:input.inputText,canonicalContext:input.context,fastVoice:options.fastVoice});options.onTiming?.("groundingCompleted");const systemPrompt=options.fastVoice&&channel==="voice"?VOICE_FAST_PROMPT:await buildRuntimeSystemPrompt(db,{customerId:input.customerId,channel,dispatchItemId:options.dispatchItemId});const result=await requestAiDraft({onTiming:options.onTiming,systemPrompt,userPrompt:JSON.stringify({channel,customerMessage:input.inputText,intent:input.intent,canonicalContext:grounded.context}),channel,intent:input.intent.intent,maxTokens:options.fastVoice&&channel==="voice"?(isExplicitCustomerActionConfirmation(input.inputText)?600:160):channel==="voice"?450:1200,...(input.onDelta?{onDelta:input.onDelta}:{})});if(!result.connected)return{text:"",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,latencyMs:0,failure:result.failure};const envelope=parseGroundedActionEnvelope(result.text);return{text:envelope?.reply||result.text,provider:result.providerRef,modelRef:result.modelRef,latencyMs:result.latencyMs,referencedCustomerIds:[input.customerId],groundingRefs:grounded.groundingRefs,highImpactAction:false,actionRequests:envelope?.actions};}};}
