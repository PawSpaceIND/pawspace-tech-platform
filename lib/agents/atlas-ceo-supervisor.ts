...
import { buildAtlasBusinessSnapshot, recordAtlasProposal } from "../intelligence/atlas-business-snapshot";
...
async function propose(db:Db,input:{instruction:AtlasManagerInstruction;basisId:string;asOf:number}) {
  const snapshot=await buildAtlasBusinessSnapshot(db,{asOf:input.asOf});
  return recordAtlasProposal(db,{
    proposalType:`ceo_${input.instruction.action}`,
...
export async function runAtlasCeoSupervisor(db: Db, input: { asOf?: number; route?: AtlasWorkerRouter; env?:Record<string,unknown>; capacitySignals?: Array<{ utilization: number; cityId: string; zoneId: string; serviceCode: string }>; exceptionSignals?: HumanEscalationSignal[] }) {
  ...
  if(executiveActive){
    for (const row of targets.results) {
      ...
      if (lagRatio > 0.25) proposals.push(await propose(db,{basisId:text(row.id),asOf,instruction:{ manager: "sales", action: "recover_target_pacing", cityId: text(row.city_id) || null, serviceCode: text(row.service_code) || null, reason: "target_pacing_lag_above_25_percent", payload: { targetId: row.id, targetType: row.target_type, goal, actual, expected, lagRatio } }}));
    }
    for (const signal of input.capacitySignals || []) for (const instruction of resolveCapacityConflict(signal)) proposals.push(await propose(db,{instruction,basisId:`capacity:${signal.cityId}:${signal.zoneId}:${signal.serviceCode}`,asOf}));
  }
  const escalations = [];
  ...
  return { evaluatedTargets: targets.results.length, proposals, directives:[], escalations, executiveActive, executiveDefault:false, proposalOnly:true, workerRouterCalled:false, lineLevelExecution:false, canonicalExecutionOnly:true };
}
