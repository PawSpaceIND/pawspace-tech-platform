...
const switchName:Record<VerticalAgentCode,string>={sales:"AI_SALES_ACTIVE",marketing:"AI_MARKETING_ACTIVE",ops:"AI_OPS_ACTIVE",hr:"AI_HR_ACTIVE",finance:"AI_FINANCE_ACTIVE"};
...
export function resolveVerticalRuntime(env:Record<string,unknown>,agent:VerticalAgentCode,requested:Exclude<VerticalRuntimeMode,"disabled">="recommend"){
 ...
 if(agent==="finance"&&requested==="execute_within_envelope"&&!enabled(env.AI_FINANCIAL_MUTATION_ACTIVE))return{agent,mode:"approval_required" as const,humanFallback:true};
...
export const PHASE2_VERTICALS:VerticalAgentCode[]=["sales","marketing","ops","hr","finance"];
