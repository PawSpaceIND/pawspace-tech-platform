...
export async function mutateCurrentGroomingAssignment(db:D1Database,booking:Row,work:Row,offer:Row|null,statements:D1PreparedStatement[]){
  ...
  try{return await db.batch([guard,...statements,db.prepare("DELETE FROM scheduling_dispatch_assertions WHERE id=?").bind(id)]);}
  catch(error){if(!(await check.first<{ok:number}>())?.ok)throw governedJsonError({error:"This assignment changed while recovery was being processed. Refresh the job before retrying.",code:"RECOVERY_STATE_CONFLICT"},409);throw error;}
}
