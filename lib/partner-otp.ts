 ...
 * was unreachable. No real SMS gateway exists yet (same sandboxed state as the customer flow and
 * WhatsApp/Razorpay elsewhere), so the OTP code is returned directly in the sandbox response
 * rather than actually sent - explicitly marked as such, never claimed as sent.
 */
...
export async function verifyPartnerOtp(db:Db,input:{challengeId:string;code:string;name?:string;cityId?:string}){
  ...
  if(text(row.code)!==text(input.code)){
    await db.prepare("UPDATE partner_otp_challenges SET attempts=attempts+1 WHERE id=?").bind(input.challengeId).run();
    throw new Error("Incorrect OTP code");
  }
  await db.prepare("UPDATE partner_otp_challenges SET consumed=1 WHERE id=?").bind(input.challengeId).run();
  ...
  if(!provider){
    const id=uid("PROV"),now=Date.now();
    await db.prepare("INSERT INTO canonical_providers (id,city_id,name,phone,email,source,created_at,updated_at) VALUES (?,?,?,?,NULL,'partner_app_otp',?,?)")
      .bind(id,input.cityId||"blr",text(input.name)||"PawSpace Caregiver",phone,now,now).run();
    provider={id,name:text(input.name)||"PawSpace Caregiver",phone,city_id:input.cityId||"blr"};
  }
