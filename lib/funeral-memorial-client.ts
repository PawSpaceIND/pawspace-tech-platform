...
/* pet_name / pet_species / customer_id / pickup_address / alternate_contact are OPTIONAL because the
 * finance projection (scope=finance) does not carry them - see app/api/funeral-memorial/route.ts
 * FUNERAL_FINANCE_FIELDS. Typing them as required let a finance screen render `undefined` as though it
 * were data. [R3-D/F4] */
export type FuneralCase={id:string;customer_id?:string;pet_name?:string;pet_species?:string;pickup_address?:string;alternate_contact?:string|null;service_type:FuneralServiceType;memorial_option:MemorialOption;urgency:string;status:string;assigned_agent_id?:string|null;vendor_id?:string|null;support_status:string;certificate_status:string;closure_note?:string|null;created_at:number;updated_at:number;media?:Array<Record<string,unknown>>;milestones?:FuneralMilestone[];ashCollection?:Record<string,unknown>|null;payment?:FuneralPayment|null;invoice?:Record<string,unknown>|null;refunds?:FuneralRefund[];supportTickets?:Array<Record<string,unknown>>;memorialRecords?:Array<Record<string,unknown>>;acknowledgement?:Record<string,unknown>|null;vendorCost?:Record<string,unknown>|null;reconciliation?:Record<string,unknown>|null;settlement?:Record<string,unknown>|null};
