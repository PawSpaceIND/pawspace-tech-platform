...
export async function ensureUnifiedCaseTables(db:Db){await db.batch([
 ...
 db.prepare("CREATE TABLE IF NOT EXISTS case_global_sla_defaults (id TEXT PRIMARY KEY CHECK(id='global'),first_response_minutes INTEGER NOT NULL CHECK(first_response_minutes>0),manager_escalation_minutes INTEGER NOT NULL CHECK(manager_escalation_minutes>0),resolution_minutes INTEGER NOT NULL CHECK(resolution_minutes>0),version INTEGER NOT NULL DEFAULT 1,updated_at INTEGER NOT NULL)"),
 db.prepare(`INSERT OR IGNORE INTO case_global_sla_defaults (id,first_response_minutes,manager_escalation_minutes,resolution_minutes,version,updated_at) VALUES ('global',${GLOBAL_DEFAULT_CASE_SLA.firstResponseMinutes},${GLOBAL_DEFAULT_CASE_SLA.managerEscalationMinutes},${GLOBAL_DEFAULT_CASE_SLA.resolutionMinutes},${GLOBAL_DEFAULT_CASE_SLA.version},0)`),
 ...
 await db.batch([
  db.prepare(`UPDATE unified_cases SET first_response_due_at=COALESCE(first_response_due_at,created_at+${GLOBAL_DEFAULT_CASE_SLA.firstResponseMinutes*60_000}),manager_escalation_due_at=COALESCE(manager_escalation_due_at,created_at+${GLOBAL_DEFAULT_CASE_SLA.managerEscalationMinutes*60_000}),resolution_due_at=COALESCE(resolution_due_at,created_at+${GLOBAL_DEFAULT_CASE_SLA.resolutionMinutes*60_000}),policy_id=COALESCE(policy_id,'${GLOBAL_DEFAULT_CASE_SLA.id}'),policy_version=COALESCE(policy_version,${GLOBAL_DEFAULT_CASE_SLA.version}) WHERE first_response_due_at IS NULL OR manager_escalation_due_at IS NULL OR resolution_due_at IS NULL`),
  db.prepare("CREATE TRIGGER IF NOT EXISTS unified_cases_deadlines_not_null_insert BEFORE INSERT ON unified_cases WHEN NEW.first_response_due_at IS NULL OR NEW.manager_escalation_due_at IS NULL OR NEW.resolution_due_at IS NULL BEGIN SELECT RAISE(ABORT,'case_sla_deadline_required'); END"),
  db.prepare("CREATE TRIGGER IF NOT EXISTS unified_cases_deadlines_not_null_update BEFORE UPDATE OF first_response_due_at,manager_escalation_due_at,resolution_due_at ON unified_cases WHEN NEW.first_response_due_at IS NULL OR NEW.manager_escalation_due_at IS NULL OR NEW.resolution_due_at IS NULL BEGIN SELECT RAISE(ABORT,'case_sla_deadline_required'); END"),
 ]);}
...
async function globalDefaultSla(db:Db){const row=await db.prepare("SELECT first_response_minutes,manager_escalation_minutes,resolution_minutes,version FROM case_global_sla_defaults WHERE id='global'").first<Row>().catch(()=>null);const first=Number(row?.first_response_minutes),manager=Number(row?.manager_escalation_minutes),resolution=Number(row?.resolution_minutes),version=Number(row?.version);return{id:GLOBAL_DEFAULT_CASE_SLA.id,version:Number.isInteger(version)&&version>0?version:GLOBAL_DEFAULT_CASE_SLA.version,first_response_minutes:Number.isInteger(first)&&first>0?first:GLOBAL_DEFAULT_CASE_SLA.firstResponseMinutes,manager_escalation_minutes:Number.isInteger(manager)&&manager>0?manager:GLOBAL_DEFAULT_CASE_SLA.managerEscalationMinutes,resolution_minutes:Number.isInteger(resolution)&&resolution>0?resolution:GLOBAL_DEFAULT_CASE_SLA.resolutionMinutes};}
...
export async function createUnifiedCase(db:Db,input:{idempotencyKey:string;caseType:CaseType;severity:CaseSeverity;title:string;description:string;customerId?:string|null;bookingId?:string|null;paymentId?:string|null;leadId?:string|null;providerId?:string|null;sourceType:string;sourceId:string;ownerTeam:string;ownerEmail?:string|null;actorId:string;asOf?:number},relatedStatements:D1PreparedStatement[]=[]){
 await ensureUnifiedCaseTables(db);
 if(!input.idempotencyKey.trim()||!input.title.trim()||!input.sourceType.trim()||!input.sourceId.trim()||!input.ownerTeam.trim())throw new Error("Case idempotency key, title, source and owner team are required");
 const prior=await db.prepare("SELECT * FROM unified_cases WHERE idempotency_key=?").bind(input.idempotencyKey).first<Row>();
 if(prior){if(relatedStatements.length)await db.batch(relatedStatements);return{case:prior,duplicatePrevented:true};}const now=input.asOf??Date.now(),configured=await activePolicy(db,input.caseType,input.severity,now),p=configured??await globalDefaultSla(db);
 const first=deadline(now,Number(p.first_response_minutes)),resolution=deadline(now,Number(p.resolution_minutes)),manager=deadline(now,Number(p.manager_escalation_minutes)),id=uid("CASE");
 // Case, audit event and related domain writes commit or roll back together.
 await db.batch([db.prepare("INSERT INTO unified_cases (id,idempotency_key,case_type,severity,status,title,description,customer_id,booking_id,payment_id,lead_id,provider_id,source_type,source_id,owner_team,owner_email,policy_id,policy_version,first_response_due_at,resolution_due_at,manager_escalation_due_at,created_by,created_at,updated_by,updated_at) VALUES (?,?,?,?,'open',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(id,input.idempotencyKey,input.caseType,input.severity,input.title.trim(),input.description.trim(),input.customerId||null,input.bookingId||null,input.paymentId||null,input.leadId||null,input.providerId||null,input.sourceType.trim(),input.sourceId.trim(),input.ownerTeam.trim(),input.ownerEmail||null,p?.id||null,p?.version||null,first,resolution,manager,input.actorId,now,input.actorId,now),db.prepare("INSERT INTO unified_case_events (id,idempotency_key,case_id,event_type,actor_id,detail_json,created_at) VALUES (?,?,?,?,?,?,?)").bind(uid("CASEE"),`created:${id}`,id,"created",input.actorId,JSON.stringify({sourceType:input.sourceType,sourceId:input.sourceId}),now),...relatedStatements]);
 return{case:await db.prepare("SELECT * FROM unified_cases WHERE id=?").bind(id).first<Row>(),duplicatePrevented:false};}
