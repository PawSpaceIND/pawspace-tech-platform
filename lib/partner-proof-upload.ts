...
export async function readPartnerProofUpload(request:Request,maxBytes:number):Promise<PartnerProofUpload>{const tooLarge=()=>governedJsonError({error:`Proof uploads must be at most ${Math.round(maxBytes/1_000_000)} MB`,code:"media_size_out_of_range"},413);const declared=Number(request.headers.get("content-length")||"0");if(Number.isFinite(declared)&&declared>maxBytes)throw tooLarge();
 // Bounded read: a chunked body without Content-Length is refused as soon as it passes the limit, never buffered whole.
 const chunks:Uint8Array[]=[];let total=0;const reader=request.body?.getReader();if(reader){for(;;){const{done,value}=await reader.read();if(done)break;total+=value.byteLength;if(total>maxBytes){await reader.cancel().catch(()=>{});throw tooLarge();}chunks.push(value);}}
 const bytes=new Uint8Array(new ArrayBuffer(total));let offset=0;for(const chunk of chunks){bytes.set(chunk,offset);offset+=chunk.byteLength;}
 if(!bytes.byteLength)throw governedJsonError({error:"The upload body is empty",code:"media_upload_empty"},400);return{contentType:String(request.headers.get("content-type")||""),bytes};}
