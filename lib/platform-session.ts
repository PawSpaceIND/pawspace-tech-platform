...
import{currentRequestD1Metrics,requestMemo}from"./request-d1-metrics";
...
async function ensurePlatformSessionTablesUncached(db:D1Database){await ensureIdentityBindingTables(db);await db.batch([
...
const platformSessionTablesReady=new WeakSet<object>();
export async function ensurePlatformSessionTables(db:D1Database){if(platformSessionTablesReady.has(db))return;await ensurePlatformSessionTablesUncached(db);platformSessionTablesReady.add(db);}
...
export async function resolvePlatformSession(db:D1Database,request:Request):Promise<PlatformSessionActor|null>{const token=cookieValue(request,PLATFORM_SESSION_COOKIE);if(!token)return null;const tokenHash=await sha256(token),now=Date.now(),ray=request.headers.get("cf-ray");if(!ray||currentRequestD1Metrics())return requestMemo(`platform-session:${tokenHash}`,()=>resolvePlatformSessionFromDb(db,request,tokenHash,now));/* no cf-ray (local, tests, internal calls), or a request scope is open (the Worker opens one for scheduling and booking writes, whose gateway and route hold different D1 wrappers): the request-scoped memo still shares nothing across requests */const key=`${ray}:${tokenHash}`;let memo=sessionMemo.get(db);if(!memo){memo=new Map();sessionMemo.set(db,memo);}const hit=memo.get(key);if(hit&&now-hit.at<SESSION_MEMO_MS&&hit.actor.expiresAt>now)return hit.actor;const actor=await resolvePlatformSessionFromDb(db,request,tokenHash,now);if(memo.size>=1000)memo.clear();else for(const [stale,entry] of memo)if(now-entry.at>=SESSION_MEMO_MS)memo.delete(stale);if(actor&&actor.identitySource!=="uat_persona")memo.set(key,{at:now,actor});else memo.delete(key);return actor;}
