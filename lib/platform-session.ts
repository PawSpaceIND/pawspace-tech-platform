 ...
 * customer latency. A session verified moments ago is reused for SESSION_MEMO_MS within this isolate; a
 * revoke or a newer sign-in for the same subject drops it here at once. UAT persona sessions depend on the
 * request, so they are never reused. */
const SESSION_MEMO_MS=2_000;
...
export async function resolvePlatformSession(db:D1Database,request:Request):Promise<PlatformSessionActor|null>{const token=cookieValue(request,PLATFORM_SESSION_COOKIE);if(!token)return null;const tokenHash=await sha256(token),now=Date.now();let memo=sessionMemo.get(db);if(!memo){memo=new Map();sessionMemo.set(db,memo);}const hit=memo.get(tokenHash);if(hit&&now-hit.at<SESSION_MEMO_MS&&hit.actor.expiresAt>now)return hit.actor;const actor=await resolvePlatformSessionFromDb(db,request,tokenHash,now);if(actor&&actor.identitySource!=="uat_persona"){if(memo.size>=500)memo.clear();memo.set(tokenHash,{at:now,actor});}else memo.delete(tokenHash);return actor;}
