...
// they are listed for manual review and left unchanged. Unpaid (payment_pending) Training BOOKINGS are not
// cancelled here: the scheduled unpaid-booking expiry (lib/training-unpaid-expiry.ts, every 5 minutes)
// cancels each one without moving money at 7 days after booking or at its first session's start, whichever
// comes first, and releases the trainer's sessions. Those older than 7 days are still counted here, as a
// health check that should read 0 once the expiry is deployed.
//
...
//
// The logic lives here so its tests execute it; scripts/uat-staging-training-cleanup.mjs is the runner the
// UAT Training workflow calls. Nothing in the application imports this module.
...
/** The seeded UAT trainers (scripts/uat-staging-provider-capacity.sql), always read even if inactive. */
export const UAT_TRAINER_IDS = ["uatcap_train_ft", "uatcap_train_ft_2", "uatcap_train_ft_3", "uatcap_train_ft_4", "uatcap_train_ft_5", "uatcap_train_east", "uatcap_train_south", "uatcap_train_north", "uatcap_train_west", "uatcap_train_central"];
...
/** Unpaid Training bookings older than 7 days. The scheduled expiry cancels these, so a non-empty list means it skipped one (paid, funded, or a session had started) and a person should look. */
...
export async function runCleanup({ env = process.env, fetcher = globalThis.fetch, log = console.log, now = Date.now() } = {}) {
  ...
  say("");
  say(`Unpaid (payment_pending) Training bookings older than 7 days: ${unpaid.length} (expected 0). Left unchanged here - the scheduled unpaid-booking expiry cancels these automatically, without moving money, at 7 days after booking or at the first session's start, whichever is earlier. Any listed were skipped by it (paid, funded, or a session had started) and need review.`);
