...
async function sessionScope(request:Request):Promise<Scope|undefined>{const url=new URL(request.url),method=request.method.toUpperCase();
  ...
  if(url.pathname==="/api/training-eligibility"&&method==="POST"){const body=await request.clone().json().catch(()=>({})) as {customerId?:string}|null;return{permission:"scheduling.book",subjectType:"customer",subjectId:String(body?.customerId||"")};}
