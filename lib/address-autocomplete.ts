...
export type ResolvedAddress={status:"configured"|"configuration_required"|"provider_error";address?:string;postalCode?:string;latitude?:number;longitude?:number;error?:string};
...
export async function resolvePlaceToAddress(input:{placeId:string;sessionToken?:string}):Promise<ResolvedAddress>{
  ...
  if(creds.fixture)return{status:"configured",address:"42, Indiranagar Double Road, Stage 2, Hoysala Nagar, Indiranagar, Bengaluru 560038",postalCode:"560038",latitude:12.9783692,longitude:77.6408356};
  try{
    ...
    const body=await response.json() as{formattedAddress?:string;addressComponents?:Array<{longText?:string;shortText?:string;types?:string[]}>;location?:{latitude?:number;longitude?:number};error?:{message?:string}};
    if(!response.ok)return{status:"provider_error",error:body.error?.message||`Places API returned ${response.status}`};
    const postalCode=body.addressComponents?.find(component=>component.types?.includes("postal_code"))?.longText?.trim()||"";
    return{status:"configured",address:body.formattedAddress,postalCode,latitude:body.location?.latitude,longitude:body.location?.longitude};
