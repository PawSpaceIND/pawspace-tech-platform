...
const seeds:Seed[]=[
 ...
 {code:"INT-KYC-01",capability:"Provider KYC / identity verification",provider:"IDfy",category:"provider_compliance",owner:"Operations + Compliance",backupOwner:"Engineering + Security",priority:"P0",required:true,environment:"none",codeBoundaryStatus:"code_ready",readinessState:"production_setup_required",credentialDetector:"idfy",dataClassification:"provider identity + private documents",notes:"Request AND callback boundaries are implemented and executed (PR #305): HMAC-signed over the raw body, correlated to a submitted check, replay-deduplicated, monotonic transitions, manual/automated authority separated. No IDfy account has ever been reached, so readiness stays production_setup_required. Credential presence alone must never approve a provider or establish readiness."},
