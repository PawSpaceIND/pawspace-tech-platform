...
const META_WHATSAPP_UAT_SECRET_NAMES=[
 "META_WHATSAPP_UAT_ACCESS_TOKEN","META_WHATSAPP_PHONE_NUMBER_ID","META_WHATSAPP_WABA_ID","META_WHATSAPP_APP_SECRET","META_WHATSAPP_VERIFY_TOKEN",
 "META_WHATSAPP_UAT_ALLOWLIST","META_WHATSAPP_TEMPLATE_ALLOWLIST",
];
...
const seeds:Seed[]=[
 ...
 {code:"INT-PAY-02",capability:"Provider/customer payout rails",provider:"RazorpayX",category:"payments",owner:"Finance",backupOwner:"Engineering",priority:"P1",required:true,environment:"sandbox",codeBoundaryStatus:"code_ready",readinessState:"sandbox_setup_required",credentialDetector:"razorpayx_sandbox",dataClassification:"financial + beneficiary data",launchGateCode:"FIN-01",notes:"RazorpayX Test Mode payout creation, idempotent retry, signed payout webhook, canonical commission/settlement reconciliation and reversal handling are implemented. Live payouts remain disabled."},
 {code:"INT-COMMS-01",capability:"WhatsApp messaging",provider:"Meta WhatsApp",category:"communications",owner:"CRM/CX + Engineering",backupOwner:"Operations",priority:"P0",required:true,environment:"sandbox",codeBoundaryStatus:"code_ready",readinessState:"sandbox_setup_required",credentialDetector:"meta_whatsapp_uat",dataClassification:"customer contact + transactional content",launchGateCode:"COMMS-01",notes:"Canonical outbox, delivery events, retry/dead-letter and direct Meta WhatsApp UAT adapter readiness exist."},
...
function detectedCredentialStatus(runtime:Record<string,unknown>,detector:unknown):CredentialStatus|null{
 switch(String(detector||"")){
  ...
  case"razorpayx_sandbox":return string(runtime.PAWSPACE_RAZORPAYX_ENV).trim().toLowerCase()==="sandbox"&&string(runtime.PAWSPACE_RAZORPAYX_LIVE_APPROVED).trim().toLowerCase()!=="true"&&configured(runtime,["RAZORPAYX_KEY_ID_SANDBOX","RAZORPAYX_KEY_SECRET_SANDBOX","RAZORPAYX_ACCOUNT_NUMBER_SANDBOX","RAZORPAYX_FUND_ACCOUNT_MAP_SANDBOX","RAZORPAYX_WEBHOOK_SECRET_SANDBOX"])?"configured":"missing";
  case"meta_whatsapp_uat":return string(runtime.PAWSPACE_COMMUNICATION_ENV).trim().toLowerCase()==="uat"&&string(runtime.META_WHATSAPP_UAT_DELIVERY_ENABLED).trim().toLowerCase()==="true"&&configured(runtime,META_WHATSAPP_UAT_SECRET_NAMES)?"configured":"missing";
...
/** Existing registries pre-date direct Meta UAT readiness. Migrate only the legacy detector contract;
 * readiness/evidence state is deliberately untouched, and credential presence is re-evaluated next. */
async function advanceMetaWhatsAppCredentialSeed(db:Db,now:number){
 const seed=seeds.find(item=>item.code==="INT-COMMS-01");
 if(!seed)return;
 await db.prepare("UPDATE integration_registry SET provider=CASE WHEN provider='LimeChat / Meta WhatsApp' THEN ? ELSE provider END,credential_detector=?,credential_status='unknown',secret_reference=NULL,updated_at=? WHERE integration_code='INT-COMMS-01' AND credential_detector='wati'")
