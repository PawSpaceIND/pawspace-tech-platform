import{eraseCustomerPersonalData,type DpdpErasureRuntime}from"./dpdp-erasure";
...
async function runDpdpTransientRetentionSweep(db:Db,input:{asOf?:number;retentionDays?:number;runtime?:DpdpErasureRuntime}={}){
 ...
 if(await tableExists(db,"app_users")&&await tableExists(db,"atlas_vector_memories")){const rows=await db.prepare("SELECT avm.vector_id FROM atlas_vector_memories avm JOIN app_users u ON u.id=avm.customer_id WHERE u.status='deleted' AND u.updated_at<?").bind(cutoff).all<Row>(),ids=rows.results.map(row=>text(row.vector_id)).filter(Boolean);if(ids.length){const vectorize=input.runtime?.ATLAS_VECTORIZE;if(!vectorize||typeof vectorize.deleteByIds!=="function")throw new Error("DPDP retention requires ATLAS_VECTORIZE.deleteByIds when stale Atlas vector memory exists");await vectorize.deleteByIds(ids);}}
...
export async function runDpdpRetentionSweep(db:Db,input:{asOf?:number;requestedBy?:string;limit?:number;runtime?:DpdpErasureRuntime}={}){
 ...
 const transient=await runDpdpTransientRetentionSweep(db,{asOf,retentionDays:30,runtime:input.runtime});
 ...
 for(const row of rows.results){
  ...
  try{const result=await eraseCustomerPersonalData(db,{customerId,idempotencyKey:`dpdp-retention:${customerId}:${cutoff}`,requestedBy,reason:"automated_retention_inactive_over_3_years",now:asOf,runtime:input.runtime});erased++;results.push({customerIdHash:result.customerIdHash,status:result.status,ledgerPreserved:result.ledgerPreserved,duplicatePrevented:result.duplicatePrevented});}
