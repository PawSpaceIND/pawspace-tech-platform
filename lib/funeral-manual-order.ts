 ...
 * hand: customer name, phone, payment method, order value, date. Funeral / memorial is GST EXEMPT (owner
 * decision 4, 26 Sept 2026): these orders carry 0 GST whether or not the legacy toggle below is on. The toggle
 * still writes the canonical Funeral finance policy for history, but it no longer adds tax. Deliberately its
 * own tiny table so it never destabilises the richer funeral case workflow; sandbox/UAT, no live money.
 */
...
export async function recordFuneralConvertedOrder(db:Db,input:{customerName:string;phone:string;paymentMethod:string;orderValue:number;orderDate:string;note?:string;actorId:string}){
 ...
 const policy=await resolveFuneralFinancePolicy(db,"blr"),tax=taxFromGrossMargin(orderValue,policy.config,{exempt:true});
