...
]);await db.prepare("ALTER TABLE booking_service_locations ADD COLUMN address_id TEXT").run().catch(()=>{});}
...
export async function computeGoogleRoute(origin:ProviderPoint,destinationAddress:string):Promise<RouteResult>{
  const{env}=await import("cloudflare:workers");const runtime=env as unknown as Record<string,unknown>;const mode=String(runtime.PAWSPACE_MAPS_ENV||"sandbox").toLowerCase();if(mode!=="sandbox")return{status:"configuration_required",error:"Maps UAT adapter is locked to sandbox"};const key=String(runtime.GOOGLE_ROUTES_SERVER_API_KEY_UAT||runtime.GOOGLE_MAPS_SERVER_API_KEY_UAT||"").trim();if(!key)return{status:"configuration_required",error:"Google Routes UAT server key is not configured"};
