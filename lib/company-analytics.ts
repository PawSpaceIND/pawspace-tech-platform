 ...
 */
export async function buildCompanyAnalytics(db:D1Database,input:{from?:string;to?:string;serviceCode?:string;zoneId?:string}={}){const from=input.from||"1970-01-01",to=input.to||"2999-12-31",
  ...
  const serviceMap=new Map<string,{bookings:number;completed:number;cancelled:number;gmv:number;collected:number;heldOnCancelled:number;customerCounts:Map<string,number>;costSum:number;costKnownBookings:number;costEligibleBookings:number}>();for(const b of bookings){const code=String(b.service_code),m=serviceMap.get(code)||{bookings:0,completed:0,cancelled:0,gmv:0,collected:0,heldOnCancelled:0,customerCounts:new Map<string,number>(),costSum:0,costKnownBookings:0,costEligibleBookings:0};m.bookings++;if(String(b.status)==="completed")m.completed++;if(String(b.status)==="cancelled")m.cancelled++;if(revenueRecognized(b))m.gmv+=Number(b.total_amount||0);const captured=capturedFor(b);if(revenueRecognized(b))m.collected+=captured;else m.heldOnCancelled+=captured;const cust=String(b.customer_id);m.customerCounts.set(cust,(m.customerCounts.get(cust)||0)+1);if(costTrackedServices.has(code)){m.costEligibleBookings++;const knownCost=costByBooking.get(String(b.id));if(knownCost!=null){m.costSum+=knownCost;m.costKnownBookings++;}}serviceMap.set(code,m);}
