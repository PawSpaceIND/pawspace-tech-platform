import { dailyBookingSeries } from "./analytics-visuals";
import { bookingSupportCases } from "./booking-support-cases";
...
export async function buildCompanyAnalytics(db:D1Database,input:{from?:string;to?:string;serviceCode?:string;zoneId?:string}={}){const from=input.from||"1970-01-01",to=input.to||"2999-12-31";if(!/^\d{4}-\d{2}-\d{2}$/.test(from)||!/^\d{4}-\d{2}-\d{2}$/.test(to)||from>to)throw new Error("Analytics dates must be valid YYYY-MM-DD values in ascending order");
 ...
 const orphanTickets=await safeAll(db,"SELECT t.booking_id FROM customer_experience_tickets t LEFT JOIN canonical_bookings b ON b.id=t.booking_id WHERE t.booking_id IS NULL OR TRIM(t.booking_id)='' OR b.id IS NULL",[],degradation,"customer experience tickets");const providers=await safeAll(db,"SELECT id,provider_model,status,live,quality_score,rating FROM provider_capacity_profiles",[],degradation,"providers");const paymentByBooking=new Map(payments.map(p=>[String(p.booking_id),p]));const completed=bookings.filter(b=>String(b.status)==="completed"),cancelled=bookings.filter(b=>String(b.status)==="cancelled");
  ...
  const refundTotal=[...refundByBooking.values()].reduce((sum,value)=>sum+value,0),uniqueCustomers=new Set(bookings.map(b=>String(b.customer_id))).size,repeatCustomers=Object.values(bookings.reduce((acc:Record<string,number>,b)=>{const id=String(b.customer_id);acc[id]=(acc[id]||0)+1;return acc;},{} as Record<string,number>)).filter(count=>count>1).length;const openTickets=tickets.filter(t=>isSupportCaseOpen(t.status)).length,resolvedTickets=tickets.filter(t=>!isSupportCaseOpen(t.status)),resolutionMs=resolvedTickets.map(t=>Number(t.resolved_at||0)-Number(t.created_at||0)).filter(v=>v>=0);const dataQuality={bookingsMissingProvider:bookings.filter(b=>!String(b.provider_id||"")).length,paymentsMissing:bookings.filter(b=>!paymentByBooking.has(String(b.id))).length,ticketsMissingBooking:orphanTickets.length};return{daily:dailyBookingSeries(bookings,capturedFor),degraded:degradationNotice(degradation.entries()),period:{from,to},filters:{serviceCode:input.serviceCode||null,zoneId:input.zoneId||null},bookings:{total:bookings.length,completed:completed.length,cancelled:cancelled.length,completionRate:bookings.length?completed.length/bookings.length:null,cancellationRate:bookings.length?cancelled.length/bookings.length:null},money:{gmv,collected,heldOnCancelled,refunds:refundTotal,netCollections:collected+heldOnCancelled-refundTotal,currency:"INR",refundsStatus:"booking_refund_cases_processing_processed_completed",contributionMarginStatus:"configuration_required"},customers:{unique:uniqueCustomers,repeat:repeatCustomers,repeatRate:uniqueCustomers?repeatCustomers/uniqueCustomers:null},cx:{tickets:tickets.length,open:openTickets,reopened:tickets.reduce((s,t)=>s+Number(t.reopened_count||0),0),averageResolutionMs:resolutionMs.length?resolutionMs.reduce((a,b)=>a+b,0)/resolutionMs.length:null},providers:{profiles:providers.length,active:providers.filter(p=>String(p.status)==="active"&&Number(p.live)===1).length,qualityConfigured:providers.filter(p=>Number(p.quality_score)>0).length},services,dataQuality,sourceStatus:{bookings:"canonical_bookings",payments:"booking_payments_plus_stay_payment_schedules",refunds:"booking_refund_cases",cx:"customer_experience_tickets_plus_unified_customer_complaints",providers:"provider_capacity_profiles",marketingSpend:"not_connected",tax:"configuration_required",payouts:"real_for_boarding_sitting_training_and_grooming"}};}
