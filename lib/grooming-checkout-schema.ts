...
export const groomingCheckoutSchema = z.object({
  ...
  // Optional. An empty field must never hold "Confirm booking" hostage; a filled one must still be a real number.
  alternativePhone: z.string().trim().refine(value => value === "" || /^[6-9]\d{9}$/.test(value), "Enter a valid 10-digit Indian mobile number").optional().default(""),
