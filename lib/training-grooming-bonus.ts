...
export type TrainingGroomingBonus={status:"awaiting_full_payment"|"issued"|"used"|"expired"|"withdrawn";code:string|null;packageName:string;value:number;validUntil:number|null};

/**
 * An unused bonus stops working when its programme is cancelled or is no longer fully paid (for example after
 * a refund), so a cancelled programme cannot keep a free grooming. A voucher already used is left alone.
 */
export async function withdrawTrainingGroomingBonus(db:D1Database,bookingId:string){
 await db.prepare("UPDATE coupon_campaigns SET status='paused',updated_at=? WHERE id=? AND status='active' AND NOT EXISTS (SELECT 1 FROM coupon_redemptions r WHERE r.campaign_id=coupon_campaigns.id AND r.status='consumed')").bind(Date.now(),`CPN-TRNBONUS-${bookingId}`).run().catch(()=>null);
}
...
function view(row:Row,now:number):TrainingGroomingBonus{
 ...
 return{status:used?"used":String(row.status)!=="active"?"withdrawn":validUntil<now?"expired":"issued",code:String(row.code),packageName:pkg?.name??"Bath & Basic",value:Number(row.max_discount),validUntil};
}
...
export async function ensureTrainingGroomingBonus(db:D1Database,input:{bookingId:string;now?:number;liveCoupons?:boolean}):Promise<TrainingGroomingBonus|null>{
 ...
 if(!booking||String(booking.service_code)!=="dog_training")return null;
 if(["cancelled","refunded"].includes(String(booking.status))){await withdrawTrainingGroomingBonus(db,bookingId);return null;}
 ...
 const read=()=>db.prepare("SELECT c.code,c.status,c.max_discount,c.valid_until,(SELECT COUNT(*) FROM coupon_redemptions r WHERE r.campaign_id=c.id AND r.status='consumed') used FROM coupon_campaigns c WHERE c.id=?").bind(id).first<Row>();
 const existing=await read(),fullyPaid=(await trainingBookingPaymentState(db,bookingId)).status==="FULLY_PAID";
 if(existing){if(!fullyPaid&&String(existing.status)==="active"&&!(Number(existing.used||0)>0)){await withdrawTrainingGroomingBonus(db,bookingId);const after=await read();return after?view(after,now):null;}return view(existing,now);}
 if(!fullyPaid)return{status:"awaiting_full_payment",code:null,packageName:pkg.name,value:pkg.singlePrice,validUntil:null};
