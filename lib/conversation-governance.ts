...
export async function listConversationThreads(db:D1Database,input:{customerId?:string;status?:string;limit?:number;actor?:ConversationAccessActor;query?:string;channel?:string;ownership?:string;before?:{at:number;id:string}}){
 ...
 for(const row of result.results){
  const [lastMessage,openTicket,communicationState]=await Promise.all([
   db.prepare("SELECT id,direction,channel,purpose,template_key,status,created_at,payload_json FROM communication_messages WHERE thread_id=? ORDER BY created_at DESC,id DESC LIMIT 1").bind(row.id).first<Row>(),
   linkedConversationTicket(db,row),
   bookingCommunicationState(db,row),
  ]);
  threads.push({...row,customer_name:String(row.customer_name||"Customer"),primary_phone:row.primary_phone?String(row.primary_phone):null,lastMessage:conversationMessagePreview(lastMessage),ticket:openTicket||null,communicationState});
 }
 return threads;
}
...
export async function recordInboundMessage(db:D1Database,input:{threadId:string;customerId:string;channel:CommunicationChannel;payload:Record<string,unknown>;provider:string;providerReference:string;eventId:string;createdBy:string}){await ensureConversationGovernance(db);
 const thread=await db.prepare("SELECT customer_id,status FROM communication_threads WHERE id=?").bind(input.threadId).first<Row>();
 if(!thread||String(thread.customer_id)!==input.customerId)throw new Error("Conversation thread/customer mismatch");
 const existing=await db.prepare("SELECT id,thread_id,customer_id,direction,channel FROM communication_messages WHERE provider_reference=? AND provider=? LIMIT 1").bind(input.providerReference,input.provider).first<Row>();
 if(existing){
  if(existing.thread_id!==input.threadId||existing.customer_id!==input.customerId||existing.direction!=="inbound"||existing.channel!==input.channel)throw new Response("Inbound provider reference conflicts with another message",{status:409});
  return{id:String(existing.id),duplicatePrevented:true};
 }
 if(String(thread.status)==="closed")throw new Error("Closed conversation cannot accept a new inbound message until reopened");
 const now=Date.now(),id=`MSG-${crypto.randomUUID().slice(0,14).toUpperCase()}`;await db.batch([
