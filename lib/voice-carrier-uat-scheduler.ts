...
export const VOICE_CARRIER_UAT_RUN_AT = "2026-09-10T15:30:00.000Z"; // already due; controlled UAT may run inside quiet hours
const RUN_ID = "VOICE-UAT-20260910-CONTROLLED-RETRY-5";
...
export async function runDueVoiceCarrierUat(db: D1Database, env: Env, asOf = Date.now()) {
  ...
  try {
    const result = await requestControlledCarrierUatCall(db, env, {
      idempotencyKey: "voice-carrier-uat:2026-09-10:controlled-retry-5",
