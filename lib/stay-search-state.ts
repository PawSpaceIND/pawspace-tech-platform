...
export function staySearchKey(input:{cityId?:string;zoneId?:string;start:string;end:string;careWindow:string;petIds:string[];species:string[]}){
 return JSON.stringify([input.cityId||"",input.zoneId||"",input.start,input.end,input.careWindow,[...input.petIds].sort(),[...input.species].sort()]);
}
