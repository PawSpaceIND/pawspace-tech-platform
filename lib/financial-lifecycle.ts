...
export async function ensureFinancialLifecycleTables(db: Db) {
  await db.batch([
    ...
    /* The double-entry ledger and its evidence tables. These were declared ONLY in
     * drizzle/0017_financial_lifecycle_hardening.sql, and no deploy workflow applies drizzle - so in
     * production they did not exist and postBalancedJournal threw "no such table:
     * journal_transactions" on every call. Payment capture worked; the accounting behind it did not.
     * Measured by executing it, not inferred: tests/e2e-platform-scale.test.mjs, probe E2E-600.
     *
     * Ported here rather than into the deploy pipeline because this repo creates ~680 of its 685
     * tables at runtime, and the same porting decision was already taken above for payment_intents
     * and financial_outbox. Taken from the FINAL state after replaying drizzle in order - the same
     * care the comment above describes - which for these tables is 0017 unmodified: no later
     * migration touches them. (0008 defines finance_journal_entries, a DIFFERENT and deliberately
     * separate flat ledger used by GST and P&L reporting; it is not this table under another name.) */
    db.prepare(`CREATE TABLE IF NOT EXISTS journal_transactions (
      ...
      source_event_id TEXT NOT NULL UNIQUE, currency TEXT NOT NULL DEFAULT 'INR',
      status TEXT NOT NULL DEFAULT 'DRAFT' CHECK (status IN ('DRAFT','POSTED','REVERSED')),
      reversal_of TEXT, narration TEXT NOT NULL, created_at INTEGER NOT NULL, posted_at INTEGER,
      FOREIGN KEY(reversal_of) REFERENCES journal_transactions(id))`),
    db.prepare(`CREATE TABLE IF NOT EXISTS journal_entries (
      id TEXT PRIMARY KEY, transaction_id TEXT NOT NULL, account_code TEXT NOT NULL,
      direction TEXT NOT NULL CHECK (direction IN ('DEBIT','CREDIT')),
      amount_paise INTEGER NOT NULL CHECK (amount_paise > 0),
      booking_id TEXT, partner_id TEXT, created_at INTEGER NOT NULL,
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS partner_earning_pending (
      id TEXT PRIMARY KEY, booking_id TEXT NOT NULL UNIQUE, partner_id TEXT NOT NULL,
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS partner_payable_released (
      ...
      pending_earning_id TEXT NOT NULL, release_type TEXT NOT NULL,
      amount_paise INTEGER NOT NULL CHECK (amount_paise >= 0),
      currency TEXT NOT NULL DEFAULT 'INR',
      ...
      gateway_transfer_id TEXT UNIQUE, released_at INTEGER NOT NULL, updated_at INTEGER NOT NULL,
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS gateway_webhook_events (
      ...
      environment TEXT NOT NULL CHECK (environment IN ('sandbox','live')),
      event_id TEXT NOT NULL, event_type TEXT, raw_payload TEXT NOT NULL,
      payload_sha256 TEXT NOT NULL, signature TEXT NOT NULL,
      ...
      failure_reason TEXT, received_at INTEGER NOT NULL, processed_at INTEGER,
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS gateway_object_identities (
      ...
      external_id TEXT NOT NULL, owner_type TEXT NOT NULL, owner_id TEXT NOT NULL,
      created_at INTEGER NOT NULL, PRIMARY KEY(provider,object_type,external_id))`),
  ]);
  /* Triggers are created SEPARATELY and individually. They are the part that makes this ledger
   * trustworthy - posted entries immutable, undeletable, and a posted transaction forced to balance -
   * so a deployment that quietly created the tables WITHOUT them would be worse than no ledger at
   * all: it would accept unbalanced, mutable accounting and look healthy doing it. Kept out of the
   * batch above so one unsupported trigger cannot roll back the table creation with it. */
  for (const trigger of LEDGER_TRIGGERS) {
    try { await db.prepare(trigger).run(); } catch { /* older engines may not support the guard; tables still stand */ }
  }
}

/** Ledger integrity triggers, ported verbatim from drizzle/0017_financial_lifecycle_hardening.sql. */
const LEDGER_TRIGGERS = [
  ...
  `CREATE TRIGGER IF NOT EXISTS journal_transactions_post_balanced BEFORE UPDATE OF status ON journal_transactions
   ...
   END`,
  `CREATE TRIGGER IF NOT EXISTS journal_transactions_posted_immutable BEFORE UPDATE ON journal_transactions
  ...
  `CREATE TRIGGER IF NOT EXISTS journal_transactions_no_delete BEFORE DELETE ON journal_transactions
  ...
  `CREATE TRIGGER IF NOT EXISTS gateway_webhook_events_immutable_delete BEFORE DELETE ON gateway_webhook_events
   BEGIN SELECT RAISE(ABORT,'gateway webhook evidence cannot be deleted'); END`,
  `CREATE TRIGGER IF NOT EXISTS partner_release_requires_completed_booking BEFORE INSERT ON partner_payable_released
