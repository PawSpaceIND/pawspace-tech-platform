...
export type CouponPackageScope="all"|"single_session"|"subscription"|"selected";
export type CouponCampaign={
  id:string;code:string;name:string;status:"active"|"paused";testOnly:boolean;serviceCodes:CouponService[];cityIds:string[];channels:CouponChannel[];customerKinds:CouponCustomerKind[];
...
export async function ensureCouponTables(db:Db){
  ...
  if(!columns.results.some(row=>String(row.name)==="customer_ids_json")){
    try{await db.prepare("ALTER TABLE coupon_campaigns ADD COLUMN customer_ids_json TEXT NOT NULL DEFAULT '[]'").run();}
    catch(error){const again=await db.prepare("PRAGMA table_info(coupon_campaigns)").all<Row>();if(!again.results.some(row=>String(row.name)==="customer_ids_json"))throw error;}
  }
}
...
export async function consumeCouponQuote(db:Db,input:{quoteId:string;bookingId:string;customerId:string;idempotencyKey:string}){
  ...
  if(!input.quoteId||!input.bookingId||!input.customerId||!input.idempotencyKey)throw new Error("Quote, booking, customer and idempotency key are required");
  const prior=await db.prepare("SELECT * FROM coupon_redemptions WHERE idempotency_key=?").bind(input.idempotencyKey).first<Row>();
  if(prior){if(String(prior.quote_id)!==input.quoteId||String(prior.booking_id)!==input.bookingId||String(prior.customer_id)!==input.customerId)throw new Error("Coupon idempotency key was already used for a different redemption");return{redemption:prior,duplicatePrevented:true};}
  const quote=await db.prepare("SELECT * FROM coupon_quotes WHERE id=?").bind(input.quoteId).first<Row>();
  ...
  if(String(quote.status)!=="open")throw new Error("Coupon quote is no longer open");
  ...
  const [totalUsed,customerUsed,campaignRow]=await Promise.all([
    ...
    db.prepare("SELECT * FROM coupon_campaigns WHERE id=?").bind(quote.campaign_id).first<Row>(),
  ]);
  ...
  try{
    ...
    // D1 batches are transactional. This guarded compensation also protects test/local adapters that
    // emulate batch sequentially: reopen only our unbacked claim, never a quote with a redemption.
    await db.prepare("UPDATE coupon_quotes SET status='open',booking_id=NULL,updated_at=? WHERE id=? AND status='consumed' AND booking_id=? AND NOT EXISTS (SELECT 1 FROM coupon_redemptions WHERE quote_id=?)").bind(now,input.quoteId,input.bookingId,input.quoteId).run();
    const winner=await db.prepare("SELECT * FROM coupon_redemptions WHERE idempotency_key=?").bind(input.idempotencyKey).first<Row>();
    if(winner){if(String(winner.quote_id)!==input.quoteId||String(winner.booking_id)!==input.bookingId||String(winner.customer_id)!==input.customerId)throw new Error("Coupon idempotency key was already used for a different redemption");return{redemption:winner,duplicatePrevented:true};}
    const [quoteNow,totalNow,customerNow]=await Promise.all([
      db.prepare("SELECT status FROM coupon_quotes WHERE id=?").bind(input.quoteId).first<Row>(),
      db.prepare("SELECT COUNT(*) count FROM coupon_redemptions WHERE campaign_id=? AND status='consumed'").bind(quote.campaign_id).first<Row>(),
      db.prepare("SELECT COUNT(*) count FROM coupon_redemptions WHERE campaign_id=? AND customer_id=? AND status='consumed'").bind(quote.campaign_id,input.customerId).first<Row>(),
    ]);
    if(String(quoteNow?.status)!=="open")throw new Error("Coupon quote is no longer open");
    if(Number(customerNow?.count||0)>=campaign.perCustomerLimit)throw new Error("Customer coupon limit reached");
    if(Number(totalNow?.count||0)>=campaign.totalLimit)throw new Error("Coupon total redemption limit reached");
    throw error;
  }
