...
/*
 * The customer's open chat thread, created on first contact. The create is conditional - in the same
 * statement - on no open thread existing, so two first messages racing past the read join one thread
 * instead of each inserting its own and splitting the conversation and its AI history in two.
 */
async function openThread(db:D1Database,customerId:string){const current=()=>db.prepare("SELECT id FROM communication_threads WHERE customer_id=? AND status='open' ORDER BY updated_at DESC LIMIT 1").bind(customerId).first<Row>();const existing=await current();if(existing)return text(existing.id);const id=`THREAD-${crypto.randomUUID().slice(0,12).toUpperCase()}`,now=Date.now();const[created]=await db.batch([db.prepare("INSERT INTO communication_threads (id,customer_id,booking_id,lead_id,ticket_id,status,assigned_to,sla_due_at,created_at,updated_at) SELECT ?,?,NULL,NULL,NULL,'open','ai-orchestrator',NULL,?,? WHERE NOT EXISTS (SELECT 1 FROM communication_threads WHERE customer_id=? AND status='open')").bind(id,customerId,now,now,customerId),db.prepare("INSERT OR IGNORE INTO communication_participants (id,thread_id,participant_type,participant_id,display_ref,role,created_at) SELECT ?,?,?,?,?,'customer',? WHERE EXISTS (SELECT 1 FROM communication_threads WHERE id=?)").bind(crypto.randomUUID(),id,"customer",customerId,customerId,now,id)]);if(Number(created?.meta?.changes||0)===1)return id;const raced=await current();if(raced)return text(raced.id);throw new Error("Customer chat thread could not be opened");}

/** One audit row per stored chat message, keyed on it: a retry can fill the row a failed attempt never wrote, and never writes a second. */
function recordAuthenticatedTurn(db:D1Database,input:{messageId:string;threadId:string;customerId:string;actorEmail:string;turn:unknown;trustSafetyRedacted:boolean;now:number}){return db.prepare("INSERT INTO ai_web_chat_events (id,thread_id,customer_id,event_type,actor_ref,detail_json,created_at) VALUES (?,?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING").bind(`AIWEBTURN-${input.messageId}`,input.threadId,input.customerId,"authenticated_turn",input.actorEmail,JSON.stringify({outcome:input.turn&&typeof input.turn==="object"?(input.turn as Row).outcome:null,autonomousExecution:false,trustSafetyRedacted:input.trustSafetyRedacted}),input.now).run();}

/*
 * A retry (same idempotency key) finds its message already stored. It used to answer duplicatePrevented
 * with no AI turn at all, so an attempt that failed after storing the message - the orchestrator
 * throwing, or the audit insert failing - could never be repaired: every retry reported the message as
 * already received and the customer never got a reply. The orchestrator is itself idempotent on the same
 * `ai:` key (it replays a completed turn, claims one a failed attempt released, or reports one still in
 * flight), so the retry resumes through it, as a replayed voice segment already does. A message this
 * adapter did not store, or one in a conversation since closed, keeps the plain duplicate answer.
 */
async function replayAuthenticatedTurn(db:D1Database,input:{actor:AuthenticatedActor;customerId:string;idempotencyKey:string},prior:Row){
 const messageId=text(prior.id),threadId=text(prior.thread_id);
 let payload:Row={};try{payload=JSON.parse(text(prior.payload_json)||"{}")as Row;}catch{}
 const trustSafetyRedacted=payload.safetyRedacted===true;
 if(text(prior.template_key)!=="web_app_chat"||text(prior.thread_status)==="closed")return{duplicatePrevented:true,messageId,threadId,autonomousExecution:false,trustSafetyRedacted};
 const result=await orchestrateAiTurn(db,{actor:input.actor,threadId,customerId:input.customerId,inputMessageId:messageId,idempotencyKey:`ai:${input.idempotencyKey}`,channel:"chat",provider:await createGroundedAiRuntimeProvider(db,input.actor,"chat")});
 if(result.turn)await recordAuthenticatedTurn(db,{messageId,threadId,customerId:input.customerId,actorEmail:input.actor.email,turn:result.turn,trustSafetyRedacted,now:Date.now()});
 return{duplicatePrevented:true,messageId,threadId,ai:result,autonomousExecution:false,trustSafetyRedacted};
}

export async function runAuthenticatedAiWebChat(db:D1Database,input:{actor:AuthenticatedActor;customerId:string;text:string;idempotencyKey:string}){await ensureAiWebChatTables(db);if(!text(input.text)||!text(input.idempotencyKey))throw new Error("Message and idempotency key are required");const[,prior]=await Promise.all([requireCustomerOwnership(db,input.actor,input.customerId),db.prepare("SELECT m.id,m.thread_id,m.customer_id,m.template_key,m.payload_json,t.status thread_status FROM communication_messages m LEFT JOIN communication_threads t ON t.id=m.thread_id WHERE m.idempotency_key=?").bind(input.idempotencyKey).first<Row>()]);if(prior&&text(prior.customer_id)!==input.customerId)throw new Response("Chat request key belongs to another customer",{status:403});if(prior)return replayAuthenticatedTurn(db,input,prior);const providerPromise=createGroundedAiRuntimeProvider(db,input.actor,"chat"),threadId=await openThread(db,input.customerId),messageId=`MSG-CHAT-${crypto.randomUUID().slice(0,12).toUpperCase()}`,now=Date.now();const inspected=await inspectTrustSafetyText(db,{text:input.text,channel:"chat",sourceReference:`ai-web-authenticated:${input.idempotencyKey}`,actorType:"customer",actorId:input.actor.email,customerId:input.customerId,threadId,messageId,asOf:now,detail:{surface:"authenticated_ai_web_chat"}});await db.batch([db.prepare("INSERT INTO communication_messages (id,thread_id,customer_id,booking_id,lead_id,ticket_id,direction,channel,purpose,template_key,payload_json,status,provider,provider_reference,idempotency_key,policy_json,created_by,created_at,updated_at) VALUES (?,?,?,NULL,NULL,NULL,'inbound','chat','transactional','web_app_chat',?,'received','pawspace_web',NULL,?,?,?, ?,?)").bind(messageId,threadId,input.customerId,JSON.stringify({text:inspected.redacted,safetyRedacted:inspected.detected}),input.idempotencyKey,JSON.stringify({authenticated:true,customerOwned:true,externalDelivery:false,trustSafetyInspected:true}),input.actor.email,now,now),db.prepare("UPDATE communication_threads SET updated_at=? WHERE id=?").bind(now,threadId)]);const result=await orchestrateAiTurn(db,{actor:input.actor,threadId,customerId:input.customerId,inputMessageId:messageId,idempotencyKey:`ai:${input.idempotencyKey}`,channel:"chat",provider:await providerPromise});await recordAuthenticatedTurn(db,{messageId,threadId,customerId:input.customerId,actorEmail:input.actor.email,turn:result.turn,trustSafetyRedacted:inspected.detected,now});return{duplicatePrevented:false,messageId,threadId,ai:result,autonomousExecution:false,trustSafetyRedacted:inspected.detected};}
