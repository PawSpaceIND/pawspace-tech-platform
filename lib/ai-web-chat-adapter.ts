...
const authenticatedChatSystemPrompt="Draft a concise PawSpace customer-chat reply using only the canonical context supplied. This is a governed conversational response, not authorization to mutate bookings, payments, refunds, provider assignments, prices, slots or discounts. Never invent a price, discount, availability, completion state or policy. If the request needs a high-impact action, unsupported fact, medical or safety advice, complaint, refund/payment dispute, or a human, allow the PawSpace handoff policy to take over.";
...
async function runtimeAiWebProvider():Promise<AiResponseProvider>{
 ...
 return{status:connection.connected?"connected":"not_connected",provider:connection.providerRef||"not_connected",modelRef:connection.modelRef,deadlineMs:connection.timeoutMs,async generate(input:AiProviderInput){
  const result=await requestAiDraft({systemPrompt:authenticatedChatSystemPrompt,userPrompt:JSON.stringify({channel:"chat",customerMessage:input.inputText,intent:input.intent,canonicalContext:input.context}),maxTokens:1200});
