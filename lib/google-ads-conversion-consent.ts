...
import{syncMarketingAdMetricsAtomic}from"./marketing-ad-atomic-sync";
import{dispatchMarketingConversionOutbox}from"./marketing-attribution-server";
...
async function conversionConsentCoverage(db: Db, from: number, to: number) {
  ...
  const row = await db.prepare(`
    ...
    INNER JOIN whatsapp_lead_attribution attribution
      ON attribution.id=(SELECT a.id FROM whatsapp_lead_attribution a WHERE a.lead_id=facts.lead_id AND a.customer_id=facts.customer_id AND a.thread_id=facts.thread_id AND a.source_platform='google' AND a.click_id IS NOT NULL AND TRIM(a.click_id)<>'' ORDER BY a.created_at DESC,a.id DESC LIMIT 1)
    LEFT JOIN google_ads_conversion_consent consent ON consent.customer_id=facts.customer_id
    WHERE facts.event_type IN ('lead_qualified','booking_created','payment_captured') AND facts.occurred_at>=? AND facts.occurred_at<=?
  `).bind(from,to).first<Row>();
  return {candidates:Number(row?.candidate_count||0),granted:Number(row?.granted_count||0),missing:Number(row?.missing_count||0),denied:Number(row?.denied_count||0)};
}
...
export async function uploadGoogleOfflineConversionsDataManager(db: Db, runtime: Runtime, input: { from?: number; to?: number; validateOnly?: boolean; fetchImpl?: Fetcher } = {}) {
  ...
  if (coverage.missing || coverage.denied || coverage.granted !== coverage.candidates) return { status: "blocked_consent", submitted: 0, externalMutation: false, consent: coverage };
...
export async function runMarketingConnectorScheduler(db: Db, input: { asOf?: number; runtime?: Runtime; fetchImpl?: Fetcher } = {}) {
  ...
  const sync=istHour(asOf)>=6?await syncMarketingAdMetricsAtomic(db,runtime,{from:yesterday,to:yesterday,fetchImpl}):[];
  const reports=await generateMarketingAdReports(db,{asOf});
  const sandbox=text(runtime.PAWSPACE_PAYMENT_ENV).toLowerCase()==="sandbox"||!truthy(runtime.PAWSPACE_MARKETING_EXTERNAL_WRITES_ENABLED);
  const conversionOutbox=await dispatchMarketingConversionOutbox(db,runtime,{now:asOf,validateOnly:sandbox||!truthy(runtime.PAWSPACE_GOOGLE_DATA_MANAGER_UPLOAD_ENABLED),fetchImpl});
  let offlineConversions:unknown={status:"not_due"};
  if(dayOfWeek(today)===1&&istHour(asOf)>=7){
    const fromDate=addDays(mondayOfWeek(today),-7),toDate=addDays(mondayOfWeek(today),-1),from=Date.parse(`${fromDate}T00:00:00+05:30`),to=Date.parse(`${toDate}T23:59:59.999+05:30`);
    if(!sandbox&&truthy(runtime.PAWSPACE_GOOGLE_DATA_MANAGER_UPLOAD_ENABLED))offlineConversions=await uploadGoogleOfflineConversionsDataManager(db,runtime,{from,to,validateOnly:false,fetchImpl});
    else offlineConversions={status:"live_upload_disabled",submitted:0,externalMutation:false};
  }
  return{sync,reports,conversionOutbox,offlineConversions,connectorStatus:await marketingAdConnectorStatus(db,runtime),externalDelivery:false};
}
