...
export type Customer360Record={
  ...
  dataQuality:{score:number;issues:string[];duplicateCandidateIds:string[]};
};

const rows=<T=Row>(result:{results?:unknown[]})=>(result.results||[]) as T[];
const normPhone=(value:unknown)=>String(value||'').replace(/\D/g,'').slice(-10);
const normEmail=(value:unknown)=>String(value||'').trim().toLowerCase();

export async function ensureCustomer360Tables(db:Db){await db.batch([
  db.prepare("CREATE TABLE IF NOT EXISTS customer_contact_preferences (customer_id TEXT PRIMARY KEY, marketing_consent INTEGER NOT NULL DEFAULT 0, service_consent INTEGER NOT NULL DEFAULT 1, whatsapp_consent INTEGER NOT NULL DEFAULT 0, sms_consent INTEGER NOT NULL DEFAULT 0, email_consent INTEGER NOT NULL DEFAULT 0, source TEXT NOT NULL DEFAULT 'customer', updated_by TEXT NOT NULL, updated_at INTEGER NOT NULL)"),
  db.prepare("CREATE TABLE IF NOT EXISTS customer_merge_reviews (id TEXT PRIMARY KEY, primary_customer_id TEXT NOT NULL, duplicate_customer_id TEXT NOT NULL, match_reason TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'open', reviewed_by TEXT, reviewed_at INTEGER, created_at INTEGER NOT NULL)"),
  db.prepare("CREATE UNIQUE INDEX IF NOT EXISTS customer_merge_pair_idx ON customer_merge_reviews(primary_customer_id,duplicate_customer_id)"),
]);}
...
export async function buildCustomer360(db:Db,customerId?:string):Promise<Customer360Record[]>{
  await ensureCustomer360Tables(db);
  const crm=await safeAll(db,"SELECT id,name,primary_phone,email,area,stage,owner,source,lifetime_value,updated_at FROM crm_contacts ORDER BY updated_at DESC LIMIT 500");
  const canonical=await safeAll(db,"SELECT id,name,primary_phone,email,city_id,consent_json,updated_at FROM canonical_customers ORDER BY updated_at DESC LIMIT 500");
  const source=new Map<string,Row>();
  for(const row of crm)source.set(String(row.id),row);
  for(const row of canonical){const id=String(row.id);source.set(id,{...(source.get(id)||{}),...row});}
  ...
  for(const [id,base] of selected){
    const [pets,bookings,tickets,prefs]=await Promise.all([
      ...
      safeAll(db,"SELECT * FROM customer_contact_preferences WHERE customer_id=?",[id]),
    ]);
    const pref=prefs[0]||{};
    const phone=normPhone(base.primary_phone),email=normEmail(base.email);
    const duplicates=[...source.entries()].filter(([otherId,row])=>otherId!==id&&((phone&&normPhone(row.primary_phone)===phone)||(email&&normEmail(row.email)===email))).map(([otherId])=>otherId);
    const issues:string[]=[];
    if(!phone)issues.push('missing_primary_phone');
    if(!String(base.name||'').trim())issues.push('missing_name');
    if(!pets.length)issues.push('no_canonical_pet');
    if(duplicates.length)issues.push('possible_duplicate');
    const score=Math.max(0,100-issues.length*20);
    const bookingRecords=bookings.map(row=>({id:String(row.id),serviceCode:String(row.service_code),packageName:String(row.package_name),status:String(row.status),scheduledStart:String(row.scheduled_start),scheduledEnd:String(row.scheduled_end),totalAmount:Number(row.total_amount||0),currency:String(row.currency||'INR')}));
    result.push({
      customerId:id,
      name:String(base.name||'Customer'),
      primaryPhone:String(base.primary_phone||''),
      ...
      lifetimeValue:bookingRecords.filter(item=>!['cancelled','refunded'].includes(item.status)).reduce((sum,item)=>sum+item.totalAmount,0),
      lastServiceAt:bookingRecords[0]?.scheduledEnd||null,
      openTicketCount:tickets.filter(row=>String(row.status)!=='resolved').length,
      dataQuality:{score,issues,duplicateCandidateIds:duplicates},
    });
  }
  return result;
}
