...
export async function readCustomerBilling(db:D1Database,customerId:string):Promise<CustomerBilling>{
 ...
 const payments=paymentsAvailable?(await db.prepare("SELECT p.id,p.booking_id,p.amount,p.amount_due_now,p.currency,p.method,p.status,p.gateway,p.created_at,b.package_name,b.service_code FROM booking_payments p JOIN canonical_bookings b ON b.id=p.booking_id WHERE p.customer_id=? AND b.customer_id=? ORDER BY p.created_at DESC LIMIT 100").bind(customerId,customerId).all<Row>()).results:[];
