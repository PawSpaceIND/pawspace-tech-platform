 ...
 * tests/control-center-column-contract.test.mjs, which checks EVERY column this module names -
 * recent()'s select lists AND count()'s predicates - against the canonical CREATE TABLE for that
 * table, and fails on any column that does not exist.
 *
 * That distinction is not academic. An earlier version of this note claimed the approvals/onboarding
 * suite already provided this cover; it did not. It pinned recent()'s columns only, so three count()
 * predicates naming columns their tables never had - payroll_approval_events.outcome,
 * partner_payout_instruction_approvals.status and communication_dead_letters.status - sat on these
 * very lines while that suite passed 17/17. count() absorbs `no such column` exactly as recent()
 * does, so each one rendered as a calm "not connected" while real work queued up behind it.
 */
...
export async function buildControlCenterOperations(db:Db,mode:ControlOpsMode){
 ...
 if(mode==="health"){
  return{mode,title:"System health",cards:[card("Integrations",await count(db,"integration_registry"),"Registered external integrations","integration_registry"),card("Integration blockers",await count(db,"integration_registry","required=1 AND readiness_state NOT IN ('sandbox_verified','controlled_live_verified')"),"Required integrations below verified state","integration_registry"),card("Outbox pending",await count(db,"communication_outbox","status IN ('pending','retry')"),"Messages awaiting delivery","communication_outbox"),card("Dead letters",await count(db,"communication_dead_letters","resolved_at IS NULL"),"Communication failures needing recovery","communication_dead_letters"),card("Outbound queue",await count(db,"outbound_routing_queue","status IN ('queued','claimed','dialing')"),"Sales/AI outbound work in progress","outbound_routing_queue")],rows:await recent(db,"integration_registry","integration_code,category,provider,readiness_state,blocker_reason,updated_at","updated_at")};
 }
