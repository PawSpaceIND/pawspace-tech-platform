...
export async function mutateSubscriptionWallet(db:Db,input:SubscriptionWalletMutation){await ensureSubscriptionWalletTables(db);if(!input.subscriptionId||!input.action||!input.idempotencyKey||!input.actorId)throw new Error("Subscription, action, idempotency key and actor are required");const prior=await priorEvent(db,input.idempotencyKey);if(prior){const eventForAction:Record<SubscriptionWalletAction,string>={reserve:"reserved",consume:"consumed",release:"released",pause:"paused",resume:"resumed",refresh_expiry:"expiry_checked"},priorBooking=String(prior.booking_id||""),inputBooking=String(input.bookingId||"");if(String(prior.subscription_id)!==input.subscriptionId||String(prior.event_type)!==eventForAction[input.action]||priorBooking!==inputBooking||String(prior.actor_id)!==input.actorId||(input.action==="reserve"&&Number(prior.credits)!==Math.floor(Number(input.credits||1))))throw new Error("Subscription wallet idempotency key was already used for a different mutation");return{duplicatePrevented:true,event:prior,wallet:await readSubscriptionWallet(db,input.subscriptionId)};}const subscription=await activeSubscription(db,input.subscriptionId),now=Date.now(),snap=await snapshot(db,input.subscriptionId),cfg=configOf(snap),graceEndsAt=Number(subscription.expires_at)+Math.max(0,Number(cfg.graceDays||0))*86_400_000;
 ...
 if(input.action==="reserve"){const existing=await db.prepare("SELECT * FROM booking_subscription_usage WHERE booking_id=?").bind(input.bookingId).first<Row>();if(existing)throw new Error("Booking already has a subscription reservation");const available=balance(subscription);if(available<credits)throw new Error("Subscription does not have enough available credits");if(["completed","cancelled"].includes(String(booking.status)))throw new Error("Credits cannot be reserved for a completed or cancelled booking");
  // Atomic reserve: the guarded decrement runs FIRST and the usage row is only written when the
  // guard actually took the credits (meta.changes>0). Previously the usage INSERT sat in the same
  // batch unconditionally, so a lost race left a phantom reservation with no backing credits.
  const reserved=await db.prepare("UPDATE customer_grooming_subscriptions SET sessions_reserved=sessions_reserved+?,updated_at=? WHERE id=? AND sessions_reserved+sessions_consumed+?<=total_sessions").bind(credits,now,input.subscriptionId,credits).run();
  if(!Number(reserved.meta?.changes||0))throw new Error("Subscription does not have enough available credits");
  await db.prepare("INSERT INTO booking_subscription_usage (id,booking_id,customer_id,plan_code,sessions_reserved,sessions_consumed,status,created_at,updated_at) VALUES (?,?,?,?,?,0,'reserved',?,?)").bind(crypto.randomUUID(),input.bookingId,subscription.customer_id,input.subscriptionId,credits,now,now).run();
  const current=await activeSubscription(db,input.subscriptionId);await addEvent(db,input,"reserved",credits,balance(current),{bookingStatus:booking.status});return{duplicatePrevented:false,wallet:await readSubscriptionWallet(db,input.subscriptionId)};}
