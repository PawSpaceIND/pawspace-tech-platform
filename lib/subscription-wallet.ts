...
export async function mutateSubscriptionWallet(db:Db,input:SubscriptionWalletMutation){await ensureSubscriptionWalletTables(db);if(!input.subscriptionId||!input.action||!input.idempotencyKey||!input.actorId)throw new Error("Subscription, action, idempotency key and actor are required");const requestedCredits=input.credits==null?1:Number(input.credits),normalizedReason=String(input.reason||"").trim(),releaseReason=normalizedReason||"booking_credit_release";if(!Number.isFinite(requestedCredits)||!Number.isInteger(requestedCredits)||requestedCredits<1)throw new Error("Credits must be a positive integer");if(input.pauseDays!=null&&(!Number.isFinite(input.pauseDays)||!Number.isInteger(input.pauseDays)||input.pauseDays<1))throw new Error("Pause days must be a positive integer");const prior=await priorEvent(db,input.idempotencyKey);if(prior){const eventForAction:Record<SubscriptionWalletAction,string>={reserve:"reserved",consume:"consumed",release:"released",pause:"paused",resume:"resumed",refresh_expiry:"expiry_checked"},priorBooking=String(prior.booking_id||""),inputBooking=String(input.bookingId||""),detail=parse<Record<string,unknown>>(prior.detail_json,{}),actionContextMismatch=(input.action==="reserve"&&Number(prior.credits)!==requestedCredits)||(input.action==="pause"&&(Number(detail.requestedDays)!==Number(input.pauseDays)||String(detail.reason||"")!==normalizedReason))||(input.action==="release"&&String(detail.reason||"")!==releaseReason);if(String(prior.subscription_id)!==input.subscriptionId||String(prior.event_type)!==eventForAction[input.action]||priorBooking!==inputBooking||String(prior.actor_id)!==input.actorId||actionContextMismatch)throw new Error("Subscription wallet idempotency key was already used for a different mutation");return{duplicatePrevented:true,event:prior,wallet:await readSubscriptionWallet(db,input.subscriptionId)};}const subscription=await activeSubscription(db,input.subscriptionId),now=Date.now(),snap=await snapshot(db,input.subscriptionId),cfg=configOf(snap),graceEndsAt=Number(subscription.expires_at)+Math.max(0,Number(cfg.graceDays||0))*86_400_000;
 ...
 if(input.action==="pause"){if(String(subscription.status)!=="active")throw new Error("Only an active subscription can be paused");const allowed=Math.max(0,Number(cfg.pauseDays||0)),days=Math.floor(Number(input.pauseDays||0));if(allowed<1)throw new Error("This plan has no configured pause entitlement");if(days<1||days>allowed)throw new Error(`Pause must be between 1 and ${allowed} configured days`);const usedRow=await db.prepare("SELECT COALESCE(SUM(requested_days),0) used FROM subscription_pause_periods WHERE subscription_id=? AND status IN ('active','completed')").bind(input.subscriptionId).first<{used:number}>(),used=Number(usedRow?.used||0);if(used+days>allowed)throw new Error("Configured pause entitlement would be exceeded");if(!normalizedReason)throw new Error("Pause reason is required");const pauseId=`SUBPAUSE-${crypto.randomUUID().slice(0,10).toUpperCase()}`;await db.batch([db.prepare("UPDATE customer_grooming_subscriptions SET status='paused',updated_at=? WHERE id=?").bind(now,input.subscriptionId),db.prepare("INSERT INTO subscription_pause_periods (id,subscription_id,status,start_at,end_at,requested_days,reason,actor_id,created_at,updated_at) VALUES (?,?,'active',?,NULL,?,?,?,?,?)").bind(pauseId,input.subscriptionId,now,days,normalizedReason,input.actorId,now,now)]);await addEvent(db,input,"paused",0,balance(subscription),{pauseId,requestedDays:days,reason:normalizedReason,expiryExtension:"policy_required"});return{duplicatePrevented:false,wallet:await readSubscriptionWallet(db,input.subscriptionId)};}
 ...
 if(!input.bookingId)throw new Error("Booking is required for credit movements");if(!["active","exhausted"].includes(String(subscription.status)))throw new Error(`Subscription is ${String(subscription.status)} and cannot move booking credits`);if(now>graceEndsAt)throw new Error("Subscription has expired beyond its configured grace period");const booking=await db.prepare("SELECT id,customer_id,service_code,package_code,status FROM canonical_bookings WHERE id=?").bind(input.bookingId).first<Row>();if(!booking)throw new Error("Canonical booking not found");if(String(booking.customer_id)!==String(subscription.customer_id))throw new Error("Booking and subscription customer do not match");if(String(booking.service_code)!=="grooming")throw new Error("This UAT wallet currently supports Grooming bookings only");const credits=requestedCredits;

 if(input.action==="reserve"){const existing=await db.prepare("SELECT * FROM booking_subscription_usage WHERE booking_id=?").bind(input.bookingId).first<Row>();if(existing)throw new Error("Booking already has a subscription reservation");const available=balance(subscription);if(available<credits)throw new Error("Subscription does not have enough available credits");if(["completed","cancelled"].includes(String(booking.status)))throw new Error("Credits cannot be reserved for a completed or cancelled booking");
  const eventId=crypto.randomUUID(),usageId=crypto.randomUUID();
  try{
   // Claim the idempotency key, move the credits, and create the booking usage in one D1 transaction.
   // A unique-key loss on either the event or booking usage rolls every local mutation back.
   const results=await db.batch([
    db.prepare("INSERT INTO subscription_wallet_events (id,subscription_id,booking_id,event_type,credits,balance_after,idempotency_key,actor_id,detail_json,created_at) SELECT ?,id,?,'reserved',?,total_sessions-sessions_reserved-sessions_consumed-?,?,?,?,? FROM customer_grooming_subscriptions WHERE id=? AND sessions_reserved+sessions_consumed+?<=total_sessions").bind(eventId,input.bookingId,credits,credits,input.idempotencyKey,input.actorId,JSON.stringify({bookingStatus:booking.status}),now,input.subscriptionId,credits),
    db.prepare("UPDATE customer_grooming_subscriptions SET sessions_reserved=sessions_reserved+?,updated_at=? WHERE id=? AND EXISTS (SELECT 1 FROM subscription_wallet_events WHERE id=?)").bind(credits,now,input.subscriptionId,eventId),
    db.prepare("INSERT INTO booking_subscription_usage (id,booking_id,customer_id,plan_code,sessions_reserved,sessions_consumed,status,created_at,updated_at) SELECT ?,?,?,?, ?,0,'reserved',?,? WHERE EXISTS (SELECT 1 FROM subscription_wallet_events WHERE id=?)").bind(usageId,input.bookingId,subscription.customer_id,input.subscriptionId,credits,now,now,eventId),
   ]);
   if(Number(results[0]?.meta?.changes||0)!==1||Number(results[1]?.meta?.changes||0)!==1||Number(results[2]?.meta?.changes||0)!==1)throw new Error("Subscription does not have enough available credits");
  }catch(error){const winner=await priorEvent(db,input.idempotencyKey);if(winner){const same=String(winner.subscription_id)===input.subscriptionId&&String(winner.event_type)==="reserved"&&String(winner.booking_id||"")===String(input.bookingId)&&String(winner.actor_id)===input.actorId&&Number(winner.credits)===credits;if(!same)throw new Error("Subscription wallet idempotency key was already used for a different mutation");return{duplicatePrevented:true,event:winner,wallet:await readSubscriptionWallet(db,input.subscriptionId)};}throw error;}
 ...
 if(input.action==="release"){if(String(booking.status)==="completed")throw new Error("Consumed/completed service credits cannot be released");if(String(usage.status)!=="reserved")throw new Error(`Only reserved credits can be released; current status is ${String(usage.status)}`);const reserved=Number(usage.sessions_reserved||0);
  const releasedNow=await db.prepare("UPDATE booking_subscription_usage SET status='released',updated_at=? WHERE booking_id=? AND status='reserved'").bind(now,input.bookingId).run();
  if(!Number(releasedNow.meta?.changes||0))throw new Error("Only reserved credits can be released; they were already moved");
  await db.prepare("UPDATE customer_grooming_subscriptions SET sessions_reserved=MAX(0,sessions_reserved-?),status=CASE WHEN status='exhausted' AND total_sessions-sessions_consumed>0 THEN 'active' ELSE status END,updated_at=? WHERE id=?").bind(reserved,now,input.subscriptionId).run();
  const current=await activeSubscription(db,input.subscriptionId);await addEvent(db,input,"released",reserved,balance(current),{bookingStatus:booking.status,reason:releaseReason});return{duplicatePrevented:false,wallet:await readSubscriptionWallet(db,input.subscriptionId)};}
