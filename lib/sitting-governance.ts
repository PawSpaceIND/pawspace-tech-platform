...
export async function ensureSittingGovernanceTables(db:D1Database){
 ...
 await db.batch([
  ...
  db.prepare("CREATE TABLE IF NOT EXISTS sitting_commercial_quotes (id TEXT PRIMARY KEY,customer_id TEXT,package_code TEXT NOT NULL,package_version INTEGER NOT NULL,mode TEXT NOT NULL,pet_count INTEGER NOT NULL,scheduled_start TEXT NOT NULL,scheduled_end TEXT NOT NULL,billable_units INTEGER NOT NULL,payment_mode TEXT NOT NULL,total_amount REAL NOT NULL,amount_due_now REAL NOT NULL,expires_at INTEGER NOT NULL,status TEXT NOT NULL DEFAULT 'open',created_at INTEGER NOT NULL,used_at INTEGER,used_booking_id TEXT)"),
