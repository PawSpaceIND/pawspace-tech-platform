...
export async function processGatewayEvent(db:Db,event:GatewayEvent){
 ...
 await postBookingCollectionOnCapture(db,{bookingId,paymentId,eventId:event.eventId}).catch(error=>{console.warn("[collection-ledger] capture posting deferred",error instanceof Error?error.message:String(error));});
    ...
    const failedPayment=await db.prepare("UPDATE booking_payments SET status='failed',gateway=?,detail_json=json_set(detail_json,'$.lastGatewayEventId',?),updated_at=? WHERE id=? AND status NOT IN ('captured','refunded','partially_refunded')").bind(event.environment==="sandbox"?"razorpay_sandbox":"razorpay",event.eventId,now,paymentId).run();
