...
type SupportedGatewayEventType="payment.authorized"|"payment.captured"|"order.paid"|"payment_link.paid"|"payment.failed"|"refund.created"|"refund.processed"|"refund.failed";
function canonicalGatewayEventType(value:string):SupportedGatewayEventType|null{
  switch(value){
    case "payment.authorized":return "payment.authorized";
    case "payment.captured":return "payment.captured";
    case "order.paid":return "order.paid";
    case "payment_link.paid":return "payment_link.paid";
    case "payment.failed":return "payment.failed";
    case "refund.created":return "refund.created";
    case "refund.processed":return "refund.processed";
    case "refund.failed":return "refund.failed";
    default:return null;
  }
}
 ...
 */
export function processedRefundStatements(db:Db,input:{bookingId:string;paymentId:string;refundCaseId:string;gatewayRefundId:string|null;eventId:string;provider:string;environment:string;expected:number;capturedCurrent:number;currency:string;now:number;purposeAware?:boolean}){
 ...
 return[
  ...
  // A refunded reschedule difference that never moved the booking returned an ADDITIONAL payment: the
  // booking's own price is still fully paid, so a captured booking stays 'captured' (its invoice, its
  // confirmation and a later paid reschedule all require it). The gateway record keeps the refund.
  input.purposeAware
    ?db.prepare("UPDATE booking_payments SET status=CASE WHEN status='captured' AND NOT EXISTS (SELECT 1 FROM booking_refund_cases c WHERE c.booking_id=? AND c.payment_id=? AND c.status IN ('processed','completed') AND COALESCE(c.purpose,'')<>'reschedule_difference') THEN 'captured' ELSE (SELECT gateway_status FROM payment_reconciliation_records WHERE payment_id=?) END,detail_json=json_set(detail_json,'$.lastGatewayEventId',?,'$.lastGatewayRefundId',?),updated_at=? WHERE id=?").bind(bookingId,paymentId,paymentId,input.eventId,input.gatewayRefundId,now,paymentId)
...
async function resolvePayment(db:Db,event:GatewayEvent){if(event.bookingId){const payment=await db.prepare("SELECT * FROM booking_payments WHERE booking_id=?").bind(event.bookingId).first<Row>();if(payment)return{bookingId:event.bookingId,payment};}if(event.gatewayPaymentLinkId){const link=await db.prepare("SELECT booking_id,payment_id FROM payment_gateway_links WHERE gateway_payment_link_id=?").bind(event.gatewayPaymentLinkId).first<Row>();if(link){const payment=await db.prepare("SELECT * FROM booking_payments WHERE id=?").bind(link.payment_id).first<Row>();if(payment)return{bookingId:String(link.booking_id),payment};}}if(event.gatewayPaymentId){const link=await db.prepare("SELECT booking_id,payment_id FROM payment_gateway_links WHERE gateway_payment_id=?").bind(event.gatewayPaymentId).first<Row>();if(link){const payment=await db.prepare("SELECT * FROM booking_payments WHERE id=?").bind(link.payment_id).first<Row>();if(payment)return{bookingId:String(link.booking_id),payment};}}if(event.gatewayOrderId){const link=await db.prepare("SELECT booking_id,payment_id FROM payment_gateway_links WHERE gateway_order_id=?").bind(event.gatewayOrderId).first<Row>();if(link){const payment=await db.prepare("SELECT * FROM booking_payments WHERE id=?").bind(link.payment_id).first<Row>();if(payment)return{bookingId:String(link.booking_id),payment};}}
 // An ADDITIONAL payment on a booking (a reschedule difference) has its own intent and order and is never
 // written onto the booking's gateway link, so a refund of it resolves through that intent.
 if(String(event.eventType||"").startsWith("refund.")&&(event.gatewayPaymentId||event.gatewayOrderId)){const intent=await db.prepare("SELECT booking_id,payment_id FROM payment_intents WHERE provider='razorpay' AND environment=? AND ((?<>'' AND gateway_payment_id=?) OR (?<>'' AND gateway_order_id=?)) LIMIT 1").bind(event.environment,event.gatewayPaymentId??"",event.gatewayPaymentId??"",event.gatewayOrderId??"",event.gatewayOrderId??"").first<Row>().catch(()=>null);if(intent){const payment=await db.prepare("SELECT * FROM booking_payments WHERE id=? AND booking_id=?").bind(intent.payment_id,intent.booking_id).first<Row>().catch(()=>null);if(payment)return{bookingId:String(intent.booking_id),payment};}}
 return null;}
...
export async function processGatewayEvent(db:Db,event:GatewayEvent){
  ...
  const eventType=canonicalGatewayEventType(event.eventType);
  const existing=await db.prepare("SELECT * FROM payment_gateway_events WHERE provider=? AND event_id=?").bind(event.provider,event.eventId).first<Row>();
  const refundReplay=existing&&String(existing.event_type)==="refund.processed"&&["received","processed"].includes(String(existing.processing_status));
  if(existing&&!refundReplay){
  ...
  // Resume only the signed refund event already persisted. Reusing an event id with a
  // different payload must not redirect its accounting repair to another booking.
  if(refundReplay){
    if(existing.payload_hash!==event.payloadHash||existing.environment!==event.environment||existing.event_type!==event.eventType||existing.gateway_refund_id!==(event.gatewayRefundId??null)||Number(existing.amount_subunits)!==Number(event.amountSubunits)||existing.currency!==(event.currency??null)||(event.bookingId&&existing.booking_id!==event.bookingId))throw new Error("Refund event replay does not match the recorded payload");
    event={...event,bookingId:String(existing.booking_id||event.bookingId||"")||undefined};
  }
  const now=Date.now(),rowId=existing?String(existing.id):`PAYEV-${crypto.randomUUID().slice(0,12).toUpperCase()}`;if(!existing)await db.prepare("INSERT INTO payment_gateway_events (id,provider,environment,event_id,event_type,booking_id,payment_id,gateway_order_id,gateway_payment_id,gateway_refund_id,amount_subunits,currency,signature_verified,payload_hash,processing_status,detail_json,received_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,1,?,'received',?,?)").bind(rowId,event.provider,event.environment,event.eventId,event.eventType,event.bookingId??null,null,event.gatewayOrderId??null,event.gatewayPaymentId??null,event.gatewayRefundId??null,event.amountSubunits??null,event.currency??null,event.payloadHash,JSON.stringify(event.detail||{}),now).run();
  ...
  const lifecycleStatement=(eventType:string,detail:unknown)=>db.prepare("INSERT OR IGNORE INTO booking_lifecycle_events (id,booking_id,event_type,entity_type,entity_id,actor_id,detail_json,occurred_at) VALUES (?,?,?,?,?,?,?,?)").bind(`PAYREF-${event.provider}-${event.environment}-${event.gatewayRefundId??event.eventId}`,bookingId,eventType,"payment",bookingId,"razorpay_webhook",JSON.stringify(detail),now);
  // Lower gateway states are allowed to arrive out of order, but they must never rewrite money that
  // the primary already considers captured/refunded. The WHERE is evaluated by SQLite at the write
  // boundary, so it still protects us when a preceding D1 read came from a lagging replica.
  const upsert=async(status:string,recon:string,captured:number,refunded:number,variance:number,preserveSettled=false)=>db.prepare(`INSERT INTO payment_reconciliation_records (payment_id,booking_id,gateway,environment,expected_amount,captured_amount,refunded_amount,currency,gateway_status,reconciliation_status,variance_amount,last_event_id,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(payment_id) DO UPDATE SET gateway=excluded.gateway,environment=excluded.environment,expected_amount=excluded.expected_amount,captured_amount=excluded.captured_amount,refunded_amount=excluded.refunded_amount,currency=excluded.currency,gateway_status=excluded.gateway_status,reconciliation_status=excluded.reconciliation_status,variance_amount=excluded.variance_amount,last_event_id=excluded.last_event_id,updated_at=excluded.updated_at${preserveSettled?" WHERE payment_reconciliation_records.gateway_status NOT IN ('captured','refunded','partially_refunded')":""}`).bind(paymentId,bookingId,event.provider,event.environment,expected,captured,refunded,currency,status,recon,variance,event.eventId,now).run();
  const current=await db.prepare("SELECT captured_amount,refunded_amount,gateway_status,reconciliation_status,variance_amount,last_event_id FROM payment_reconciliation_records WHERE payment_id=?").bind(paymentId).first<Row>();const capturedCurrent=Number(current?.captured_amount||0),refundedCurrent=Number(current?.refunded_amount||0),gatewayCurrent=String(current?.gateway_status||"not_started"),reconCurrent=String(current?.reconciliation_status||"pending"),varianceCurrent=Number(current?.variance_amount||0);const settled=["captured","refunded","partially_refunded"].includes(gatewayCurrent)||["captured","refunded","partially_refunded"].includes(String(payment.status));
  ...
  if(eventType==="payment.authorized"){
    ...
    const written=await upsert("authorized","pending",capturedCurrent,refundedCurrent,0,true);
    if(Number(written.meta?.changes||0)===0){await finish("processed","Out-of-order authorization ignored after settled state");return{duplicate:false,status:"processed",ignored:true,reason:"out_of_order_authorized"};}
  }else if(eventType==="payment.captured"||eventType==="order.paid"||eventType==="payment_link.paid"){
    ...
    // A refused capture is an exception and a variance; the collected total stays what it was. Writing THIS
    // capture's amount here wiped out every earlier capture (Rs 1,899 + Rs 285 collected read Rs 99 after
    // one short difference capture), under-reporting collections and every refund ceiling built on them.
    if(!amountGuard.ok){await upsert("captured","amount_mismatch",capturedCurrent,refundedCurrent,variance);await addException(db,{bookingId,paymentId,eventId:event.eventId,type:"capture_amount_mismatch",detail:{expected,variance,...amountGuard}});await finish("exception",amountGuard.reason||"Capture amount mismatch");return{duplicate:false,status:"exception",reason:amountGuard.reason||"capture_amount_mismatch"};}
 ...
 await postBookingCollectionOnCapture(db,{bookingId,paymentId,eventId:event.eventId}).catch(error=>{console.warn("[collection-ledger] capture posting deferred",error instanceof Error?error.message:String(error));});
    ...
    if(settled){await finish("processed","Out-of-order failure ignored after settled state");return{duplicate:false,status:"processed",ignored:true,reason:"out_of_order_failed"};}
    const failedPayment=await db.prepare("UPDATE booking_payments SET status='failed',gateway=?,detail_json=json_set(detail_json,'$.lastGatewayEventId',?),updated_at=? WHERE id=? AND status NOT IN ('captured','refunded','partially_refunded') AND NOT EXISTS (SELECT 1 FROM payment_reconciliation_records WHERE payment_id=? AND gateway_status IN ('captured','refunded','partially_refunded'))").bind(event.environment==="sandbox"?"razorpay_sandbox":"razorpay",event.eventId,now,paymentId,paymentId).run();
    if(Number(failedPayment.meta?.changes||0)===0){await finish("processed","Out-of-order failure ignored after settled state");return{duplicate:false,status:"processed",ignored:true,reason:"out_of_order_failed"};}
    const failedRecon=await upsert("failed","gateway_failed",capturedCurrent,refundedCurrent,0,true);
    if(Number(failedRecon.meta?.changes||0)===0){await finish("processed","Out-of-order failure ignored after settled state");return{duplicate:false,status:"processed",ignored:true,reason:"out_of_order_failed"};}
    await lifecycle(db,bookingId,"payment_failed",{gateway:event.provider,eventId:event.eventId});
  ...
  }else if(eventType!==null&&["refund.created","refund.processed","refund.failed"].includes(eventType)){
    ...
    const expectedRefund=Number(refund.amount||0),sameGatewayRefund=Boolean(event.gatewayRefundId&&String(refund.gateway_reference||"")===event.gatewayRefundId),alreadyProcessed=["processed","completed"].includes(String(refund.status))&&sameGatewayRefund;
    // Legacy partial commits may already contain the refund total but not its ledger marker.
    // Identify that fact from the persisted refund event, never from the replay's amount alone.
    const priorRefundFact=alreadyProcessed&&event.gatewayRefundId?await db.prepare("SELECT id FROM booking_lifecycle_events WHERE booking_id=? AND event_type='refund_processed' AND json_extract(detail_json,'$.gatewayRefundId')=? LIMIT 1").bind(bookingId,event.gatewayRefundId).first<Row>():null;
    const priorReconciledRefund=alreadyProcessed&&current?.last_event_id?await db.prepare("SELECT id FROM payment_gateway_events WHERE provider=? AND event_id=? AND payment_id=? AND event_type='refund.processed' AND gateway_refund_id=?").bind(event.provider,current.last_event_id,paymentId,event.gatewayRefundId??null).first<Row>():null;
    const refundAlreadyCounted=Boolean(priorRefundFact||priorReconciledRefund);
    if(eventType==="refund.processed"&&alreadyProcessed&&refundAlreadyCounted){
      ...
      if(["refunded","partially_refunded",...(String(refund.purpose??"")==="reschedule_difference"?["captured"]:[])].includes(String(payment.status))&&refundedCurrent>=expectedRefund&&posted){await finish("processed","Duplicate logical refund ignored");return{duplicate:Boolean(existing),status:"processed",ignored:true,reason:"refund_already_processed"};}
    }
    if(eventType!=="refund.failed"&&Math.abs(amount-expectedRefund)>0.009){await addException(db,{bookingId,paymentId,eventId:event.eventId,type:"refund_amount_mismatch",detail:{expected:expectedRefund,received:amount}});await finish("exception","Refund amount mismatch");return{duplicate:false,status:"exception",reason:"refund_amount_mismatch"};}
    if(eventType==="refund.created"&&!alreadyProcessed)await db.prepare("UPDATE booking_refund_cases SET status='processing',gateway_reference=?,updated_at=? WHERE id=?").bind(event.gatewayRefundId??null,now,refund.id).run();
    ...
    if(eventType==="refund.processed"){
      // Totals are derived from the canonical processed cases INSIDE the transaction.
      // Pre-reading refundedCurrent and adding in JavaScript loses one of two concurrent refunds.
      ...
      /* FIN-D6: refund facts, canonical journal/marker and inbox completion commit as ONE batch.
      ...
      await db.batch([
        ...ledger.statements,
        ...processedRefundStatements(db,{bookingId,paymentId,refundCaseId:String(refund.id),gatewayRefundId:event.gatewayRefundId??null,eventId:event.eventId,provider:event.provider,environment:event.environment,expected,capturedCurrent,currency,now,purposeAware:"purpose" in refund}),
  ...
  }else{await upsert(gatewayCurrent||eventType||"unsupported_event","ignored",capturedCurrent,refundedCurrent,varianceCurrent);}
