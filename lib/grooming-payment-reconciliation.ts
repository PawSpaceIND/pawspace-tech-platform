...
export async function createPostServicePaymentRequest(db:Db,env:Record<string,unknown>,input:{bookingId:string;providerId:string;actorId:string}){
 ...
 const now=Date.now(),id=String(link.paymentLink.id),path=String(link.paymentLink.short_url),qrPayload=path,expiresAt=Number(link.paymentLink.expire_by)*1000,gatewayLinkId=`PAYLINK-${crypto.randomUUID().slice(0,10).toUpperCase()}`;
 // Persist the provider reference, request and canonical reconciliation mapping atomically. Without
 // the gateway link row, a verified capture could update booking_payments (via notes.booking_id) but
 // could not retain its gateway payment id for the governed refund path.
 await db.batch([
  db.prepare("INSERT INTO post_service_payment_requests (id,booking_id,payment_id,provider_id,amount,currency,status,payment_path,qr_payload,expires_at,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,'awaiting_payment',?,?,?,?,?,?)").bind(id,input.bookingId,row.payment_id,input.providerId,Number(row.amount||0),String(row.currency||"INR"),path,qrPayload,expiresAt,input.actorId,now,now),
  db.prepare("INSERT INTO payment_gateway_links (id,booking_id,payment_id,provider,environment,gateway_order_id,status,created_at,updated_at) VALUES (?,?,?,?,?,?,'active',?,?) ON CONFLICT(booking_id) DO UPDATE SET gateway_order_id=excluded.gateway_order_id,provider=excluded.provider,environment=excluded.environment,status='active',updated_at=excluded.updated_at").bind(gatewayLinkId,input.bookingId,row.payment_id,"razorpay","sandbox",id,now,now),
  db.prepare("INSERT INTO payment_reconciliation_records (payment_id,booking_id,gateway,environment,expected_amount,captured_amount,refunded_amount,currency,gateway_status,reconciliation_status,variance_amount,last_event_id,updated_at) VALUES (?,?,?,?,?,0,0,?,'payment_linked','pending',0,NULL,?) ON CONFLICT(payment_id) DO UPDATE SET gateway=excluded.gateway,environment=excluded.environment,expected_amount=excluded.expected_amount,currency=excluded.currency,gateway_status=CASE WHEN payment_reconciliation_records.gateway_status IN ('captured','refunded','partially_refunded') THEN payment_reconciliation_records.gateway_status ELSE 'payment_linked' END,updated_at=excluded.updated_at").bind(row.payment_id,input.bookingId,"razorpay","sandbox",Number(row.amount||0),String(row.currency||"INR"),now),
 ]);
 return{id,bookingId:input.bookingId,amount:Number(row.amount||0),currency:String(row.currency||"INR"),status:"awaiting_payment",paymentStatus:String(row.payment_status),paymentPath:path,qrPayload,providerReference:id,collectable:true,expiresAt,sandboxOnly:true,liveCapture:false};
}

function paymentRequestView(row:Row,paymentStatus:string){const paymentPath=String(row.payment_path),expiresAt=Number(row.expires_at),settled=["captured","refunded","partially_refunded"].includes(paymentStatus);return{id:String(row.id),bookingId:String(row.booking_id),amount:Number(row.amount||0),currency:String(row.currency||"INR"),status:settled?paymentStatus:String(row.status),paymentStatus,paymentPath,qrPayload:String(row.qr_payload),providerReference:String(row.id),collectable:!settled&&expiresAt>Date.now()&&paymentPath.startsWith("https://"),expiresAt,sandboxOnly:true,liveCapture:false};}
