...
export async function buildWhatsAppAiAnalytics(db: Db, input: { from?: number; to?: number } = {}) {
  ...
  const [messages, delivery, templateMessages, templateDelivery, response, resolution, ai, chatbot, agents, queues, consent, attribution, feedback, sources] = await Promise.all([
    ...
    db.prepare("SELECT template_key,status,COUNT(*) count FROM communication_messages WHERE channel='whatsapp' AND direction='outbound' AND template_key IS NOT NULL AND template_key!='' AND created_at>=? AND created_at<=? GROUP BY template_key,status ORDER BY template_key,status").bind(from, to).all<Row>(),
    db.prepare("SELECT m.template_key,e.event_type,COUNT(*) count FROM communication_message_delivery_events e JOIN communication_messages m ON m.id=e.message_id WHERE m.channel='whatsapp' AND m.direction='outbound' AND m.template_key IS NOT NULL AND m.template_key!='' AND e.created_at>=? AND e.created_at<=? GROUP BY m.template_key,e.event_type ORDER BY m.template_key,e.event_type").bind(from, to).all<Row>(),
    db.prepare("SELECT AVG(first_outbound-first_inbound) avg_ms,COUNT(*) threads FROM (SELECT inbound.thread_id,inbound.first_inbound,(SELECT MIN(reply.created_at) FROM communication_messages reply WHERE reply.thread_id=inbound.thread_id AND reply.channel='whatsapp' AND reply.direction='outbound' AND reply.created_at>=inbound.first_inbound AND reply.created_at<=?) first_outbound FROM (SELECT thread_id,MIN(created_at) first_inbound FROM communication_messages WHERE channel='whatsapp' AND direction='inbound' AND created_at>=? AND created_at<=? GROUP BY thread_id) inbound) WHERE first_outbound IS NOT NULL").bind(to, from, to).first<Row>(),
  ...
  return {
    ...
    whatsapp: {
      funnel: {
        ...
        sentMessages: countWhere(messageRows, (row) => text(row.direction) === "outbound" && ["sent", "delivered", "read", "queued"].includes(text(row.status))),
