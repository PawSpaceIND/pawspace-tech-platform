...
export async function upsertCustomerPet(input: { customerId?: string; idempotencyKey?: string; pet: PetProfileInput }) { const response=await fetch("/api/customer-account",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({customerId:input.customerId,action:"upsert_pet",idempotencyKey:input.idempotencyKey||`pet-manager:${crypto.randomUUID()}`,pet:input.pet})}); return payload<{entityId:string;duplicatePrevented?:boolean}>(response,"Unable to save the pet"); }
