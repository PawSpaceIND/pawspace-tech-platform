...
export async function requiredPermission(request:Request):Promise<Permission|null>{const url=new URL(request.url),method=request.method.toUpperCase();if(url.pathname==="/api/pricing-quote"||url.pathname==="/api/training-commercial"||url.pathname==="/api/training-trainers"||url.pathname==="/api/boarding-commercial"||url.pathname==="/api/sitting-commercial"||url.pathname==="/api/taxi-commercial"||url.pathname==="/api/food-commercial"||url.pathname==="/api/walking-commercial"||url.pathname==="/api/razorpay-webhook"||url.pathname==="/api/razorpay-checkout-return"||url.pathname==="/api/razorpayx-webhook"||url.pathname==="/api/voice-provider-webhook"||url.pathname==="/api/webhooks/exotel/call-event"||url.pathname==="/api/provider-verification-callback"||url.pathname==="/api/communication-provider-callback"||url.pathname==="/api/haptik"||url.pathname==="/api/whatsapp-uat-webhook"||url.pathname==="/api/whatsapp/meta-webhook"||url.pathname==="/api/email-provider-webhook"||url.pathname==="/api/identity-session"||url.pathname==="/api/service-availability"||url.pathname==="/api/public-contact"||url.pathname==="/api/inquiries"||url.pathname==="/api/provider-public-profile"||url.pathname==="/api/staging-login"||url.pathname==="/api/partner-otp"||url.pathname==="/api/pet-passport-public"
  ...
  if(url.pathname==="/api/walking-lifecycle"){if(method==="GET")return url.searchParams.get("scope")==="customer"?"scheduling.book":"bookings.view";const body=await request.clone().json().catch(()=>({})) as Record<string,unknown>,action=String(body.action||"");if(action==="no_show")return "bookings.manage";if(action==="request_session_reschedule")return "scheduling.book";return "bookings.view";}
  ...
  if(url.pathname==="/api/taxi-fleet-control")return "settings.manage";
  if(url.pathname==="/api/taxi-bookings")return "scheduling.book";
  if(url.pathname==="/api/taxi-ride-bookings")return "scheduling.book";
  if(url.pathname==="/api/taxi-adjustments"){if(method==="GET")return url.searchParams.get("scope")==="customer"?"scheduling.book":"bookings.view";return "bookings.view";}
  ...
  /* scope=finance is a READ ONLY, and only on GET. /team/finance/relocation records payment evidence
   * and resolves refunds on relocation cases, but the unscoped staff read is a booking read, and the
   * `finance` role holds neither bookings.view nor the customers.view the handler then demands - so
   * the screen was unreachable for the only role whose job it is. The handler answers this scope with
   * an allow-list of the commercial fields alone (no customer id, no pet, no route, no documents), and
   * every mutation below is untouched: a finance write is still finance.manage. Same shape as the
   * scope=customer and scope=provider reads already registered here and in lib/session-api-gateway.ts. */
  if(url.pathname==="/api/relocation"){if(method==="GET"){const scope=url.searchParams.get("scope");if(scope==="customer")return "scheduling.book";if(scope==="finance")return "finance.view";return "bookings.view";}const body=await request.clone().json().catch(()=>({})) as Record<string,unknown>,action=String(body.action||"create");if(["create","register_document","accept_quote","request_refund","open_support"].includes(action))return "scheduling.book";if(["record_payment","resolve_refund","approve_vendor_settlement","reconcile_finance"].includes(action))return "finance.manage";return "bookings.manage";}
  /* Same finance-scoped read, for the same reason, for /team/finance/funeral-memorial. GET only, and
   * the handler answers it with the commercial fields alone - the pickup address and alternate
   * contact this record carries are exactly what the unscoped read's customers.view guard exists to
   * protect, and they stay out of it. set_service_amount, record_payment and resolve_refund keep
   * requiring finance.manage. */
  if(url.pathname==="/api/funeral-memorial"){if(method==="GET"){if(url.searchParams.get("config")==="1")return "pricing.view";if(url.searchParams.get("report")==="summary")return "reports.view";const scope=url.searchParams.get("scope");if(scope==="customer")return "scheduling.book";if(scope==="finance")return "finance.view";return "bookings.view";}const body=await request.clone().json().catch(()=>({})) as Record<string,unknown>,action=String(body.action||"create");if(action==="save_service_config")return "pricing.manage";if(["create","register_customer_media","schedule_ash_collection","request_refund","open_support"].includes(action))return "scheduling.book";if(["set_service_amount","record_payment","resolve_refund","set_vendor_cost","approve_vendor_settlement","reconcile_finance"].includes(action))return "finance.manage";return "bookings.manage";}
