...
export async function recordAtomicDeliveryEvent(db:D1Database,input:{messageId:string;provider:string;eventId:string;eventType:DeliveryEventType;providerReference?:string|null;detail?:Record<string,unknown>;now?:number}){
 ...
 const now=input.now??Date.now(),current=text(row.status),apply=shouldApplyDeliveryTransition(current,input.eventType),statements:D1PreparedStatement[]=[];
 ...
 if(apply&&input.eventType==="failed"){
 ...
 }else if(apply&&input.eventType!=="failed"){status=nextStatus[input.eventType];outboxStatus=input.eventType==="accepted"?"sent":input.eventType;statements.push(db.prepare("UPDATE communication_outbox SET status=?,last_error=NULL,locked_at=NULL,updated_at=? WHERE message_id=?").bind(outboxStatus,now,input.messageId));}
 ...
 statements.unshift(db.prepare("INSERT OR IGNORE INTO communication_message_delivery_events (id,message_id,provider,event_id,event_type,detail_json,created_at) VALUES (?,?,?,?,?,?,?)").bind(crypto.randomUUID(),input.messageId,input.provider,input.eventId,input.eventType,JSON.stringify(eventDetail),now));
