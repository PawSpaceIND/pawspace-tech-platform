...
export async function recordAtomicDeliveryEvent(db:D1Database,input:{messageId:string;provider:string;eventId:string;eventType:DeliveryEventType;providerReference?:string|null;detail?:Record<string,unknown>;now?:number}){
 ...
 }else if(apply){status=nextStatus[input.eventType];outboxStatus=input.eventType==="accepted"?"sent":input.eventType;statements.push(db.prepare("UPDATE communication_outbox SET status=?,last_error=NULL,locked_at=NULL,updated_at=? WHERE message_id=?").bind(outboxStatus,now,input.messageId));}
