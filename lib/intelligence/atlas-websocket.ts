...
import { executeAtlasApprovedAction, listAtlasMessages, recordAtlasMessage, requireAtlasApprovalAssurance, requireFounderRole } from "./atlas-data";
...
export async function handleAtlasWebSocket(request:Request){
 ...
 server.addEventListener("message",event=>{void(async()=>{try{
  ...
  if(type==="approve"){const messageId=String(payload.messageId||"").trim(),approvalAssurance=await requireAtlasApprovalAssurance(db,request,actor);await recordAtlasMessage(db,{role:"founder",actorEmail:actor.email,content:"Yes, execute.",replyToId:messageId});send({type:"action_result",data:await executeAtlasApprovedAction(db,{messageId,actorEmail:actor.email,actorRoleCode:actor.roleCode,approvalAssurance})});return}
  if(type==="ask"){
