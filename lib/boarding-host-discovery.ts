...
export async function discoverBoardingHosts(db:D1Database,input:BoardingHostDiscoveryInput){
 ...
 for(const row of candidates.results){
  ...
  let reservations:{group_id:string;capacity_units:number;scheduled_start:string;scheduled_end:string}[]=[];const schedulingTable=await db.prepare("SELECT 1 present FROM sqlite_master WHERE type='table' AND name='scheduling_reservations'").first<Row>();if(schedulingTable){const scheduled=await db.prepare("SELECT group_id,capacity_units,scheduled_start,scheduled_end FROM scheduling_reservations WHERE provider_id=? AND service_code='boarding' AND status!='cancelled' AND scheduled_start<? AND scheduled_end>?").bind(row.provider_id,input.scheduledEnd,input.scheduledStart).all<Row>();reservations=scheduled.results.filter(item=>overlaps(input.scheduledStart,input.scheduledEnd,item)).map(item=>({group_id:String(item.group_id),capacity_units:Number(item.capacity_units||0),scheduled_start:String(item.scheduled_start),scheduled_end:String(item.scheduled_end)}));}
