...
export function mfaEligibleRole(roleCode:string){const role=String(roleCode).toLowerCase();return role==="founder"||PRIVILEGED.has(role);}
...
export async function validPrivilegedSessionProof(db:Db,request:Request,userId:string){await ensureAdminMfaTables(db);const raw=readCookie(request,COOKIE);if(!raw)return null;const hash=await sha256(raw);return db.prepare("SELECT id,mfa_verified_at,issued_at,expires_at FROM active_sessions WHERE user_id=? AND token_hash=? AND revoked_at IS NULL AND expires_at>? LIMIT 1").bind(userId,hash,Date.now()).first<Row>();}
export async function hasValidPrivilegedSession(db:Db,request:Request,userId:string){return Boolean(await validPrivilegedSessionProof(db,request,userId));}
