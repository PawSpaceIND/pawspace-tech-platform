...
function metaPayload(input:{eventType:MarketingConversionEvent;businessReference:string;occurredAt:number;customerId:string;bookingId?:string|null;valueMinor:number;currency:string;attribution:Row}){
 const eventName=input.eventType==="lead_qualified"?"Lead":input.eventType==="booking_created"?"InitiateCheckout":"Purchase";
 const userData:Record<string,unknown>={external_id:[input.customerId]};
 const fbc=text(input.attribution.fbclid);if(fbc)userData.fbc=`fb.1.${Math.trunc(input.occurredAt/1000)}.${fbc}`;
...
export async function recordMarketingConversionFact(db:Db,input:{eventType:MarketingConversionEvent;businessReference:string;leadId:string;customerId:string;bookingId?:string|null;paymentId?:string|null;valueMinor?:number;currency?:string;occurredAt?:number}){
 await ensureFirstPartyMarketingAttribution(db);
 const businessReference=text(input.businessReference);if(!businessReference)throw new Error("Conversion business reference is required");
 const existing=await db.prepare("SELECT * FROM marketing_conversion_facts WHERE event_type=? AND business_reference=?").bind(input.eventType,businessReference).first<Row>();
 let factId=text(existing?.id),duplicatePrevented=Boolean(existing);const occurredAt=input.occurredAt??Date.now(),valueMinor=Math.max(0,Math.trunc(Number(input.valueMinor||0))),currency=text(input.currency)||"INR";
 ...
 const attribution=await attributionForLead(db,input.leadId);let queued=0;
 if(attribution){const google=googleAdIdentifier(attribution as MarketingAttributionCapture),fbclid=text(attribution.fbclid),now=Date.now();
...
export async function dispatchMarketingConversionOutbox(db:Db,runtime:Runtime,input:{now?:number;limit?:number;validateOnly?:boolean;fetchImpl?:Fetcher}={}){
 await ensureFirstPartyMarketingAttribution(db);const now=input.now??Date.now(),limit=Math.min(100,Math.max(1,input.limit||25)),fetchImpl=input.fetchImpl||fetch;
 const rows=await db.prepare("SELECT * FROM marketing_conversion_outbox WHERE status IN ('pending','retry') AND next_attempt_at<=? ORDER BY created_at LIMIT ?").bind(now,limit).all<Row>();const results:Record<string,unknown>[]=[];
 for(const row of rows.results){const id=text(row.id),platform=text(row.platform),attempts=Number(row.attempts||0)+1;try{
   let requestId="";let externalMutation=false;
   if(platform==="meta"){
    if(!truthy(runtime.PAWSPACE_META_CAPI_UPLOAD_ENABLED))throw new Error("Meta CAPI upload is disabled");
    ...
    const response=await fetchImpl(`https://graph.facebook.com/${version}/${pixelId}/events`,{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:text(row.payload_json)});const body=await providerJson(response,"Meta CAPI");requestId=text(response.headers.get("x-fb-trace-id")||body.fbtrace_id);externalMutation=true;
