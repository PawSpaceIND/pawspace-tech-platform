...
function indiaDate(value:string|number|Date){if(typeof value==="string"){const text=value.trim();if(/^\d{4}-\d{2}-\d{2}/.test(text))return text.slice(0,10);const parsed=Date.parse(text);if(!Number.isFinite(parsed))throw new Error("Invalid TCS effective date");return new Date(parsed+IST_OFFSET_MS).toISOString().slice(0,10);}const time=value instanceof Date?value.getTime():Number(value);if(!Number.isFinite(time))throw new Error("Invalid TCS effective date");return new Date(time+IST_OFFSET_MS).toISOString().slice(0,10);}
