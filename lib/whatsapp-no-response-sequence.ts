...
export async function processDueWhatsAppNoResponseSequences(db: D1Database, input: { now?: number; limit?: number; actorEmail?: string } = {}) {
  ...
  for (const step of due.results || []) {
    ...
    const queued = await queueWhatsAppUatOutbound(db, { provider, threadId, customerId, text: template.body, idempotencyKey: text(step.idempotency_key), createdBy: actorEmail, templateKey: text(step.template_key), language: template.language });
