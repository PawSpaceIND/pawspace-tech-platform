...
export async function saveWhatsAppNoResponseConfig(db: D1Database, input: { enabled: boolean; templateKeys: string[]; offerType: string; offerReference: string; actorEmail: string }) {
  ...
  if (!/^[a-z0-9_]{3,64}$/.test(offerType)) throw new Response("Offer type must be a stable lowercase business identifier", { status: 400 });
...
async function cancelAllActive(db: D1Database, reason: string, now: number) {
  const active = await db.prepare("SELECT id FROM whatsapp_no_response_sequences WHERE status='active'").all<Row>();
...
export async function cancelWhatsAppNoResponseSequences(db: D1Database, input: { threadId: string; reason: string; now?: number }) {
  ...
  for (const row of active.results || []) await cancelSequence(db, text(row.id), text(input.reason) || "cancelled", now);
  return { cancelled: (active.results || []).length, threadId: input.threadId, reason: text(input.reason) || "cancelled", externalDelivery: false };
}
...
export async function armWhatsAppNoResponseSequence(db: D1Database, input: { threadId: string; customerId: string; anchorMessageId: string; routingMode: WhatsAppRecoveryRoutingMode; now?: number }) {
  ...
  const templates = config.templateKeys;
  const steps = whatsappRecoveryDelaysMinutes.map((delay, index) => ({
    id: uid("WARECSTEP"), index: index + 1, delay, dueAt: anchorCreatedAt + delay * 60_000, templateKey: templates[index],
...
export async function processDueWhatsAppNoResponseSequences(db: D1Database, input: { now?: number; limit?: number; actorEmail?: string } = {}) {
  ...
  const config = await getWhatsAppNoResponseConfig(db);
  if (!config.enabled) { await cancelAllActive(db, "automation_disabled", now); return { processed: 0, queued: 0, cancelled: 0, blocked: 0, externalDelivery: false, environment: "uat" }; }
  const due = await db.prepare("SELECT s.*,q.thread_id,q.customer_id,q.provider,q.routing_mode,q.anchor_message_id,q.anchor_created_at,q.status AS sequence_status,q.offer_type,q.offer_reference FROM whatsapp_no_response_steps s JOIN whatsapp_no_response_sequences q ON q.id=s.sequence_id WHERE q.status='active' AND s.due_at<=? AND (s.status='pending' OR (s.status='processing' AND s.updated_at<=?)) ORDER BY s.due_at ASC LIMIT ?")
  ...
  for (const step of due.results || []) {
    ...
    const thread = await db.prepare("SELECT status,booking_id,assigned_to FROM communication_threads WHERE id=? AND customer_id=?").bind(threadId, customerId).first<Row>();
    ...
    if (routing.mode !== text(step.routing_mode)) { await cancelSequence(db, sequenceId, routing.mode === "human_only" ? "human_takeover" : "routing_changed", now); results.push({ stepId, status: "cancelled", reason: routing.mode === "human_only" ? "human_takeover" : "routing_changed" }); continue; }
  ...
  return { processed: results.length, queued: results.filter((row) => row.status === "queued").length, cancelled: results.filter((row) => row.status === "cancelled").length, blocked: results.filter((row) => row.status === "blocked").length, results, externalDelivery: false, environment: "uat" };
}
