...
[data,setData]=useState<FoodSubscriptionSnapshot|null>(null),[loadedId,setLoadedId]=useState(""),[error,setError]=useState(""),[busy,setBusy]=useState("");useEffect(()=>{if(!renewalId)return;void loadFoodSubscription({renewalId}).then(snapshot=>{setData(snapshot);setLoadedId(renewalId)}).catch(problem=>{setError(problem instanceof Error?problem.message:"Unable to load payment request");setLoadedId(renewalId)})},[renewalId]);const renewal=data?.renewals.find(row=>String(row.id)===renewalId);
 ...
 if(screen!=="ready"){
  ...
  return <main style={{maxWidth:720,margin:"0 auto",padding:32,fontFamily:"system-ui",display:"grid",gap:12}}><Link href="/food/subscriptions">← Food subscriptions</Link><p>FOOD RENEWAL PAYMENT</p><h1>{renewalId||"Payment request"}</h1><p role={screen==="failed"?"alert":undefined}>{detail}</p></main>;
 }
 const paid=String(renewal?.status||"")==="paid_invoiced"||String(renewal?.status||"").includes("paid");
 return <main style={{maxWidth:720,margin:"0 auto",padding:32,fontFamily:"system-ui",display:"grid",gap:14}}><Link href="/food/subscriptions">← Food subscriptions</Link><p>FOOD RENEWAL PAYMENT</p><h1>{renewalId}</h1>{renewal&&<><h2>₹{Number(renewal.total_amount||0).toLocaleString("en-IN")}</h2><p>Status: {String(renewal.status||"payment_pending").replaceAll("_"," ")}</p>{error&&<p role="alert">{error}</p>}{paid?<p>Payment recorded. Invoice will follow the paid confirmation path.</p>:<div style={{display:"flex",gap:12,flexWrap:"wrap"}}><button disabled={Boolean(busy)} onClick={()=>void pay("online_sandbox")}>{busy==="online_sandbox"?"Recording…":"Pay online (sandbox)"}</button><button disabled={Boolean(busy)} onClick={()=>void pay("cash")}>{busy==="cash"?"Recording…":"Pay cash"}</button></div>}<p>Cash posts to Cash in Hand after the collector is the signed-in customer or staff. Online sandbox records a canonical payment reference — live Razorpay capture still needs payment-live approval.</p></>}</main>}
