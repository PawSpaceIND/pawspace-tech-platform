...
export default function FoodSubscriptionsPage(){
 ...
 return <main style={{maxWidth:1180,margin:"0 auto",padding:28,fontFamily:"system-ui",display:"grid",gap:16}}>
  ...
  <section style={box}><h2>Renewal cycles</h2>{renewals.length===0?<p>No renewal cycle has become due yet.</p>:renewals.map(row=><article key={String(row.id)} style={{padding:"12px 0",borderBottom:"1px solid #e9f1ee"}}><strong>{String(row.id)} · {label(row.status)}</strong><div>₹{Number(row.total_amount||0).toLocaleString("en-IN")} · cycle {String(row.cycle_no)}</div>{!!row.payment_link_path&&<Link href={String(row.payment_link_path)}>Open UAT payment request</Link>}{!!row.invoice_id&&<div><Link href={`/food/subscription-invoice?invoiceId=${encodeURIComponent(String(row.invoice_id))}`}>Open invoice</Link></div>}</article>)}</section>
