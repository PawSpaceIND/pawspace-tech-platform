...
export default function V2StayExperience({mode:initialMode}:{mode:Mode}){
 ...
 return <main className={styles.page}><div className={styles.shell}><header className={styles.nav}><Link href="/v2" className={styles.brand}><img src="/assets/pawspace-icon.jpeg" alt=""/><span>PawSpace</span></Link><Link href="/v2" className={styles.back}>← Home</Link></header><section className={styles.hero}><div><small>{c.eyebrow}</small><h1>{c.title}</h1><p>{c.body}</p><div className={styles.modeSwitch}><Link href="/v2/boarding" className={mode==="boarding"?undefined:styles.ghost}>Boarding</Link><Link href="/v2/sitting" className={mode==="sitting"?undefined:styles.ghost}>Pet Sitting</Link></div></div><div className={styles.art} aria-hidden="true">{c.art}</div></section>
 <section className={styles.surface}>{loading?<p role="status">Loading your PawSpace family…</p>:account?<StayFlow key={mode+":"+account.customerId} mode={mode} onModeChange={setMode} customer={{customerId:account.customerId,customerName:account.name,phone:account.primaryPhone}}/>:<div className={styles.login}><h2>Sign in to plan care.</h2><p>{error||"Use your PawSpace customer account to choose pets, dates and verified caregivers."}</p><button onClick={()=>setAttempt(x=>x+1)}>Retry account</button><CustomerLogin embedded onLoggedIn={()=>setAttempt(x=>x+1)}/></div>}</section></div>
