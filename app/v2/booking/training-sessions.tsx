...
import {customerTrainingSessionStatus} from "../../../lib/training-session-status";
...
export default function TrainingBookingSessions({bookingId,inactive,onReady}:Props){
 ...
 {loading?<p role="status">Loading session details…</p>:record?<ol>{record.sessions.map(session=><li key={session.id}>Session {session.sequence_no} · {formatIndiaDateTime(session.scheduled_start,{weekday:true})} — {customerTrainingSessionStatus(session.status)}</li>)}</ol>:<><p>Your booking is saved. Prepare its reserved sessions before payment; this keeps the same booking and trainer.</p>{!inactive&&<button disabled={preparing} onClick={()=>void prepare()}>{preparing?"Preparing sessions…":"Prepare reserved sessions"}</button>}</>}
