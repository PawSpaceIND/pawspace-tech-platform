...
export default function V2BoardingTaxiExperience({sourceBookingId,routeScope="v2"}:{sourceBookingId?:string;routeScope?:"legacy"|"v2"}){
 ...
 const root=routeScope==="v2"?"/v2":"";
 const stayHref=sourceBookingId?`${root}/boarding/manage?bookingId=${encodeURIComponent(sourceBookingId)}`:root||"/mobile-app";
 return <main className={styles.page}><div className={styles.shell}>
  <header className={styles.nav}><Link href={root||"/mobile-app"} className={styles.brand}>PawSpace</Link><Link href={stayHref} className={styles.back}>{sourceBookingId?"← Boarding booking":"← Home"}</Link></header>
