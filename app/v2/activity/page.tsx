...
import CustomerRelocationInquiries from "../../relocation/customer-inquiries";
import {formatIndiaDateTime} from "../../../lib/india-time";
import styles from "../customer-detail.module.css";
import V2PendingFeedback from "../service-feedback";
const CLOSED=new Set(["completed","cancelled","canceled","refunded","closed"]);
const label=(v:string)=>v.replaceAll("_"," ");
const money=(v:number,c:string)=>new Intl.NumberFormat("en-IN",{style:"currency",currency:c,maximumFractionDigits:2}).format(v);
...
export default function V2ActivityPage(){
 ...
 {error&&<div className={styles.notice} role="alert">{error}</div>}{loading?<section className={styles.card}>Loading your care activity…</section>:!account?<section className={styles.card}><h2>Sign in to see your care history.</h2><p className={styles.muted}>Return home and use secure mobile OTP sign-in.</p><Link href="/v2" className={styles.back}>Open PawSpace home</Link></section>:<div className={styles.grid}><V2PendingFeedback customerId={account.customerId}/><section className={styles.card}><h2>Upcoming & active</h2><p className={styles.muted}>{open.length} booking{open.length===1?"":"s"} currently in motion.</p>{list(open,"Nothing active right now.")}</section><section className={styles.card}><h2>Care history</h2><p className={styles.muted}>Completed, cancelled and refunded bookings remain attached to your family record.</p>{list(history,"No closed bookings yet.")}</section><section className={styles.card}><h2>Fresh Food orders</h2><p className={styles.muted}>{foodOrders.length} order{foodOrders.length===1?"":"s"} on your family record.</p>{foodList}</section><CustomerRelocationInquiries customerId={account.customerId} routeScope="v2"/></div>}</div>
