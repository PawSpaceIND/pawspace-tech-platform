...
type View={
  ...
  salesIncentiveTruth?:{monthStart:string;dailyAccruedTotal:number;monthlyBonus:number;total:number;status:string;approvedBy:string|null;payrollRunId:string|null}|null;
...
export default function MyPortalPage(){
  ...
  return <StaffModule><main style={{minHeight:"100vh",background:`var(--staff-bg, ${C.ground})`,color:`var(--staff-text, ${C.ink})`,fontFamily:"var(--staff-font, system-ui,-apple-system,Segoe UI,sans-serif)"}}>
    <div style={{maxWidth:1080,margin:"0 auto",padding:"28px 20px 60px"}}>
      <p style={{margin:0}}><Link href="/" style={{color:`var(--staff-muted, ${C.dim})`,textDecoration:"none"}}>← PawSpace</Link></p>
      <p style={{fontWeight:800,letterSpacing:2,color:`var(--staff-muted, ${C.dim})`,fontSize:14,marginTop:10}}>PAWSPACE · MY WORKSPACE</p>
      {error?<p style={{color:"var(--staff-danger)"}}>{error}</p>:null}
      {loading&&!data?<p style={{color:`var(--staff-muted, ${C.dim})`}}>Loading your workspace…</p>:null}

      {data&&!data.linked?<section style={{...card,marginTop:16}}>
        <h1 style={{marginTop:0}}>No employee record linked yet</h1>
        <p style={{color:`var(--staff-muted, ${C.dim})`}}>Your identity <b>{data.email}</b> is signed in, but it is not yet linked to an active employee record. Ask People Ops to link your work email to your employee profile — then your salary, payslips, incentives, leave and ranking appear here automatically.</p>
      </section>:null}
      ...
      {e?<>
        <header style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:14,marginTop:8}}>
          <div><h1 style={{margin:"6px 0",fontSize:30}}>Hello, {e.name.split(" ")[0]}</h1>
            <p style={{margin:0,color:`var(--staff-muted, ${C.dim})`}}>{e.code} · {e.workEmail} · joined {day(e.joinedAt)}</p></div>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            <button disabled={busy} style={btn} onClick={()=>void clock("check_in")}>Check in</button>
            <button disabled={busy} style={{...btn,background:`var(--staff-primary, ${C.purple})`,color:`var(--staff-on-primary, ${C.ink})`}} onClick={()=>void clock("check_out")}>Check out</button>
          </div>
        </header>
        {msg?<p style={{color:`var(--staff-primary, ${C.gold})`}}>{msg}</p>:null}

        <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginTop:18}} data-staff-grid="stats">
          {data.engagement!=="contract"?<div style={stat}><small style={{color:`var(--staff-muted, ${C.dim})`}}>Net take-home (latest)</small><strong style={{display:"block",fontSize:26,marginTop:6}}>{INR(data.payslips?.latest?.net??data.compensation?.netMonthly)}</strong></div>:null}
          <div style={stat}><small style={{color:`var(--staff-muted, ${C.dim})`}}>Approved incentives</small><strong style={{display:"block",fontSize:26,marginTop:6,color:`var(--staff-primary, ${C.gold})`}}>{INR(data.incentives?.approvedTotal)}</strong></div>
          <div style={stat}><small style={{color:`var(--staff-muted, ${C.dim})`}}>Sales incentive · canonical</small><strong style={{display:"block",fontSize:26,marginTop:6,color:`var(--staff-primary, ${C.gold})`}}>{INR(data.salesIncentiveTruth?.total)}</strong><small style={{color:`var(--staff-muted, ${C.dim})`}}>{data.salesIncentiveTruth?.status??"not generated"}</small></div>
          <div style={stat}><small style={{color:`var(--staff-muted, ${C.dim})`}}>Advance outstanding</small><strong style={{display:"block",fontSize:26,marginTop:6}}>{INR(data.advances?.outstanding)}</strong></div>
          <div style={stat}><small style={{color:`var(--staff-muted, ${C.dim})`}}>Peer rank</small><strong style={{display:"block",fontSize:26,marginTop:6,color:`var(--staff-warning, ${C.orange})`}}>{data.performance?.appears?`#${data.performance.rank} / ${data.performance.ofEmployees}`:"—"}</strong></div>
        </section>
        ...
        <div style={card}>
          {data.compensation?<>
            <p style={{marginTop:0,color:`var(--staff-muted, ${C.dim})`}}>{data.compensation.structureCode} v{data.compensation.version} · gross {INR(data.compensation.grossMonthly)}/mo · fixed deductions {INR(data.compensation.fixedDeductions)} · net {INR(data.compensation.netMonthly)}</p>
            <div style={{display:"grid",gap:6}}>{data.compensation.components.map(c=><div key={c.code} style={{display:"flex",justifyContent:"space-between",borderBottom:`1px solid ${`var(--staff-line, ${C.line})`}`,padding:"6px 0",flexWrap:"wrap"}}><span>{c.label} <small style={{color:`var(--staff-muted, ${C.dim})`}}>({c.kind})</small></span><span style={{fontVariantNumeric:"tabular-nums"}}>{INR(c.amount)}</span></div>)}</div>
          </>:<p style={{margin:0,color:`var(--staff-muted, ${C.dim})`}}>No active compensation assignment yet.</p>}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}} data-staff-grid="stats">
          <div style={card}><b>Incentives</b>{data.incentives?.list.length?<div style={{display:"grid",gap:6,marginTop:8}}>{data.incentives.list.map((r,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:14,borderBottom:`1px solid ${`var(--staff-line, ${C.line})`}`,padding:"5px 0",flexWrap:"wrap"}}><span>{r.scheme} <small style={{color:`var(--staff-muted, ${C.dim})`}}>{day(r.periodStart)}</small></span><span style={{color:r.status==="approved"?`var(--staff-primary, ${C.gold})`:`var(--staff-muted, ${C.dim})`}}>{r.status} · {INR(r.approved||r.calculated)}</span></div>)}</div>:<p style={{color:`var(--staff-muted, ${C.dim})`,marginBottom:0}}>No incentive results yet.</p>}</div>
          <div style={card}><b>Sales incentive evidence</b>{data.salesIncentiveTruth?<p style={{color:`var(--staff-muted, ${C.dim})`,fontSize:15}}>Canonical month result: <b style={{color:`var(--staff-primary, ${C.gold})`}}>{INR(data.salesIncentiveTruth.total)}</b> · {data.salesIncentiveTruth.status} · daily accrual {INR(data.salesIncentiveTruth.dailyAccruedTotal)} · monthly bonus {INR(data.salesIncentiveTruth.monthlyBonus)}{data.salesIncentiveTruth.payrollRunId?` · payroll ${data.salesIncentiveTruth.payrollRunId}`:""}</p>:<p style={{color:`var(--staff-muted, ${C.dim})`,fontSize:15}}>Monthly canonical sales incentive has not been generated yet.</p>}<b>Daily evidence</b>{data.dailyIncentive?.list.length?<div style={{display:"grid",gap:6,marginTop:8}}>{data.dailyIncentive.list.slice(0,10).map((d,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:14,borderBottom:`1px solid ${`var(--staff-line, ${C.line})`}`,padding:"5px 0",flexWrap:"wrap"}}><span>{d.date} <small style={{color:`var(--staff-muted, ${C.dim})`}}>{d.baseVertical}{d.blitz?" · BLITZ":""}</small></span><span style={{color:`var(--staff-primary, ${C.gold})`}}>{INR(d.incentive)}</span></div>)}</div>:<p style={{color:`var(--staff-muted, ${C.dim})`,marginBottom:0}}>No daily incentive accrued yet (needs a sales base vertical + attributed bookings).</p>}</div>
          {data.engagement!=="contract"?<div style={card}><b>Salary advances</b>{data.advances?.list.length?<div style={{display:"grid",gap:6,marginTop:8}}>{data.advances.list.map(a=><div key={a.id} style={{fontSize:14,borderBottom:`1px solid ${`var(--staff-line, ${C.line})`}`,padding:"5px 0"}}>{INR(a.amount)} over {a.recoveryMonths} mo · <span style={{color:`var(--staff-muted, ${C.dim})`}}>{a.status}</span><div style={{color:`var(--staff-muted, ${C.dim})`}}>recovered {INR(a.recovered)} · outstanding {INR(a.outstanding)}</div></div>)}</div>:<p style={{color:`var(--staff-muted, ${C.dim})`,marginBottom:0}}>No advances.</p>}</div>:null}
        </div>

        <h2 style={h2}>My performance</h2>
        <div style={card}>
          {data.performance?.appears?<p style={{margin:0}}>Team <b>{data.performance.teamCode}</b> · rank <b style={{color:`var(--staff-warning, ${C.orange})`}}>#{data.performance.rank}</b> of {data.performance.ofEmployees} · net collected {INR(data.performance.netCollectedRevenue)} · {data.performance.bookingConversions} conversions · {data.performance.qualifiedLeads} qualified leads · first-response {data.performance.firstResponseRate==null?"—":`${data.performance.firstResponseRate}%`}</p>
          :<p style={{margin:0,color:`var(--staff-muted, ${C.dim})`}}>You are not in this period&apos;s operational leaderboard facts yet (rank appears once your team&apos;s productivity facts are generated).</p>}
          <p style={{margin:"10px 0 0"}}><Link href="/leaderboard" style={{color:`var(--staff-primary, ${C.gold})`}}>See the live company leaderboard →</Link></p>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}} data-staff-grid="stats">
          <div style={card}>
            <b>Apply for leave</b>
            <label style={{fontSize:15,color:`var(--staff-muted, ${C.dim})`}}>Leave type (code)<input style={inp} value={leave.leaveCode} placeholder="e.g. CL / SL / EL" onChange={ev=>setLeave({...leave,leaveCode:ev.target.value})}/></label>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <label style={{fontSize:15,color:`var(--staff-muted, ${C.dim})`,flex:1}}>From<input type="date" style={inp} value={leave.startDate} onChange={ev=>setLeave({...leave,startDate:ev.target.value})}/></label>
              <label style={{fontSize:15,color:`var(--staff-muted, ${C.dim})`,flex:1}}>To<input type="date" style={inp} value={leave.endDate} onChange={ev=>setLeave({...leave,endDate:ev.target.value})}/></label>
              <label style={{fontSize:15,color:`var(--staff-muted, ${C.dim})`,width:80}}>Days<input type="number" min="0.5" step="0.5" style={inp} value={leave.units} onChange={ev=>setLeave({...leave,units:ev.target.value})}/></label>
            </div>
            <label style={{fontSize:15,color:`var(--staff-muted, ${C.dim})`}}>Reason<input style={inp} value={leave.reason} onChange={ev=>setLeave({...leave,reason:ev.target.value})}/></label>
        ...
        <div style={{...card,overflowX:"auto"}}>
          {data.attendance?.length?<table style={{width:"100%",borderCollapse:"collapse",fontSize:15}}><thead><tr style={{color:`var(--staff-muted, ${C.dim})`,textAlign:"left"}}><th style={{padding:"6px 8px"}}>Date</th><th>Status</th><th>Worked</th><th>Exception</th></tr></thead>
            <tbody>{data.attendance.map((a,i)=><tr key={i} style={{borderTop:`1px solid ${`var(--staff-line, ${C.line})`}`}}><td style={{padding:"6px 8px"}}>{a.workDate}</td><td>{a.status}</td><td>{a.workedMinutes?`${Math.floor(a.workedMinutes/60)}h ${a.workedMinutes%60}m`:"—"}</td><td style={{color:a.exception?"var(--staff-danger)":`var(--staff-muted, ${C.dim})`}}>{a.exception||"—"}</td></tr>)}</tbody></table>
          :<p style={{margin:0,color:`var(--staff-muted, ${C.dim})`}}>No attendance recorded yet. Use Check in / Check out above.</p>}
        </div>

        <footer style={{marginTop:30,color:`var(--staff-muted, ${C.dim})`,fontSize:14}}>You are viewing your own record only. {data.engagement==="contract"?"Contract earnings stay in the Partner workspace; this People view covers attendance, leave and governed incentives.":"Salary and incentive figures are drawn from governed payroll runs and approved incentive schemes."} Leave requests follow maker/checker — your manager approves. Sandbox / UAT — not production payments.</footer>
      </>:null}
    </div>
  </main></StaffModule>;
}
