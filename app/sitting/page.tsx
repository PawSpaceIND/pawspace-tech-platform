...
// This page used to hardcode customer TST-101. Measured on a public host, that made it non-functional
// for EVERY real customer: anonymous callers get 401 and a signed-in customer asking for TST-101 is
// refused by requireCustomerOwnership. Same treatment app/training/page.tsx already had. [PTJA-P1-F37]
...
export default function SittingPage(){
 ...
 useEffect(()=>{let active=true;loadCustomerAccount().then(record=>{if(!active)return;setAccount(record);setAccountError("")}).catch(problem=>{if(active)setAccountError(problem instanceof Error?problem.message:"Sign in as a customer to book Pet Sitting")}).finally(()=>{if(active)setAccountLoading(false)});return()=>{active=false}},[]);
