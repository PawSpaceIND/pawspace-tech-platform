...
export default function SittingPage(){
 ...
 <section className={styles.content}><aside className={styles.filters}><p className={styles.kicker}>HOME & PET NEEDS</p><h2>Care preferences</h2>{carePreferences.map((x)=><button className={selectedPreferences.has(x)?styles.on:""} key={x} onClick={()=>togglePreference(x)}><i>{selectedPreferences.has(x)?"✓":""}</i>{x}</button>)}<div><strong>Bruno + Coco</strong><span>Dog & cat · family profile</span><span>Care instructions 85% ready</span><Link href="/account">Complete pet profiles →</Link></div></aside><div className={styles.results}><div className={styles.resultHead}><div><p className={styles.kicker}>3 UAT SITTER PROFILES</p><h2>Trusted care near your home</h2></div><select><option>Best match</option><option>Top rated</option></select></div>{sitters.map((s,i)=><button className={`${styles.sitter} ${chosen.name===s.name?styles.chosen:""}`} key={s.name} onClick={()=>setChosen(s)}><i className={styles[s.tone]}>{s.initials}</i><div><span>{s.badge}</span><h3>{s.name}</h3><p>📍 {s.area} · replies in {5+i*4} min</p><div>{s.skills.map(skill=><b key={skill}>✓ {skill}</b>)}</div></div><aside><strong>{s.rating} ★</strong><small>{s.reviews} reviews</small><em>{s.repeat} repeat families</em></aside><footer><span>UAT profile · canonical scheduler verifies final assignment</span><strong>{activePackage?`₹${Number(activePackage.base_price_per_pet).toLocaleString("en-IN")}`:"—"}<small> / {mode==="overnight"?"night":"visit"}</small></strong></footer></button>)}</div></section>
