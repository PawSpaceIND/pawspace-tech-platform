...
export default function GroomingCustomerBooking({bookingId}:{bookingId:string}) {
 ...
 useEffect(()=>{
  ...
  void loadCustomerAccount(undefined,{signal:controller.signal}).then(account=>{
   if(active){setCustomerId(account.customerId);setError("");setBooking(account.bookings.find(item=>item.id===bookingId&&item.serviceCode==="grooming")||null);setLoadedId(bookingId);}
