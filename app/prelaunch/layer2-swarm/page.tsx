...
export default function Layer2SwarmPage(){
 ...
 return <main style={{maxWidth:1050,margin:"0 auto",padding:"32px 20px",fontFamily:"system-ui,sans-serif"}}>
  ...
  <div style={{border:"1px solid #ddd",borderRadius:14,padding:18,margin:"20px 0",background:"white"}}><button disabled={running} onClick={run} style={{padding:"12px 18px",fontWeight:800,cursor:running?"wait":"pointer"}}>{running?"Layer 2 running…":"Run Layer 2 — 60 real bookings"}</button><p><b>Status:</b> {progress}</p>{runId?<p><b>Run ID:</b> <code>{runId}</code></p>:null}{error?<p style={{fontWeight:800}}>FAIL: {error}</p>:null}</div>
  {summary?<section><h2>{summary.status==="PASSED"?"PASS":"FAIL"} · {summary.totalBookings} D1 bookings</h2><p><b>Production ready:</b> NO · <b>UAT closed:</b> NO</p><div style={{display:"grid",gap:8}}>{summary.assertions?.map(item=><article key={item.name} style={{border:"1px solid #ddd",borderRadius:10,padding:12,background:"white"}}><b>{item.pass?"✅":"❌"} {item.name}</b><div>{item.detail}</div><small>{item.severity}</small></article>)}</div>{failures.length?<p><b>{failures.length} reconciliation issue(s) need engineering action before Layer 2 can close.</b></p>:<p><b>All blocker assertions passed. Human staff UAT evidence is still required separately.</b></p>}<p>{summary.knownBoundary}</p></section>:null}
 </main>;
}
