...
const surfaces:Surface[]=[
 ...
 {label:"Partner Mobile App",href:"/partner-mobile",audience:"Provider",note:"Mobile-first field app that also opens in a laptop browser.",check:"Owned jobs, GPS/ETA, navigation, earnings and profile."},
 ...
 {label:"CRM / Sales",href:"/team/sales",audience:"Sales / CRM",note:"Canonical Customer 360 and governed Revenue Intelligence worklist.",check:"Customer record, consent, data quality, opportunities, claim/complete actions."},
 {label:"Control Center",href:"/control",audience:"Founder / Admin",note:"Governance, launch gates, pricing, finance, access, audit and system controls.",check:"Readable fonts/sidebar, configuration, approvals, security and evidence views."},
...
const card={border:"1px solid #e3dbea",borderRadius:14,padding:16,background:"white"} as const;
export default function PrelaunchAccessHub(){return <main style={{maxWidth:1180,margin:"0 auto",padding:"32px 20px 64px",fontFamily:"system-ui,sans-serif",color:"#24133f"}}>
 <header style={{marginBottom:24}}><p style={{fontWeight:900,letterSpacing:1.2,margin:0,color:"#6a2daf"}}>PAWSPACE · FINAL HUMAN UAT HUB</p><h1 style={{margin:"8px 0",fontSize:36}}>Test the whole platform from eight front doors.</h1><p style={{maxWidth:900,lineHeight:1.6,color:"#6e6576"}}>Use this page for founder visual review and staff UAT navigation. Deep routes still exist, but they are modules—not separate products. Synthetic test data is clearly labelled and must never be treated as live operating truth.</p><p style={{padding:"10px 12px",background:"#fff4d9",borderRadius:10}}><b>Engineering candidate: READY FOR HUMAN UAT after exact-SHA deployment verification.</b> · <b>PRODUCTION READY = FALSE.</b></p></header>

 <section><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"end",marginBottom:12}}><div><small style={{fontWeight:800,color:"#6a2daf"}}>FINAL REVIEW LINKS</small><h2 style={{margin:"5px 0"}}>Open only these eight.</h2></div><span style={{color:"#706779"}}>Everything else should be reachable from these hubs.</span></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12}}>{surfaces.map(surface=><article key={surface.href} style={card}><small style={{fontWeight:800,color:"#6a2daf"}}>{surface.audience}</small><h3 style={{fontSize:19,margin:"7px 0"}}><Link href={surface.href}>{surface.label} →</Link></h3><p style={{margin:"0 0 8px",lineHeight:1.45}}>{surface.note}</p><small style={{display:"block",color:"#6e6576",lineHeight:1.45}}><b>Check:</b> {surface.check}</small><code style={{display:"block",marginTop:10,padding:"6px 8px",background:"#f7f3fa",borderRadius:7}}>{surface.href}</code></article>)}</div></section>

 <section style={{marginTop:32}}><small style={{fontWeight:800,color:"#6a2daf"}}>CONTROLLED TEST DATA</small><h2 style={{margin:"5px 0"}}>Synthetic dataset already loaded for visual/UAT testing.</h2><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,margin:"14px 0"}}>{[["Customers",testDataSummary.customers],["Linked pets",testDataSummary.pets],["Service families",testDataSummary.services],["Subscribers",testDataSummary.subscribers]].map(([label,value])=><article key={String(label)} style={card}><small>{label}</small><strong style={{display:"block",fontSize:26,marginTop:5}}>{value}</strong></article>)}</div><p style={{lineHeight:1.6}}>The Test Lab covers New, Repeat, Subscriber and Dormant segments; Dog, Cat and Mixed households; one to four pets; Paid online, Pay after service and Subscription-credit simulations. It sends <b>no live calls, messages or payments</b>.</p>
 <div style={{overflowX:"auto",...card}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}><thead><tr>{["Test ID","Primary service","Segment","Area","Pets","Payment"].map(h=><th key={h} style={{textAlign:"left",padding:"8px",borderBottom:"1px solid #e8e0ed"}}>{h}</th>)}</tr></thead><tbody>{samples.map(customer=><tr key={customer.id}><td style={{padding:8}}><code>{customer.id}</code></td><td style={{padding:8}}>{customer.service}</td><td style={{padding:8}}>{customer.segment}</td><td style={{padding:8}}>{customer.area}</td><td style={{padding:8}}>{customer.petCount} · {customer.species}</td><td style={{padding:8}}>{customer.payment}</td></tr>)}</tbody></table></div>
 <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:12}}><article style={card}><b>Address-autocomplete checks</b><p style={{lineHeight:1.5}}>Type these into customer address/location fields and confirm suggestions appear where applicable:</p><p>{addressChecks.map(x=><code key={x} style={{display:"inline-block",margin:"3px 5px 3px 0",padding:"4px 7px",background:"#f7f3fa",borderRadius:6}}>{x}</code>)}</p></article><article style={card}><b>Identity boundary</b><p style={{lineHeight:1.5}}>Customer/provider/staff ownership and permission tests require the relevant verified UAT identity/session. Do not substitute legacy synthetic provider names or browser-only demo records as canonical UAT evidence.</p></article></div></section>

 <section style={{marginTop:32}}><small style={{fontWeight:800,color:"#6a2daf"}}>WHAT IS READY NOW</small><h2 style={{margin:"5px 0"}}>Engineering modules ready for deployed human testing.</h2><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:10}}>{[
 ...
 <section style={{marginTop:32}}><small style={{fontWeight:800,color:"#a34f00"}}>STILL REQUIRED FOR FINAL PRODUCTION</small><h2 style={{margin:"5px 0"}}>Do not close these from screenshots or green CI.</h2><ol style={{lineHeight:1.7,paddingLeft:22}}>{productionGaps.map(gap=><li key={gap}>{gap}</li>)}</ol><p style={{padding:12,background:"#f6eefc",borderRadius:10}}><b>Human-test rule:</b> screenshots are useful defect/evidence inputs, but formal PASS also needs exact deployed SHA, actor/role, canonical record IDs where applicable, negative/replay outcomes and the required hosted real-D1 evidence.</p></section>
</main>}
