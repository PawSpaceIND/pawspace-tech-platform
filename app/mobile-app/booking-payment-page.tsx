...
function BookingPaymentInner({serviceName,totalAmount,amountDueNow,mode,bookingId,busy=false,autoStart=false,onCreateBooking,onVerified,onBack,stage,paidAmount,balanceDueAt}:Props){
 ...
 return <section className={styles.page} aria-label={`${serviceName} payment`}>
  {balance?<header><small>BALANCE PAYMENT</small><h3>Pay the remaining balance</h3><p>Your first payment is verified. Only the balance below is left.</p></header>:<header><small>PAYMENT · FINAL STEP</small><h3>Review payment</h3><p>Your service details are locked while you finish this step.</p></header>}
  {balance?<article className={styles.amount}><span>Total service value<b>{money(totalAmount)}</b></span><span>Paid so far<b>{money(paidAmount??Math.max(0,totalAmount-dueNow))}</b></span><span>Balance{balanceDueAt?` · due by ${formatIndiaDateTime(balanceDueAt)}`:""}<b>{money(dueNow)}</b></span></article>
  ...
  <button type="button" className={styles.primary} disabled={working} onClick={()=>void primary()}>{working?"Please wait…":state.phase==="captured"?"Retry booking confirmation":!bookingId?payAfter?"Accept pay-after-service & confirm booking":"Reserve slot & continue to Razorpay":payAfter?"Confirm booking":state.canCheck?"Check payment status":balance?`Pay balance · ${money(dueNow)}`:`Pay securely · ${money(dueNow)}`}</button>
