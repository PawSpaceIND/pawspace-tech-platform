...
export default function SittingCustomerPanel({bookingId}:{bookingId:string}){
 ...
 const load=useCallback(()=>{const version=++readVersion.current;return loadSittingCustomerView(bookingId).then(value=>{if(version!==readVersion.current)return;setData(value);setPlan(value.carePlan);}).catch(problem=>{if(version!==readVersion.current)return;setData(null);setError(problem instanceof Error?problem.message:'Unable to load your sitting booking.');}).finally(()=>{if(version===readVersion.current)setLoading(false);});},[bookingId]);
 ...
 return <section style={{display:'grid',gap:16,padding:16,overflowWrap:'anywhere'}} aria-label="Your sitting booking">
  ...
  {data&&!loading&&<><section><h3>Booking status</h3><p>{label(data.status)}</p><p>Booking total: {data.totalAmount==null?"Unavailable":new Intl.NumberFormat("en-IN",{style:"currency",currency:"INR"}).format(data.totalAmount)}</p><p>{when(data.scheduledStart)} – {when(data.scheduledEnd)}</p></section>
   ...
   <p>Sitter messaging is currently unavailable.</p>
  </>}
 </section>;
}
