...
export default function SittingCustomerPanel({bookingId}:{bookingId:string}){
 ...
 const load=useCallback(async()=>{const version=++readVersion.current;setLoading(true);setError('');try{const value=await loadSittingCustomerView(bookingId);if(version!==readVersion.current)return;setData(value);setPlan(value.carePlan);}catch(problem){if(version!==readVersion.current)return;setData(null);setError(problem instanceof Error?problem.message:'Unable to load your sitting booking.');}finally{if(version===readVersion.current)setLoading(false);}},[bookingId]);
 useEffect(()=>{void load();return()=>{readVersion.current++;};},[load]);
 ...
 return <section style={{display:'grid',gap:16,padding:16,overflowWrap:'anywhere'}} aria-label="Your sitting booking">
  <header><h2 style={{fontSize:24,fontWeight:700}}>Your sitting booking</h2><p>{bookingId}</p><button disabled={loading||busy} onClick={()=>void load()} style={{minHeight:44}}>Refresh booking</button></header>
  ...
  {data&&!loading&&<><section><h3>Booking status</h3><p>{label(data.status)}</p><p>Booking total: {data.totalAmount==null?"Unavailable":new Intl.NumberFormat("en-IN",{style:"currency",currency:"INR"}).format(data.totalAmount)}</p><p>{when(data.scheduledStart)} – {when(data.scheduledEnd)}</p></section>
   ...
   <p>Live sitter messaging is not connected. This screen does not send messages or show simulated care updates.</p>
  </>}
 </section>;
}
