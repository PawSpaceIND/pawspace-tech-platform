...
export default function StayBookingPage({mode}:{mode:"boarding"|"sitting"}){
 ...
 return <main className={styles.stage} data-theme="emerald" data-mode="light" style={{display:"block",padding:16}}><section style={{maxWidth:620,margin:"0 auto",background:"var(--ps-surface)",borderRadius:24,padding:16}}><Link href="/mobile-app">My PawSpace</Link><h1 style={{fontSize:28,fontWeight:700,margin:"16px 0"}}>{selectedMode==="boarding"?"Boarding":"Pet Sitting"}</h1>{loading?<p role="status">Loading your PawSpace account…</p>:account?<StayFlow key={`${selectedMode}:${account.customerId}`} mode={selectedMode} onModeChange={setSelectedMode} customer={{customerId:account.customerId,customerName:account.name,phone:account.primaryPhone}}/>:<><p role="alert">{error}</p><button style={{minHeight:44}} onClick={()=>setAttempt(value=>value+1)}>Retry account</button><p>Sign in to choose your pets, care dates and available caregivers.</p><CustomerLogin embedded onLoggedIn={()=>setAttempt(value=>value+1)}/></>}</section></main>;
}
