...
export default function AddressPicker({onZoneResolved}:{onZoneResolved?:(zone:ZoneResult|null)=>void}){
 ...
 const[address,setAddress]=useState(""),[pincode,setPincode]=useState(""),[resolvedZone,setResolvedZone]=useState<ZoneResult|null>(null),[zones,setZones]=useState<Zone[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(""),[manualFallback,setManualFallback]=useState(false);
 ...
 async function continueManualAddress(){setLoading(true);setError("");try{if(address.trim().length<8)throw new Error("Enter the complete doorstep address");const coverage=await resolveServiceCoverage(pincode);const zone:Zone={zoneId:coverage.zone.zoneId,zoneName:coverage.zone.zoneName,description:coverage.zone.description,color:coverage.zone.color,serviceAvailable:coverage.zone.serviceAvailable};const result:ZoneResult={zone,assignment:{pincode:coverage.pincode,zoneId:coverage.zoneId,cityId:coverage.cityId,city:coverage.city,area:coverage.area},address:address.trim(),latitude:0,longitude:0,placeId:`manual:${coverage.pincode}`,verification:"manual"};setResolvedZone(result);setSuggestions([]);remember(result);onZoneResolved?.(result);}catch(e){setError(e instanceof Error?e.message:"Unable to verify this service PIN");}finally{setLoading(false);}}
