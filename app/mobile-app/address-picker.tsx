...
export default function AddressPicker({onZoneResolved}:{onZoneResolved?:(zone:ZoneResult|null)=>void}){
 ...
 return<div style={container}><section style={box}><label style={label}>Where should we visit?<p style={{fontSize:13,color:"var(--ds-text-muted)",margin:0}}>Add your home address so your care professional can reach the right doorstep. We’ll check that care is available here.</p></label><div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr)",gap:8}}><input style={input} type="text" placeholder="House / flat, street and landmark" value={address} onChange={e=>{setAddress(e.target.value);clear();}} disabled={loading} aria-label="Complete doorstep address"/><input style={input} type="text" inputMode="numeric" aria-label="Service address pincode" placeholder="e.g., 560034" value={pincode} onChange={e=>{setPincode(e.target.value.replace(/\D/g,"").slice(0,6));clear();}} disabled={loading}/><button style={primaryButton} onClick={()=>void resolveZone()} disabled={loading||address.trim().length<8||pincode.length!==6}>{loading?"Verifying…":"Verify map"}</button></div>{error&&<p role="alert" style={{color:"var(--ds-danger-500)",fontSize:14,margin:0}}>{error}</p>}</section>
