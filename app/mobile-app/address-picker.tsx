...
export default function AddressPicker({onZoneResolved}:{onZoneResolved?:(zone:ZoneResult|null)=>void}){
 ...
 const[address,setAddress]=useState(""),[pincode,setPincode]=useState(""),[resolvedZone,setResolvedZone]=useState<ZoneResult|null>(null),[zones,setZones]=useState<Zone[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(""),[mapsUnavailable,setMapsUnavailable]=useState(false);
 ...
 function clear(){setResolvedZone(null);setSuggestions([]);setLookupSession("");setError("");setMapsUnavailable(false);remember(null);onZoneResolved?.(null);}
 async function resolveZone(){clear();setLoading(true);try{
  ...
  if(lookup.status!=="configured"){
    ...
    setError("Map lookup is unavailable. You can continue with the typed address and PIN.");
    return;
  }
 ...
 async function useTypedAddress(){
  ...
  try{
    ...
    const result:ZoneResult={zone,assignment:{pincode:coverage.pincode,zoneId:coverage.zoneId,cityId:coverage.cityId,city:coverage.city,area:coverage.area},address:address.trim(),latitude:Number((coverage as {latitude?:number}).latitude)||12.9716,longitude:Number((coverage as {longitude?:number}).longitude)||77.5946,placeId:`manual:${pincode}`};
    setResolvedZone(result);setMapsUnavailable(false);remember(result);onZoneResolved?.(result);
  }catch(e){setError(e instanceof Error?e.message:"Could not save typed address");}
 ...
 {resolvedZone&&<section className={styles.box}><p className={styles.caption}>Verified service doorstep</p><div className={styles.activeZone} style={{"--zone-accent":resolvedZone.zone.color} as CSSProperties}><div className={styles.zoneRow}><div><h3 className={styles.zoneTitle}>{resolvedZone.zone.zoneName}</h3><p className={styles.caption}>{resolvedZone.address}</p><p className={styles.locationLine}>📍 {resolvedZone.assignment.area}, {resolvedZone.assignment.city} · {resolvedZone.placeId.startsWith("manual:")?"typed address":"map verified"}</p></div><span className={styles.badge}>{resolvedZone.zone.serviceAvailable?"✓ Available":"Not available"}</span></div></div></section>}
