...
export default function AddressPicker({onZoneResolved,restoreSaved=true,summaryLabel}:{onZoneResolved?:(zone:ZoneResult|null)=>void;restoreSaved?:boolean;summaryLabel?:string}){
 ...
 async function applyCoverage(addressText:string,pincode:string,place?:{latitude:number;longitude:number;placeId:string;verification:"map"|"typed"}){
  ...
  const conflict=serviceAddressConflict([addressText,line2.trim()].filter(Boolean).join(", "),coverage.city,coverage.pincode);if(conflict)throw new Error(conflict);
  ...
  const result:ZoneResult={zone,assignment:{pincode:coverage.pincode,zoneId:coverage.zoneId,cityId:coverage.cityId,city:coverage.city,area:coverage.area},address:[addressText,line2.trim()].filter(Boolean).join(", "),addressLine1:addressText,addressLine2:line2.trim(),latitude:place?.latitude||12.925,longitude:place?.longitude||77.5938,placeId:place?.placeId||`typed:${pincode}`,verification:place?.verification||"typed"};
 ...
 async function verifyTyped(source:string){
  const pincode=inferPin(`${source} ${line2}`);
