...
export default function AddressPicker({onZoneResolved}:{onZoneResolved?:(zone:ZoneResult|null)=>void}){
  ...
  useEffect(()=>{
    async function loadZones(){
      try{const r=await fetch("/api/service-zone?action=list");const body=await r.json() as{data?:Zone[]};if(body.data)setZones(body.data);}catch(e){console.error("Failed to load zones:",e);}
    }
    loadZones();
  },[]);
  ...
  async function resolveZone(){
    ...
    try{
      ...
      const coverage=await resolveServiceCoverage(pincode),sessionToken=crypto.randomUUID();
      const lookup=await searchAddresses(`${address.trim()}, ${pincode}`,sessionToken);
      ...
      const suggestion=lookup.suggestions[0],place=await resolveAddress(suggestion.placeId,sessionToken);
      ...
      const mappedAddress=String(place.address||suggestion.fullText||address.trim());
      if(!mappedAddress.includes(pincode))throw new Error("The mapped doorstep does not match the selected pincode");
      ...
      setAddress(mappedAddress);setResolvedZone(result);onZoneResolved?.(result);
    }catch(e){setResolvedZone(null);onZoneResolved?.(null);setError(e instanceof Error?e.message:"Failed to resolve zone");}
    finally{setLoading(false);}
  }

  return<div style={container}>
    <section style={box}>
      <label style={label}>Service area lookup<p style={{fontSize:13,color:"var(--ds-text-muted)",margin:0}}>Enter your doorstep and pincode. PawSpace verifies the mapped coordinates before the booking can continue.</p></label>
      <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8}}>
        <input style={input} type="text" placeholder="House / flat, street and landmark" value={address} onChange={e=>{setAddress(e.target.value);setResolvedZone(null);onZoneResolved?.(null);}} disabled={loading} aria-label="Complete doorstep address"/>
        <input style={input} type="text" inputMode="numeric" placeholder="e.g., 560034" value={pincode} onChange={e=>{setPincode(e.target.value.replace(/\D/g,"").slice(0,6));setResolvedZone(null);onZoneResolved?.(null);}} disabled={loading}/>
        <button style={primaryButton} onClick={()=>void resolveZone()} disabled={loading||address.trim().length<8||pincode.length!==6}>{loading?"Verifying…":"Verify map"}</button>
      </div>
      {error&&<p role="alert" style={{color:"var(--ds-danger-500)",fontSize:14,margin:0}}>{error}</p>}
    </section>

    {resolvedZone&&<section style={box}>
      <p style={{margin:0,fontSize:13,color:"var(--ds-text-muted)"}}>Verified service doorstep</p>
      <div style={{...zoneCardActive(resolvedZone.zone.color),padding:16}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"start",gap:12}}><div><h3 style={{margin:"0 0 4px 0",fontSize:16,color:"var(--ds-text)"}}>{resolvedZone.zone.zoneName}</h3><p style={{margin:0,fontSize:13,color:"var(--ds-text-muted)"}}>{resolvedZone.address}</p><p style={{margin:"8px 0 0 0",fontSize:13,color:"var(--ds-text)"}}>📍 {resolvedZone.assignment.area}, {resolvedZone.assignment.city} · map verified</p></div><span style={{...badge,background:resolvedZone.zone.color,color:"#fff"}}>{resolvedZone.zone.serviceAvailable?"✓ Available":"Not available"}</span></div></div>
    </section>}

    {zones.length>0&&!resolvedZone&&<section style={box}><p style={{margin:0,fontSize:13,color:"var(--ds-text-muted)"}}>Configured UAT service zones</p><div style={{display:"grid",gap:8}}>{zones.map(zone=><div key={zone.zoneId} style={zoneCard} aria-disabled="true"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}><div><h4 style={{margin:"0 0 4px 0",fontSize:14,fontWeight:600}}>{zone.zoneName}</h4><p style={{margin:0,fontSize:12,color:"var(--ds-text-muted)"}}>{zone.description}</p></div><span style={{fontSize:20}}>{zone.color}</span></div></div>)}</div></section>}
  </div>;
}
