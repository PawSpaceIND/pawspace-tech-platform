...
function Recovery({bookingId,service,routeScope='legacy',className}:Props){
 ...
 // useInitialBookingReference reads the reference once per page load, so leaving recovery reloads the page without it.
 const startSeparate=<p><button type="button" onClick={()=>window.location.assign(withoutBookingReference(window.location.href))}>{service==='taxi'?'Book another ride':'Start a new stay request'}</button></p>;
