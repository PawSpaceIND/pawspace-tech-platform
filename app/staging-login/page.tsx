...
export default function StagingLoginPage(){
  ...
  async function login(useEmail:string){
    ...
    try{const r=await fetch("/api/staging-login",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code:code.trim(),email:useEmail.trim()})});
      ...
      if(j.role==="finance")window.location.assign("/mfa?next=%2Fteam%2Ffinance");else if(j.role==="admin")window.location.assign("/mfa?next=%2Fcontrol");else if(j.role==="manager")window.location.assign("/booking-command-center");else window.location.assign("/me");
