...
export default function GroomingRouteCard({bookingId,providerId}:{bookingId:string;providerId:string}){
  ...
  return <section style={{marginTop:14,padding:16,borderRadius:18,border:"1px solid #e7dcef",background:"#fff"}}>
    ...
    {data&&<><div style={{margin:"12px 0",padding:12,borderRadius:12,background:"#f8f5fb"}}><small style={{color:"#786b81"}}>DESTINATION</small><b style={{display:"block",marginTop:4,fontSize:13}}>{data.destinationAddress}</b></div>{data.providerLocation&&<p style={{fontSize:11,margin:"0 0 8px",color:"#6d6275"}}>Latest GPS captured {data.providerLocation.capturedAt?new Date(data.providerLocation.capturedAt).toLocaleTimeString("en-IN",{hour:"numeric",minute:"2-digit",second:"2-digit"}):"just now"}{Number.isFinite(data.providerLocation.accuracyMeters)?` · ±${Math.round(Number(data.providerLocation.accuracyMeters))} m`:""}</p>}{data.route?.status==="configured"&&<p style={{fontSize:13,margin:"0 0 8px"}}><b>ETA:</b> {mins(data.route.durationSeconds)} · {km(data.route.distanceMeters)}</p>}{data.route?.status==="configuration_required"&&<p style={{fontSize:12,margin:"0 0 8px",color:"#7a5b20"}}>GPS capture works. Google Routes ETA waits for the approved UAT server key.</p>}{data.route?.status==="route_unavailable"&&<p style={{fontSize:12,margin:"0 0 8px",color:"#9a3d32"}}>Route unavailable: {data.route.error}</p>}
      ...
      <a href={data.navigationUrl} target="_blank" rel="noreferrer" style={{display:"block",marginTop:9,padding:"12px",borderRadius:12,border:"1px solid #d8cae7",textDecoration:"none",textAlign:"center",color:"#4b168c",fontWeight:900}}>Open turn-by-turn in Google Maps ↗</a>
    </>}
    ...
    <p style={{fontSize:10,color:"#817887",lineHeight:1.45,margin:"10px 0 0"}}>Location sharing is user-started and foreground-only in UAT. Leaving this screen or tapping Stop GPS ends the browser watch. Background tracking is not enabled.</p>
  </section>;
}
