...
export default function GroomingRouteCard({bookingId,providerId}:{bookingId:string;providerId:string}){
  ...
  return <section style={{marginTop:14,padding:16,borderRadius:18,border:"1px solid #dcece5",background:"#fff"}}>
    ...
    {tracking&&<div role="status" style={{marginTop:10,padding:"9px 11px",borderRadius:10,background:"#eaf8ef",color:"#176e45",fontSize:13,fontWeight:800}}>● GPS tracking on · updates while this screen stays open</div>}
    {error&&<p style={{fontSize:12,color:"#9a3d32",marginBottom:0}}>{error}</p>}
    <p style={{fontSize:12,color:"#798683",lineHeight:1.45,margin:"10px 0 0"}}>Location sharing is user-started and foreground-only in UAT. Leaving this screen or tapping Stop GPS ends the browser watch. Background tracking is not enabled.</p>
  </section>;
}
