...
import GroomingUpload from "../../components/customer/grooming-upload";
import { PHOTO_UPLOADS_DEFERRED } from "../../lib/mobile/human-uat-scope";
...
type MediaAsset={ref:string;purpose:"before_service"|"after_service";proofReady:boolean};
...
export default function CanonicalGroomingJobs(){
  ...
  return <div className={styles.board}>
    <section style={{background:"white",border:"1px solid #eadff4",borderRadius:18,padding:20}}>
      ...
      {!providerId&&!error&&<div style={{padding:26,textAlign:"center",color:"#746b7d"}}>Resolving verified provider identity…</div>}
      {providerId&&jobs.length===0&&!error&&<div style={{padding:26,textAlign:"center",color:"#746b7d"}}>No canonical Grooming work orders are assigned to this provider yet.</div>}
    ...
    <section style={{background:"white",border:"1px solid #eadff4",borderRadius:18,padding:22}}>
      {!selected?<div style={{padding:30,textAlign:"center",color:"#746b7d"}}>Select a canonical work order.</div>:<>
        ...
        {["arrived", "in_service"].includes(selected.status) && <GroomingUpload key={selected.bookingId} bookingId={selected.bookingId} deferred={PHOTO_UPLOADS_DEFERRED} />}
        {!PHOTO_UPLOADS_DEFERRED && <p><a href={`/team/operations/media-review?bookingId=${encodeURIComponent(selected.bookingId)}`}>Internal photo review · separate reviewer sign-in required</a></p>}
        ...
        <p style={{fontSize:12,color:"#776d80",marginTop:16}}>Canonical UAT data only. Customer phone remains masked. Proof is accepted only from registered private media with storage confirmation and a clean scan.</p>
      </>}
    </section>
  </div>;
}
