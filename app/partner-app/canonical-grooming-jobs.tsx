  ...
  return <div style={{display:"grid",gridTemplateColumns:"minmax(300px,.9fr) minmax(420px,1.1fr)",gap:18}}>
    ...
    <section style={{background:"white",border:"1px solid #eadff4",borderRadius:18,padding:22}}>
      {!selected?<div style={{padding:30,textAlign:"center",color:"#746b7d"}}>Select a canonical work order.</div>:<>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:18}}><button disabled={busy||!nextAction} onClick={()=>nextAction&&void act(nextAction)} style={{padding:"11px 16px",border:0,borderRadius:10,background:"#4b168c",color:"white",fontWeight:800,opacity:busy||!nextAction?.7:1}}>{busy?"Updating…":actionLabel}</button><button onClick={()=>setRefreshKey(value=>value+1)} style={{padding:"11px 16px",borderRadius:10,border:"1px solid #d8cae7",background:"white",fontWeight:700}}>Reload timeline</button></div>
