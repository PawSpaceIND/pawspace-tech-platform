...
type IdentitySession={subjectType?:string;subjectId?:string;roleCode?:string;error?:string};
...
export default function CanonicalGroomingJobs(){
  ...
  useEffect(()=>{let cancelled=false;fetch("/api/identity-session",{cache:"no-store"}).then(async response=>{const body=await response.json() as {data?:IdentitySession;error?:string};if(!response.ok)throw new Error(body.error||"Verified provider identity session required");if(body.data?.subjectType!=="provider"||!body.data.subjectId)throw new Error("Verified provider identity session required");return body.data.subjectId;}).then(id=>{if(!cancelled)setProviderId(id);}).catch(err=>{if(!cancelled)setError(err instanceof Error?err.message:"Verified provider identity session required");});return()=>{cancelled=true};},[]);

  useEffect(()=>{if(!providerId)return;let cancelled=false;fetch(`/api/partner-grooming-jobs?providerId=${encodeURIComponent(providerId)}&v=${refreshKey}`,{cache:"no-store"}).then(async response=>{const body=await response.json() as JobsResponse;if(!response.ok)throw new Error(body.error||"Unable to load canonical jobs");return body;}).then(body=>{if(cancelled)return;const next=body.jobs??[];setJobs(next);setError("");setSelectedId(current=>current&&next.some(job=>job.bookingId===current)?current:(next[0]?.bookingId??""));}).catch(err=>{if(!cancelled)setError(err instanceof Error?err.message:"Unable to load canonical jobs");});return()=>{cancelled=true};},[providerId,refreshKey]);
  ...
  return <div style={{display:"grid",gridTemplateColumns:"minmax(300px,.9fr) minmax(420px,1.1fr)",gap:18}}>
    <section style={{background:"white",border:"1px solid #dcece5",borderRadius:18,padding:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:16}}><div><small style={{fontWeight:800,color:"#1f6b57",letterSpacing:1}}>CANONICAL WORK ORDERS</small><h2 style={{margin:"6px 0"}}>Grooming assignments</h2>{providerId?<small>Provider {providerId}</small>:null}</div><button onClick={()=>setRefreshKey(value=>value+1)} disabled={!providerId} style={{border:"1px solid #dcece5",background:"white",borderRadius:10,padding:"9px 12px",fontWeight:700}}>Refresh</button></div>
      {error&&<div style={{padding:12,borderRadius:10,background:"#fff1f1",marginBottom:12}}>{error}</div>}
      {!providerId&&!error&&<div style={{padding:26,textAlign:"center",color:"#6c7c78"}}>Resolving verified provider identity…</div>}
      {providerId&&jobs.length===0&&!error&&<div style={{padding:26,textAlign:"center",color:"#6c7c78"}}>No canonical Grooming work orders are assigned to this provider yet.</div>}
      <div style={{display:"grid",gap:10}}>{jobs.map(job=><button key={job.bookingId} onClick={()=>setSelectedId(job.bookingId)} style={{textAlign:"left",padding:14,borderRadius:12,border:selected?.bookingId===job.bookingId?"2px solid #1f6b57":"1px solid #dcece5",background:selected?.bookingId===job.bookingId?"#f2f7f5":"white"}}><div style={{display:"flex",justifyContent:"space-between",gap:10}}><strong>{when(job.scheduledStart)}</strong><em style={{fontStyle:"normal",fontSize:12,textTransform:"capitalize"}}>{statusLabel(job.status)}</em></div><b style={{display:"block",marginTop:7}}>{job.packageName}</b><small>{job.pets.map(pet=>pet.name).join(", ")} · {job.zoneId}</small><div style={{display:"flex",justifyContent:"space-between",marginTop:8}}><span>{job.customer.name}</span><strong>{money(job.totalAmount)}</strong></div></button>)}</div>
    </section>

    <section style={{background:"white",border:"1px solid #dcece5",borderRadius:18,padding:22}}>
      {!selected?<div style={{padding:30,textAlign:"center",color:"#6c7c78"}}>Select a canonical work order.</div>:<>
        <div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"flex-start"}}><div><small style={{fontWeight:800,color:"#1f6b57"}}>BOOKING {selected.bookingId}</small><h2 style={{margin:"7px 0"}}>{selected.packageName}</h2><p style={{margin:0,color:"#667672"}}>{when(selected.scheduledStart)} → {when(selected.scheduledEnd)}</p></div><span style={{padding:"7px 10px",borderRadius:999,background:"#e9f1ee",fontWeight:800,textTransform:"capitalize"}}>{statusLabel(selected.status)}</span></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10,marginTop:18}}>
          {[["Customer",selected.customer.name],["Contact",selected.customer.maskedPhone],["Pets",selected.pets.map(pet=>`${pet.name}${pet.breed?` · ${pet.breed}`:""}`).join(" | ")],["Vaccination",selected.pets.map(pet=>pet.vaccinationStatus).join(", ")],["Payment",`${statusLabel(selected.payment.mode)} · ${statusLabel(selected.payment.status)}`],["Package value",money(selected.totalAmount)],["Work order",selected.workOrderId],["Provider model",statusLabel(selected.providerModel)]].map(([label,value])=><div key={label} style={{border:"1px solid #e9f1ee",borderRadius:11,padding:12}}><small style={{color:"#6c7d79"}}>{label}</small><b style={{display:"block",marginTop:4}}>{value}</b></div>)}
        </div>
        <div style={{marginTop:18,padding:14,borderRadius:12,background:"#f2f7f5"}}><b>Service proof</b><p style={{margin:"7px 0 0",fontSize:13}}>{selected.proof?`${selected.proof.beforePhotoRef?"Before ✓":"Before —"} · ${selected.proof.afterPhotoRef?"After ✓":"After —"} · Checklist ${selected.proof.checklist.length}`:"Not captured yet"}</p>{selected.invoice&&<p style={{margin:"6px 0 0",fontSize:13}}><b>Invoice:</b> {selected.invoice.invoiceNumber} · {money(selected.invoice.netAmount)}</p>}</div>
        <GroomingRouteCard bookingId={selected.bookingId} providerId={selected.providerId} />
        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:18}}><button disabled={busy||!nextAction} onClick={()=>nextAction&&void act(nextAction)} style={{padding:"11px 16px",border:0,borderRadius:10,background:"#01261F",color:"white",fontWeight:800,opacity:busy||!nextAction?0.7:1}}>{busy?"Updating…":actionLabel}</button>{canDecline&&<button disabled={busy} onClick={()=>void act("decline")} style={{padding:"11px 16px",borderRadius:10,border:"1px solid #dcece5",background:"white",fontWeight:700}}>Decline job</button>}<button onClick={()=>setRefreshKey(value=>value+1)} style={{padding:"11px 16px",borderRadius:10,border:"1px solid #dcece5",background:"white",fontWeight:700}}>Reload timeline</button></div>
        <div style={{marginTop:20}}><small style={{fontWeight:800,color:"#1f6b57"}}>CANONICAL TIMELINE</small><div style={{display:"grid",gap:8,marginTop:9}}>{selected.events.slice(0,8).map((event,index)=><div key={`${event.eventType}-${event.occurredAt}-${index}`} style={{display:"flex",justifyContent:"space-between",gap:12,borderBottom:"1px solid #e9f1ee",paddingBottom:8}}><span style={{textTransform:"capitalize"}}>{statusLabel(event.eventType)}</span><small>{new Date(event.occurredAt).toLocaleString("en-IN")}</small></div>)}</div></div>
        <p style={{fontSize:12,color:"#6e7f7b",marginTop:16}}>Canonical UAT data only. Customer phone remains masked; proof references are synthetic until secure media storage is connected.</p>
      </>}
    </section>
  </div>;
}
