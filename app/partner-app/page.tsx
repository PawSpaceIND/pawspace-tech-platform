...
export default function PartnerMobileApp() {
  ...
  const paymentRefreshPending = Boolean(paymentRequest && !["captured", "refunded", "partially_refunded"].includes(paymentRequest.paymentStatus));
  ...
  // A provider webhook can settle the link while this screen stays open. Refresh only while the
  // request is pending; terminal payment truth stops the timer and the checkout disappears.
  useEffect(() => { if (!selected?.bookingId || !paymentRefreshPending) return; let active=true; const timer=setInterval(() => { void fetch(`/api/grooming-payment-sandbox?bookingId=${encodeURIComponent(selected.bookingId)}`, { cache: "no-store" }).then(async response => { const body=await response.json() as { data?: PaymentRequest }; if(active&&response.ok)setPaymentRequest(body.data??null); }); }, 5_000); return()=>{active=false;clearInterval(timer);}; }, [selected?.bookingId, paymentRefreshPending]);
