...
type Job = {
  ...
  invoice: { invoiceNumber: string; status: string; netAmount: number; totalAmount?: number; issuedAt: number } | null;
  events: JobEvent[];
};
  ...
  return <main className={styles.viewport}>
    ...
    <section className={styles.phoneShell}>
      ...
      <section className={styles.content}>
        ...
        {identity?.subjectId && <section className={styles.notice} aria-live="polite"><b>{statusQueue.connection==="Online"?"🟢":statusQueue.connection==="Offline"?"🔴":"🟠"} {statusQueue.connection}</b>{statusQueue.connection!=="Online"&&<p>Connection lost — reconnecting…</p>}{dutyJob&&<><h2>You are ON DUTY</h2><p>{trackingNotice}</p></>}{statusQueue.pending.filter(item=>item.error).map(item=><p key={item.id} role="alert"><b>{label(item.action)} was not accepted:</b> {item.error} <button type="button" onClick={()=>void statusQueue.dismiss(item.id)}>Dismiss</button></p>)}{statusQueue.pending.length>0&&<>{statusQueue.pending.some(item=>!item.error)&&<p>{statusQueue.pending.filter(item=>!item.error).length} update(s) saved on this device, waiting to sync.</p>}{statusQueue.pending.filter(item=>!item.error).map(item=><p key={item.id}>{label(item.action)} · Will sync automatically when connected</p>)}<button onClick={statusQueue.retry}>Retry saved updates</button></>}{statusQueue.error&&<p role="alert">{statusQueue.error}</p>}</section>}
