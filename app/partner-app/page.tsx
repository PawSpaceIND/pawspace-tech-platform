...
type PaymentRequest = { status: string; paymentStatus: string; amount: number; paymentPath: string; qrPayload: string; sandboxOnly: boolean; liveCapture: boolean };
      ...
      <section className={styles.content}>
        ...
        {tab === "jobs" && <>
          ...
          {selected && <section className={styles.detailCard}>
            ...
            {selected.status === "completed" && selected.payment.mode === "pay_after_service" && selected.payment.status !== "captured" && <section className={styles.notice}><b>Payment due after service</b>{!paymentRequest ? <><p>Create a shareable UAT payment request and QR payload. This does not capture money.</p><button disabled={busy} onClick={() => void requestPayment()}>Create payment request</button></> : <><p><b>{money(paymentRequest.amount)}</b> · {label(paymentRequest.status)}</p><p><code>{paymentRequest.paymentPath}</code></p><p><code>{paymentRequest.qrPayload}</code></p><small>Sandbox request only. Payment remains unpaid until a signature-verified gateway capture is reconciled.</small></>}</section>}
