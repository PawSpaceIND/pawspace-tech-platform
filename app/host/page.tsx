...
const awaitingStatuses=["awaiting_host_acceptance","recovery_pending"];
/** Where an awaiting stay belongs for this host: its own request list only while the offer is open and the stay is still ahead. */
function requestBucket(stay:BoardingStay){if(windowEnded(stay.check_out_at,stay.check_in_at))return"past";const state=stay.offer?.state;return state==="expired"||state==="withdrawn"?"needs_operations":"active";}
...
function HostPageContent(){
 const searchParams=useSearchParams(),pathname=usePathname(),requestedBookingId=searchParams.get("bookingId")||"",inV2=pathname.startsWith("/v2/partner/host"),customerHref=inV2?"/v2/boarding":"/boarding",hostBase=inV2?"/v2/partner/host":"/host";
 ...
 // Only an open offer on a stay that has not already ended is waiting on this host. Expired or reassigned offers are Operations' to resolve, and a
 // stay whose window has passed is history: neither is counted as a pending request or offered an Accept that the lifecycle would refuse.
 const pending=useMemo(()=>stays.filter(item=>["awaiting_host_acceptance","recovery_pending"].includes(item.status)&&requestBucket(item)==="active"),[stays]);
 ...
 const selectedPaymentCaptured=selected?.payment_status==="captured";
 const selectedBucket=selected&&awaitingStatuses.includes(selected.status)?requestBucket(selected):undefined,selectedOffer=describeProviderOffer(selected?.offer??(selected&&awaitingStatuses.includes(selected.status)?{state:"open",expiresAt:null}:null),{noun:"stay",bucket:selectedBucket}),selectedCanAccept=Boolean(selected)&&acceptAvailable(selected?.offer??{state:"open"},selectedBucket);
 ...
 return <main className={styles.shell}>
  ...
  <section className={styles.main}>
   ...
   {!loading&&tab==="requests"&&<section className={styles.requestLayout}>
    ...
    <aside className={styles.panel}>{selected&&awaitingStatuses.includes(selected.status)?<><span className={styles.match}>{selected.status==="recovery_pending"?"Recovery offer":"Canonical offer"}</span><h2>{selected.package_name||selected.package_code}</h2><p>{stayWindow(selected)}<br/>{petSummary(selected)} · Booking {selected.booking_id}</p><div className={styles.marketSync} role="status" aria-label="Offer status"><b>{selectedOffer.label}</b><span>{selectedOffer.detail}</span></div><HostStayCare stay={selected}/><div className={styles.careNotes}><strong>Pre-acceptance controls</strong><span>✓ Host identity is server-owned</span><span>✓ Home verification and capacity are rechecked</span><span>{selectedPaymentCaptured?"✓ Customer payment captured":"! Customer payment pending · acceptance locked"}</span><span>✓ Host cannot override PawSpace price</span><span>✓ Payout formula is not yet configured</span></div><dl><div><dt>Customer booking value</dt><dd>₹{Number(selected.total_amount||0).toLocaleString("en-IN")}</dd></div><div><dt>Host price override</dt><dd>Disabled</dd></div><div><dt>Payout</dt><dd>Rule pending</dd></div></dl>{selectedCanAccept?<div className={styles.actions}><button disabled={!selectedPaymentCaptured||isBusy(selected,"accept")} onClick={()=>act(selected,"accept")}>{!selectedPaymentCaptured?"Payment pending · acceptance locked":selected.status==="recovery_pending"?"Accept replacement & lock capacity":"Accept & lock capacity"}</button><button disabled={isBusy(selected,"decline")} onClick={()=>decline(selected)}>Decline with reason</button><button disabled={isBusy(selected,"host_unavailable")} onClick={()=>unavailable(selected)}>Mark unavailable</button></div>:<p>Nothing to accept here: {selectedOffer.detail}</p>}</>:<p>Select a pending canonical stay request.</p>}</aside>
   </section>}
