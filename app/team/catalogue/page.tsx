...
export default function CataloguePage() {
  ...
  return <main style={wrap}><div style={{ maxWidth: 1200, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 18 }}><div><small style={{ fontWeight: 800, color: "#6c39a8" }}>PAWSPACE TEAM · CATALOGUE</small><h1 style={{ margin: "7px 0" }}>Packages & prices</h1><p style={{ margin: 0, color: "#746b7d" }}>Create packages for any service, city- and zone-wise. Zone &gt; city &gt; global price precedence.</p></div><StaffGatedLink href="/team" permission="dashboard.view" style={{ padding: 10, background: "#4b168c", color: "white", borderRadius: 10, textDecoration: "none" }}>Team home</StaffGatedLink></header>
    ...
    <div style={{ ...card, overflow: "hidden" }}>
      <div style={{ padding: 14, borderBottom: "1px solid #eee6f5" }}><b>{rows.length} package{rows.length === 1 ? "" : "s"}</b></div>
      {rows.length === 0 ? <p style={{ padding: 18, color: "#746b7d" }}>No packages yet — add one above.</p> : rows.map(p => <article key={p.id} style={{ padding: 14, borderBottom: "1px solid #f0ebf4", display: "flex", justifyContent: "space-between", gap: 12 }}><div><strong>{p.name}</strong> <small style={{ color: "#746b7d" }}>· {p.serviceCode} · {p.packageCode}</small><small style={{ display: "block", marginTop: 3, color: "#746b7d" }}>{p.cityId}{p.zoneId ? ` / ${p.zoneId}` : ""} · v{p.version} · {p.active ? "active" : "inactive"}</small></div><div style={{ fontWeight: 800, fontSize: 17 }}>₹{Number(p.basePrice).toLocaleString("en-IN")}</div></article>)}
    </div>
  </div></main>;
}
