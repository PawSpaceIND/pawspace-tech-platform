...
export default function AcquisitionFunnelPage() {
  ...
  const stat = (label: string, value: number | undefined, hint?: string) => <div style={{ ...card }}><div style={{ fontSize: 12, color: "#6c7c78", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em" }}>{label}</div><div style={{ fontSize: 30, fontWeight: 800, margin: "6px 0" }}>{Number(value || 0).toLocaleString("en-IN")}</div>{hint && <div style={{ fontSize: 12, color: "#6c7c78" }}>{hint}</div>}</div>;
  return <main style={wrap}><div style={{ maxWidth: 1200, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 18 }}>
      <div><small style={{ fontWeight: 800, color: "#1f6b57" }}>PAWSPACE TEAM · ACQUISITION FUNNEL</small><h1 style={{ margin: "7px 0" }}>App-to-Revenue funnel</h1><p style={{ margin: 0, color: "#6c7c78" }}>Download → identify → booked / payment-pending / not-booked → convert. Payment-truthful.</p></div>
      <div style={{ display: "flex", gap: 8 }}><button disabled={busy} onClick={refresh} style={{ padding: 10, background: "#F6920A", color: "white", border: 0, borderRadius: 10, fontWeight: 800 }}>{busy ? "Refreshing…" : "Refresh sweep"}</button><Link href="/team" style={{ padding: 10, background: "#01261F", color: "white", borderRadius: 10, textDecoration: "none" }}>Team home</Link></div>
    </header>
    ...
    <p style={{ fontSize: 12, color: "#6c7c78", marginTop: 18 }}>Advisory intelligence — outreach stays human-launched. Refresh recomputes the funnel from the live book of record.</p>
  </div></main>;
}
