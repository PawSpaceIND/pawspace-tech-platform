...
export default function AcquisitionFunnelPage() {
  ...
  return <StaffModule><main style={wrap}><div style={{ maxWidth: 1200, margin: "0 auto" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} data-staff-grid="split">
      <div><h3>Payment recovery (₹300)</h3><section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }} data-staff-grid="split">{stat("Issued", rec.issued)}{stat("Active", rec.active)}{stat("Redeemed", rec.redeemed)}{stat("Recovered", rec.recoveredBookings, "bookings")}</section></div>
      <div><h3>Inbound to Sales</h3><section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }} data-staff-grid="split">{stat("App-Inbound active", (inb.appInbound || {}).active)}{stat("Recovery tasks active", (inb.paymentRecovery || {}).active)}</section></div>
    </div>
    <p style={{ fontSize: 14, color: "var(--staff-muted)", marginTop: 18 }}>Advisory intelligence — outreach stays human-launched. Refresh recomputes the funnel from the live book of record.</p>
