...
const card = { background: "white", border: "1px solid #dcece5", borderRadius: 14, padding: 16 } as const;
...
export default function ProviderVerificationPage() {
  ...
  return <main style={wrap}><div style={{ maxWidth: 1100, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 18 }}><div><small style={{ fontWeight: 800, color: "#1f6b57" }}>PAWSPACE TEAM · PROVIDER VERIFICATION</small><h1 style={{ margin: "7px 0" }}>KYC mandate (IDfy)</h1><p style={{ margin: 0, color: "#6c7c78" }}>Per-category checks. A provider takes assignments only when every mandated check is verified. IDfy fail-closed.</p></div><Link href="/team" style={{ padding: 10, background: "#01261F", color: "white", borderRadius: 10, textDecoration: "none" }}>Team home</Link></header>
    ...
    <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12, marginBottom: 18 }}>
      {(snap.categories || []).map(c => <div key={c.category} style={card}><div style={{ fontWeight: 800, textTransform: "capitalize", marginBottom: 6 }}>{c.category.replace("_", " ")}</div>{c.required.map(t => <div key={t} style={{ fontSize: 13, color: "#01261F", padding: "2px 0" }}>• {t.replace(/_/g, " ")}</div>)}</div>)}
    </section>
    <div style={{ ...card, marginBottom: 14 }}>
      ...
      {status && <div style={{ marginTop: 12 }}><span style={{ fontWeight: 800, color: status.canTakeAssignments ? "#1f8a5b" : "#c47a00" }}>{status.canTakeAssignments ? "✓ Eligible to take assignments" : "⏳ Not yet eligible"}</span>{(status.checks || []).map(c => <div key={c.verificationType} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #e9f1ee" }}><span>{c.verificationType.replace(/_/g, " ")} <small style={{ color: "#6c7c78" }}>· {c.automatable ? "IDfy" : "manual"}</small></span><span style={{ fontWeight: 700 }}>{c.status}</span></div>)}</div>}
    </div>
    <p style={{ fontSize: 12, color: "#6c7c78" }}>Automatable checks (Aadhaar/PAN/address) run via IDfy once its keys are set; house/police/pet-proofing are agent-recorded. Nothing auto-approves.</p>
  </div></main>;
}
