...
type AiAudit = { call: Record<string, unknown>; segments: Array<Record<string, unknown>>; events: Array<Record<string, unknown>>; truth: { productionTelephony: boolean; rawAudioStored: boolean } };
...
const stateTone = (state: string): "success" | "warning" | "danger" | "info" | "neutral" =>
  state === "completed" ? "success" : state.startsWith("blocked_") ? "danger" : state.startsWith("provider_") || state === "failed" ? "warning" : state === "ai_handoff" || state === "handoff_requested" ? "info" : "neutral";
...
async function aiApi<T>(callId: string): Promise<T> {
  const response = await fetch(`/api/ai-voice-uat?callId=${encodeURIComponent(callId)}`, { cache: "no-store" });
  const payload = await response.json() as { data?: T; error?: string };
  if (!response.ok) throw new Error(payload.error || `AI voice audit failed (${response.status})`);
  return payload.data as T;
}
...
export default function VoiceOperatorPage() {
  ...
  const refreshAudit = async (callId: string) => {
    const next = await api<Audit>({ query: `?scope=audit&callId=${encodeURIComponent(callId)}` });
    setAudit(next);
    if (next.call.transcriptRef) {
      try { setAiAudit(await aiApi<AiAudit>(next.call.transcriptRef)); }
      catch { setAiAudit(null); }
    } else setAiAudit(null);
  };
  ...
  const callAction = (callId: string, action: string, extra: Record<string, unknown> = {}) => run(`${action}:${callId}`, async () => {
    ...
    await load();
    if (audit?.call.callId === callId) await refreshAudit(callId);
  });

  const openAudit = (callId: string) => run(`audit:${callId}`, async () => { await refreshAudit(callId); });
  const closeAudit = () => { setAudit(null); setAiAudit(null); };
    ...
    >
      ...
      <TeamSection title="Environment" note="Read-only. Secret NAMES are shown so a missing one can be identified; no value is ever returned to this page.">
        <TeamTable head={["Setting", "State", "Detail"]} rows={gate ? [
          ...
          ["UAT approved", <Badge key="u" tone={gate.uatApproved ? "success" : "neutral"}>{gate.uatApproved ? "yes" : "no"}</Badge>, "Approval to run controlled UAT"],
          ...
          ["Outbound sales", <Badge key="o" tone={gate.salesOutboundApproved ? "warning" : "neutral"}>{gate.salesOutboundApproved ? "approved" : "not approved"}</Badge>, "Sales use cases stay unavailable without approval"],
      ...
      <TeamSection title="Place a call" note="Check policy first. The dial button stays disabled until a policy check for this exact request comes back allowed.">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 12, marginBottom: 12 }}>
          <label>Use case<select value={form.useCase} onChange={setField("useCase")} disabled={!dialAllowedByGate}><option value="">Select a use case…</option>{(readiness?.useCases ?? []).map(item => <option key={item.code} value={item.code} disabled={!item.availableNow}>{item.label}{item.availableNow ? "" : " — needs outbound sales approval"}</option>)}</select></label>
        ...
        {previewIsCurrent && preview && <div style={{ marginTop: 14 }}><TeamTable head={["Policy check", "Result", "Detail"]} rows={preview.result.checks.map(check => [check.code, <Badge key={check.code} tone={check.passed ? "success" : "danger"}>{check.passed ? "pass" : "blocked"}</Badge>, check.detail])} /></div>}
      </TeamSection>

      <TeamSection title="Call ledger" note="Every refusal is recorded here with the reason. Only the last four digits of a number are returned by the API." actions={<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><select value={stateFilter} onChange={event => setStateFilter(event.target.value)}><option value="">All states</option>{states.map(state => <option key={state} value={state}>{state}</option>)}</select><Button variant="ghost" onClick={() => void run("refresh", load)} disabled={Boolean(busy)}>Refresh</Button></div>}>
        <TeamTable head={["Call", "State", "Use case", "Number", "Consent / opt-out / quiet hours", "Provider", "Operator actions"]} rows={ledger.map(row => [
          ...
          <span key="uc">{row.useCase}<small> · {row.purpose}</small></span>, maskedNumber(row.phoneLast4),
          ...
          <span key="ac" style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><Button size="sm" variant="ghost" onClick={() => void openAudit(row.callId)}>Audit</Button><Button size="sm" variant="secondary" onClick={() => void callAction(row.callId, "handoff", { reason: "Operator escalated to a human" })} disabled={Boolean(busy)}>Hand off</Button><Button size="sm" variant="secondary" onClick={() => void callAction(row.callId, "opt_out", { reason: "Recipient asked not to be called" })} disabled={Boolean(busy)}>Opt out</Button><Button size="sm" variant="secondary" onClick={() => void callAction(row.callId, "retry")} disabled={Boolean(busy) || !dialAllowedByGate}>Retry</Button><Button size="sm" variant="danger" onClick={() => void callAction(row.callId, "cancel", { reason: "Cancelled by operator" })} disabled={Boolean(busy)}>Cancel</Button></span>,
        ])} empty="No calls have been requested. A refused call is still recorded, so an empty ledger means nothing has been attempted." />
      </TeamSection>

      {audit && <TeamSection title={`Audit · ${audit.call.callId}`} note={audit.truth.productionCallExecuted ? "A production call was executed for this record." : "No production call was executed for this record."} actions={<Button variant="ghost" onClick={closeAudit}>Close</Button>}>
        ...
        {audit.call.transcriptRef && !aiAudit && <TeamAlert tone="info">A transcript reference is attached, but its governed AI voice audit is unavailable.</TeamAlert>}
        {aiAudit && <>
          <TeamTable head={["Segment", "Speaker", "Transcript", "STT", "Confidence", "Interrupted", "At"]} rows={aiAudit.segments.map(segment => [String(segment.segment_index), String(segment.speaker), String(segment.transcript_text), String(segment.stt_provider ?? "—"), segment.stt_confidence == null ? "—" : String(segment.stt_confidence), Number(segment.interrupted) === 1 ? <Badge key={`i${segment.segment_index}`} tone="warning">barge-in</Badge> : "no", IST(segment.created_at)])} empty="No transcript segments recorded." />
          <TeamTable head={["Voice event", "Detail", "At"]} rows={aiAudit.events.map((event, index) => [<Badge key={`e${index}`} tone={String(event.event_type) === "barge_in" || String(event.event_type) === "live_agent_transfer" ? "warning" : "neutral"}>{String(event.event_type)}</Badge>, String(event.detail_json ?? "{}"), IST(event.created_at)])} empty="No AI voice events recorded." />
        </>}
      </TeamSection>}
      ...
      <TeamSection title="Scripts" note="A script containing a claim word cannot be activated until a human has approved that script's claims.">
        <TeamTable head={["Use case", "Active", "Claims approved", "Version"]} rows={(readiness?.scripts ?? []).map(script => [script.useCase, <Badge key={`a${script.useCase}`} tone={script.active ? "success" : "neutral"}>{script.active ? "active" : "inactive"}</Badge>, <Badge key={`c${script.useCase}`} tone={script.claimsApproved ? "warning" : "neutral"}>{script.claimsApproved ? "approved" : "not approved"}</Badge>, script.version])} empty="No scripts seeded." />
      </TeamSection>
    </TeamShell>
  );
}
