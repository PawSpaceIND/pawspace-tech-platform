...
export default function I18nPage() {
  ...
  return <main style={wrap}><div style={{ maxWidth: 900, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 18 }}><div><small style={{ fontWeight: 800, color: "#6c39a8" }}>PAWSPACE TEAM · LANGUAGES</small><h1 style={{ margin: "7px 0" }}>Multi-language coverage</h1><p style={{ margin: 0, color: "#746b7d" }}>8 locales · English is the always-present fallback · AI drafts translations you review &amp; publish.</p></div><Link href="/team" style={{ padding: 10, background: "#4b168c", color: "white", borderRadius: 10, textDecoration: "none" }}>Team home</Link></header>
    ...
    <div style={{ ...card, overflow: "hidden" }}>
      <div style={{ padding: 14, borderBottom: "1px solid #eee6f5" }}><b>{base} base keys (English)</b></div>
      {(cov.locales || []).map(l => <div key={l.locale} style={{ padding: 14, borderBottom: "1px solid #f0ebf4", display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 130 }}><strong>{l.name}</strong> <small style={{ color: "#746b7d" }}>{l.locale}</small></div>
        <div style={{ flex: 1, background: "#eee6f5", borderRadius: 8, height: 10, overflow: "hidden" }}><div style={{ width: `${l.coveragePct}%`, height: "100%", background: "#F6920A" }} /></div>
        <div style={{ width: 130, textAlign: "right", fontSize: 13, color: "#746b7d" }}>{l.published} pub · {l.draft} draft · {l.coveragePct}%</div>
        {l.locale !== "en" && <button disabled={Boolean(busy)} onClick={() => aiTranslate(l.locale)} style={{ background: "#472792", color: "white", border: 0, borderRadius: 8, fontWeight: 800, padding: "8px 12px" }}>{busy === l.locale ? "…" : "AI draft"}</button>}
      </div>)}
    </div>
    <p style={{ fontSize: 12, color: "#746b7d", marginTop: 14 }}>AI translation is fail-closed — with no AI key set it drafts nothing. Drafts never auto-publish; a human publishes before they go live.</p>
  </div></main>;
}
