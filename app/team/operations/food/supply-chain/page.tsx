...
export default function FoodSupplyChainPage(){
 ...
 return <StaffModule><main style={{maxWidth:1400,margin:"0 auto",padding:24,fontFamily:"inherit",display:"grid",gap:16}}>
  ...
  {snapshot&&<section style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(140px,1fr))",gap:12}} data-staff-grid="stats">
  ...
  {snapshot&&snapshot.reorderSuggestions.length>0&&<section style={{border:"1px solid var(--staff-line)",borderRadius:14,padding:16}}>
  ...
  {snapshot&&snapshot.expiry.expiringWithin48h.length>0&&<section style={{border:"1px solid var(--staff-line)",borderRadius:14,padding:16}}>
  ...
  <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:16}} data-staff-grid="stats">
  ...
  {snapshot&&<section style={{overflowX:"auto"}}>
   ...
   <table style={{borderCollapse:"collapse",width:"100%"}}>
    <thead><tr>{["Batch","SKU","Zone","Kitchen","Received","Remaining","Prepared","Expiry","Status"].map(header=><th key={header} style={{textAlign:"left",borderBottom:"2px solid var(--staff-line)",padding:"6px 10px"}}>{header}</th>)}</tr></thead>
    <tbody>{snapshot.batches.map(batch=><tr key={String(batch.id)}>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}><code>{String(batch.id)}</code></td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{String(batch.sku)}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{String(batch.zone_id)}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{String(batch.kitchen_id||"—")}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{Number(batch.quantity_received)}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{Number(batch.quantity_remaining)}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{String(batch.preparation_date)}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{String(batch.expiry_date)}</td>
     <td style={{padding:"6px 10px",borderBottom:"1px solid var(--staff-line)"}}>{label(batch.status)}</td>
    </tr>)}</tbody>
   </table>
  </section>}
