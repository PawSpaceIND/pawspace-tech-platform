...
const card={background:"var(--staff-surface)",border:"1px solid var(--staff-line)",borderRadius:14,padding:14};
...
function Requirement({label,met,detail}:{label:string;met:boolean;detail:string}){
  return <li style={{display:"grid",gridTemplateColumns:"22px 1fr",gap:8,alignItems:"start",padding:"6px 0"}}>
    <span aria-hidden="true" style={{fontWeight:800,color:met?"var(--staff-success)":"var(--staff-danger)"}}>{met?"✓":"✗"}</span>
    <span><b>{label}</b> <small style={{display:"block",color:"var(--staff-muted)"}}>{detail}</small></span>
  </li>;
}
...
export default function AiConfigurationPage(){
  ...
  return <StaffModule><main style={{minHeight:"100vh",background:"var(--staff-bg)",padding:28,fontFamily:"inherit",color:"var(--staff-text)"}}><div style={{maxWidth:1300,margin:"0 auto"}}>
    ...
    {notice&&<div role="status" style={{padding:12,background:"var(--staff-success-bg)",borderRadius:10,marginBottom:12}}>{notice}</div>}

    {status&&<section style={{...card,marginBottom:14}}>
      <h2 style={{marginTop:0}}>Is the assistant switched on?</h2>
      <p style={{margin:"0 0 8px",color:"var(--staff-muted)"}}>
        Answering staff: <b style={{color:status.answersStaff?"var(--staff-success)":"var(--staff-danger)"}}>{status.answersStaff?"yes":"no"}</b> ·
        answering customers: <b style={{color:status.answersCustomers?"var(--staff-success)":"var(--staff-danger)"}}>{status.answersCustomers?"yes":"no"}</b>.
      ...
      <div style={{marginTop:12,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        <button disabled={busy==="bootstrap"} onClick={()=>void installStarterGrounding()} style={{padding:"10px 14px",background:"var(--staff-primary)",color:"var(--staff-on-primary)",border:"none",borderRadius:10,fontWeight:700,cursor:"pointer"}}>{busy==="bootstrap"?"Installing…":status.configurationRequired?"Install starter assistant grounding":"Reinstall starter grounding (new version)"}</button>
        <small style={{color:"var(--staff-muted)"}}>Creates the PawSpace assistant profile, system policy, approved knowledge and intent catalogue through the normal draft → review → approve → activate lifecycle. Re-running supersedes the current versions rather than editing them.</small>
      </div>
    </section>}
    ...
    {!data?<p>Loading…</p>:<>
      ...
      <section style={card}><h2 style={{marginTop:0}}>Kill switches &amp; audit</h2><p>Active switches: {data.killSwitches.filter(row=>Number(row.disabled)===1).length} · Audit events: {data.auditEvents.length}</p><p style={{fontSize:14,color:"var(--staff-muted)"}}>Configuration changes require settings.manage and are separately security-audited. Active versions are immutable snapshots; changes create new versions.</p></section>
    </>}
