...
export function AtlasChat(){
 ...
 const load=useCallback(async(signal?:AbortSignal)=>{const response=await fetch("/api/admin/atlas-chat?limit=30",{cache:"no-store",signal});const body=await response.json();if(!response.ok)throw new Error(body.error||"Founder access is required to read Atlas conversations.");setMessages(body.data?.messages||[]);},[]);
 useEffect(()=>{const controller=new AbortController();void load(controller.signal).catch(e=>{if(!controller.signal.aborted)setError(e instanceof Error?e.message:"Unable to load Atlas conversations.");}).finally(()=>{if(!controller.signal.aborted)setLoading(false);});return()=>controller.abort();},[load]);
 async function ask(event:React.FormEvent){event.preventDefault();const message=question.trim();if(!message||pending.current)return;pending.current=true;setBusy(true);setError("");setAnswer(null);const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),150000);
  try{const response=await fetch("/api/admin/atlas-chat",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"ask",message}),signal:controller.signal});const body=await response.json();if(!response.ok)throw new Error(body.error||"Atlas could not answer this question.");setAnswer(body.data);setQuestion("");try{await load(controller.signal);}catch{setError("Your answer was received, but history could not refresh. Reload history before sending again.");}}
