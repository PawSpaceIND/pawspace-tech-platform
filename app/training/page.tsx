...
import{TRAINING_FALLBACK_GOALS}from"../../lib/training-goals";
...
export default function TrainingPage({routeScope="legacy"}:{routeScope?:"legacy"|"v2"}={}){
 ...
 const calendarPlan=useMemo(()=>{if(!currentQuote)return{windows:[],problem:""};try{return{windows:trainingCalendarWindows(scheduledStart,currentQuote,cadenceDays),problem:""};}catch(problem){return{windows:[],problem:problem instanceof Error?problem.message:"This programme calendar does not fit its validity."};}},[currentQuote,scheduledStart,cadenceDays]);
  const calendar=calendarPlan.windows;
 ...
 if(pendingCheckout)return <main className={styles.shell}>
  ...
  <section className={styles.card} aria-label="Reserved training details"><h2>{pendingCheckout.summary.packageName}</h2><p>Booking reference: {pendingCheckout.booking.bookingId}</p><p>Trainer: {pendingCheckout.summary.providerName} · {pendingCheckout.summary.petNames}</p><p>First session: {formatIndiaRange(pendingCheckout.summary.start,pendingCheckout.summary.end)}</p><p>Service address: {pendingCheckout.summary.address}</p><Link href={bookingRecordHref(pendingCheckout.booking.bookingId)}>View reserved sessions and payment status</Link></section>
 ...
 if(booking&&programme)return <main className={styles.shell}><header><Link href={href("/")}>PawSpace</Link><p>{routeScope==="v2"?"DOG TRAINING":"TRAINING · CANONICAL UAT"}</p><h1>Training programme confirmed</h1><p>{routeScope==="v2"?"Your trainer and sessions are booked. You can see and manage them under Activity.":"Booking, trainer assignment, payment ledger and programme sessions now share one canonical identity."}</p></header>{error&&<p role="alert">{error}</p>}<section className={styles.grid3}><article className={styles.card}><small>Booking</small><strong className={styles.block}>{booking.bookingId}</strong><span>{label(booking.status)}</span></article><article className={styles.card}><small>Programme</small><strong className={styles.block}>{programme.programme.id}</strong><span>{routeScope==="v2"?trainingSessionCount(Number(programme.programme.total_sessions)):`${programme.programme.total_sessions} session(s)`}</span></article><article className={styles.card}><small>Trainer</small><strong className={styles.block}>{confirmedProviderName||programme.programme.provider_id}</strong><span>{routeScope==="v2"?"Your trainer":"Canonical scheduler assignment"}</span></article></section><section className={styles.card}><h2>Programme sessions</h2>{programme.sessions.map(session=><article key={session.id} className={styles.sessionRow}><strong>Session {session.sequence_no} · {routeScope==="v2"?customerTrainingSessionStatus(session.status):label(session.status)}</strong><div>{formatIndiaRange(session.scheduled_start,session.scheduled_end)}</div>{routeScope!=="v2"&&<small>{session.id} · trainer {session.provider_id}</small>}</article>)}</section>{routeScope!=="v2"&&<section className={styles.card}><h2>UAT boundaries</h2><p>Payment is confirmed only from verified Razorpay sandbox evidence. Production media storage/scanning, GST/tax invoicing, payout execution and external messaging remain configuration/launch dependencies.</p><div className={styles.actions}><Link href={href("/mobile-app")}>My PawSpace</Link><button onClick={()=>{window.location.assign(pathname)}}>Book another programme</button></div></section>}</main>;
 ...
 return <main className={styles.shell}><header><Link href={href("/")}>PawSpace</Link><p>{routeScope==="v2"?"DOG TRAINING":"DOG TRAINING · CANONICAL UAT"}</p><h1>{routeScope==="v2"?"Choose the right training programme":"Choose a server-owned Training programme"}</h1><p>{routeScope==="v2"?"Pick your dogs, a plan and a time. Prices and trainer availability are checked live for every session.":"Catalogue, price, trainer eligibility and schedule are read from PawSpace governance. The browser no longer invents a plan, trainer or progress journey."}</p></header>{routeScope==="v2"&&recoveryBookingId&&<section className={styles.card} aria-label="Existing training booking"><p>Already reserved a programme? Your booking, its payment status and Manage link are on its own page.</p><div className={styles.actions}><Link href={bookingRecordHref(recoveryBookingId)}>View booking & payment →</Link></div></section>}{error&&<p role="alert">{error}</p>}<section className={styles.card}><h2>1. Pet and first session</h2><div className={styles.grid2}><div role="group" aria-labelledby="training-dogs-label"><span id="training-dogs-label">Dogs</span>{accountLoading?<p>Loading your pets…</p>:accountError?<p role="alert">{accountError} <Link href={href("/mobile-app")}>Sign in →</Link></p>:dogs.length===0?<p>No dogs on your profile yet. <Link href={href("/mobile-app")}>Add one in My PawSpace →</Link></p>:<div className={styles.petList}>{dogs.map(pet=><button key={pet.id} type="button" onClick={()=>togglePet(pet.id)} aria-pressed={selectedPetIds.includes(pet.id)} className={styles.choice}><strong>{pet.name}</strong><small className={styles.block}>{petLabel(pet)}</small></button>)}<small>{petCount} {petCount===1?"dog":"dogs"} selected{account?` · booking as ${account.name}`:""}</small>{packageCode!=="trainer-meet-greet"&&selectedPets.some(pet=>!isVaccinatedStatus(pet.vaccinationStatus))&&<p role="note">Vaccinations for {selectedPets.filter(pet=>!isVaccinatedStatus(pet.vaccinationStatus)).map(pet=>pet.name).join(", ")} must be verified before the first programme session. You can book now and update it in My PawSpace; a Meet &amp; Greet needs no proof.</p>}</div>}</div>{!accountLoading&&!accountError&&dogs.length>0&&<fieldset className={styles.petList}><legend>What should training focus on? (optional)</legend>{goalOptions.map(goal=><button key={goal} type="button" aria-pressed={goals.includes(goal)} className={styles.choice} onClick={()=>setGoals(current=>current.includes(goal)?current.filter(item=>item!==goal):[...current,goal])}>{goal}</button>)}<label>Anything your trainer should know? (optional)<textarea value={behaviourNotes} maxLength={500} onChange={event=>setBehaviourNotes(event.target.value)} className={styles.input} placeholder="For example: pulls on the lead, nervous around other dogs"/></label></fieldset>}<label>First session date<input type="date" value={date} onChange={event=>setDate(event.target.value)} className={styles.input}/></label><label>First session time (IST)<input type="time" value={time} step={900} onChange={event=>setTime(event.target.value)} className={styles.input}/></label><label>Days between sessions<select value={cadenceDays} disabled={packageCode==="trainer-meet-greet"} onChange={event=>setCadenceDays(Number(event.target.value))}>{[1,2,3,4,5,6,7].map(value=><option key={value} value={value}>{value===7?"Every week":`Every ${value} day${value===1?"":"s"}`}</option>)}</select></label></div><section aria-label="Training service address"><strong>Service address</strong><p>{savedAddressText}</p><Link href={routeScope==="v2"?"/v2/account":"/mobile-app"}>Manage saved address</Link><p>The same saved doorstep is used for all sessions. Each session must fit the programme validity.</p></section>{locationLoading?<p>Confirming your training zone…</p>:locationError?<p role="alert">{locationError}</p>:location?<small>Training zone {location.zoneName} · confirmed from your address PIN code</small>:null}</section><section className={styles.card}><h2>2. Programme</h2>{catalogueLoading?<p>{routeScope==="v2"?"Loading training plans…":"Loading canonical Training catalogue…"}</p>:<div className={styles.grid2}>{packages.map(item=><button key={item.package_code} onClick={()=>setPackageCode(item.package_code)} aria-pressed={packageCode===item.package_code} className={styles.choice}><strong>{item.name}</strong><div>{routeScope==="v2"?trainingSessionCount(item.sessions):`${item.sessions} session(s)`} · valid {item.validity_days} days</div><b>{money(trainingPriceForPets(item.base_price,Math.max(1,petCount),item.extra_pet_percent))}</b>{petCount>1&&<small className={styles.block}>for {petCount} dogs</small>}<small className={styles.block}>{item.meet_and_greet?(routeScope==="v2"?"Meet & Greet · paid in full":"Meet & Greet · prepaid"):(routeScope==="v2"?"Pay in full, or 50% now":"Programme · prepaid or approved split")}</small></button>)}</div>}<div className={styles.topGap}><label>Payment mode <select value={paymentMode} disabled={packageCode==="trainer-meet-greet"} onChange={event=>setPaymentMode(event.target.value as "prepaid"|"split")}><option value="split">{routeScope==="v2"?"50% now, the rest before the final session":"Approved split"}</option><option value="prepaid">{routeScope==="v2"?"Pay in full":"Full prepaid"}</option></select></label></div></section><section className={styles.card}><h2>3. Available trainer</h2><p>Availability is checked for every session in this programme. A search does not reserve a trainer.</p><button type="button" disabled={!location||petCount===0||busy||availabilityLoading} onClick={()=>{setError("");setAvailabilityRefresh(value=>value+1);}}>Refresh trainer availability</button>{availabilityLoading?<p role="status">Checking availability for every programme session...</p>:!location?<p>Your training zone must be confirmed before trainers can be listed.</p>:calendarPlan.problem?<p role="alert">{calendarPlan.problem}</p>:trainers.length===0?<p>No available trainer has been confirmed in {location.zoneName} for this programme. Refresh availability or choose another date.</p>:<div className={styles.grid3}>{trainers.map(item=><button key={item.id} onClick={()=>setTrainerId(item.id)} aria-pressed={activeTrainer?.id===item.id} className={styles.choice}><strong>{item.name}</strong><div>{item.rating.toFixed(1)} ★{routeScope==="v2"?"":` · quality ${item.qualityScore}`}</div>{routeScope!=="v2"&&<small>{item.model.replaceAll("_"," ")} · capacity {item.capacity}</small>}</button>)}</div>}</section><section className={styles.card} aria-label="Proposed training calendar"><h2>Review your session calendar</h2>{calendar.length?<ol>{calendar.map((window,index)=><li key={window.start}>Session {index+1}: {formatIndiaRange(window.start,window.end)}</li>)}</ol>:<p>Choose a valid start time and frequency to preview every session before reserving.</p>}</section><section className={styles.stickyCard}><div><strong>{currentQuote?money(currentQuote.totalAmount):"—"}</strong><div>{currentQuote?(routeScope==="v2"?`${currentQuote.packageName} · ${trainingSessionCount(currentQuote.sessions)} · ${currentQuote.minutesPerSession} min each`:`${currentQuote.packageName} · ${currentQuote.sessions} session(s) · ${currentQuote.minutesPerSession} min/session`):activePackage?.name||(routeScope==="v2"?"Choose a plan to see its price":"Canonical quote unavailable")}</div><small>{currentQuote?(routeScope==="v2"?(currentQuote.amountDueNow<currentQuote.totalAmount?`${money(currentQuote.amountDueNow)} due now · ${money(currentQuote.totalAmount-currentQuote.amountDueNow)} before your final session`:`${money(currentQuote.amountDueNow)} due now`):`${money(currentQuote.amountDueNow)} sandbox amount due now · live money disabled`):locationError?"Booking unavailable for your location":locationLoading?"Confirming your training zone…":petCount===0?"Select at least one of your dogs to continue":"Repricing for your current selections…"}</small></div><Button data-v2-action={routeScope==="v2"?"primary":undefined} size="lg" disabled={!currentQuote||!activeTrainer||busy||!account||!location||petCount===0} onClick={()=>void confirm()}>{busy?"Reserving trainer…":"Reserve trainer & continue to payment →"}</Button></section></main>;
}
