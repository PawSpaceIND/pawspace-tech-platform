...
/** A replayed order may not carry an amount at all; "—" is an answer, a crashed page is not. */
const amount=(value:number|null|undefined)=>typeof value==="number"&&Number.isFinite(value)?money(value):"—";
    ...
    <main>
      ...
      <div className={styles.grid}>
        ...
        <section className={styles.workspace}>
          ...
          {result&&<div className={styles.stage}><small>{result.duplicatePrevented?"THIS BOOKING ALREADY EXISTS":"CANONICAL RESULT"}</small><h3>{result.bookingId||result.assistedOrderId}</h3>
            {/* A replay carries the existing order, not a new one, and may carry less of it: the route
                answers a duplicate idempotency key with whatever it can read back. Rendering
                result.provider.name unconditionally threw here and the error boundary replaced the
                whole page with "This page didn't load", so the operator was never told the booking
                already existed. Every optional field now degrades to a readable line. */}
            {result.duplicatePrevented&&<div className={styles.info}>This customer, package and slot were already booked as <b>{result.assistedOrderId}</b>. Nothing was charged and no second booking was created — open the booking to change it.</div>}
            <div className={styles.review}><div><small>Assisted order</small><b>{result.assistedOrderId}</b></div><div><small>Provider</small><b>{result.provider?.name??"Assigned on the original order"}</b></div><div><small>Governed total</small><b>{amount(result.totalAmount)}</b></div><div><small>Due now</small><b>{amount(result.amountDueNow)}</b></div><div><small>Duplicate safe</small><b>{result.duplicatePrevented?"Existing order reused":"New order"}</b></div><div><small>Lead</small><b>{result.lead?(result.lead.converted?`Closed · ${result.lead.leadId}`:result.lead.reason==="no_open_lead_for_this_customer"?"No open lead":"Still open"):result.duplicatePrevented?"Closed on the original order":"—"}</b></div><div><small>Live money</small><b>{result.liveMoney?"Unexpected":"No"}</b></div></div></div>}
        </section>
      </div>
