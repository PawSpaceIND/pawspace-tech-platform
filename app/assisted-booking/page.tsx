...
export default function AssistedBooking(){
  ...
  const [selectedPetKeys,setSelectedPetKeys]=useState<{customerId:string;keys:string[]}|null>(null);
  ...
  async function submit(event:FormEvent){event.preventDefault();setError("");if(crmNeedsSpecies&&!crmSpecies){setError("Confirm the pet species before creating this CRM booking");return;}if(!customer||!selectedPets.length||!selectedPackage){setError("Complete customer, pet and package details before creating the order");return;}if(!consentCaptured||consentReference.trim().length<5){setError("Record explicit customer authority and its evidence reference first.");return;}setBusy(true);setNotice("");setResult(null);try{
    const chosenPets=selectAssistedPets(customer.pets,petKeys);
    const idempotencyKey=`uat-${customer.id}-${selectedPackage.code}-${scheduledStart}-${scheduledEnd}-${petKeys.slice().sort().join("-")}${couponCode.trim()?`-coupon-${couponCode.trim()}`:""}`.replace(/[^A-Za-z0-9:_-]/g,"-");
    const created=await createAssistedOrder({idempotencyKey,customer:{id:customer.id,name:customer.name,primaryPhone:customer.primaryPhone,secondaryPhone:customer.secondaryPhone,email:customer.email},pets:chosenPets,cityId:"blr",zoneId:"blr-east",serviceAddress:serviceAddress.trim()||undefined,servicePincode:servicePincode||undefined,packageCode:selectedPackage.code,scheduledStart:new Date(`${scheduledStart}:00+05:30`).toISOString(),scheduledEnd:new Date(`${scheduledEnd}:00+05:30`).toISOString(),consent:{captured:consentCaptured,method:consentMethod,reference:consentReference},...(couponCode.trim()?{couponCode:couponCode.trim()}:{})});
    ...
    <main>
      ...
      <div className={styles.grid}>
        <section className={styles.customerPanel}>
          ...
          {requestedCustomerId?<div className={styles.customerList}>{crmLoading?<p style={{padding:14}}>Loading selected CRM customer…</p>:customer?<button type="button" className={styles.selected}><span>{customer.name.split(" ").map(part=>part[0]).join("")}</span><div><b>{customer.name}</b><small>{customer.primaryPhone.replace(/(\d{5})\d{3}(\d{2})$/, "$1•••$2")} · {customer.pets.length?customer.pets.map(p=>p.name).join(", "):crmPendingPetName}</small><em className={styles.repeat}>CRM selected</em></div><strong>{customer.id}</strong></button>:<p style={{padding:14}}>CRM customer unavailable.</p>}</div>:<div className={styles.customerList}>{config?.customers.map((item,index)=><button key={item.id} className={selected===index?styles.selected:""} onClick={()=>{setSelected(index);setPackageCode("");setResult(null);setConsentCaptured(false);setConsentReference("")}}><span>{item.name.split(" ").map(part=>part[0]).join("")}</span><div><b>{item.name}</b><small>{item.primaryPhone.replace(/(\d{5})\d{3}(\d{2})$/, "$1•••$2")} · {item.pets.map(p=>p.name).join(", ")}</small><em className={styles.repeat}>UAT fixture</em></div><strong>{item.id}</strong></button>)}</div>}
        </section>
        <section className={styles.workspace}>
          ...
          <form className={styles.builder} onSubmit={submit}>
            ...
            <div className={styles.stage}><small>SERVER-GOVERNED PACKAGE</small><h3>Choose the Grooming service to test</h3><div className={styles.serviceGrid}>{eligiblePackages.map(item=><button type="button" className={effectivePackageCode===item.code?styles.chosen:""} key={item.code} onClick={()=>setPackageCode(item.code)}><span>✂</span><b>{item.name}{item.code.startsWith("young-")?" · Puppy & kitten (under 6 months)":""}</b><small>{money(item.singlePrice)} single · {money(item.multiPetPrice)} multi-pet</small></button>)}</div><div className={styles.info}>Displayed catalogue values come from the server. The server recomputes the final governed amount when the assisted order is created.</div></div>
            <div className={styles.stage}><small>CANONICAL SCHEDULE</small><h3>Choose the UAT service window</h3><div className={styles.two}><label>Start<input type="datetime-local" value={scheduledStart} onChange={e=>setScheduledStart(e.target.value)} required/></label><label>End<input type="datetime-local" value={scheduledEnd} onChange={e=>setScheduledEnd(e.target.value)} required/></label></div><div className={styles.two} style={{marginTop:12}}><label>Service address<input value={serviceAddress} onChange={e=>setServiceAddress(e.target.value)} placeholder="Leave blank to use the saved address" autoComplete="street-address"/></label><label>Service PIN code<input value={servicePincode} onChange={e=>setServicePincode(e.target.value.replace(/\D/g,"").slice(0,6))} inputMode="numeric" maxLength={6} autoComplete="postal-code"/></label></div><div className={styles.assignment}><div><b>Assignment rule</b><p>The staff member does not choose or fabricate a provider. The existing canonical UAT scheduler selects and reserves the eligible provider.</p></div><span>Auto</span></div></div>
            <div className={styles.stage}><h3>Pets for this grooming appointment</h3><p>Choose one to four pets of the same species. Other family pets stay on the customer profile.</p><div className={styles.serviceGrid}>{customer?.pets.map(pet=><button key={assistedPetKey(pet)} type="button" aria-pressed={petKeys.includes(assistedPetKey(pet))} onClick={()=>{if(!customer)return;const key=assistedPetKey(pet),keys=petKeys.includes(key)?petKeys.filter(id=>id!==key):[...petKeys,key];if(keys.length>4){setError("Choose up to four pets per appointment.");return;}setSelectedPetKeys({customerId:customer.id,keys});setError("");}}>{pet.name} · {pet.species}</button>)}</div></div>
