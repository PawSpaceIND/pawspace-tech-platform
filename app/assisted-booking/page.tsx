...
export default function AssistedBooking(){
  ...
  useEffect(()=>{if(eligiblePackages.length&&!eligiblePackages.some(item=>item.code===packageCode))setPackageCode(eligiblePackages[0].code);},[eligiblePackages,packageCode]);
    ...
    <main>
      ...
      <div className={styles.grid}>
        <section className={styles.customerPanel}>
          ...
          <div className={styles.customerList}>{config?.customers.map((item,index)=><button key={item.id} className={selected===index?styles.selected:""} onClick={()=>{setSelected(index);setResult(null)}}><span>{item.name.split(" ").map(part=>part[0]).join("")}</span><div><b>{item.name}</b><small>{item.primaryPhone.replace(/(\d{5})\d{3}(\d{2})$/, "$1•••$2")} · {item.pets.map(p=>p.name).join(", ")}</small><em className={styles.repeat}>UAT fixture</em></div><strong>{item.id}</strong></button>)}</div>
        </section>
        <section className={styles.workspace}>
          ...
          <form className={styles.builder} onSubmit={submit}>
            <div className={styles.stage}><small>SERVER-GOVERNED PACKAGE</small><h3>Choose the Grooming service to test</h3><div className={styles.serviceGrid}>{eligiblePackages.map(item=><button type="button" className={packageCode===item.code?styles.chosen:""} key={item.code} onClick={()=>setPackageCode(item.code)}><span>✂</span><b>{item.name}</b><small>{money(item.singlePrice)} single · {money(item.multiPetPrice)} multi-pet</small></button>)}</div><div className={styles.info}>Displayed catalogue values come from the server. The server recomputes the final governed amount when the assisted order is created.</div></div>
