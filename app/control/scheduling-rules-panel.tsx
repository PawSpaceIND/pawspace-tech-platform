    ...
    <section className={styles.two}>
      <div className={styles.panel}><h3>Add a scheduling constraint</h3><p>Changes apply to future provider matching in the selected location.</p>
        <fieldset disabled={busy||loading||rules===null} style={{border:0,padding:0,minWidth:0}}>
          ...
          <label>Service<select value={service} onChange={event=>setService(event.target.value)}>{services.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label>
          <label>Location<select value={zone} onChange={event=>setZone(event.target.value)}><option value="blr-east">Bengaluru · East zone</option><option value="blr-south">Bengaluru · South zone</option><option value="">All Bengaluru</option></select></label>
          <label>Constraint<select value={field} onChange={event=>{setField(event.target.value);setValue(event.target.value==="model"?"commission":"");}}><option value="rating">Minimum rating (0–5)</option><option value="qualityScore">Minimum quality score (0–100)</option><option value="capacity">Minimum capacity</option><option value="model">Provider model</option></select></label>
          <label>Required value{field==="model"?<select value={value} onChange={event=>setValue(event.target.value)}><option value="commission">Commission</option><option value="full_time">Full-time</option></select>:<input inputMode="decimal" value={value} onChange={event=>setValue(event.target.value)}/>}</label>
