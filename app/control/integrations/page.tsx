...
type Payload={data:{items:Integration[];summary:{total:number;required:number;p0Required:number;p0ControlledLive:number;controlledLiveVerified:number};productionReady:false};blockers:Array<{integrationCode:string;capability:string;owner:string;readinessState:string;blockerReason:string}>;uatSandbox:{status:string;configuredForExternalTest:boolean;sandboxEvidenceVerified:boolean;syntheticLogicReady:true;productionEnabled:false;modules:SandboxModule[]};productionReady:false};
...
export default function IntegrationReadinessControl(){
 ...
 return <StaffModule><main className={styles.shell}>
  ...
  <section className={styles.workspace}>
   ...
   <section className={styles.panel}><header><div><span>STRICT UAT SANDBOX</span><h2>Five-provider test readiness</h2></div><em className={data.uatSandbox.status==="sandbox_verified"?styles.good:styles.blocked}>{pretty(data.uatSandbox.status)}</em></header><p>Logic tests, test configuration and real sandbox evidence are separate. No secret values are returned, and production stays disabled.</p><div className={styles.controls}>{data.uatSandbox.modules.map(item=><article key={item.code}><i className={item.status==="sandbox_verified"?styles.passDot:item.configuredForExternalTest?styles.warnDot:styles.blockDot}></i><div><h3>{item.label}</h3><p>{pretty(item.configurationStatus)} · external evidence {item.sandboxEvidenceVerified?"verified":"not verified"}{item.blockers.length?` · ${item.blockers.join("; ")}`:""}</p></div><b>{pretty(item.status)}</b></article>)}</div></section>
   ...
   <section className={styles.panel}><header><div><span>FULL REGISTER</span><h2>Integration evidence states</h2></div><em>{data.data.summary.controlledLiveVerified} controlled-live verified</em></header><div className={styles.controls}>{data.data.items.map(item=><article key={item.integrationCode}><i className={item.readinessState==="controlled_live_verified"?styles.passDot:item.readinessState==="blocked"?styles.blockDot:styles.warnDot}></i><div><h3>{item.integrationCode} · {item.capability}</h3><p>{item.category} · {item.provider} · code {pretty(item.codeBoundaryStatus)} · credentials {pretty(item.credentialStatus)}{item.blockerReason?` · ${item.blockerReason}`:""}</p>{editing===item.integrationCode&&<div style={{marginTop:8,display:"grid",gap:6}}>
    ...
    {formError&&<small style={{color:"var(--staff-danger, crimson)"}}>{formError}</small>}
