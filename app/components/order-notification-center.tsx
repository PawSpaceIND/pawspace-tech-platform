...
export default function OrderNotificationCenter(){
 ...
 return <div className="ps-order-fab" style={{position:"fixed",right:18,zIndex:80,fontFamily:"inherit"}}>{open&&<section role="dialog" aria-label="PawSpace order notifications" style={{position:"absolute",right:0,bottom:58,width:"min(360px,calc(100vw - 32px))",maxHeight:"65vh",overflow:"auto",background:"var(--ps-surface,#fff)",border:"1px solid var(--ps-border,#e7e1e8)",borderRadius:18,boxShadow:"0 18px 50px rgba(0,0,0,.18)",padding:12}}><header style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 4px 10px"}}><div><b>Order updates</b><small style={{display:"block",opacity:.65}}>{error?"Updates unavailable":loading?"Loading updates…":unread?`${unread} unread`:"You're all caught up"}</small></div><button aria-label="Close notifications" onClick={()=>setOpen(false)} style={{border:0,background:"transparent",fontSize:18}}>×</button></header>{error&&<div role="alert"><p>{error}</p><button onClick={()=>setReload(value=>value+1)}>Retry order updates</button></div>}{loading&&<p role="status">Loading order updates…</p>}{items.length===0&&!error&&!loading?<p style={{fontSize:13,opacity:.7,padding:8}}>New booking, service, cancellation and refund updates will appear here.</p>:items.map(item=><button key={item.id} onClick={()=>void markRead(item)} style={{display:"block",width:"100%",textAlign:"left",border:0,borderTop:"1px solid var(--ps-border,#eee)",background:item.status==="unread"?"rgba(111,72,172,.07)":"transparent",padding:"12px 8px",cursor:"pointer"}}><strong style={{display:"block",fontSize:13}}>{item.title}</strong><span style={{display:"block",fontSize:12,opacity:.75,lineHeight:1.45,marginTop:3}}>{item.body}</span><small style={{display:"block",fontSize:10,opacity:.55,marginTop:6}}>{new Date(item.created_at).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"})} IST</small></button> )}{(history.length>0||nextCursor)&&<nav aria-label="Order update pages" style={{display:"flex",justifyContent:"space-between",padding:8}}><button disabled={loading||history.length===0} onClick={newer}>Newer updates</button><button disabled={loading||!nextCursor} onClick={older}>Older updates</button></nav>}</section>}<button aria-label={unread?`${unread} unread order updates`:"Order notifications"} onClick={()=>setOpen(value=>!value)} style={{width:52,height:52,borderRadius:26,border:"1px solid var(--ps-border,#ddd)",background:"var(--ps-surface,#fff)",boxShadow:"0 8px 26px rgba(0,0,0,.14)",fontSize:22,cursor:"pointer",position:"relative"}}>♢{unread>0&&<i style={{position:"absolute",right:-3,top:-3,minWidth:20,height:20,borderRadius:10,background:"#b3261e",color:"white",fontStyle:"normal",fontSize:11,display:"grid",placeItems:"center",padding:"0 3px"}}>{unread>99?"99+":unread}</i>}</button></div>;
}
