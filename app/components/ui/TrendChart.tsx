...
export interface TrendChartProps {
  ...
  onSelect?: (row: Record<string, unknown>, seriesKey: string) => void;
...
export default function TrendChart({ data, xKey, series, type = "line", height = 260, valueFormatter, axisValueFormatter, onSelect }: TrendChartProps) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        {type === "line" ? (
          ...
          <BarChart accessibilityLayer data={data} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
            ...
            {series.length > 1 && <Legend wrapperStyle={{ fontSize: 11 }} />}
            {series.map((s, i) => (
              <Bar onClick={entry => { if (entry.payload) onSelect?.(entry.payload, s.key); }} cursor={onSelect ? "pointer" : undefined} key={s.key} dataKey={s.key} name={s.label ?? s.key} fill={s.color ?? palette[i % palette.length]} radius={[6, 6, 0, 0]} isAnimationActive={false} />
            ))}
          </BarChart>
        )}
      </ResponsiveContainer>
