...
function VisualAnalyticsContent({ serviceCode, title = "Business performance" }: Props) {
  ...
  const services = current && previous ? [...new Set([...Object.keys(current.services), ...Object.keys(previous.services)])].map(code => ({ name: code.replaceAll("_", " "), current: current.services[code]?.gmv ?? 0, previous: previous.services[code]?.gmv ?? 0 })) : [];
  ...
  return <section className={css.panel} aria-label={title}>
    ...
    {range && prior && <p className={css.muted}><b>{range.from} → {range.to}</b> compared with <b>{prior.from} → {prior.to}</b> · {rangeDays(range)} days each. Dates below apply to this performance panel.</p>}
    ...
    {result && !degraded && current && previous && <>
      ...
      <div className={css.grid}>
        <article className={`${css.chart} ${css.wide}`}><h3>Revenue over time</h3><p className={css.muted}>Equal-length periods aligned by day. The data table includes both exact dates. Cancelled and draft bookings are excluded from GMV.</p>{daily.length ? <><TrendChart data={daily} xKey="date" series={[{ key: "current", label: "Selected period" }, { key: "previous", label: "Previous period", dashed: true }]} valueFormatter={money} axisValueFormatter={compact} height={280} /><details className={css.table}><summary>View daily values and comparison dates</summary><table><thead><tr><th>Selected date</th><th>Revenue</th><th>Comparison date</th><th>Revenue</th></tr></thead><tbody>{daily.map(row => <tr key={row.date}><td>{row.date}</td><td>{money(row.current)}</td><td>{row.previousDate}</td><td>{money(row.previous)}</td></tr>)}</tbody></table></details></> : <p className={css.empty}>Daily history is unavailable from this source.</p>}</article>
        <article className={css.chart}><h3>Service revenue comparison</h3><TrendChart data={services} xKey="name" type="bar" series={[{ key: "current", label: "Selected period" }, { key: "previous", label: "Previous period" }]} valueFormatter={money} axisValueFormatter={compact} /></article>
        <article className={css.chart}><h3>Booking outcomes</h3><p className={css.muted}>Selected service-date period · current booking statuses</p><div className={css.meter} role="img" aria-label={outcomes.map(row => `${row.name}: ${row.value}`).join(", ")}>{outcomes.map(row => <span key={row.name} style={{ width: `${current.bookings.total ? row.value / current.bookings.total * 100 : 0}%`, background: row.color }} />)}</div><div className={css.legend}>{outcomes.map(row => <span key={row.name}><i className={css.dot} style={{ background: row.color }} />{row.name}: <b>{compact(row.value)}</b></span>)}</div><p className={css.muted}>{current.bookings.total ? `${(current.bookings.completed / current.bookings.total * 100).toFixed(1)}% completed` : "No bookings to calculate completion."}</p></article>
      </div>
    </>}
  </section>;
}
