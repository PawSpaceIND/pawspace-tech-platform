...
type Props={name:string;children:React.ReactNode;resetHref?:string};
...
export default class CriticalErrorBoundary extends React.Component<Props,State>{
 ...
 render(){if(this.state.failed)return <section role="alert" style={{padding:20,border:"1px solid #d8cfe1",borderRadius:16,display:"grid",gap:12}}><h2 style={{margin:0}}>Something went wrong.</h2><p style={{margin:0}}>We couldn’t continue this {this.props.name.toLowerCase()} safely. Your saved account data is unchanged.</p><button type="button" onClick={()=>{this.setState({failed:false});window.location.assign(this.props.resetHref??"/mobile-app");}}>Reload section</button></section>;return this.props.children;}
}
