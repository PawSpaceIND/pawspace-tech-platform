...
export async function GET(request:Request){try{const actor=await resolveActor(request);const db=await database();const providerId=await resolveProviderForActor(db,actor.email);if(!providerId)return json({data:{linked:false,email:actor.email},productionReady:false});const{env}=await import("cloudflare:workers");const enforce=["1","true","on","yes"].includes(String((env as unknown as Record<string,unknown>).PAWSPACE_SHIFT_LIVENESS_ENFORCE??"").toLowerCase());const liveness=await assertPartnerDailyScheduleLiveness(db,{providerId,enforce});return json({data:{linked:true,liveness,...await providerWorkspace(db,{providerId})},productionReady:false});}catch(error){return authError(error,"Unable to load your provider workspace");}}
