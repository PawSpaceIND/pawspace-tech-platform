...
import{captureTrainingQuoteSandbox,collectTrainingRemainingBalanceSandbox}from"../../../lib/training-commercial-governance";
...
export async function POST(request:Request){try{
 ...
 const body=await request.json() as {quoteId?:string;amount?:number;action?:"deposit"|"remaining_balance"};const quoteId=String(body.quoteId||"").trim(),amount=Number(body.amount),action=body.action??"deposit";
 ...
 const db=await database(),result=action==="remaining_balance"?await collectTrainingRemainingBalanceSandbox(db,{quoteId,amount,paymentKey}):await captureTrainingQuoteSandbox(db,{quoteId,amount,paymentKey});return json({data:{...result,liveMoney:false,synthetic:true}},201);
