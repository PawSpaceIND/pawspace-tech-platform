...
export async function POST(request:Request){try{const actor=await authorize(request,"scheduling.manage");const db=await database();const body=await request.json() as {name?:unknown;serviceCode?:string;cityId?:string;zoneId?:string;priority?:number;conditions?:unknown};if(typeof body.name!=="string"||!body.name.trim()||!Array.isArray(body.conditions)||body.conditions.length===0||!body.conditions.every(validScheduleRule))return response({error:"Rule name and valid conditions are required"},400);const id=`rule_${crypto.randomUUID().slice(0,12)}`;const now=Date.now();
