...
export async function GET(request:Request){try{const actor=await resolveActor(request);requirePermission(actor,"finance.view");const db=await database();const url=new URL(request.url);const returnsFilter={returnType:url.searchParams.get("returnType")||undefined,period:url.searchParams.get("period")||undefined};return json({data:{...await getGstAccountingSnapshot(db),serviceInvoices:await serviceInvoiceOwnershipSnapshot(db)},returns:await getGstReturnsSnapshot(db,returnsFilter),actor:{email:actor.email,roleCode:actor.roleCode},productionReady:false});}catch(error){return authError(error,"Unable to load GST/accounting control");}}
...
export async function POST(request:Request){try{sameOrigin(request);const actor=await resolveActor(request);requirePermission(actor,"finance.manage");const db=await database(),body=await request.json() as Record<string,unknown>,action=String(body.action||"");let data:unknown;
 if(action==="assign_service_invoice_owner"){await ensureGstAccountingTables(db);data=await assignServiceInvoiceOwnership(db,{invoiceId:text(body.invoiceId),entityId:text(body.entityId),registrationId:text(body.registrationId),reason:text(body.reason)},actor.email);}
