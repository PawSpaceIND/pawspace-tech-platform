...
function travelStatus(booking:Row){
  return String(booking.work_order_status||booking.booking_status||"");
}

async function addressDisclosure(db:Awaited<ReturnType<typeof database>>,actor:{email:string;roleCode:string;permissions?:readonly string[]},booking:Row){
  const policy=await resolveDataAccessPolicy(db,{serviceCode:"grooming",cityId:null});
  const status=travelStatus(booking);
  const scheduledStart=Date.parse(String(booking.scheduled_start||""))||null;
  const decision=decideCustomerDataAccess(policy.config,{
    actor,
    subject:{
      customerId:String(booking.customer_id||""),
      name:"Customer",
      address:{line1:String(booking.address_text||""),area:null,city:null,pincode:null},
    },
    purpose:"service_delivery",
    assignment:{
      type:"booking",
      id:String(booking.id),
      assignedTo:String(booking.provider_id||""),
      status,
      scheduledStart,
      completedAt:status==="completed"?Date.now():null,
    },
  });
  const eligible=policy.config.addressEligibleStatuses.map(String).includes(status);
  const full=decision.address.precision==="full"&&eligible;
  return{
    eligible,
    precision:decision.address.precision,
    destinationAddress:full?(decision.address.line1||String(booking.address_text||"")):null,
    destinationCoordinates:full&&booking.destination_latitude!=null&&booking.destination_longitude!=null
      ?{lat:Number(booking.destination_latitude),lng:Number(booking.destination_longitude)}
      :null,
  };
}

function validInput(input:Input){
  return Boolean(input.bookingId&&input.providerId)&&validGpsCoordinates(Number(input.latitude),Number(input.longitude))&&Number.isFinite(Number(input.accuracyMeters))&&Number(input.accuracyMeters)>=0&&Number.isFinite(Number(input.capturedAt))&&Number(input.capturedAt)>0;
}
...
export async function GET(request:Request){try{
  ...
  if(!disclosure.eligible)return json({error:"Route access is unavailable outside a policy-eligible provider travel state"},409);
  ...
  return json({data:{
    ...
    addressPrecision:disclosure.precision,
    destinationAddress:disclosure.destinationAddress,
    destinationCoordinates:disclosure.destinationCoordinates,
    providerLocation:point,
    navigationUrl:disclosure.destinationAddress?mapsNavigationUrl(destination,point||undefined):null,
    travelState:travelStatus(booking),
    route:snapshot?{status:String(snapshot.provider_status),distanceMeters:snapshot.distance_meters==null?undefined:Number(snapshot.distance_meters),durationSeconds:snapshot.duration_seconds==null?undefined:Number(snapshot.duration_seconds),error:detail.error?String(detail.error):undefined}:undefined,
  }});
...
export async function POST(request:Request){try{
  ...
  const destination=disclosure.destinationAddress||String(booking.address_text||""),origin:ProviderPoint={lat:Number(input.latitude),lng:Number(input.longitude),accuracyMeters:Number(input.accuracyMeters),capturedAt:Number(input.capturedAt)};
  const route=prepared.verdict.trustState==="accepted"?await computeGoogleRoute(origin,destination):null;
  let data:Record<string,unknown>;
  try{data=await commitGroomingTelemetry(db,{telemetry,prepared,destinationAddress:destination,route,actor,travelState:travelStatus(booking)}) as Record<string,unknown>;}catch(error){
  ...
  const response={
    ...
    destinationAddress:disclosure.destinationAddress,
    destinationCoordinates:disclosure.destinationCoordinates,
    navigationUrl:disclosure.destinationAddress?mapsNavigationUrl(destination,origin):null,
  };
