...
async function bindingReadiness(){const{env}=await import("cloudflare:workers");const runtime=env as unknown as BotCallBindings;return{missingRequired:runtime.DB?[]:["DB"],optional:{IDEMPOTENCY_KV:Boolean(runtime.IDEMPOTENCY_KV),CALL_ARTIFACTS:Boolean(runtime.CALL_ARTIFACTS)}};}
async function requireBotCallBindings(){const state=await bindingReadiness();if(state.missingRequired.length)return{response:json({error:"Bot call outcome runtime bindings are unavailable",code:"BOT_CALL_BINDINGS_MISSING",missingBindings:state.missingRequired,optionalBindings:state.optional},503),optionalBindings:state.optional};return{response:null,optionalBindings:state.optional};}
...
export async function GET(request:Request){
  try{
    const actor=await authorize(request,"customers.view");void actor;
