...
export async function GET(request:Request){try{const convActor=await authorize(request,"communications.manage");const db=await database();
  ...
  const maskPayload=(value:unknown,view:{contact:{phone:string|null}}):unknown=>{if(!value||typeof value!=="object")return value;const record={...value as Record<string,unknown>};if(record.customerPhone)record.customerPhone=view.contact.phone;return record;};
