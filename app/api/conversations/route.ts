...
import{assignConversation,recordConversationInternalNote,getConversation,listConversationThreads,recordInboundMessage,setConversationStatus}from"../../../lib/conversation-governance";
import{actorCanAccessConversation}from"../../../lib/conversation-access";

type Body={action?:string;note?:string;idempotencyKey?:string;threadId?:string;customerId?:string;channel?:"whatsapp"|"sms"|"email"|"push"|"chat"|"voice";payload?:Record<string,unknown>;provider?:string;providerReference?:string;eventId?:string;assignedTo?:string;reason?:string;slaMinutes?:number;status?:"open"|"pending_customer"|"resolved"|"closed"};
...
export async function GET(request:Request){try{const convActor=await authorize(request,"communications.manage");const db=await database();
  ...
  const maskPayload=(value:unknown,view:{contact:{phone:string|null}}):unknown=>{if(!value||typeof value!=="object")return value;const record={...value as Record<string,unknown>};if(record.customerPhone)record.customerPhone=view.contact.phone;
    ...
    return record;};
  const url=new URL(request.url),threadId=url.searchParams.get("threadId");if(threadId){if(!(await actorCanAccessConversation(db,convActor,threadId)))return json({error:"Conversation access denied"},403);const data=await getConversation(db,threadId,"staff");if(data){const shaped=data as Record<string,unknown>;const threadRow=shaped.thread as Record<string,unknown>|undefined;const view=threadRow?threadView(threadRow):{contact:{phone:null}};const thread=threadRow?maskThread(threadRow):shaped.thread;const messages=Array.isArray(shaped.messages)?(shaped.messages as Record<string,unknown>[]).map(message=>({...message,payload:maskPayload(message.payload,view)})):shaped.messages;const notes=Array.isArray(shaped.notes)?(shaped.notes as Record<string,unknown>[]).map(note=>({...note,body:notesReadable?note.body:"[internal note withheld]"})):[];return json({data:{...shaped,thread,messages,notes}});}return json({error:"Conversation not found"},404);}const limit=Number(url.searchParams.get("limit")||100);if(!Number.isInteger(limit)||limit<1||limit>200)return json({error:"Conversation limit must be between 1 and 200"},400);
 let before:{at:number;id:string}|undefined;const cursor=url.searchParams.get("cursor");if(cursor){try{before=JSON.parse(cursor);if(!before||typeof before!=="object")throw new Error("Invalid cursor");}catch{return json({error:"Invalid conversation cursor"},400);}}
 const rows=await listConversationThreads(db,{actor:convActor,customerId:url.searchParams.get("customerId")||undefined,status:url.searchParams.get("status")||undefined,query:url.searchParams.get("q")||undefined,channel:url.searchParams.get("channel")||undefined,ownership:url.searchParams.get("ownership")||undefined,before,limit:limit+1});
 const data=rows.slice(0,limit),last=data.at(-1);
 return json({data:{threads:(data as Record<string,unknown>[]).map(maskThread),nextCursor:rows.length>limit&&last?{at:Number(last.updated_at),id:String(last.id)}:null,source:"canonical_conversation_threads",liveDelivery:false}});}catch(error){return authError(error,"Unable to load conversations");}}
export async function POST(request:Request){try{sameOrigin(request);const actor=await authorize(request,"communications.manage"),body=await request.json() as Body,db=await database();if(body.threadId&&!(await actorCanAccessConversation(db,actor,body.threadId))){await securityAudit(db,actor,`conversation.${String(body.action||"unknown")}`,"conversation",body.threadId,"denied",{reason:"row_scope"});return json({error:"Conversation access denied"},403);}if(body.action==="record_inbound"){if(!body.threadId||!body.customerId||!body.channel||!body.provider||!body.providerReference||!body.eventId)return json({error:"Complete inbound message identity is required"},400);const data=await recordInboundMessage(db,{threadId:body.threadId,customerId:body.customerId,channel:body.channel,payload:body.payload||{},provider:body.provider,providerReference:body.providerReference,eventId:body.eventId,createdBy:actor.email});await securityAudit(db,actor,"conversation.inbound","conversation",body.threadId,"completed",{messageId:data.id,provider:body.provider});return json({data},data.duplicatePrevented?200:201);}
 if(body.action==="add_internal_note"){if(!body.threadId)return json({error:"Thread is required"},400);const data=await recordConversationInternalNote(db,{threadId:body.threadId,actorEmail:actor.email,body:body.note as string,idempotencyKey:body.idempotencyKey as string});await securityAudit(db,actor,"conversation.internal_note","conversation",body.threadId,"completed",{noteId:data.id,duplicatePrevented:data.duplicatePrevented});return json({data},data.duplicatePrevented?200:201);}
