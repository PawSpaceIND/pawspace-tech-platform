...
export async function POST(request:Request){try{
  ...
  let data:unknown;
  if(action==="save_tax_policy"){const cityId=String(body.cityId||"").trim();if(!cityId)return Response.json({error:"City is required for Grooming tax policy"},{status:400});data=await saveGroomingTaxPolicy(db,{cityId,taxMode:String(body.taxMode||"") as"inclusive"|"exclusive",taxRate:Number(body.taxRate),effectiveFrom:String(body.effectiveFrom||new Date().toISOString().slice(0,10)),reason,actorId:actor.email});}
  ...
  await securityAudit(db,actor,`grooming.finance.${action}`,"grooming_finance",String(body.bookingId||body.cityId||"unspecified"),"completed",{liveMoney:false,executionMode:"sandbox_not_connected"});
