...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "refund_status") {
      ...
      await db.batch([
        db.prepare("UPDATE booking_refund_cases SET status=?,approved_by=CASE WHEN ?='approved' THEN ? ELSE approved_by END,gateway_reference=COALESCE(?,gateway_reference),updated_at=? WHERE id=?").bind(input.refundStatus,input.refundStatus,actor.email,input.gatewayReference??null,now,input.refundCaseId),
        db.prepare("INSERT INTO booking_operational_events (id,booking_id,provider_id,event_type,reason,impact_minutes,detail_json,actor_id,created_at) VALUES (?,?,?,?,?,?,?,?,?)").bind(eventId,input.bookingId,input.providerId,`refund.${input.refundStatus}`,input.reason,0,JSON.stringify({refundCaseId:input.refundCaseId,gatewayReference:input.gatewayReference}),actor.email,now),
        db.prepare("INSERT INTO booking_lifecycle_events (id,booking_id,event_type,entity_type,entity_id,actor_id,detail_json,occurred_at) VALUES (?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(),input.bookingId,`refund.${input.refundStatus}`,"refund",input.refundCaseId,actor.email,JSON.stringify({gatewayReference:input.gatewayReference}),now),
