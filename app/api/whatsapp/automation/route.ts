...
type Body = { action?: string; enabled?: boolean; templateKeys?: string[]; offerType?: string; offerReference?: string; quietHoursStart?: string; quietHoursEnd?: string; quietHoursTimezone?: string; maxMarketingMessagesPer24h?: number; limit?: number };
  ...
  try {
    ...
    if (action === "save_no_response") {
      ...
      await securityAudit(db, actor, "whatsapp.automation.no_response_saved", "whatsapp_automation", "no_response", "completed", {
        delaysMinutes: [10, 30, 180],
        offerType: body.offerType,
        offerReferenceConfigured: Boolean(String(body.offerReference || "").trim()),
        quietHoursConfigured: Boolean(String(body.quietHoursStart || "").trim() && String(body.quietHoursEnd || "").trim() && String(body.quietHoursTimezone || "").trim()),
        maxMarketingMessagesPer24h: Number(body.maxMarketingMessagesPer24h || 0),
