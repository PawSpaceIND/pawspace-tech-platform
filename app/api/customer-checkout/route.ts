...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "status") {
      ...
      if (requestedOrder) {
        const capturedOrder = gatewayEventsTable ? await db.prepare(`SELECT id FROM payment_gateway_events
        ...
        status = capturedOrder ? "captured" : "awaiting_confirmation";
      } else if (paymentReady && bookingReady) status = "captured";
