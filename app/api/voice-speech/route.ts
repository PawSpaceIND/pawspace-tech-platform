...
export async function POST(request:Request){
  try{
    ...
    const body=await request.json().catch(()=>({})) as {action?:string;audioRef?:string;text?:string;language?:string};
    ...
    if(error instanceof VoiceSpeechError){
      const env=await runtime().catch(()=>({}) as Record<string,unknown>);
      if(String(env.PAWSPACE_DEPLOYMENT_ENV||"").trim()==="staging")return json({error:"Unable to process voice speech request",diagnostic:{stage:error.stage,code:error.code}},502);
    }
