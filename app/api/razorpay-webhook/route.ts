...
function targetFor(eventType:string):PaymentState|null{if(eventType==="payment.authorized")return"AUTHORIZED";if(eventType==="payment.captured"||eventType==="order.paid"||eventType==="payment_link.paid")return"CAPTURED";return null;}
async function claimInbox(db:D1Database,row:Row,eventType:string){const result=await db.prepare("UPDATE gateway_webhook_events SET processing_status='PROCESSING',event_type=?,failure_reason=NULL,processed_at=NULL WHERE id=? AND processing_status IN ('RECEIVED','DEFERRED','FAILED')").bind(eventType,String(row.id)).run();return Number(result.meta?.changes||0)===1;}
async function markInbox(db:D1Database,row:Row,status:"PROCESSED"|"DEFERRED"|"REJECTED"|"FAILED",eventType?:string,reason?:string){const terminal=status!=="DEFERRED";await db.prepare("UPDATE gateway_webhook_events SET processing_status=?,event_type=COALESCE(?,event_type),failure_reason=?,processed_at=? WHERE id=?").bind(status,eventType||null,reason||null,terminal?Date.now():null,String(row.id)).run();}
async function matchedIntent(db:D1Database,event:GatewayEvent){if(!event.gatewayOrderId)return null;return db.prepare("SELECT * FROM payment_intents WHERE provider='razorpay' AND gateway_order_id=?").bind(event.gatewayOrderId).first<Row>();}
function transitionWouldDefer(intent:Row,target:PaymentState){const current=String(intent.state||"") as PaymentState;if(!(current in rank))throw new Error("Payment intent contains an unknown state");if(rank[current]>=90)return true;return rank[target]>rank[current]+1;}
async function ensureCaptureJournal(db:D1Database,event:GatewayEvent,intent:Row){const amountPaise=Number(event.amountSubunits||0);if(!Number.isSafeInteger(amountPaise)||amountPaise<=0)throw new Error("Captured Razorpay amount must be positive integer paise");return postBalancedJournal(db,{sourceType:"razorpay_capture",sourceId:String(intent.id),sourceEventId:`razorpay:${event.eventId}:capture`,narration:`Razorpay capture ${event.eventId}`,currency:event.currency||String(intent.currency||"INR"),entries:[{accountCode:ACCT.GATEWAY_CLEARING,direction:"DEBIT",amountPaise,bookingId:String(intent.booking_id)},{accountCode:ACCT.CUSTOMER_COLLECTIONS,direction:"CREDIT",amountPaise,bookingId:String(intent.booking_id)}]});}
...
export async function POST(request:Request){
  try{
    ...
    try{accepted=await acceptRazorpayWebhook(db,{rawBody:raw,signature,webhookSecret:gate.secret,eventId,environment:gate.environment});}
    catch(error){const message=error instanceof Error?error.message:String(error);if(message==="Invalid Razorpay webhook signature")return json({error:message},401);if(message==="Signed Razorpay webhook body is not valid JSON")return json({error:"Invalid webhook JSON"},400);if(message.includes("replayed with a different payload"))return json({error:"Razorpay event ID payload mismatch"},409);throw error;}
    const payload=(accepted.duplicate?JSON.parse(String(accepted.row.raw_payload||"{}")):accepted.event) as RazorPayload;
    const eventType=String(payload.event||"").trim();if(!eventType){await markInbox(db,accepted.row,"REJECTED",undefined,"missing_event_type");return json({error:"Webhook event type is required"},400);}
    ...
    try{
      ...
      if(intent&&target&&transitionWouldDefer(intent,target)){await markInbox(db,accepted.row,"DEFERRED",eventType,`payment_state_${String(intent.state).toLowerCase()}_awaits_prior_transition`);return json({ok:true,environment:gate.environment,deferred:true,state:String(intent.state),target});}
      const result=await processGatewayEvent(db,event);const failed=String(result.status||"")==="exception";if(failed){await markInbox(db,accepted.row,"FAILED",eventType,String(result.reason||"reconciliation_exception"));return json({ok:true,environment:gate.environment,...result});}
      let transition:Awaited<ReturnType<typeof advancePaymentState>>|null=null;if(intent&&target)transition=await advancePaymentState(db,{intentId:String(intent.id),target,gatewayPaymentId:event.gatewayPaymentId});
      let journal:null|Awaited<ReturnType<typeof postBalancedJournal>>=null;const captureEvent=target==="CAPTURED",sameEventRecovery=Boolean((result as Record<string,unknown>).duplicate),repeatCapture=Boolean((result as Record<string,unknown>).ignored)&&String((result as Record<string,unknown>).reason||"")==="capture_already_collected";if(intent&&captureEvent&&!repeatCapture&&(transition?.changed||sameEventRecovery||String(intent.state)==="CAPTURED"))journal=await ensureCaptureJournal(db,event,intent);
      await markInbox(db,accepted.row,"PROCESSED",eventType);return json({ok:true,environment:gate.environment,...result,paymentState:transition,journal:journal?{transactionId:journal.transactionId,duplicate:journal.duplicate}:null});
    }catch(error){await markInbox(db,accepted.row,"FAILED",eventType,error instanceof Error?error.message:"domain_processing_failed").catch(()=>null);throw error;}
