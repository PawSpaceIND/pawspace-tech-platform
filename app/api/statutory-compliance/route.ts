...
export async function POST(request:Request){try{
 ...
 if(action==="record_tcs_deposit"){
  const result=await recordTcsDeposit(db,{period,challanReference:String(body.challanReference||""),amount:Number(body.amount),actorId:actor.email});
  await securityAudit(db,actor,"statutory.tcs_deposit","tcs_period",period,"completed",{amount:result.amount,duplicatePrevented:result.duplicatePrevented});
  return json({data:result},result.duplicatePrevented?200:201);
 }
 if(action==="reconcile_partner_tax"){
  const result=await reconcilePartnerPayoutTax(db,{period});
  await securityAudit(db,actor,"statutory.reconcile_partner_tax","tax_reconciliation",period,"completed",{reconciled:result.reconciled});
  return json({data:result});
 }
