...
export async function GET(request:Request){try{const actor=await resolveActor(request);requirePermission(actor,"bookings.view");requirePermission(actor,"bookings.manage");const db=await database(),data=await getSittingOpsSnapshot(db);return json({data,sandboxOnly:true});}catch(error){return authError(error,"Unable to load Sitting Operations queue");}}
