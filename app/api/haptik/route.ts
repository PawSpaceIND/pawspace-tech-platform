...
import{loadAiKnowledgeCenter}from"../../../lib/ai-knowledge-center";
...
export async function POST(request:Request){
  try{
    ...
    if(action==="knowledge_context")return json({data:{locale:canonicalVoiceLocale(body.locale||body.language),knowledge:await loadAiKnowledgeCenter(db,{cityId:String(body.city||body.cityId||"blr"),zoneId:body.zone as string,query:String(body.query||body.message||""),visibilityScopes:["public"]})}});
    ...
    return json({error:"Unsupported Haptik action. Use knowledge_context | capture_lead | capture_callback | fetch_slots | request_booking | record_call_outcome"},400);
  }catch(error){
