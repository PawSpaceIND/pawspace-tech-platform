...
export async function POST(request:Request){
  try{
    ...
    if(action==="fetch_slots"){const cityId=String(body.city||body.cityId||"").trim(),zoneId=String(body.zone||body.zoneId||"").trim();if(!cityId||!zoneId)return json({error:"City and zone are required to fetch serviceable Haptik slots"},400);return json({data:await fetchHaptikTimeSlots(db,{serviceCode:String(body.service||body.serviceCode||""),cityId,zoneId,fromDate:body.fromDate as string,days:body.days as number})});}
