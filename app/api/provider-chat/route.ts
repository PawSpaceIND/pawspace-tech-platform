...
import { ensureTrustSafetyTables, recordProviderChatMessage, redactTrustSafetyText } from "../../../lib/trust-safety-governance";
...
export async function GET(request: Request) {
  try {
    ...
    const data = messages.results.map(row => {
      ...
      // Persisted transport payloads may contain private staff/finance/media metadata.
      // Provider chat exposes a text projection, never the underlying payload.
      const body = [stored.text, stored.message, stored.body, stored.content].find(value => typeof value === "string") as string | undefined;
      const inspected = redactTrustSafetyText(body || "");
      return { id: row.id, direction: row.direction, channel: row.channel, payload: { text: inspected.redacted, safetyRedacted: inspected.detections.length > 0 || stored.safetyRedacted === true }, status: row.status, createdAt: row.created_at };
    });
