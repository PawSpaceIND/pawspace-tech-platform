...
import {authError,requireCustomerOwnership,requirePermission,resolveActor,type AuthenticatedActor} from "../../../lib/server-auth";
...
import {cityBookingVerdict,type BookingChannel} from "../../../lib/city-status-authority";
...
/* A booking may FILL a saved contact field; it may NEVER blank one.
 * No booking client sends `email`, and step 4's "Alternative Phone Number" posts "" when the customer
 * leaves it alone, so both absent spellings are normalised to NULL HERE - at the binding site, once,
 * for every statement in the batch - and the COALESCE in the conflict clause reads that NULL as "keep
 * what is stored". Normalising at the bind rather than in SQL also keeps "" out of the FIRST insert,
 * so a stored contact is either real or NULL and never an empty string a later COALESCE cannot tell
 * from a real one. A booking that DOES carry a contact still lands: it is not NULL, so COALESCE takes it. */
const optionalContact=(value:unknown):string|null=>{const text=typeof value==="string"?value.trim():"";return text.length?text:null};
...
async function projectCanonicalCustomerToCrm(db:D1Database,input:LifecycleInput,packageName:string,now:number){
  ...
  await db.prepare("INSERT INTO crm_contacts (id,name,primary_phone,secondary_phone,email,area,pet_names,pet_summary,stage,owner,source,lifetime_value,next_action,opportunity,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,'Active customer','Unassigned','canonical_booking',0,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,primary_phone=excluded.primary_phone,secondary_phone=COALESCE(excluded.secondary_phone,crm_contacts.secondary_phone),email=COALESCE(excluded.email,crm_contacts.email),area=excluded.area,pet_names=excluded.pet_names,pet_summary=excluded.pet_summary,next_action=excluded.next_action,opportunity=excluded.opportunity,updated_at=excluded.updated_at")
    .bind(input.customer.id,input.customer.name,input.customer.primaryPhone,optionalContact(input.customer.secondaryPhone),optionalContact(input.customer.email),input.cityId==="blr"?"Bengaluru":input.cityId,petNames,petSummary,"Post-booking care follow-up",packageName,now,now).run();
}
...
export async function executeCanonicalBookingRequest(request:Request,actorOverride?:AuthenticatedActor){try{const db=await database();await ensureSchedulingReservationLeaseGovernance(db);await cleanupExpiredReservationLeases(db);const actor=actorOverride??await resolveActor(request);requirePermission(actor,"scheduling.book");const input=await request.json() as LifecycleInput;const problem=validate(input);if(problem)return json({error:problem},400);await ensureTables(db);await requireCustomerOwnership(db,actor,input.customer.id);const replayInput={customerId:input.customer.id,serviceCode:input.serviceCode,idempotencyKey:input.idempotencyKey,scheduleGroupId:input.scheduleGroupId};/*
   * WHO booked this, and THROUGH WHAT. Both were constants: channel was bound "customer_app" and
   * created_by was bound input.customer.id for EVERY caller, so a booking a staff member took on a
   * call was recorded as the customer booking themselves in the app - and every
   * booking_lifecycle_events row was attributed to the customer for the same reason.
   * app/api/assisted-orders corrected its own two columns afterwards with a second UPDATE; no other
   * staff caller did, and nothing corrected the event log at all.
   *
   * Derived from the ACTOR, never from a caller-supplied field: a customer books through an identity
   * subject (customer OTP), a staff member through their workspace email, and a caller cannot choose
   * which of the two it is. The local development-preview actor is deliberately NOT read as staff -
   * it is a stand-in for whoever the operator wants to be (lib/development-preview.ts), so it is no
   * evidence that a staff member placed the order, and a UAT booking keeps reading as the customer's
   * own rather than silently becoming staff-assisted.
   *
   * The two names are two different vocabularies, and they are kept apart on purpose.
   * canonical_bookings.channel records "assisted_staff" - what app/api/assisted-orders has always
   * written there, and what the coupon and Pet Taxi channel types call it. The CITY gate speaks
   * lib/city-status-authority.ts's BOOKING_CHANNELS, where the same thing is "ops_assisted"; a
   * pilot city enables channels one at a time, so evaluating a staff-assisted booking as though it
   * arrived through the app is the same contradiction in the other direction.
   */
  const staffPlacedThisOrder=actor.principalType==="email"&&!actor.developmentPreview;
  const bookedByCustomerThemselves=!staffPlacedThisOrder;
  const bookingChannel=bookedByCustomerThemselves?"customer_app":"assisted_staff";
  const cityChannel:BookingChannel=bookedByCustomerThemselves?"customer_app":"ops_assisted";
  const createdBy=bookedByCustomerThemselves?input.customer.id:actor.email;
  const prior=await findCustomerReplay(db,replayInput);if(prior)return json({data:await readBundle(db,prior,true)});if(await hasForeignReplayConflict(db,replayInput))return json({error:BOOKING_REPLAY_CONFLICT},409);
  /*
   ...
   */
  const cityVerdict=await cityBookingVerdict(db,{cityId:input.cityId,serviceCode:input.serviceCode,pincode:(input as{pincode?:string}).pincode??null,channel:cityChannel});
  ...
  const statements=[
    ...
    db.prepare("INSERT INTO canonical_customers (id,city_id,name,primary_phone,secondary_phone,email,source,consent_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?, ?,?,?) ON CONFLICT(id) DO UPDATE SET city_id=excluded.city_id,name=excluded.name,primary_phone=excluded.primary_phone,secondary_phone=COALESCE(excluded.secondary_phone,canonical_customers.secondary_phone),email=COALESCE(excluded.email,canonical_customers.email),updated_at=excluded.updated_at").bind(input.customer.id,input.cityId,input.customer.name,input.customer.primaryPhone,optionalContact(input.customer.secondaryPhone),optionalContact(input.customer.email),"uat_customer_app",JSON.stringify({serviceUpdates:true,marketing:false}),now,now),
...
["payment_recorded","payment",paymentId,{status:paymentStatusPersisted,submittedStatus:input.payment.status,gateway:"uat_sandbox",liveMoney:false,amount:governed.totalAmount,amountDueNow:governed.amountDueNow,sittingQuoteId:sittingCommercial?.quoteId}]];
  ...
  for(const [eventType,entityType,entityId,detail] of events)statements.push(db.prepare("INSERT INTO booking_lifecycle_events (id,booking_id,event_type,entity_type,entity_id,actor_id,detail_json,occurred_at) VALUES (?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(),bookingId,eventType,entityType,entityId,createdBy,JSON.stringify(detail),now));
