...
export async function POST(request:Request){const requestStarted=Date.now();let traceContext:Record<string,unknown>={};try{const db=await timedBookingStage("database",traceContext,()=>database());await timedBookingStage("lease_governance",traceContext,()=>ensureSchedulingReservationLeaseGovernance(db));await timedBookingStage("lease_cleanup",traceContext,()=>cleanupExpiredReservationLeases(db));const actor=await timedBookingStage("actor_resolution",traceContext,()=>resolveActor(request));requirePermission(actor,"scheduling.book");const input=await request.json() as LifecycleInput;traceContext={serviceCode:String(input.serviceCode||""),cityId:String(input.cityId||""),bookingKey:String(input.idempotencyKey||input.scheduleGroupId||"").slice(-24)};const problem=validate(input);if(problem)return json({error:problem},400);await timedBookingStage("canonical_schema",traceContext,()=>ensureTables(db),true);await timedBookingStage("customer_ownership",traceContext,()=>requireCustomerOwnership(db,actor,input.customer.id));const replayInput={customerId:input.customer.id,serviceCode:input.serviceCode,idempotencyKey:input.idempotencyKey,scheduleGroupId:input.scheduleGroupId};const prior=await timedBookingStage("replay_lookup",traceContext,()=>findCustomerReplay(db,replayInput));if(prior)return json({data:await readBundle(db,prior,true)});if(await timedBookingStage("foreign_replay_check",traceContext,()=>hasForeignReplayConflict(db,replayInput)))return json({error:BOOKING_REPLAY_CONFLICT},409);
   ...
   */
  const[cityVerdict,replayConflict]=await Promise.all([timedBookingStage("city_booking_verdict",traceContext,()=>cityBookingVerdict(db,{cityId:input.cityId,serviceCode:input.serviceCode,pincode:(input as{pincode?:string}).pincode??null,channel:(input as{channel?:string}).channel??"customer_app"})),timedBookingStage("replay_conflict_check",traceContext,()=>hasReplayConflict(db,replayInput))]);
  if(!cityVerdict.allowed)return json({error:"PawSpace is not taking new bookings in this city right now",code:cityVerdict.reason,cityId:cityVerdict.cityId,cityStatus:cityVerdict.status,existingBookingsUnaffected:cityVerdict.existingWorkHandling==="continue"},409);if(replayConflict)return json({error:BOOKING_WRITE_CONFLICT},409);
  ...
  const commercialPolicy=input.serviceCode==="grooming"?await timedBookingStage("grooming_policy",traceContext,()=>resolveGroomingPolicy(db,input.cityId,input.zoneId)):null;
  ...
  if(input.serviceCode==="grooming"){
    await timedBookingStage("provider_booking_guard",traceContext,()=>ensureProviderBookingGuard(db),true);
  ...
  const[assignment,reservations]=await Promise.all([timedBookingStage("assignment_read",traceContext,()=>db.prepare("SELECT selected_provider_id,status,shortlist_json FROM scheduling_assignment_decisions WHERE group_id=?").bind(input.scheduleGroupId).first<Record<string,unknown>>()),timedBookingStage("reservation_read",traceContext,()=>db.prepare("SELECT id,provider_id,customer_id,service_code,city_id,zone_id,scheduled_start,scheduled_end,occurrence_number,status FROM scheduling_reservations WHERE group_id=? AND status!='cancelled' ORDER BY occurrence_number").bind(input.scheduleGroupId).all<Record<string,unknown>>())]);if(!assignment||assignment.status!=="assigned")return json({error:"Scheduling must be assigned before booking confirmation"},409);if(String(assignment.selected_provider_id)!==input.provider.id)return json({error:"The provider does not match the scheduling decision"},409);if(!schedulingGroupBelongsToCustomer(reservations.results,input.customer.id))return json({error:SCHEDULING_GROUP_OWNERSHIP_CONFLICT},409);if(!reservations.results.length||reservations.results.some(row=>String(row.provider_id)!==input.provider.id))return json({error:"A valid provider reservation is required"},409);if(reservations.results.some(row=>String(row.service_code)!==input.serviceCode))return json({error:"The booking service does not match the scheduling reservation"},409);
  ...
  if(input.serviceCode==="grooming"){
    const unavailable=(await timedBookingStage("provider_unavailability",traceContext,()=>Promise.all(reservations.results.map(row=>providerUnavailableForWindow(db,{providerId:String(row.provider_id),scheduledStart:String(row.scheduled_start),scheduledEnd:String(row.scheduled_end)})))).then(rows=>rows)).some(Boolean);
    if(unavailable){
...
["payment_recorded","payment",paymentId,{status:paymentStatusPersisted,submittedStatus:input.payment.status,gateway:"uat_sandbox",liveMoney:false,amount:governed.totalAmount,amountDueNow:governed.amountDueNow,sittingQuoteId:sittingCommercial?.quoteId,sittingPaymentReference:sittingCapture?.reference}]];
  ...
  if(String(paymentStatusPersisted)==="captured"){
    await timedBookingStage("collection_ledger",traceContext,()=>postCollectionEvent(db,{
    ...
    }),true).catch(error=>{console.warn("[collection-ledger] posting deferred",error instanceof Error?error.message:String(error));});
  }
  if(trainingCommercial)await consumeTrainingQuote(db,trainingCommercial.quoteId,bookingId);if(boardingCommercial)await consumeBoardingQuote(db,boardingCommercial.quoteId,bookingId);if(sittingCommercial)await consumeSittingQuote(db,sittingCommercial.quoteId,bookingId);await timedBookingStage("lead_attribution",traceContext,()=>attributeBookingToOpenLead(db,{customerId:input.customer.id,bookingId,serviceCode:input.serviceCode,paymentStatus:paymentStatusPersisted}),true);const responseData={bookingId,customerId:input.customer.id,petIds:ids,scheduleGroupId:input.scheduleGroupId,workOrderId,paymentId,status:"confirmed",duplicatePrevented:false};bookingTrace("request","complete",requestStarted,traceContext);return json({data:responseData},201);
