...
// The crm_contacts DDL is idempotent, so it is sent once per D1 binding rather than on every booking.
const crmContactsTableEnsured=new WeakSet<D1Database>();
async function projectCanonicalCustomerToCrm(db:D1Database,input:LifecycleInput,packageName:string,now:number){
  if(!crmContactsTableEnsured.has(db)){await db.prepare("CREATE TABLE IF NOT EXISTS crm_contacts (id TEXT PRIMARY KEY, name TEXT NOT NULL, primary_phone TEXT NOT NULL, secondary_phone TEXT, email TEXT, area TEXT, pet_names TEXT, pet_summary TEXT, stage TEXT NOT NULL DEFAULT 'New lead', owner TEXT DEFAULT 'Unassigned', source TEXT DEFAULT 'Website', lifetime_value REAL DEFAULT 0, next_action TEXT, opportunity TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)").run();crmContactsTableEnsured.add(db);}
...
export async function executeCanonicalBookingRequest(request:Request,actorOverride?:AuthenticatedActor){try{const db=await database();await ensureSchedulingReservationLeaseGovernance(db);await cleanupExpiredReservationLeases(db);const actor=actorOverride??await resolveActor(request);requirePermission(actor,"scheduling.book");const input=await request.json() as LifecycleInput;const problem=validate(input);if(problem)return json({error:problem},400);await ensureTables(db);await requireCustomerOwnership(db,actor,input.customer.id);const replayInput={customerId:input.customer.id,serviceCode:input.serviceCode,idempotencyKey:input.idempotencyKey,scheduleGroupId:input.scheduleGroupId};const prior=await findCustomerReplay(db,replayInput);if(prior)return json({data:await readBundle(db,prior,true)});const closedPrior=await findClosedCustomerBooking(db,replayInput);if(closedPrior)return json(closedBookingRequestRefusal(closedPrior),409);if(await hasForeignReplayConflict(db,replayInput))return json({error:BOOKING_REPLAY_CONFLICT},409);
  ...
  // A Training split pays its deposit now and the rest before the final session (the lifecycle refuses
  // that session until FULLY_PAID). Recording the balance in the same split schedule the checkout reads
  // lets the customer pay it through Razorpay; without it the booking read "settled" after the deposit.
  if(trainingCommercial&&trainingCommercial.paymentMode==="split"&&trainingCommercial.amountDueNow<trainingCommercial.totalAmount){await ensureStayPaymentTables(db);const finalStart=Math.max(...reservations.results.map(row=>Date.parse(String(row.scheduled_start))).filter(Number.isFinite));statements.push(staySplitScheduleStatement(db,{bookingId,serviceCode:"dog_training",customerId:input.customer.id,totalAmount:trainingCommercial.totalAmount,paidNowAmount:trainingCommercial.amountDueNow,balanceAmount:Math.round((trainingCommercial.totalAmount-trainingCommercial.amountDueNow)*100)/100,balanceDueAt:Number.isFinite(finalStart)?finalStart:Date.parse(input.scheduledStart)}));}
