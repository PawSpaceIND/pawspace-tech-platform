 ...
 * merely padded or oddly cased is still the customer's own value, so blankness is TESTED with TRIM
...
const petFills=(column:string)=>`(TRIM(COALESCE(canonical_pets.${column},''))='' AND TRIM(COALESCE(excluded.${column},''))<>'')`;
const petKeep=(column:string)=>`${column}=CASE WHEN ${petFills(column)} THEN excluded.${column} ELSE canonical_pets.${column} END`;
const VACCINATION_FILLS="(LOWER(TRIM(COALESCE(canonical_pets.vaccination_status,''))) IN ('','not_provided') AND LOWER(TRIM(COALESCE(excluded.vaccination_status,''))) NOT IN ('','not_provided'))";
