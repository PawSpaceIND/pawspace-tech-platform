...
export async function POST(request:Request){try{const input=await request.json() as LifecycleInput;const problem=validate(input);if(problem)return json({error:problem},400);const db=await database();await ensureTables(db);const actor=await resolveActor(request);await requireCustomerOwnership(db,actor,input.customer.id);const prior=await db.prepare("SELECT * FROM canonical_bookings WHERE idempotency_key=? OR schedule_group_id=?").bind(input.idempotencyKey,input.scheduleGroupId).first<Record<string,unknown>>();if(prior)return json({data:await readBundle(db,prior,true)});
  ...
  // whose profile was filled in. Most booking flows also send only sourceId/name/species, so the
  // profile could not come from the request either.
  //
  // Resolve each booking pet against the customer's own pets first — by source id, else by name — and
  // reuse that row rather than minting a duplicate, inheriting whatever the caller did not send.
  ...
  // A pet already booked before this fix left an empty duplicate row behind whose source id IS the
  // pet name, so an id-only match would keep selecting the blank row and the card would stay empty
  // for exactly the customers who hit the bug. Gather every candidate for this animal — matched by
  // source id first, then by name — and prefer one that actually carries a profile.
  const hasProfile=(row:Record<string,unknown>)=>Boolean(key(row.breed))||(key(row.vaccination_status)&&key(row.vaccination_status)!=="not_provided");
  const resolvedPets=input.pets.map(pet=>{
    const candidates=[
      ...existingPets.results.filter(row=>key(row.source_pet_id)&&key(row.source_pet_id)===key(pet.sourceId)),
      ...existingPets.results.filter(row=>key(row.name)===key(pet.name)),
    ];
    const match=candidates.find(hasProfile)??candidates[0];
    const inheritedBreed=match&&match.breed!=null&&key(match.breed)?String(match.breed):null;
    const inheritedVaccination=match&&key(match.vaccination_status)&&key(match.vaccination_status)!=="not_provided"?String(match.vaccination_status):null;
    ...
    // caller that has nothing to say cannot out-rank the profile the customer actually saved.
    const suppliedBreed=key(pet.breed)?String(pet.breed):null;
    const suppliedVaccination=key(pet.vaccinationStatus)&&key(pet.vaccinationStatus)!=="not_provided"?String(pet.vaccinationStatus):null;
    return {
