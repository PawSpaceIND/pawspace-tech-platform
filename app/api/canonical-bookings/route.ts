...
export async function POST(request:Request){const denied=await refuseUnlessGatewayPermits(request);if(denied)return denied;try{const input=await request.json() as LifecycleInput;const problem=validate(input);if(problem)return json({error:problem},400);const db=await database();await ensureTables(db);const actor=await resolveActor(request);await requireCustomerOwnership(db,actor,input.customer.id);const prior=await db.prepare("SELECT * FROM canonical_bookings WHERE idempotency_key=? OR schedule_group_id=?").bind(input.idempotencyKey,input.scheduleGroupId).first<Record<string,unknown>>();if(prior)return json({data:await readBundle(db,prior,true)});
  ...
  // a pet whose details were filled in. Most booking flows send only sourceId/name/species, so the
  // profile could not arrive on the request either.
  //
  // Resolution is TIERED, and IDENTITY OUTRANKS PROFILE RICHNESS. A same-name row must never
  // displace an unambiguous source-id match: two of a customer's pets can share a name, and binding
  // a booking to the wrong animal is worse than showing a blank profile — that id flows into the
  // work order the groomer works from and into boarding's vaccination gating.
  ...
  // issued `account-<hash>` (or a client id), never its own name. Naming the legacy condition this
  // precisely is what lets duplicate-healing stay without opening the door to same-name substitution.
  ...
  const resolvedPets=input.pets.map(pet=>{
    const identityMatches=existingPets.results.filter(row=>petKey(row.source_pet_id)&&petKey(row.source_pet_id)===petKey(pet.sourceId));
    const nameMatches=existingPets.results.filter(row=>petKey(row.name)===petKey(pet.name));
    // TIER 1 — identity. Profile preference is applied only WITHIN this tier, so it can break a tie
    // between rows that are all the same animal but can never reach across to a different one.
    let match=identityMatches.find(petHasProfile)??identityMatches[0];
    // LEGACY EXCEPTION, narrowly gated: the sole identity match is a pre-fix booking artifact for
    // this name, with no profile to offer. Only then may a saved pet of the same name be adopted, and
    // only a genuine saved pet — another artifact cannot stand in.
    ...
    // TIER 2 — no identity match at all. This is the ordinary case: the flows send the pet's name as
    // the source id and nothing has been minted yet, so the customer's saved pet of that name is the
    // intended animal.
