 ...
 * these rules existed are still replayable — a matching idempotency key returns its original booking
 * even though its payload would be refused today.
 */
 ...
 * needs no read at all — the same identity always yields the same id, so concurrent bookings converge,
 * and DIFFERENT identities yield different ids even when they sanitize alike. The readable prefix is
 * kept for operators; the digest carries the identity, so `pet-1` and `pet_1` never collide the way
 * the sanitized-alphanumeric form did.
 */
async function mintPetId(customerId:string,sourceId:string){
  const bytes=new TextEncoder().encode(`${petKey(customerId)}\u0000${petKey(sourceId)}`);
...
export async function POST(request:Request){const denied=await refuseUnlessGatewayPermits(request);if(denied)return denied;try{const input=await request.json() as LifecycleInput;const problem=validate(input);if(problem)return json({error:problem},400);const db=await database();await ensureTables(db);const actor=await resolveActor(request);await requireCustomerOwnership(db,actor,input.customer.id);const prior=await db.prepare("SELECT * FROM canonical_bookings WHERE idempotency_key=? OR schedule_group_id=?").bind(input.idempotencyKey,input.scheduleGroupId).first<Record<string,unknown>>();if(prior)return json({data:await readBundle(db,prior,true)});
  ...
  // PHASE 4 — mint the rest from the identity digest. No read, so nothing to race: the same identity
  // always produces the same id, and distinct identities produce distinct ids even from one snapshot.
  const mintedIds=new Map<number,string>();
  for(const [index,pet] of input.pets.entries())if(!matches[index])mintedIds.set(index,await mintPetId(input.customer.id,pet.sourceId));
