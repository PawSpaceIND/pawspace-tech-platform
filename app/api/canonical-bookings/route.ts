import {listCanonicalBookingsForAdmin} from "../../../lib/canonical-booking-admin-list";
...
async function ensureTables(db:Awaited<ReturnType<typeof database>>){await db.batch([
  ...
  // Index the bounded admin list and its child joins as booking history grows.
  db.prepare("CREATE INDEX IF NOT EXISTS idx_canonical_bookings_created ON canonical_bookings(created_at DESC,id DESC)"),
...
export async function GET(request:Request){try{const actor=await resolveActor(request);requirePermission(actor,"bookings.manage");const db=await database();await ensureTables(db);return json({bookings:await listCanonicalBookingsForAdmin(db)});}catch(error){return authError(error,"Unable to load lifecycle records");}}
