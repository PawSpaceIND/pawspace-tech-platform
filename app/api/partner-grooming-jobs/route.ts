...
export async function GET(request:Request){
  try{
    ...
    // Partner opens a fresh browser/session immediately after customer payment. Read the assignment
    // projection from the latest primary state so a just-confirmed work order cannot disappear behind
    // a stale replica. Test doubles without Sessions support safely fall back to the base binding.
    const readDb=(db as Db&{withSession?:(constraint:string)=>Pick<Db,"prepare"|"batch">}).withSession?.("first-primary")??db;
    const rows=await readDb.prepare(`SELECT w.id work_order_id,w.booking_id,w.provider_id,w.provider_name,w.provider_model,w.status work_order_status,w.occurrence_count,
