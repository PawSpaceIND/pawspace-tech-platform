...
type Input={
 idempotencyKey:string;scheduleGroupId:string;sittingQuoteId:string;
...
export async function POST(request:Request){try{
 ...
 const statements=[
  ...
  db.prepare("INSERT INTO booking_lifecycle_events (id,booking_id,event_type,entity_type,entity_id,actor_id,detail_json,occurred_at) VALUES (?,?,?,?,?,?,?,?)").bind(`EVT-SIT-${crypto.randomUUID().slice(0,8).toUpperCase()}`,bookingId,"sitting_payment_pending","booking",bookingId,actor.email,JSON.stringify({quoteId:input.sittingQuoteId,scheduleGroupId:input.scheduleGroupId,providerId:input.provider.id,confirmationRequiresVerifiedCapture:true}),now),
