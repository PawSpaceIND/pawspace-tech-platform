...
async function postBalancedJournal(db:Db,sourceType:string,sourceId:string,date:string,debitAccount:string,creditAccount:string,amount:number,narration:string,costCentre:string,vertical:string){
 ...
 const period=date.slice(0,7),now=Date.now(),group=id("journal");
...
export async function GET(request:Request){const denied=await refuseUnlessGatewayPermits(request);if(denied)return denied;try{const db=await database();await seed(db);const [expenses,bills,vendors,journals,bank,budgets,periods,audits]=await Promise.all([
...
export async function POST(request:Request){const denied=await refuseUnlessGatewayPermits(request);if(denied)return denied;try{const actor=await authorize(request,"finance.manage");const db=await database();await seed(db);const body=await request.json() as Record<string,unknown>,entity=String(body.entity??""),now=Date.now();
...
export async function PATCH(request:Request){const denied=await refuseUnlessGatewayPermits(request);if(denied)return denied;try{const actor=await authorize(request,"finance.manage");const db=await database();await seed(db);const body=await request.json() as {entity?:string;id?:string;action?:string;reason?:string};if(!body.id||!body.action||!body.reason||body.reason.trim().length<5)return json({error:"A reason of at least 5 characters is required"},400);const now=Date.now();
