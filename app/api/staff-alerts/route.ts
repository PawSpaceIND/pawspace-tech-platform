...
export async function POST(request:Request){try{const actor=await authorize(request,"customers.manage"),db=await database(),body=await request.json() as Row,action=String(body.action||"").trim();
 if(action==="sweep"){const data=await runStaffAlertSweep(db,{actorId:actor.email,asOf:body.asOf==null?undefined:Number(body.asOf)}),scheduler=await backgroundSchedulerStatus(db);await securityAudit(db,actor,"staff_alert.sweep","staff_alert","scheduler","completed",data);return json({data,scheduler,productionReady:true,externalDelivery:false});}
 if(action==="acknowledge"||action==="resolve"){const data=await updateStaffAlert(db,{alertId:String(body.alertId||""),action,actorId:actor.email});await securityAudit(db,actor,`staff_alert.${action}`,"staff_alert",String(body.alertId||""),"completed",{});return json({data,productionReady:true,externalDelivery:false});}
