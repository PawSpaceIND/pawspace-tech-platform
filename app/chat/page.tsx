...
import {CustomerChatUiError,customerChatResponseError,customerChatErrorMessage} from "../../lib/communication-ui-state";
...
export default function AiChatPage(){
 ...
 function choose(next:"public"|"authenticated"){if(sending.current)return;setMode(next);setTurns([]);setError("");pending.current=null;}
 async function submit(event:FormEvent){event.preventDefault();if(sending.current||busy||!message.trim()||(mode==='authenticated'&&identity!=='customer'))return;
  ...
  const controller=new AbortController();request.current=controller;const timer=setTimeout(()=>controller.abort(),20000);sending.current=true;setBusy(true);setError('');
  try{const body=mode==='public'?{mode,query:question}:{mode,message:question,idempotencyKey:'web-'+pending.current.key};
   ...
   if(!payload?.data)throw new CustomerChatUiError('Chat returned an incomplete response. Please try again.');
  ...
  finally{clearTimeout(timer);sending.current=false;setBusy(false);request.current=null;}
 }
