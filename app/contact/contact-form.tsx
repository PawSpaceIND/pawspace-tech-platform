...
export default function ContactForm(){
  ...
  async function onSubmit(event:React.FormEvent<HTMLFormElement>){
    ...
    try{const response=await fetch("/api/public-contact",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const result=await response.json() as{error?:string};if(!response.ok){setError(result.error||"Something went wrong - please try again.");setStatus("error");return;}setStatus("sent");form.reset();}catch{setError("Something went wrong - please check your connection and try again.");setStatus("error");}
  }
