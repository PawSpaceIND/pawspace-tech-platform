...
export default function LandingLeadForm({service,pet,formTitle,formCta}:{service:string;pet:string;formTitle:string;formCta:string}){
  ...
  async function onSubmit(event:React.FormEvent<HTMLFormElement>){
    ...
    const body={
      name:String(data.get("name")||""),phone:String(data.get("phone")||""),area:String(data.get("area")||""),petNames:pet,service,whatsappConsent:data.get("whatsappConsent")==="yes",
  ...
  return <div className={styles.formCard}>
    ...
    <form className={styles.formGrid} onSubmit={onSubmit}>
      ...
      <input aria-label="Phone number" name="phone" type="tel" required pattern="[0-9+\s\-]{10,15}" title="10-15 digits; + spaces and hyphens are allowed" placeholder="Phone number"/>
