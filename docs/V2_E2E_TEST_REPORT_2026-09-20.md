...
|---|---|---|---|---|---|
| SW-024 | P2 | wiring | Founder · `/team/customer-experience` | Page probes `/api/whatsapp/conversation-control` for every open thread, including non-WhatsApp ones, and receives a 409 on each load (repo sweep marks the page DATA) | `findings/sweep.md` SW-024 |
...
173 routes discovered from `app/`; 628 route × persona × viewport loads over 177 route states (desktop, Pixel 7, iPhone 14) across anonymous, customer, provider and founder; 455 buttons clicked in 75 dead-button passes with a reload between clicks; 124 unique same-origin links fetched (0 broken); 18 form probes; 929 screenshots. Result: 0 P0, 4 P1 (all covered above), 13 P2, 120 observations, 194 passes. No broken internal links, no uncaught page errors on customer/partner V2 surfaces, no horizontal overflow on `/mobile-app`, `/partner-app` or V2 pages on either phone viewport. Placeholder copy: only the word "placeholder" in the Revenue Mission page's own text. Console errors observed were the expected 401 identity probes plus the API failures listed above.
