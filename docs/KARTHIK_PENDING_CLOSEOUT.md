...
- [ ] **OTP/SMS/WhatsApp UAT delivery** — choose/confirm the UAT delivery provider/account and add its sandbox/test credentials. Configure `PAWSPACE_IDENTITY_ENV=sandbox` and a random 32+ character `PAWSPACE_IDENTITY_ASSERTION_SECRET_UAT` as a Cloudflare UAT secret shared only with the verified OTP adapter. The PawSpace application now has the signed assertion -> canonical identity binding -> HttpOnly customer/provider session contract; actual OTP delivery/verification remains external. Do not enable production messaging until authenticated UAT passes.
- [ ] **Unified communications UAT adapters** — configure the chosen sandbox accounts/credentials and webhook secrets for LimeChat/WhatsApp, SMS, email/push, Exotel voice and the autodialer. The PawSpace canonical thread/outbox/delivery/retry/dead-letter layer is application-ready; adapter credentials must be enabled and tested one at a time. Keep all production sending OFF until sandbox delivery, replay, failure and consent tests pass.
- [ ] **Exotel UAT** — add test account/number credentials and verify masked/routed provider-customer calling after identity/session UAT. This is the voice/telephony portion of the unified communications adapter test above.
