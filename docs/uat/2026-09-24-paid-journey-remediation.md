...
## Deployed voice finding and full-suite compatibility

A visible Chromium virtual-microphone test sent 380 PCM frames / 1,037,400 bytes to deployed staging, but no STT audio-flow event or transcript returned. Only the opening TTS audio arrived; the upstream then closed with an unsupported KeepAlive error. The browser receive socket omitted `binaryType`, although this Worker's 2026-05-22 compatibility date defaults binary messages to Blob. Its ArrayBuffer guard silently discarded those frames. The fix explicitly sets `server.binaryType = "arraybuffer"` BEFORE accept; an executable current-default mock proves PCM forwarding, transcript -> answer -> TTS and single-use/environment guards. No physical microphone or customer call was used. A new deployed voice proof remains required.

The protected 6,847-test run found four source-contract assertions referring to pre-refactor locations/wording. The assertions now follow the extracted care-before-payment gate, preserve the actual multiline long-stay due notice, and require a committed booking to be called created rather than falsely payment-confirmed. They are backed by the six independent browser fault-injection cases. Local source-contract follow-ups: 32 passed plus the focused long-stay notice check passed. No assertions were removed to waive payment readiness.
