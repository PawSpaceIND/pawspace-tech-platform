...
### CX rendering, status recovery and live refresh

Local browser verification found that the client live-refresh template caused a server-component serialization error, preventing the inbox from rendering. The template is now a server boundary; the page owns SSE updates and the existing polling fallback, refreshing data without remounting the operator's workspace. Status navigation now retrieves pending, resolved and closed threads, and Reopen returns them to Open. Rows display their actual status. The status service rejects unsupported values and avoids success audits for nonexistent threads; thread changes and audits remain transactional.

21 selected tests pass, including real status API calls through staff authentication, pending/resolved/open queries, invalid input and an injected audit failure with rollback/retry. Build/artifact validation and typecheck pass. In the authenticated local browser, the existing synthetic QA thread was moved to Awaiting customer, retrieved through its filter, reopened and restored to Open twice. SQLite confirmed two pending and two open audit events. Operator routing input survived refreshes. The temporary local UAT configuration is ignored by Git and keeps payment flags sandbox/true/false; it is not deployed. Full reply-draft preservation under an actual WhatsApp transport, all profile/booking context, inbox pagination and hosted SSE remain open.


### CX customer, booking and support context

The thread detail previously omitted the customer profile, booking summary and linked ticket expected by its inspector. Staff detail now attaches canonical customer data (still masked by the existing API policy), a customer-owned booking summary and an explicitly linked unified case or legacy ticket. Ticket reads check customer and booking linkage in both queue and detail. The customer/provider scope does not receive this added staff context. Support notice bodies render from their stored notice/body fields rather than falling back to a template key.

39 selected checks passed (29 context/access/platform/provider checks plus 10 connected journeys), with build/artifact validation and typecheck successful. The authenticated local browser now shows the masked QA customer, its completed grooming booking, and its linked high-priority one-star case with the response deadline. It also renders the actual overdue-support notice. The mismatch regression changes linked records to another customer and confirms they are withheld. No money or external communication is generated by these reads. Global inbox search/pagination, preview content, internal notes, assignment UX and full deployed coverage remain open.


### Full-suite and CI lint follow-through

Commit `68cab731` passed all 4,693 local tests (zero failures/skips). GitHub reported both Lint and Staging isolation/full certification failing at the same React lint error in the customer support-notification dialog. The dialog now uses a customer-keyed component for identity resets, resets retry state in the user action, captures its dialog element for cleanup, and aborts/ignores pending pagination when its lifetime ends.

Local lint passes with zero errors (81 existing warnings), build/artifact validation and typecheck pass, and all 10 connected journey tests pass. The local browser signed in with the existing QA customer's sandbox OTP, opened the actual support notification, displayed its booking context, and closed the dialog successfully. Remote reruns after this fix remain pending. Vercel independently reports that the deployment account is blocked; this is not resolved by the code changes and no deployment certification is claimed.


### CX internal notes and draft isolation

Added a dedicated staff-note action and editor. Notes are stored as conversation audit records with a deterministic, thread-scoped request key. The note and thread refresh timestamp commit together; duplicate/concurrent retries create one note, while a conflicting body returns 409. Customer communications/outbox are untouched. Staff reads use the existing restricted-note policy; customer/provider scopes receive no notes. Inbox projection ownership alone remains insufficient for row access: the tests exercise canonical lead assignment, membership, service and city scope. Switching conversations or status views clears unsaved reply/note drafts.

37 selected tests pass, including duplicate/concurrent requests, transaction rollback/retry, invalid input, row authorization and restricted-note masking. Build/artifact validation, typecheck and lint pass (zero lint errors, 81 existing warnings). In the authenticated local browser, a synthetic note was saved once, appeared on its original thread, and did not increase that thread's customer-message count. An unsaved note cleared on switching to a second synthetic customer; returning showed only the original saved note. A transient local Miniflare error during direct fixture seeding recovered on subsequent reads; this is not deployed resilience evidence. Latest GitHub observation confirms the earlier lint failure cleared; certification was still running. Notes beyond the latest 100, global search/pagination and agent-assignment UX remain open.


### CX global search and cursor navigation — local evidence

Search now runs inside the authorized database query before pagination, covering thread, customer, booking, lead, name and phone. Channel and ownership filters also apply before the page limit. The UI requests 50 rows, exposes previous/next cursor navigation, and labels counts as page counts. Literal percent/underscore searches remain literal; malformed limits, cursors and filters return 400. Ordering uses updated time and thread ID for tied timestamps. Live updates can move threads between pages; this is not a frozen snapshot.

Validation: 32 focused executed tests pass; build and typecheck pass; lint has zero errors and 81 existing warnings. Route/SQLite coverage checks multi-page traversal without duplicates, search beyond the initial page, ownership filters, malformed requests and restricted staff visibility. Authenticated local browser search for synthetic booking B-QA-INBOX reduces two rows to its one linked conversation. No notification or financial mutation is part of this read-only flow. Hosted verification remains open; this evidence does not establish 95% overall readiness.


### Inbound replay ownership verification

The staff inbound-recording path now verifies the current thread/customer relationship before returning a duplicate message. Reusing a provider reference for another thread, customer, direction or channel is rejected rather than reported as successful. Matching retries still return the original message. Executed route/database coverage proves mismatched and reassigned ownership cannot replay the original success; 20 focused tests pass, with build/artifact validation and typecheck passing. This does not certify all external webhook concurrency paths.
