...
# The built deployment config intentionally carries production-only AI and Vectorize bindings.
# Miniflare cannot emulate Workers AI and must never open a remote session during browser E2E, so
# serve a harness-only copy with those two binding classes removed. The shipping artifact is untouched.
E2E_WRANGLER_CONFIG="$ROOT/dist/server/wrangler.e2e.json"
node - "$ROOT/dist/server/wrangler.json" "$E2E_WRANGLER_CONFIG" <<'NODE'
