...
async function address(page){await page.getByLabel("Complete doorstep address",{exact:true}).fill(ADDRESS);await page.getByLabel("Pincode",{exact:true}).fill(PIN);await page.getByRole("button",{name:"Verify map",exact:true}).click();const matches=page.getByRole("region",{name:"Matching map addresses",exact:true});const doorstep=matches.getByRole("button",{name:/42.*Indiranagar Double Road/i}).first();await doorstep.waitFor({state:"visible",timeout:SERVER_TIMEOUT});await doorstep.click();await page.getByText("Verified service doorstep",{exact:true}).waitFor({state:"visible",timeout:SERVER_TIMEOUT});}
async function stay(page,sitting){
 const name=sitting?"Pet Sitting":"Boarding";await openService(page,name);await text(page,sitting?"Care at home, around their routine.":"A stay that feels like home.");await address(page);const available=page.getByRole("button",{name:sitting?/See available sitters/i:/See available homes/i});await ensurePetProgress(page,available,`${name} trip details`);await available.click();await text(page,sitting?"Choose your sitter":"Choose your host");if(sitting){const rateButtons=page.locator("button").filter({hasText:"/ night"});await rateButtons.first().waitFor({state:"visible",timeout:SERVER_TIMEOUT});const rates=await rateButtons.allTextContents();report.sittingProfileRateEvidence=rates.slice(0,3).map(x=>x.replace(/\s+/g," ").trim());if(!rates.length)die("Sitting profile rates not rendered for checkout-vs-card review");}
