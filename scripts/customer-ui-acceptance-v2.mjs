...
async function address(page){await page.getByLabel("Complete doorstep address").fill(ADDRESS);await page.getByPlaceholder("e.g., 560034").fill(PIN);await page.getByRole("button",{name:"Check",exact:true}).click();await text(page,"Koramangala",SERVER_TIMEOUT);}
