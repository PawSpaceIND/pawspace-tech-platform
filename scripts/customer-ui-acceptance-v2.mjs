...
async function grooming(page){await openService(page,"Grooming");await text(page,"Who needs grooming?");await petsReady(page);await petButton(page).click();const next=page.getByRole("button",{name:/Choose a package/i});await ready(page,next,"Grooming package progression");await next.click();for(const label of["Essential Bath","Bath & Basic","Complete Makeover","Just Trim"])await text(page,label);return"pet -> package stage + legacy packages";}
