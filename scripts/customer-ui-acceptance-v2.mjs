...
const TIMEOUT=Number(readArg("timeout","45000"));
...
async function ready(page,button,label){
 await button.waitFor({state:"visible",timeout:TIMEOUT});
 try{await button.click({trial:true,timeout:TIMEOUT});}
...
async function petsReady(page){const loading=page.getByText("Loading your pets…",{exact:true}).first();if(await loading.count())await loading.waitFor({state:"hidden",timeout:TIMEOUT});}
const petButton=(page)=>page.getByRole("button",{name:new RegExp(PET.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),"i")}).first();
async function selectPet(page){await petsReady(page);const button=petButton(page);await button.waitFor({state:"visible",timeout:TIMEOUT});const selected=await button.evaluate(element=>element.getAttribute("aria-pressed")==="true"||String(element.className).includes("selected")||element.textContent?.includes("✓"));if(!selected)await button.click();}
...
async function observeFinal(page,button,target,safePosts=[]){
 const seen=[],unexpected=[];let targetObserved=()=>{};const observed=new Promise(resolve=>{targetObserved=resolve;});const handler=async(route)=>{const req=route.request(),method=req.method(),path=new URL(req.url()).pathname,label=`${method} ${path}`;if(!["POST","PUT","PATCH","DELETE"].includes(method)){await route.continue();return;}if(target.test(label)){seen.push(label);targetObserved();await route.abort("blockedbyclient");return;}if(safePosts.some(pattern=>pattern.test(label))){await route.continue();return;}unexpected.push(label);await route.abort("blockedbyclient");};
 await page.route("**/api/**",handler);try{await button.click({timeout:TIMEOUT});await Promise.race([observed,page.waitForTimeout(TIMEOUT)]);}finally{await page.unroute("**/api/**",handler);}if(unexpected.length)die(`unexpected committing request(s) before target: ${unexpected.join(", ")}`);if(!seen.length)die(`expected final mutation ${target} not attempted`);return seen.join(", ");
}
...
async function grooming(page){await openService(page,"Grooming");await text(page,"Who needs grooming?");await selectPet(page);const next=page.getByRole("button",{name:/Choose a package/i});await ready(page,next,"Grooming package progression");await next.click();for(const label of["Essential Bath","Bath & Basic","Complete Makeover","Just Trim"])await text(page,label);return"pet -> package stage + legacy packages";}

async function training(page){
 await openService(page,"Training");await text(page,"Build better days together.");await petsReady(page);const plans=page.getByRole("button",{name:/See PawSpace plans/i});await ready(page,plans,"Training plan progression");await plans.click();await text(page,"All training programmes");await page.getByPlaceholder("Enter six-digit PIN code").first().fill(PIN);await page.getByRole("button",{name:"Choose trainer"}).click();await text(page,"Your trainer matches");const calendar=page.getByRole("button",{name:"Build session calendar"});await ready(page,calendar,"Training trainer match");await calendar.click();await text(page,"Plan your sessions");await page.getByRole("button",{name:"Review & pay"}).click();await text(page,"Review your programme");const final=page.getByRole("button",{name:/request trainer approval|Refreshing server quote/i});await ready(page,final,"Training server quote");const seen=await observeFinal(page,final,/POST \/api\/uat-scheduling/,[/POST \/api\/training-commercial/]);return`5 stages + final scheduler wiring (${seen})`;
}

async function address(page){await page.getByLabel("Complete doorstep address").fill(ADDRESS);await page.getByPlaceholder("e.g., 560034").fill(PIN);await page.getByRole("button",{name:"Check",exact:true}).click();await text(page,"Indiranagar");}
async function stay(page,sitting){
 const name=sitting?"Pet Sitting":"Boarding";await openService(page,name);await text(page,sitting?"Care at home, around their routine.":"A stay that feels like home.");await address(page);await selectPet(page);const available=page.getByRole("button",{name:sitting?/See available sitters/i:/See available homes/i});await ready(page,available,`${name} trip details`);await available.click();await text(page,sitting?"Choose your sitter":"Choose your host");if(sitting){const rates=await page.locator("button").filter({hasText:"/ night"}).allTextContents();report.sittingProfileRateEvidence=rates.slice(0,3).map(x=>x.replace(/\s+/g," ").trim());if(!rates.length)die("Sitting profile rates not rendered for checkout-vs-card review");}
 const next=page.getByRole("button",{name:/Continue with/i});await ready(page,next,`${name} caregiver match`);await next.click();await text(page,"Build the Care Card");await page.getByRole("button",{name:"Review protected booking"}).click();await text(page,"Review and confirm");const final=page.getByRole("button",{name:/create canonical stay|request final partner approval/i});await ready(page,final,`${name} server quote`);const seen=await observeFinal(page,final,sitting?/POST \/api\/(sitting-payment|uat-scheduling|canonical-bookings)/:/POST \/api\/uat-scheduling/,[/POST \/api\/(boarding|sitting).*quote/,/POST \/api\/live-price-quote/]);return`4 stages + caregiver + final wiring (${seen})`;
}

async function walking(page){
 await openService(page,"Dog Walking");await text(page,"Choose a walk package");const schedule=page.getByRole("button",{name:"Plan the schedule"});await ready(page,schedule,"Walking catalogue");await schedule.click();await page.getByRole("button",{name:"Choose your dog"}).click();await text(page,"Who's walking?");await selectPet(page);const review=page.getByRole("button",{name:"Review & confirm"});await ready(page,review,"Walking pet selection");await review.click();await text(page,"Review your walks");await page.getByPlaceholder("Enter six-digit PIN code").fill(PIN);const final=page.getByRole("button",{name:/Confirm \d+ walk|Refreshing server quote/i});await ready(page,final,"Walking server quote");const seen=await observeFinal(page,final,/POST \/api\/uat-scheduling/,[/POST \/api\/walking-commercial/]);return`4 stages + fresh quote + scheduler wiring (${seen})`;
}

async function taxi(page){
 await openService(page,"Pet Taxi");await text(page,"Choose a route class");const route=page.getByRole("button",{name:"Set pickup & drop"});await ready(page,route,"Taxi catalogue");await route.click();await page.getByPlaceholder("e.g. Indiranagar, 100 Feet Road").fill("Indiranagar");await page.getByPlaceholder("e.g. Whitefield vet clinic").fill("Whitefield Vet Clinic");const choose=page.getByRole("button",{name:"Choose your pet"});await ready(page,choose,"Taxi locations");await choose.click();await selectPet(page);const review=page.getByRole("button",{name:"Review & confirm"});await ready(page,review,"Taxi pet selection");await review.click();await text(page,"Review your trip");await page.getByPlaceholder("Enter six-digit PIN code").fill(PIN);const final=page.getByRole("button",{name:/Confirm trip|Refreshing server quote/i});await ready(page,final,"Taxi server quote");const seen=await observeFinal(page,final,/POST \/api\/uat-scheduling/,[/POST \/api\/taxi-commercial/]);return`4 stages + fresh quote + scheduler wiring (${seen})`;
}

async function food(page){
 await openService(page,"Fresh Food");await text(page,"Fresh food for your pets");await page.getByPlaceholder("Enter six-digit PIN code").first().fill(PIN);await page.getByRole("button",{name:"Check service area & load catalogue"}).click();await text(page,"Delivery coverage confirmed");const add=page.getByRole("button",{name:"Add",exact:true}).first();await add.waitFor({state:"visible",timeout:TIMEOUT});await add.click();const cart=page.getByRole("button",{name:/Review cart/i});await ready(page,cart,"Food cart");await cart.click();await text(page,"Your cart");await page.getByRole("button",{name:"Choose delivery plan"}).click();await text(page,"One-time or repeat?");await page.getByRole("button",{name:"Delivery details"}).click();await text(page,"Where and when?");const deliveryAddress=page.getByLabel("Delivery address",{exact:true});await deliveryAddress.fill(ADDRESS,{timeout:TIMEOUT});const review=page.getByRole("button",{name:"Review with server quote"});await ready(page,review,"Food delivery coverage");await review.click();await text(page,"Review and confirm");const final=page.getByRole("button",{name:/Confirm food order|Confirm order \+ repeat plan/i});await ready(page,final,"Food server quote");const seen=await observeFinal(page,final,/POST \/api\/food-orders/);return`5 stages + catalogue + quote + order wiring (${seen})`;
}
