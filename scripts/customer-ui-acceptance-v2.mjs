...
async function stay(page,sitting){
 const name=sitting?"Pet Sitting":"Boarding";await openService(page,name);await text(page,sitting?"Care at home, around their routine.":"A stay that feels like home.");await address(page);const available=page.getByRole("button",{name:sitting?/See available sitters/i:/See available homes/i});await ensurePetProgress(page,available,`${name} trip details`);await available.click();await text(page,sitting?"Choose your sitter":"Choose your host");if(sitting)await sittingRates(page);
