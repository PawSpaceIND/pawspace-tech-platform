...
// displaced. Absence of evidence stays a failure.
...
export const CONTROL_RESULT = {
  ...
  identityGated: "blocked_identity_precondition",
};

// A finding is a gate failure when the probe could not obtain wiring evidence, or could not
// trust that it probed the control it meant to probe. blocked_identity_precondition is not in
// this set: it means the surface refused the probe's session, which is the RBAC boundary doing
// its job rather than a broken control. It is counted and printed separately so it can never
// grow silently into a blind spot.
 ...
 * @param {string|null} [evidence.requestSeen]       Non-mutating API request observed.
 ...
 * @param {boolean} [evidence.identityGated]        Surface refused the probe's session.
...
export function classifyControl(evidence = {}) {
  if (evidence.identityGated) return CONTROL_RESULT.identityGated;
  ...
  // is committed. Clicking a sibling first moves that state away; the control must then commit a
  // change. Evidence, not exemption.
