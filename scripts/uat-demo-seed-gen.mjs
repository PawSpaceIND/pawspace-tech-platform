...
const BOOKINGS = [
  ...
  { id: "UATD-BK-TRAIN-3", cus: "UATD-CUS-5", svc: "dog_training", provider: "train_kiran", pkg: "Basic Obedience", amount: 4999, status: "in_progress", start: 0, pay: "captured" },
...
insert("training_session_consumptions", { session_id: "UATD-TS-1", programme_id: "UATD-TP-1", booking_id: "UATD-BK-TRAIN-1", actor_id: "train_kiran", consumed_at: at(-10, 12) });
...
insert("provider_commercial_terms", { id: "UATD-PCT-STD", service_code: "dog_walking", provider_id: null, version: 1, status: "active", engagement_model: "commission_standard", provider_share_pct: 0.7, gst_mode: "none", platform_gst_rate: 0.18, cash_allowed: 0, onboarding_fee: 0, renewal_fee: 0, renewal_months: 12, effective_from: dateAt(-200), reason: "Demo seed walking commercial baseline", created_by: "uat_demo_seed", approved_by: "founder@pawspace.in", approval_reference: "UATD-APPR-PCT-2", created_at: at(-200), updated_at: at(-200) });
for (const b of BOOKINGS.filter((x) => x.status === "completed" && ["grooming", "dog_walking"].includes(x.svc))) {
  ...
  const gstDeducted = 0; // owner decision 2 (26 Sept 2026): nothing is carved off the paid amount before the split
