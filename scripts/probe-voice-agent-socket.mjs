...
const socket=new WebSocket(b.signed_url);let sent=false,started=Date.now();
await new Promise((resolve,reject)=>{
 const timer=setTimeout(()=>{socket.close();reject(Error('No substantive agent response within 45 seconds'));},45000);
 ...
 socket.addEventListener('message',event=>{const d=JSON.parse(String(event.data));
  ...
  if(d.type==='agent_response'){
   ...
   else {const substantive=reply.replace(/^I'm checking the details[. ]*/i,'').trim();if(substantive&&!/^(one moment|just a moment|let me check|please wait)/i.test(substantive))finish();}
 ...
 socket.addEventListener('close',event=>{console.log('VOICE_SOCKET_CLOSED='+JSON.stringify({code:event.code,ms:Date.now()-started}));if(event.code!==1000)finish(Error('Agent socket closed before substantive reply'));});
});
