...
s.push("CREATE TABLE IF NOT EXISTS booking_invoices (id TEXT PRIMARY KEY,booking_id TEXT NOT NULL UNIQUE,customer_id TEXT NOT NULL,invoice_number TEXT NOT NULL UNIQUE,status TEXT NOT NULL DEFAULT 'draft',currency TEXT NOT NULL DEFAULT 'INR',gross_amount REAL NOT NULL,tax_amount REAL NOT NULL DEFAULT 0,net_amount REAL NOT NULL,issued_at INTEGER,created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL);");
s.push("CREATE TABLE IF NOT EXISTS provider_payout_computations (booking_id TEXT PRIMARY KEY,provider_id TEXT NOT NULL,service_code TEXT NOT NULL,order_value REAL NOT NULL,provider_net_payout REAL NOT NULL,platform_fee REAL NOT NULL,platform_gst REAL NOT NULL,provider_gst_deducted REAL NOT NULL,pawspace_gst_on_order REAL NOT NULL,breakdown_json TEXT NOT NULL,term_id TEXT NOT NULL,computed_by TEXT NOT NULL,computed_at INTEGER NOT NULL);");
  ...
  if (i % 10 < 7) {
    ...
    for (let b = 0; b < nb; b++) {
      ...
      if (bstatus === "completed") {
        // 18% GST on the taxable value, consistent with the statutory config the finance screens read.
        const gross = amount, tax = Math.round(amount - amount / 1.18), net = gross;
        s.push(`INSERT OR IGNORE INTO booking_invoices (id,booking_id,customer_id,invoice_number,status,currency,gross_amount,tax_amount,net_amount,issued_at,created_at,updated_at) VALUES ('INV-${bid}','${bid}','${id}','PS/2026/${bid}','issued','INR',${gross},${tax},${net},${at},${at},${BASE});`);
        // 20% platform commission; the provider keeps the rest.
        const fee = Math.round(amount * 0.2), payout = gross - fee;
        s.push(`INSERT OR IGNORE INTO provider_payout_computations (booking_id,provider_id,service_code,order_value,provider_net_payout,platform_fee,platform_gst,provider_gst_deducted,pawspace_gst_on_order,breakdown_json,term_id,computed_by,computed_at) VALUES ('${bid}','PROV-${city}-${svc}','${svc}',${gross},${payout},${fee},${Math.round(fee * 0.18)},0,${tax},'{"basis":"uat_seed"}','TERM-${svc}','uat_seed',${BASE});`);
      }
