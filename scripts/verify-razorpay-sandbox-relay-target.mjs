...
const host=/^(pawspace-checkout-736-[1-9][0-9]{0,19}-[1-9][0-9]{0,5})\.([a-z0-9-]+)\.workers\.dev$/i.exec(url.hostname);
if(url.protocol!=="https:"||url.username||url.password||url.port||url.search||url.hash||!['','/'].includes(url.pathname)||!host)throw new Error("Relay target must be an exact isolated certified-checkout workers.dev origin");
const worker=host[1];
