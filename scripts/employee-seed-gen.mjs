...
if (trainer) {
  ...
  // The conversion pays the trainer an incentive on this order value, and /team/people/incentives
  // states that pipeline revenue never qualifies as earned incentive. So the converted booking has to
  // be backed by money actually collected - otherwise the seed pays a bonus on revenue that does not
  // exist. The ₹0 meet & greet needs no payment; there is nothing to collect on a free visit.
  ...
  s.push(`INSERT OR IGNORE INTO booking_payments (id,booking_id,customer_id,amount,amount_due_now,method,mode,status,idempotency_key,created_at,updated_at) VALUES ('PAY-${conv}','${conv}','SEEDCUS1',18000,18000,'card','prepaid','captured','pidem-${conv}',${BASE},${BASE});`);
  s.push(`INSERT OR IGNORE INTO payment_reconciliation_records (payment_id,booking_id,gateway,environment,expected_amount,captured_amount,refunded_amount,currency,gateway_status,reconciliation_status,variance_amount,last_event_id,updated_at) VALUES ('PAY-${conv}','${conv}','uat_sandbox','sandbox',18000,18000,0,'INR','captured','matched',0,'evt-${conv}',${BASE});`);
