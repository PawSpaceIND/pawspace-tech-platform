...
async function d1(sql, params = []) {
  ...
  if (!response.ok || body?.success !== true || body?.result?.[0]?.success !== true) {
    throw new Error(body?.errors?.[0]?.message || body?.result?.[0]?.error || `D1 HTTP ${response.status}`);
  }
...
const intentRows = await d1("SELECT id,state,order_request_state,gateway_order_id,amount_paise,currency FROM payment_intents WHERE booking_id=? ORDER BY created_at DESC LIMIT 1", [bookingId]);
