...
# Every remaining old marker call is post-claim and must go through the fencing wrapper.
text = text.replace('await markInbox(db,accepted.row,', 'await finishInbox(')
# The replacement above also sees the fallback inside finishInbox. Restore that one deliberate legacy
# capture-path call; there must be exactly one self-recursive shape to repair.
one('await finishInbox(status,type,reason);\n    };', 'await markInbox(db,accepted.row,status,type,reason);\n    };')
...
if 'await finishInbox(status,type,reason);' in text:
    raise SystemExit('finishInbox must not recurse')
