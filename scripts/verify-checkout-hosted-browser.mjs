...
if(hosting.hosted!==true||hosting.candidateSha!==process.env.EXPECTED_SHA||hosting.worker!==`pawspace-checkout-674-${process.env.GITHUB_RUN_ID}-${process.env.GITHUB_RUN_ATTEMPT}`)throw Error("Hosted browser candidate provenance mismatch");
...
if(!/^https:\/\/pawspace-checkout-674-[0-9]+-[0-9]+\.[a-z0-9-]+\.workers\.dev$/.test(origin||""))throw Error("Only the dedicated checkout sandbox origin is allowed");
