...
try{
 for(const viewport of [{width:1440,height:1000},{width:390,height:844}]){
  ...
  try{
   ...
   await page.getByRole("button",{name:"Change PawSpace appearance"}).click();await appearance.getByRole("radio",{name:/^Professional/}).check();await appearance.getByRole("button",{name:"Done",exact:true}).click();await expect(page.locator("html")).toHaveAttribute("data-paw-style","professional");await page.screenshot({path:resolve(output,`${label}-professional.png`),fullPage:true});await page.reload();await expect(home).toBeVisible();await expect(page.locator("html")).toHaveAttribute("data-paw-style","professional");row.preferencePersistence=true;
