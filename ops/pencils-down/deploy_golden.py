...
WORKER = 'pawspace-beta-golden-8745bbca'
ENVIRONMENT = 'pencils-down-preview'
ROOT = Path(os.environ.get('GITHUB_WORKSPACE', '.')).resolve()
CANDIDATE = ROOT / 'candidate'
EVIDENCE = ROOT / 'pencils-down-evidence'
PRIVATE = Path(os.environ.get('RUNNER_TEMP', '/tmp')) / 'pawspace-golden-8745bbca'
...
MAPS = ['GOOGLE_MAPS_SERVER_API_KEY_UAT']
...
SAFE_VARS = {
    ...
    'PAWSPACE_LOCAL_PREVIEW':'off', 'PAWSPACE_SCHEDULING_ENV':'uat', 'PAWSPACE_MAPS_ENV':'sandbox',
...
def configured() -> tuple[str,str,dict]:
    ...
    names = ['CLOUDFLARE_API_TOKEN','CLOUDFLARE_ACCOUNT_ID',*UAT,*MAPS]
    ...
    values = {name:os.environ[name].strip() for name in UAT+MAPS}
    for name,value in {name:values[name] for name in UAT}.items():
        require(len(value)>=32 and not re.match(r'^pawspace[-_]',value,re.I), name+' fails the existing UAT credential policy.')
    require(len(values['GOOGLE_MAPS_SERVER_API_KEY_UAT'])>=20, 'Maps UAT credential is unavailable.')
...
def active_version(database_id:str) -> str:
    ...
    allowed_names=set(SAFE_VARS)|set(UAT)|set(RAZOR)|set(MAPS)|{'DB','ASSETS','IMAGES'}
