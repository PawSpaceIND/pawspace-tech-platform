...
for (let index = 0; index < CUSTOMER_COUNT; index += 1) {
  ...
  const dayIndex = Math.floor(index / 40);
  ...
  pets.set(petId, { id: petId, customerId, legacyIds: [], name: `QA Pet ${index + 1}`, species: "dog", breed: "QA", allergies: [], vaccinationStatus: "verified", createdAt: start, updatedAt: start });

  const decision = await schedule(repository, { cityId: CITY, zoneId: ZONE, serviceCode: "grooming", petIds: [petId], scheduledStart: start, scheduledEnd: end, preferredProviderId: expectedProvider.id });
  assert.equal(decision.provider?.id, expectedProvider.id);
  assert.equal(decision.occurrences.length, 1);

  const booking = { id: `QA-BK-${String(index + 1).padStart(4, "0")}`, legacyIds: [], idempotencyKey: `qa-booking-${index + 1}`, cityId: CITY, zoneId: ZONE, customerId, petIds: [petId], serviceCode: "grooming", packageCode: "qa-grooming", addonCodes: [], scheduledStart: start, scheduledEnd: end, status: "confirmed", channel: "qa_certification", totalAmount: BOOKING_AMOUNT, providerId: decision.provider.id, assignmentMode: decision.mode, scheduleGroupId: `QA-GRP-${index + 1}`, occurrenceNumber: 1, capacityUnits: 1, createdBy: customerId, createdAt: start, updatedAt: start };
  bookings.push(booking);

  const created = await createPayment(repository, { booking, customerId, method: "upi", mode: "prepaid", idempotencyKey: `qa-payment-${index + 1}`, gatewayOrderId: `qa-order-${index + 1}` });
  assert.equal(created.duplicatePrevented, false);
  const replay = await createPayment(repository, { booking, customerId, method: "upi", mode: "prepaid", idempotencyKey: `qa-payment-${index + 1}`, gatewayOrderId: `qa-order-${index + 1}` });
  ...
  if (index % 10 === 0) await requestRefund(repository, captured, { id: `QA-FIN-MAKER-${index % 2}`, role: "finance" }, REFUND_AMOUNT, "QA partial refund path");

  const completedAt = new Date(new Date(end).getTime() + 15 * 60 * 1000).toISOString();
  await createProviderEarning(repository, booking, { baseEarning: 400, incentive: index % 20 === 0 ? 50 : 0, deductions: index % 30 === 0 ? 10 : 0, completedAt });
  journeyMs.push(performance.now() - started);
}

const first = bookings[0];
const collision = await schedule(repository, { cityId: CITY, zoneId: ZONE, serviceCode: "grooming", petIds: [first.petIds[0]], scheduledStart: first.scheduledStart, scheduledEnd: first.scheduledEnd, preferredProviderId: first.providerId });
assert.equal(collision.provider, null);
assert.equal(collision.mode, "manual_review");

const refundedPayment = payments.find(p => refunds.some(r => r.paymentId === p.id));
await assert.rejects(() => requestRefund(repository, refundedPayment, { id: "QA-FIN-MAKER-OVER", role: "finance" }, BOOKING_AMOUNT - REFUND_AMOUNT + 0.01, "QA refund ceiling probe"), /Refund exceeds refundable balance/);

for (const provider of providers) {
  const key = `qa-payout-${provider.id}`;
  const firstPayout = await createPayout(repository, { providerId: provider.id, cityId: CITY, idempotencyKey: key, asOf: "2026-12-01T00:00:00.000Z" });
  assert.equal(firstPayout.duplicatePrevented, false);
  const replayPayout = await createPayout(repository, { providerId: provider.id, cityId: CITY, idempotencyKey: key, asOf: "2026-12-01T00:00:00.000Z" });
  assert.equal(replayPayout.duplicatePrevented, true);
}
...
const summary = { customers: 520, providers: 10, bookings: bookings.length, providerDistribution, payments: payments.length, invoices: invoices.length, uniqueInvoiceNumbers: new Set(invoiceNumbers).size, invoiceNumberDuplicates, refunds: refunds.length, earnings: earnings.length, payouts: payouts.length, gross, refunded, earningTotal, payoutTotal, journeyP95Ms: Number(p95.toFixed(3)), collisionBlocked: collision.provider === null };
...
assert.equal(new Set(invoiceNumbers).size, 520);
...
assert.equal(payoutTotal, earningTotal);
assert.equal(invoiceNumberDuplicates, 0);
