...
test("the handoff screen opens on a live escalation instead of an arbitrary thread", () => {
  ...
  assert.match(page, /const escalated = allRows\.find/, "it must prefer a thread that actually has a handoff");
  assert.match(page, /\|\| escalated \|\| allRows\[0\]/, "and select it ahead of the first arbitrary thread");
