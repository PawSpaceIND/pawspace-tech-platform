...
const workersUrl=`data:text/javascript,${encodeURIComponent("export const env=new Proxy({}, {get:(_,key)=>globalThis.__PAWSPACE_TEST_ENV?.[key]});")}`;
if(typeof nodeModule.registerHooks==="function")nodeModule.registerHooks({resolve(specifier,context,nextResolve){if(specifier==="cloudflare:workers")return{url:workersUrl,shortCircuit:true};try{return nextResolve(specifier,context);}catch(error){if(specifier.startsWith(".")&&!specifier.endsWith(".ts"))return nextResolve(`${specifier}.ts`,context);throw error;}}});
else nodeModule.register(new URL(`data:text/javascript,${encodeURIComponent(`const workersUrl=${JSON.stringify(workersUrl)}; export async function resolve(specifier,context,nextResolve){if(specifier==="cloudflare:workers")return{url:workersUrl,shortCircuit:true};try{return await nextResolve(specifier,context);}catch(error){if(specifier.startsWith(".")&&!specifier.endsWith(".ts"))return nextResolve(specifier+".ts",context);throw error;}}`)}`));
...
test("legacy plaintext OTP rows are invalidated and scrubbed during in-place schema upgrade",async()=>{
 ...
 globalThis.__PAWSPACE_TEST_ENV={DB:db,PAWSPACE_IDENTITY_ASSERTION_SECRET_UAT:"legacy-upgrade-secret-0123456789abcdef0123456789abcdef",PAWSPACE_IDENTITY_ENV:"sandbox",PAWSPACE_PAYMENT_ENV:"sandbox"};
