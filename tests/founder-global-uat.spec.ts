...
test("founder global UAT: customer booking -> partner assignment -> SLA -> commission calculation",async()=>{
 ...
 try{
  ...
  const journey=await runCompletedJourney(ctx,{customerId:"CUST-FOUNDER-GLOBAL",customerName:"Founder Global UAT",phone:"+919900009911",petSourceId:"PET-FOUNDER-GLOBAL",petName:"Milo",cityId:"blr",zoneId:"blr-east",pincode:"560038",latitude:12.9716,longitude:77.5946,preferredProviderId:"groom_arun",groupId:`FOUNDER-GLOBAL-${Date.now()}`,start:futureSlot()});
  ...
  const offer=ctx.sqlite.prepare("SELECT offered_at,expires_at,responded_at,status FROM provider_assignment_offers WHERE group_id=?").get((journey as any).booked.body.data.scheduleGroupId??(journey as any).bookingPayload?.scheduleGroupId) as Record<string,unknown>|undefined;
