...
test('management links retain the exact booking identifier and only target supported routes',()=>{for(const [serviceCode,path] of [['pet_sitting','sitting'],['dog_walking','walking'],['pet_taxi','taxi']])assert.equal(customerBookingManageHref({id:'B & 1',serviceCode,status:'assigned',scheduledStart:''}),`/${path}/manage?bookingId=B%20%26%201`);assert.equal(customerBookingManageHref({id:'B',serviceCode:'pet_food',status:'assigned',scheduledStart:''}),null);});
