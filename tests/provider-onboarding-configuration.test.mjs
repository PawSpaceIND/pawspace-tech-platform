...
test("PO6 keeps package/process requirements versioned by vertical and jurisdiction",()=>{for(const token of["provider_onboarding_policy_versions","vertical_key","country_code","region_code","city_code","process_steps_json","package_config_json","verification_rules_json","verification_adapters_json","quiz_policy_json","interview_policy_json","media_requirements_json","sla_template_ref","activation_requirements_json","pricing_policy_ref","immutable_hash"])assert.match(lib,new RegExp(token));assert.match(lib,/status='draft'/);assert.match(lib,/status='review'/);assert.match(lib,/status='approved'/);assert.match(lib,/status='active'/);assert.match(lib,/rollback/);});
