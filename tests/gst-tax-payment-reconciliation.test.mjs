...
const PERIOD = "2026-08", COMPLETED = Date.parse("2026-08-14T12:00:00+05:30");
...
async function paymentWorld() {
  ...
  sqlite.prepare("INSERT INTO canonical_bookings VALUES ('BK-G','CUS-G','blr','blr-east','grooming','pkg','Package','PRV-G','2026-08-14T05:00:00.000Z','2026-08-14T06:00:00.000Z','completed',1000,'INR',1,1)").run();
...
test("recording the GST and TCS paid debits 2130 and 2140 against Bank with the challan, audited, and never for more than is owed", async () => {
  ...
  const noReason = await attempt(() => payments.recordTaxPayment(db, { taxKind: "gst", periodCode: PERIOD, amount: 54, challanReference: "CPIN-26100001", paidOn: "2026-09-18", reason: "" }, FINANCE));
  assert.equal(noReason.status, 400, "a reason is required");
  const tooMuch = await attempt(() => payments.recordTaxPayment(db, { taxKind: "gst", periodCode: PERIOD, amount: 60, challanReference: "CPIN-26100001", paidOn: "2026-09-18", reason: "August GSTR-3B paid" }, FINANCE));
  assert.equal(tooMuch.status, 409);
  assert.match(tooMuch.body, /GST payable up to 2026-08 is 54; a payment of 60 is more than the books owe/);

  const gst = await payments.recordTaxPayment(db, { taxKind: "gst", periodCode: PERIOD, amount: 54, challanReference: "CPIN-26100001", paidOn: "2026-09-18", reason: "August GSTR-3B paid", returnReference: "ARN-AA2909260001" }, FINANCE);
  ...
  assert.deepEqual(lines.map((l) => [l.account_code, l.debit, l.credit, l.period_code, l.source_type]), [["2130-GST Payable", 54, 0, "2026-09", "gst_payment"], ["1010-Bank", 0, 54, "2026-09", "gst_payment"]]);
  ...
  assert.deepEqual([audit.action, audit.actor_id, audit.reason], ["gst_paid", FINANCE, "August GSTR-3B paid"]);
  const again = await payments.recordTaxPayment(db, { taxKind: "gst", periodCode: PERIOD, amount: 54, challanReference: "CPIN-26100001", paidOn: "2026-09-18", reason: "August GSTR-3B paid" }, FINANCE);
  assert.equal(again.duplicatePrevented, true, "the same challan is recorded once");
  assert.equal(balance(sqlite, "2130-GST Payable"), 0);

  const wrongTcs = await attempt(() => payments.recordTaxPayment(db, { taxKind: "tcs", periodCode: PERIOD, amount: 4, challanReference: "CPIN-TCS-0001", paidOn: "2026-09-09", reason: "August GSTR-8 TCS paid" }, FINANCE));
  assert.equal(wrongTcs.status, 409, "a TCS deposit must equal the TCS computed for GSTR-8");
  const tcs = await payments.recordTaxPayment(db, { taxKind: "tcs", periodCode: PERIOD, amount: 5, challanReference: "CPIN-TCS-0001", paidOn: "2026-09-09", reason: "August GSTR-8 TCS paid" }, FINANCE);
  ...
  const august = await payments.taxPayableReconciliation(db, { periodCode: PERIOD });
  assert.deepEqual([august.gst.paymentsRecordedForPeriod, august.gst.unpaidForPeriod, august.tcs.paymentsRecordedForPeriod], [54, 0, 5]);
  const september = await payments.taxPayableReconciliation(db, { periodCode: "2026-09" });
  assert.deepEqual([september.gst.opening, september.gst.paid, september.gst.closing, september.tcs.opening, september.tcs.paid, september.tcs.closing], [54, 54, 0, 5, 5, 0]);
});
...
test("the route records a tax payment only for finance.manage, and serves the reconciliation view", async () => {
  ...
  const body = { action: "record_tax_payment", taxKind: "gst", periodCode: PERIOD, amount: 54, challanReference: "CPIN-ROUTE-01", paidOn: "2026-09-18", reason: "August GSTR-3B paid" };
