...
test("checkout identity requires the exact merged certified head and same repository",()=>{
 const pr={state:"closed",merged:true,head:{sha,ref:CHECKOUT_BRANCH,repo:{full_name:CHECKOUT_REPOSITORY}}};
...
test("retired provisioning workflow contains no hosted-browser mutation path; browser verifier remains independently hardened",()=>{
 ...
 assert.match(browser,/expect\.poll\(\(\)=>images\.evaluateAll\(nodes=>nodes\.some\(n=>n\.complete&&n\.naturalWidth>0\)\),\{timeout:20_000\}\)\.toBe\(true\)/,"hosted artwork readiness must accept any loaded responsive image in the service card and keep the verifier’s bounded 20s timeout");
