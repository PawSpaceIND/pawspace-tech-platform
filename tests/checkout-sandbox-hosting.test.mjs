...
test("actual provisioning runner tolerates bounded post-deploy propagation before certifying the root document",()=>{
 const f=runnerFixture("propagation");try{
  assert.equal(f.result.status,0,f.result.stderr+f.result.stdout);
  assert.equal(f.requests.split("GET https://pawspace-checkout-674-123456789-1.fixture.workers.dev/mobile-app").length-1,2);
  const report=JSON.parse(readFileSync(resolve(f.dir,"evidence/hosting-report.json"),"utf8"));assert.equal(report.checks.rootDocumentReachable,true);
 }finally{rmSync(f.dir,{recursive:true,force:true});}
});
...
test("hosted UI workflow uses actual UAT authentication and keeps provider secrets out of the browser step",()=>{
 ...
 const deploy=readFileSync(new URL("../scripts/deploy-checkout-sandbox.mjs",import.meta.url),"utf8");
 ...
 assert.match(browser,/context.request.get\(origin\+"\/api\/address-autocomplete/);
 assert.ok(browser.indexOf('/api/staging-login')<browser.indexOf('/api/address-autocomplete'));
 assert.doesNotMatch(deploy,/api\/address-autocomplete/);
