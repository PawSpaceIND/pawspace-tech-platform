...
test("partner onboarding page uses canonical self service without fake activation",()=>{assert.match(page,/\/api\/identity-session/);assert.match(page,/\/api\/provider-onboarding-self-service/);assert.match(page,/20-question qualification/);assert.match(page,/15-minute Ops interview/);assert.match(page,/PRODUCTION READY = FALSE/);assert.match(page,/Marketplace live: <b>No<\/b>/);assert.match(page,/Provider self-service cannot activate itself/);assert.doesNotMatch(page,/100%|Ready to take bookings|Partner activated|Digitally signed|Police verification.*Approved/);});
