...
test("dashboard-managed staging values are restored then preserved without raw Wrangler output", () => {
  ...
  assert.match(isolated, /raw binding values withheld/);
  assert.match(isolated, /configuredForExternalTest === true/);
});

test("prior contaminated workflow logs are deleted before another deploy", () => {
  assert.match(isolated, /permissions:[\s\S]*actions:\s*write/);
  assert.match(isolated, /actions\/runs\/33747290999\/logs/);
  assert.match(isolated, /--request DELETE/);
  assert.match(isolated, /if \[ "\$status" != "204" \]/);
});
