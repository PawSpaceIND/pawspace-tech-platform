...
test("Grooming fails closed until a governed address is resolved and saved for routing", async () => {
  ...
  const theirs = await post({
    ...
    latitude: 12.9752, longitude: 77.6050,
...
test("Grooming refuses a service address in a city that is not open for fulfilment", async () => {
  ...
  const open = await route.POST(new Request(`${OPS_ORIGIN}/api/grooming-service-location`, {
    ...
    body: JSON.stringify({ bookingId: booking.bookingId, customerId: CUSTOMER, address: "42 Church Street, Bengaluru 560001", pincode: "560001", latitude: 12.9752, longitude: 77.6050 }),
  }));
...
test("Grooming carries the governed coupon quote through to a single redemption", async () => {
  ...
  if (large.valid && campaign.maxDiscount) assert.ok(large.discount <= campaign.maxDiscount);
  ...
  seedCanonicalStayBooking(sqlite, { bookingId: "BKG-COUPON-1", customerId: CUSTOMER, serviceCode: "grooming", cityId: "blr", zoneId: "blr-central" });
  seedCanonicalStayBooking(sqlite, { bookingId: "BKG-COUPON-OTHER", customerId: "CUST-SOMEBODY-ELSE", serviceCode: "grooming", cityId: "blr", zoneId: "blr-central", groupId: "GRP-COUPON-OTHER", reservationId: "RES-COUPON-OTHER" });
  const otherCustomer = await coupons.consumeCouponQuote(db, { quoteId: quote.quoteId, customerId: "CUST-SOMEBODY-ELSE", bookingId: "BKG-COUPON-OTHER", orderValue: base.orderValue, idempotencyKey: "idem-other" }).then(() => null, (error) => error);
  assert.match(String(otherCustomer?.message ?? otherCustomer), /customer mismatch/i);
  const otherAmount = await coupons.consumeCouponQuote(db, { quoteId: quote.quoteId, customerId: CUSTOMER, bookingId: "BKG-COUPON-1", orderValue: base.orderValue + 1000, idempotencyKey: "idem-amount" }).then(() => null, (error) => error);
  assert.match(String(otherAmount?.message ?? otherAmount), /does not match/i);
  assert.equal((await db.prepare("SELECT status FROM coupon_quotes WHERE id=?").bind(quote.quoteId).first()).status, "open");
  await db.prepare("UPDATE coupon_quotes SET expires_at=? WHERE id=?").bind(Date.now() - 1000, quote.quoteId).run();
  const expired = await coupons.consumeCouponQuote(db, { quoteId: quote.quoteId, customerId: CUSTOMER, bookingId: "BKG-COUPON-1", orderValue: base.orderValue, idempotencyKey: "idem-expired" }).then(() => null, (error) => error);
  assert.match(String(expired?.message ?? expired), /expired/i);
  assert.equal(Number((await db.prepare("SELECT COUNT(*) AS n FROM coupon_redemptions").first()).n), 0);
});
...
test("the embedded Sitting screen offers no coupon field, because the governed quote refuses one", async () => {
  ...
  assert.doesNotMatch(rendered, /promo code|coupon/i);
  assert.doesNotMatch(rendered, /Partner accepted . live calendar verified/i);
  assert.doesNotMatch(rendered, /\bpayment (?:captured|successful)\b/i);
});
...
test("the governed Sitting quote is the only source of a Sitting price", async () => {
  ...
  assert.equal(one.billableUnits, 1);
  assert.equal(one.basePricePerPet, 399);
  assert.equal(one.totalAmount, 399);
  assert.equal(one.amountDueNow, 399);
  const three = await quoteFor({ petCount: 3 });
  assert.equal(three.totalAmount, 399 + 2 * 149);
  assert.notEqual(three.quoteId, one.quoteId);
  const tooMany = await refusal(quoteFor({ petCount: 5 }));
  assert.equal(tooMany.status, 409);
  assert.match(tooMany.message, /Sitting supports 1-4 pets per booking/);
  const past = await refusal(quoteFor({ scheduledStart: new Date(Date.now() - 3_600_000).toISOString(), scheduledEnd: new Date(Date.now() + 3_600_000).toISOString() }));
  ...
  const twoNights = await quoteFor({ packageCode: "sitting-overnight", scheduledEnd: new Date(Date.now() + 86_400_000 + 26 * 3_600_000).toISOString() });
  assert.equal(twoNights.billableUnits, 2);
...
test("a Sitting booking is governed against its server quote and consumes it exactly once", async () => {
  ...
  const submit = (overrides = {}) => sitting.governSittingBooking(db, {
    ...
    petCount: quote.petCount, cityId: "blr", zoneId: "blr-east", scheduledStart: start, scheduledEnd: end,
  ...
  assert.equal(String((await db.prepare("SELECT status FROM sitting_commercial_quotes WHERE id=?").bind(quote.quoteId).first()).status), "open");
  ...
  assert.equal(governed.catalogueVersion, "sitting-v1");
