...
test("real execution: a cold GET stays read-only and does not initialize work-queue tables", async () => {
  freshDb();
  const before = sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all();
  const result = await call("GET");
  assert.equal(result.status, 200, JSON.stringify(result.body));
  assert.equal(result.body.data.metrics.total, 0);
  assert.deepEqual(sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all(), before);
});
...
test("real execution: explicit POST sweep then GET returns queues and the command-centre TODAY block with exact numbers", async () => {
  freshDb(); seedSourceTables(); seedOneOfEachCondition();
  ...
  const sweep = await call("POST", { action: "sweep" });
  assert.equal(sweep.status, 200, JSON.stringify(sweep.body));
  const res = await call("GET");
  assert.equal(res.status, 200, JSON.stringify(res.body));
  const snapshot = res.body.data;
  assert.equal(snapshot.metrics.open, 7, "the explicit write sweep populates the queue before the read");
...
test("contract: gateway permission line, DB access rule, and the team surface exist", () => {
  ...
  assert.match(gateway, /ops-work-queue"\)return method==="GET"\?"bookings\.view":"bookings\.manage"/);
...
// Repeated sweeps and CONCURRENT sweeps are different guarantees. The scheduled worker can overlap
// with an explicit staff sweep, so two sweeps genuinely can run at once in production.
