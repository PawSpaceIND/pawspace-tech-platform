...
test("missing Origin is accepted here because that is the repository-wide convention, not a local lapse", async () => {
  ...
  const helpers = ROUTES.map((route) => {
    ...
    return match[0].replace(/"[^"]*"/g, '"MSG"');
  });
  assert.equal(new Set(helpers).size, 1,
    "all six copies must stay semantically identical — divergence between them is the real hazard here");
  assert.match(helpers[0], /if\(origin&&origin!==new URL\(request\.url\)\.origin\)/,
