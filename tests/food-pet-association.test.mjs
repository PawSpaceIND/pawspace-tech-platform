...
class FoodD1{
 constructor(){this.catalogue=new Map();this.inventory=new Map();this.pets=new Map();this.quotes=new Map();this.quotePets=[];this.orders=new Map();this.orderPets=[];this.failOrderInsert=false}
 ...
 async run(sql,args){
  ...
  if(sql.includes("INSERT INTO food_quote_pets")){const[quote_id,pet_id,customer_id,pet_type,created_at]=args;this.quotePets.push({quote_id,pet_id,customer_id,pet_type,created_at});return{meta:{changes:1}}}
  ...
  if(sql.includes("UPDATE food_inventory_uat SET reserved_units=reserved_units+")){const[quantity,updated_at,sku,zoneId]=args,row=this.inventory.get(`${sku}|${zoneId}`);row.reserved_units=Number(row.reserved_units)+Number(quantity);row.updated_at=updated_at;return{meta:{changes:1}}}
