...
test("V2 bridge pages reuse the existing canonical service implementations",()=>{for(const route of routes){const src=fs.readFileSync(new URL("../app/v2/"+route+"/page.tsx",import.meta.url),"utf8");assert.match(src,/routeScope="v2"/);if(route==="taxi"){assert.match(src,/V2BoardingTaxiExperience/);const wrapper=fs.readFileSync(new URL("../app/v2/taxi/boarding-taxi-experience.tsx",import.meta.url),"utf8");assert.match(wrapper,/from "\.\.\/\.\.\/mobile-app\/taxi-flow"/);assert.match(wrapper,/<TaxiFlow routeScope=\{routeScope\}/);}else assert.match(src,/from "\.\.\/\.\.\//);assert.doesNotMatch(src,/fetch\(|api\//);}});
