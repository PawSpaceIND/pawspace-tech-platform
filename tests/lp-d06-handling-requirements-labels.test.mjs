 ...
 * showing the raw code. This extracts the REAL function bodies from app/partner-app/page.tsx (not a
...
const source = readFileSync(new URL("../app/partner-app/page.tsx", import.meta.url), "utf8");

function extractArrow(name) {
  const marker = `const ${name} = (`;
  const start = source.indexOf(marker);
  assert.ok(start >= 0, `${name} must be defined in app/partner-app/page.tsx`);
...
const categoryLabels = extractObjectLiteral("REQUIREMENT_CATEGORY_LABELS");
...
const requirementLabelFn = (() => {
  ...
  return new Function("value", "label", "REQUIREMENT_CATEGORY_LABELS", body);
})();
...
test("LP-D06 source contract: the Handling requirements list renders through requirementLabel, not the bare label()", () => {
  assert.match(source, /Handling requirements[\s\S]{0,40}<ul>\{selected\.safetyRequirements\.map\(item => <li key=\{item\}>\{requirementLabel\(item\)\}<\/li>\)\}/);
});
