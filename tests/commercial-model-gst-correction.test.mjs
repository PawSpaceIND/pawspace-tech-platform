...
async function payoutWorld() {
  ...
  sqlite.exec(`
    CREATE TABLE IF NOT EXISTS canonical_bookings (id TEXT PRIMARY KEY,service_code TEXT,provider_id TEXT,total_amount REAL,scheduled_start TEXT,status TEXT);
...
test("commission_standard splits a GST-inclusive order true-inclusive: ₹1000 @70% -> provider 593.22, fee 254.24, GST 45.76", async () => {
  ...
  assert.equal(p.providerGstDeducted, 152.54);
  assert.equal(p.providerGrossShare, 593.22);
  assert.equal(p.providerNetPayout, 593.22);
  assert.equal(p.platformFee, 254.24);
  assert.equal(p.platformGst, 45.76);
...
test("commission_groomer keeps the full-order split unchanged (no carve; GST on the fee only)", async () => {
  ...
  assert.equal(p.providerGstDeducted, 0);
  assert.equal(p.providerNetPayout, 700);
  assert.equal(p.platformFee, 300);
  assert.equal(p.platformGst, 54);
...
test("funeral_exempt is GST-exempt and pays the vendor a share of PawSpace's OWN standard price", async () => {
  ...
  assert.equal(g.providerNetPayout, 4400);
...
test("direct_employee carves the embedded GST from the inclusive order (18/118), not 18% on top", async () => {
  ...
  assert.equal(d.pawspaceGstOnOrder, 180);
  assert.equal(d.platformFee, 1000);
  assert.equal(d.platformGst, 0);
  assert.equal(d.providerNetPayout, 0);
...
test("statutory package + monthly close count only PawSpace's commission GST as own output for a costed marketplace booking", async () => {
  ...
  assert.equal(pkg.summary.serviceOutputTax, 45.76);
  assert.equal(pkg.summary.outputTax, 45.76);
...
test("GSTR-3B counts only PawSpace's commission GST as output; the provider-supply GST is disclosed for GSTR-8", async () => {
  ...
  assert.equal(r.summary.serviceVerticalTax, 45.76);
  assert.equal(r.summary.taxCollectedFromCustomers, 152.54);
  assert.equal(r.summary.providerSupplyGstCollectedOnBehalf, 106.78);
  assert.equal(r.summary.totalOutputTax, 45.76);
  assert.equal(r.payload.sup_details.osup_det.txval, 254.24);
