...
test("sales closing tools are registered as explicit-confirmation canonical mutations",async()=>{const{db}=await world();for(const toolCode of["schedule.reserve","checkout.payment_order.create"]){const prepared=await registry.prepareAiToolExecution(db,{actor:staffActor,toolCode,threadId:"THREAD-1",customerId:"CUS-1",intent:"booking_create",channel:"chat",arguments:{bookingId:"BKG-1",dispatchItemId:"AISITEM-1",discountBps:0},idempotencyKey:`closing-${toolCode}`});assert.equal(prepared.status,"confirmation_required");assert.equal(prepared.executed,false);assert.equal(prepared.autonomousExecution,false);}});

test("schedule.reserve is rejected by the real margin gate before the scheduler is invoked",async()=>{const{sqlite,db}=await world(),goals=await import("../lib/ai-sales-goal-orchestrator.ts"),finance=await import("../lib/finance-margin-validator.ts"),commercial=await import("../lib/provider-commercial-terms.ts"),now=Date.now();sqlite.exec("ALTER TABLE canonical_bookings ADD COLUMN city_id TEXT NOT NULL DEFAULT 'blr'");sqlite.prepare("UPDATE canonical_bookings SET provider_id='PROV-1',scheduled_start='2026-09-11T16:00:00Z' WHERE id='BKG-1'").run();await goals.ensureAiSalesGoalTables(db);await finance.ensureMarginPolicyTables(db);await commercial.ensureCommercialTermsTables(db);sqlite.prepare("INSERT INTO provider_commercial_terms (id,service_code,provider_id,version,status,engagement_model,provider_share_pct,gst_mode,platform_gst_rate,cash_allowed,onboarding_fee,renewal_fee,renewal_months,effective_from,reason,created_by,approved_by,approval_reference,created_at,updated_at) VALUES ('TERM-1','grooming',NULL,1,'active','commission_groomer',0.70,'none',0.18,1,0,0,12,'2026-01-01','approved baseline','ops','finance','APR-1',?,?)").run(now,now);sqlite.prepare("INSERT INTO finance_ai_margin_policies VALUES ('MP-1','grooming','blr',1,'active',1000,15000,200,0,?,NULL,'finance',?,?,?)").run(now-1000,now,now,now);sqlite.prepare("INSERT INTO ai_sales_targets (id,target_date,target_type,service_code,city_id,daily_goal,status,max_discount_bps,minimum_margin_bps,free_upgrade_codes_json,authorized_channels_json,max_contacts_per_day,offer_policy_version,prompt_policy_version,approved_by,approved_at,starts_at,ends_at,created_by,created_at,updated_at) VALUES ('TARGET-1','2026-09-11','booking_conversion','grooming','blr',5,'active',1500,1000,'[]','[\"whatsapp\"]',20,'offer-v1',?,'ops',?,?,?,?,?,?)").run(goals.AI_SALES_PROMPT_POLICY_VERSION,now,now-1000,now+60_000,'ops',now,now);sqlite.prepare("INSERT INTO ai_sales_dispatch_runs (id,run_key,target_id,scheduled_for,gap_before,historical_conversion_rate,required_contacts,status,started_at) VALUES ('RUN-1','run-1','TARGET-1',?,5,0.2,20,'completed',?)").run(now,now);sqlite.prepare("INSERT INTO ai_sales_dispatch_items (id,run_id,target_id,lead_id,customer_id,channel,probability,quota_gap_snapshot,pressure_level,offer_envelope_json,status,created_at,updated_at) VALUES ('ITEM-1','RUN-1','TARGET-1','LEAD-1','CUS-1','whatsapp',0.8,5,'urgent',?,'queued',?,?)").run(JSON.stringify({targetId:'TARGET-1',policyVersion:'offer-v1',maxDiscountBps:1500,minimumMarginBps:1000,freeUpgradeCodes:[],expiresAt:now+60_000}),now,now);const prepared=await registry.prepareAiToolExecution(db,{actor:staffActor,toolCode:'schedule.reserve',threadId:'THREAD-1',customerId:'CUS-1',intent:'booking_create',channel:'whatsapp',arguments:{bookingId:'BKG-1',dispatchItemId:'ITEM-1',discountBps:1500,schedule:{petIds:['PET-CUS-1'],scheduledStart:'2026-09-11T16:00:00Z',scheduledEnd:'2026-09-11T17:00:00Z'}},idempotencyKey:'margin-blocked-reserve'});await assert.rejects(registry.confirmAiToolExecution(db,{actor:staffActor,requestId:prepared.requestId,canonicalRequest:new Request('https://pawspace.test/api/ai-tools',{method:'POST'})}),new RegExp(finance.MINIMUM_MARGIN_REJECTION.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));assert.equal(sqlite.prepare("SELECT decision FROM finance_ai_margin_decisions").get().decision,'blocked');assert.equal(sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='scheduling_reservations'").get(),undefined);});
