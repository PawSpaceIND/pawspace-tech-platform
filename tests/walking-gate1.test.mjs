...
test("Walking scheduling uses shared canonical provider and reservation tables",async()=>{const source=await read("lib/walking-scheduling.ts"),api=await read("app/api/walking-scheduling/route.ts"),client=await read("lib/walking-scheduling-client.ts");assert.match(source,/provider_capacity_profiles/);assert.match(source,/scheduling_assignment_decisions/);assert.match(source,/scheduling_reservations/);assert.match(source,/dog_walking/);assert.match(source,/No governed Walking provider is available/);assert.match(source,/walking_uat_roster/);assert.match(api,/requireCustomerOwnership/);assert.match(api,/securityAudit/);assert.match(client,/\/api\/walking-scheduling/);});

test("Walking canonical booking remains the last Gate 1 boundary",async()=>{const page=await read("app/walking/page.tsx");assert.match(page,/Scheduler \+ canonical booking wiring next/);await assert.rejects(()=>read("app/api/walking-bookings/route.ts"));});
