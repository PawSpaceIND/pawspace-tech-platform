...
test("booking command center streams canonical booking status changes over authenticated SSE",async()=>{const [route,page]=await Promise.all([read("app/api/booking-command-center/route.ts"),read("app/booking-command-center/page.tsx")]);assert.match(route,/text\/event-stream/);assert.match(route,/bookingStatusFingerprint/);assert.match(route,/SELECT id,status,updated_at FROM canonical_bookings/);assert.match(route,/event: \${event}/);assert.match(page,/new EventSource\("\/api\/booking-command-center\?stream=1"\)/);assert.match(page,/canonical booking status updates automatically/);assert.match(page,/Refresh now/);});
