...
test("booking command center is explicitly a refresh snapshot rather than a false live feed",async()=>{const [route,page]=await Promise.all([read("app/api/booking-command-center/route.ts"),read("app/booking-command-center/page.tsx")]);assert.match(route,/canonical UAT database snapshot/);assert.match(page,/Snapshot from shared UAT database/);assert.match(page,/Refresh snapshot/);assert.doesNotMatch(page,/Live from shared UAT database/);});
