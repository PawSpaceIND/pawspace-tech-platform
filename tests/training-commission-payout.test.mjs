...
test("50% sessions unlock only 50% commission after five days",async()=>{const w=await world();complete(w.sqlite,5,BASE);await w.mod.syncTrainingCommissionPayoutMilestones(w.db,BASE+2*DAY);let row=w.sqlite.prepare("SELECT * FROM training_commission_payout_milestones WHERE milestone_code='first_50_percent'").get();assert.equal(Number(row.payout_amount),3500);assert.equal(row.status,"waiting_payout_hold");await assert.rejects(()=>w.mod.approveTrainingCommissionMilestone(w.db,{bookingId:"BK",milestoneCode:"first_50_percent",idempotencyKey:"M1",actorId:"finance",reason:"approved fixture",asOf:BASE+2*DAY}),e=>e instanceof Response&&e.status===409);await w.mod.syncTrainingCommissionPayoutMilestones(w.db,BASE+8*DAY);row=w.sqlite.prepare("SELECT * FROM training_commission_payout_milestones WHERE milestone_code='first_50_percent'").get();assert.equal(row.status,"ready_for_finance_approval");const approved=await w.mod.approveTrainingCommissionMilestone(w.db,{bookingId:"BK",milestoneCode:"first_50_percent",idempotencyKey:"M1",actorId:"finance",reason:"approved fixture",asOf:BASE+8*DAY});assert.equal(approved.amount,3500);assert.equal(approved.status,"instruction_ready_sandbox");});

test("final 50% is separate and waits five days after package completion",async()=>{const w=await world();complete(w.sqlite,5,BASE);await w.mod.syncTrainingCommissionPayoutMilestones(w.db,BASE+8*DAY);await w.mod.approveTrainingCommissionMilestone(w.db,{bookingId:"BK",milestoneCode:"first_50_percent",idempotencyKey:"M1",actorId:"finance",reason:"approved fixture",asOf:BASE+8*DAY});complete(w.sqlite,10,BASE+10*DAY);await w.mod.syncTrainingCommissionPayoutMilestones(w.db,BASE+12*DAY);let rows=w.sqlite.prepare("SELECT milestone_code,payout_amount,status FROM training_commission_payout_milestones ORDER BY milestone_code").all();assert.equal(rows.length,2);assert.equal(rows.find(r=>r.milestone_code==='first_50_percent').status,"instruction_ready_sandbox");assert.equal(Number(rows.find(r=>r.milestone_code==='final_50_percent').payout_amount),3500);assert.equal(rows.find(r=>r.milestone_code==='final_50_percent').status,"waiting_payout_hold");await w.mod.syncTrainingCommissionPayoutMilestones(w.db,BASE+18*DAY);assert.equal(w.sqlite.prepare("SELECT status FROM training_commission_payout_milestones WHERE milestone_code='final_50_percent'").get().status,"ready_for_finance_approval");});
