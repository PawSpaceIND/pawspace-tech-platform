  ...
  const r = await providerPreflight(env, async (url, init) => { calls.push({ url, init });
    return Response.json(calls.length === 1 ? { status: "completed", output: [{ content: [{ type: "output_text", text: "OK" }] }] } : {
      agent_id: "agent_test", phone_numbers: [], conversation_config: { agent: { prompt: { custom_llm: {
        url: "https://pawspace-staging.karthik-fce.workers.dev/api/elevenlabs/v1", api_type: "responses", api_key: { secret_id: "test-secret-reference" } } } } } });
  });
...
test("agent identity alone does not validate a duplicated Responses path", async () => {
  let calls = 0;
  const result = await providerPreflight(env, async () => Response.json(++calls === 1
    ? { status: "completed", output_text: "OK" }
    : { agent_id: "agent_test", conversation_config: { agent: { prompt: { custom_llm: {
      url: "https://pawspace-staging.karthik-fce.workers.dev/api/elevenlabs/v1/responses",
      api_type: "responses", api_key: { secret_id: "synthetic-reference" }
    } } } } }));
  assert.equal(result.elevenlabs.verified, true);
  assert.equal(result.elevenlabs.customLlmEndpointMatches, false);
  assert.equal(result.configurationVerified, false);
  assert.equal(providerPreflightPassed(result), false);
});
