...
test("scheduling rule reads require view, writes require manage, and denied operations preserve D1",{timeout:120000},async()=>{
 await rm(persistDir,{recursive:true,force:true,maxRetries:5,retryDelay:100});const port=await freePort();let logs="";
 const child=spawn(process.platform==="win32"?"npx.cmd":"npx",["wrangler","dev","--config","wrangler.scheduling-rules-authorization.jsonc","--persist-to",persistDir,"--port",String(port),"--inspector-port","0"],{stdio:["ignore","pipe","pipe"],detached:process.platform!=="win32",env:{...process.env,XDG_CONFIG_HOME:`${process.cwd()}/${persistDir}/xdg`,WRANGLER_SEND_METRICS:"false"}});
