...
function applyOwnedDdl(sqlite, path) {
  ...
  for (const match of source.matchAll(/\.prepare\(\s*(["'`])([\s\S]*?)\1/g)) {
    if (/^\s*CREATE (TABLE|INDEX|UNIQUE INDEX)/i.test(match[2])) { try { sqlite.exec(match[2]); } catch { } }
  }
}
...
test("a bot call writes the attempt, the activity and the lead state a human rep's call writes", async () => {
  ...
  assert.equal(attempt[0].channel, "call");
  ...
  assert.ok(lead.first_action_at);
  assert.ok(lead.next_action_at);
...
test("a callback tag schedules a real governed callback at the time the customer asked for", async () => {
  ...
  const result = await disposition.recordBotCallDisposition(db, {idempotencyKey:"call-cb",leadId:"LEAD-BOT",botProvider:BOT,primaryTag:"callback_requested",callbackAt,notes:"Asked to be called after 6pm",actorId:BOT});
  ...
  assert.equal(Number(callback.requested_at), callbackAt);
  ...
  assert.equal(Number(lead.next_action_at), callbackAt);
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM lead_callback_events WHERE lead_id='LEAD-BOT' AND event_type='scheduled'").get().c, 1);
  await assert.rejects(() => disposition.recordBotCallDisposition(db, { idempotencyKey: "call-cb-bad", leadId: "LEAD-BOT", botProvider: BOT, primaryTag: "callback_requested", actorId: BOT }), /real future time/);
  await assert.rejects(() => disposition.recordBotCallDisposition(db, { idempotencyKey: "call-cb-past", leadId: "LEAD-BOT", botProvider: BOT, primaryTag: "callback_requested", callbackAt: Date.now() - 60000, actorId: BOT }), /real future time/);
});

test("needs-a-human and complaint tags open a real case in the queue Ops already works", async () => {
  const { sqlite, db, disposition } = await leadWorld();
  const needsHuman = await disposition.recordBotCallDisposition(db, {idempotencyKey:"call-human",leadId:"LEAD-BOT",botProvider:BOT,primaryTag:"human_intervention_needed",notes:"Customer has a medical question about her senior dog the bot must not answer",actorId:BOT});
  assert.equal(needsHuman.escalated, true);
  assert.ok(needsHuman.caseId);
  ...
  const complaint = await disposition.recordBotCallDisposition(db, {idempotencyKey:"call-complaint",leadId:"LEAD-BOT",botProvider:BOT,primaryTag:"complaint",notes:"Unhappy about the last groomer being late",actorId:BOT});
  ...
  assert.equal(complaintCase.severity, "high");
...
test("a do-not-call outcome opts the lead out and closes it, so no bot or rep dials again", async () => {
  ...
  assert.ok(/l\.opt_out=0/.test(source));
});
...
test("contradictory or unsupported tags are refused instead of producing an incoherent CRM", async () => {
  ...
  for (const [primary, secondary] of [["interested", "not_interested"], ["do_not_call", "callback_requested"], ["wrong_number", "converted"], ["not_interested", "cross_sell_potential"]]) {
    await assert.rejects(() => disposition.recordBotCallDisposition(db, { idempotencyKey: `bad-${primary}-${secondary}`, leadId: "LEAD-BOT", botProvider: BOT, primaryTag: primary, secondaryTags: [secondary], crossSellServices: ["boarding"], callbackAt: Date.now() + 3600_000, actorId: BOT }), /Contradictory bot call tags/);
  }
  ...
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM bot_call_dispositions").get().c, 0);
...
test("three bot RNRs inside the window escalate the lead exactly like three human RNRs", async () => {
  ...
  const now = Date.now();
  for (const email of ["rep.one@pawspace.in", "rep.two@pawspace.in"]) sqlite.prepare("INSERT INTO app_users (id,email,name,role_code,status,created_at,updated_at) VALUES (?,?,?,?,'active',?,?)").run(`USR-${email}`, email, "Rep", "sales_associate", now, now);
  ...
  for (const email of ["rep.one@pawspace.in", "rep.two@pawspace.in"]) await assignment.saveLeadAssignmentMember(db, { employeeEmail: email, teamCode: "sales_blr", serviceCodes: ["grooming"], cityIds: ["Bengaluru"], active: true, actorId: "founder@pawspace.in" });
  ...
  assert.equal(first.autoReassignment?.triggered, false);
  ...
  assert.equal(third.autoReassignment?.triggered, true);
  assert.notEqual(third.autoReassignment.newOwner, firstOwner);
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM lead_attempts WHERE lead_id='LEAD-BOT' AND outcome='RNR'").get().c, 3);
...
test("a bot saying 'paid' records a claim for reconciliation and moves no money", async () => {
  const { sqlite, db, disposition } = await leadWorld();
  const result = await disposition.recordBotCallDisposition(db, {idempotencyKey:"call-paid",leadId:"LEAD-BOT",botProvider:BOT,primaryTag:"paid",secondaryTags:["converted"],notes:"Customer says she paid on the UPI link during the call",actorId:BOT});
  ...
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM canonical_bookings").get().c, 0);
  assert.equal(sqlite.prepare("SELECT converted_booking_id FROM lead_work_items WHERE id='LEAD-BOT'").get().converted_booking_id, null);
...
test("the outcome summary is computed from real dispositions and is honest about money", async () => {
  const { sqlite, db, disposition } = await leadWorld();
  const now = Date.now();
  sqlite.prepare("INSERT INTO crm_contacts (id,name,primary_phone,email,area,stage,owner,source,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").run("CU-BOT2", "Rohit Menon", "9876500102", "rohit@example.test", "Bengaluru East", "New lead", "Unassigned", "Website", now, now);
  sqlite.prepare("INSERT INTO lead_work_items (id,customer_id,source,service,owner,manager,status,stage,work_day,assigned_at,first_action_due_at,manager_alert_at,call_attempts,whatsapp_attempts,recycle_cycle,opt_out,created_at,updated_at) VALUES (?,?,?,?,?,?,'active','day_1',1,?,?,?,0,0,0,0,?,?)").run("LEAD-BOT2", "CU-BOT2", "Website", "grooming", "Unassigned", "Sales Manager", now, now + 600000, now + 1800000, now, now);
  ...
  assert.equal(summary.contacted, 2);
...
test("a bot call is matched to an existing lead by the dialled number, not a bot-specific id", async () => {
  ...
  assert.equal(byPhone.leadId, "LEAD-BOT");
  assert.equal(byPhone.contactId, "CU-BOT");
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM crm_contacts").get().c, 1);
  await assert.rejects(() => disposition.recordBotCallDisposition(db, { idempotencyKey: "phone-unknown", phone: "9000000000", botProvider: BOT, primaryTag: "interested", actorId: BOT }), /capture the lead before recording a call outcome/);
  await assert.rejects(() => disposition.recordBotCallDisposition(db, { idempotencyKey: "no-target", botProvider: BOT, primaryTag: "interested", actorId: BOT }), /lead id or a valid dialled phone number/);
  await assert.rejects(() => disposition.recordBotCallDisposition(db, { idempotencyKey: "bad-lead", leadId: "LEAD-NOPE", botProvider: BOT, primaryTag: "interested", actorId: BOT }), /Lead not found/);
});
...
test("Haptik lead capture attaches to an existing CRM contact instead of forking a duplicate", async () => {
  ...
  assert.equal(captured.contactId, "CU-BOT");
  assert.equal(captured.leadId, "LEAD-BOT");
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM crm_contacts").get().c, 1);
...
test("Haptik callbacks go through the governed callback ledger, with no past-dated placeholders", async () => {
  ...
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM lead_callback_events WHERE lead_id='LEAD-BOT' AND event_type='scheduled'").get().c, 1);
  assert.equal(Number(sqlite.prepare("SELECT next_action_at FROM lead_work_items WHERE id='LEAD-BOT'").get().next_action_at), preferredAt);
  ...
  assert.ok(noTime.requestedAt > Date.now());
  ...
  assert.equal(sqlite.prepare("SELECT COUNT(*) c FROM lead_callbacks WHERE lead_id='LEAD-BOT' AND status='superseded'").get().c, 1);
});
...
test("a Haptik booking request reads the lead's real contact and never moves money", async () => {
  ...
  assert.equal(requested.contactId, "CU-BOT");
  ...
  await assert.rejects(() => haptik.requestHaptikBooking(db, { idempotencyKey: "hbr-bad", phone: "9876500101", leadId: "LEAD-NOPE", serviceCode: "grooming", actorId: BOT }), /Lead not found/);
});
...
test("the bot outcome endpoints are permission-gated and the Haptik webhook stays key-authenticated", () => {
  ...
  assert.ok(/customers\.manage/.test(gateway.slice(gateway.indexOf("/api/bot-call-outcomes"), gateway.indexOf("/api/bot-call-outcomes") + 400)), "recording an outcome needs customer-management authority because it can close or opt out a lead");
...
test("shared CRM tables are written with the column names their owning route defines", () => {
  ...
  for (const table of ["crm_contacts", "crm_activities", "lead_work_items"]) {
    ...
    for (const insert of moduleSource.matchAll(new RegExp(`INSERT (?:OR IGNORE )?INTO ${table} \\(([a-z_,]+)\\)`, "g"))) {
      for (const column of insert[1].split(",")) assert.ok(ownerColumns.includes(column), `${table}.${column} does not exist in the owning route's schema`);
    }
