...
test("birthday sweep: exactly one reward per pet per year, and the reward is single-use", async () => {
  ...
  // Keep redemption assertions independent of wall-clock time: this test validates ownership/scope/single-use, not expiry.
...
test("birthday reward cannot be double-spent by two concurrent redeems", async () => {
  ...
  const issued = await birthday.runPetBirthdaySweep(db, { today: "2026-08-15" });
  const code = issued.rewards[0].code;
  // Keep the concurrency test focused on atomic claiming rather than a calendar-dependent expiry boundary.
  sqlite.prepare("UPDATE pet_birthday_rewards SET expires_at=? WHERE code=?").run(Date.now() + 30 * DAY, code);
