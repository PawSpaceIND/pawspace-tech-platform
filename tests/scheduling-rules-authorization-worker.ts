...
async function run(db:D1Database){
 ...
 const anonymousGet=await getRules(req("GET"));await expectDenied(anonymousGet,"anonymous GET");await deniedMutations(db,{},"anonymous",before,beforeCount);
 ...
 const customerHeaders={cookie:sessions.customerCookie};const customerGet=await getRules(req("GET",undefined,customerHeaders));await expectDenied(customerGet,"customer GET");await deniedMutations(db,customerHeaders,"customer",before,beforeCount);
