...
async function setup(db:D1Database){
 ...
 await db.prepare("INSERT INTO scheduling_rules (id,name,service_code,city_id,zone_id,priority,condition_json,active,created_by,created_at,updated_at) VALUES ('rule_existing','Original rule','pet_sitting','blr',NULL,50,?,1,'seed',?,?)").bind(JSON.stringify([validCondition]),now,now).run();
...
function req(method:string,body?:unknown,headers:Record<string,string>={},suffix=""){const hasBody=body!==undefined;return new Request(`${baseUrl}${suffix}`,{method,headers:{...(hasBody?{"content-type":"application/json"}:{}),...headers},body:hasBody?JSON.stringify(body):undefined});}
async function row(db:D1Database,id:string){return db.prepare("SELECT id,name,priority,active,created_by FROM scheduling_rules WHERE id=?").bind(id).first<Rule>();}
...
async function deniedMutations(db:D1Database,headers:Record<string,string>,label:string,before:Rule|null,beforeCount:number){
 const post=await postRule(req("POST",{name:`${label} insert`,conditions:[validCondition]},headers));await expectDenied(post,`${label} POST`);await assertBaseline(db,before,beforeCount,`${label} POST`);
...
async function deniedRead(headers:Record<string,string>,label:string){await expectDenied(await getRules(req("GET",undefined,headers)),`${label} GET`);}
async function allowedRead(headers:Record<string,string>,label:string){const response=await getRules(req("GET",undefined,headers));if(response.status!==200)throw new Error(`${label} GET expected 200, got ${response.status}`);const body=await response.json() as {data?:Array<{id?:string}>};if(!body.data?.some(rule=>rule.id==="rule_existing"))throw new Error(`${label} GET did not return the existing rule`);}
async function run(db:D1Database){
 ...
 await deniedRead({},"anonymous");await deniedMutations(db,{},"anonymous",before,beforeCount);
 const providerHeaders={cookie:sessions.providerCookie};await allowedRead(providerHeaders,"provider");await deniedMutations(db,providerHeaders,"provider",before,beforeCount);
 const customerHeaders={cookie:sessions.customerCookie};await deniedRead(customerHeaders,"customer");await deniedMutations(db,customerHeaders,"customer",before,beforeCount);
 ...
 await allowedRead(managerHeaders,"manager");
 const invalidPost=await postRule(req("POST",{name:"Malformed",conditions:"not-an-array"},managerHeaders));if(invalidPost.status!==400)throw new Error(`malformed conditions expected 400, got ${invalidPost.status}`);await assertBaseline(db,before,beforeCount,"malformed conditions");
 const invalidPatch=await patchRule(req("PATCH",{id:"rule_existing",active:"false"},managerHeaders));if(invalidPatch.status!==400)throw new Error(`non-boolean active expected 400, got ${invalidPatch.status}`);await assertBaseline(db,before,beforeCount,"non-boolean active");
 const managerPost=await postRule(req("POST",{name:"Manager insert",conditions:[validCondition],reason:"manager adds rule"},managerHeaders));if(managerPost.status!==201)throw new Error(`manager POST failed: ${managerPost.status}`);const created=await managerPost.json() as {data?:{id?:string}};const createdId=String(created.data?.id||"");const createdRow=createdId?await row(db,createdId):null;if(!createdRow)throw new Error("manager POST did not persist");if(createdRow.created_by!=="manager@pawspace.test")throw new Error(`created_by was not the verified actor: ${createdRow.created_by}`);
 const managerPatch=await patchRule(req("PATCH",{id:createdId,name:"Manager patched",priority:7,active:false,reason:"manager updates rule"},managerHeaders));if(managerPatch.status!==200)throw new Error(`manager PATCH failed: ${managerPatch.status}`);const patched=await row(db,createdId);if(patched?.name!=="Manager patched"||Number(patched.priority)!==7||Number(patched.active)!==0)throw new Error("manager PATCH did not persist expected values");
