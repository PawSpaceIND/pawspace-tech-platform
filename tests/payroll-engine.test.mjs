...
test("payroll API separates compensation payroll and approval permissions",()=>{const api=read("app/api/payroll/route.ts"),security=read("lib/platform-security.ts");assert.match(api,/authorize\(request,\s*"payroll\.view"\)/);assert.match(api,/authorize\(request,\s*"payroll\.approve"\)/);assert.match(api,/authorize\(request, "compensation\.manage"\)/);assert.match(api,/authorize\(request, "payroll\.manage"\)/);assert.doesNotMatch(api,/const permission=.*body\.action/);for(const p of ["compensation.view","compensation.manage","payroll.view","payroll.manage","payroll.approve"])assert.ok(security.includes(`\"${p}\"`),p);});


test("Payroll page exposes governed review approval and sandbox preparation controls",()=>{const page=read("app/team/people/payroll/page.tsx");assert.match(page,/Review payroll/);assert.match(page,/Approve payroll/);assert.match(page,/Prepare sandbox payment/);assert.match(page,/action:"review"|"review"/);assert.match(page,/prepare_payment/);});
