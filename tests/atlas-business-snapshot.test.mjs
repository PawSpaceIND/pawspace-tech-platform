...
test("Atlas review events form a tamper-evident hash chain",async()=>{const{sqlite,db}=world(),snapshot=await atlas.buildAtlasBusinessSnapshot(db,{asOf:Date.now()}),p=await atlas.recordAtlasProposal(db,{proposalType:'report.generate',summary:'x',snapshot,basisId:'review-chain',riskClass:'low',createdBy:'atlas'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'proposed',to:'approved',actorId:'founder@pawspace.test',reviewBasisCode:'explicit_founder_approval'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'approved',to:'rejected',actorId:'founder@pawspace.test',reviewBasisCode:'founder_reversal'});const rows=sqlite.prepare("SELECT previous_review_hash,review_hash FROM atlas_proposal_reviews WHERE proposal_id=? ORDER BY reviewed_at ASC,id ASC").all(p.id);assert.equal(rows.length,2);assert.equal(rows[0].previous_review_hash,null);assert.equal(typeof rows[0].review_hash,'string');assert.equal(rows[1].previous_review_hash,rows[0].review_hash);});
