...
test("Team AI exposes precedent readiness as Founder review only",async()=>{
 const fs=await import('node:fs');const route=fs.readFileSync(new URL('../app/api/ai-intelligence/route.ts',import.meta.url),'utf8'),page=fs.readFileSync(new URL('../app/team/ai/page.tsx',import.meta.url),'utf8');assert.match(route,/precedentReadiness=buildAtlasPrecedentReadiness\(proposalFreshness,challengeReview,decisionQuality,recommendationConsistency,policyCompatibility,decisionInputIntegrity,reviewChainIntegrity\)/);assert.match(page,/precedent readiness .* Founder reuse review/i);assert.match(page,/eligible for human review only/i);
});
...
test("Atlas human review preserves immutable reviewed fingerprint and execution does not overwrite reviewer timestamp",async()=>{
 const{sqlite,db}=world(),snapshot=await atlas.buildAtlasBusinessSnapshot(db,{asOf:Date.now()}),p=await atlas.recordAtlasProposal(db,{proposalType:'report.generate',summary:'x',snapshot,basisId:'review-proof',riskClass:'low',createdBy:'atlas'}),a=await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'proposed',to:'approved',actorId:'founder@pawspace.test',reviewBasisCode:'explicit_founder_approval',reviewNote:'Reviewed evidence and approved'}),before=sqlite.prepare("SELECT reviewed_by,reviewed_at FROM atlas_proposals WHERE id=?").get(p.id),review=sqlite.prepare("SELECT decision,reviewer_id,reviewed_decision_fingerprint,basis_code,review_note FROM atlas_proposal_reviews WHERE proposal_id=?").get(p.id);assert.equal(a.reviewRecorded,true);assert.equal(review.reviewed_decision_fingerprint,p.decisionFingerprint);assert.equal(review.basis_code,'explicit_founder_approval');assert.equal(review.review_note,'Reviewed evidence and approved');await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'approved',to:'executed',actorId:'founder@pawspace.test'});const after=sqlite.prepare("SELECT reviewed_by,reviewed_at,executed_at FROM atlas_proposals WHERE id=?").get(p.id);assert.equal(after.reviewed_by,before.reviewed_by);assert.equal(after.reviewed_at,before.reviewed_at);assert.ok(after.executed_at>=before.reviewed_at);
});

test("Atlas lineage exposes review basis and exact reviewed proposal fingerprint",async()=>{
 const{db}=world(),snapshot=await atlas.buildAtlasBusinessSnapshot(db,{asOf:Date.now()}),p=await atlas.recordAtlasProposal(db,{proposalType:'report.generate',summary:'x',snapshot,basisId:'review-lineage',riskClass:'low',createdBy:'atlas'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'proposed',to:'approved',actorId:'founder@pawspace.test',reviewBasisCode:'explicit_founder_approval'});const proposals=await atlas.listAtlasProposals(db,10),lineage=await import('../lib/intelligence/atlas-decision-lineage.ts'),r=lineage.buildAtlasDecisionLineage(proposals,[],[],[],[]).find(x=>x.proposalId===p.id);assert.equal(r.reviewBasisCode,'explicit_founder_approval');assert.equal(r.reviewedDecisionFingerprint,p.decisionFingerprint);assert.equal(r.reviewedFingerprintMatchesCurrent,true);assert.equal(r.missingLineage.includes('review_basis_missing'),false);
});

test("Atlas review events form a tamper-evident hash chain",async()=>{const{sqlite,db}=world(),snapshot=await atlas.buildAtlasBusinessSnapshot(db,{asOf:Date.now()}),p=await atlas.recordAtlasProposal(db,{proposalType:'report.generate',summary:'x',snapshot,basisId:'review-chain',riskClass:'low',createdBy:'atlas'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'proposed',to:'approved',actorId:'founder@pawspace.test',reviewBasisCode:'explicit_founder_approval'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'approved',to:'rejected',actorId:'founder@pawspace.test',reviewBasisCode:'founder_reversal'});const rows=sqlite.prepare("SELECT review_sequence,previous_review_hash,review_hash FROM atlas_proposal_reviews WHERE proposal_id=? ORDER BY review_sequence ASC").all(p.id);assert.equal(rows.length,2);assert.equal(rows[0].review_sequence,1);assert.equal(rows[0].previous_review_hash,null);assert.equal(typeof rows[0].review_hash,'string');assert.equal(rows[1].review_sequence,2);assert.equal(rows[1].previous_review_hash,rows[0].review_hash);});

test("Atlas review-chain verifier detects tampering and legacy unchained history",async()=>{const{sqlite,db}=world(),snapshot=await atlas.buildAtlasBusinessSnapshot(db,{asOf:Date.now()}),p=await atlas.recordAtlasProposal(db,{proposalType:'report.generate',summary:'x',snapshot,basisId:'review-chain-check',riskClass:'low',createdBy:'atlas'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'proposed',to:'approved',actorId:'founder@pawspace.test',reviewBasisCode:'explicit_founder_approval'});const integrity=await import('../lib/intelligence/atlas-review-chain-integrity.ts');let r=(await integrity.buildAtlasReviewChainIntegrity(db,[p.id]))[0];assert.equal(r.chainValid,true);sqlite.prepare("UPDATE atlas_proposal_reviews SET basis_code='tampered' WHERE proposal_id=?").run(p.id);r=(await integrity.buildAtlasReviewChainIntegrity(db,[p.id]))[0];assert.equal(r.chainValid,false);assert.equal(r.reason,'review_chain_broken');sqlite.prepare("INSERT INTO atlas_proposal_reviews (id,proposal_id,decision,reviewer_id,reviewed_at,reviewed_decision_fingerprint,basis_code,review_note) VALUES ('LEGACY-1','LEGACY-P','approved','founder',1,'fp','legacy',NULL)").run();r=(await integrity.buildAtlasReviewChainIntegrity(db,['LEGACY-P']))[0];assert.equal(r.reason,'legacy_unchained_reviews');assert.equal(r.chainVerifiable,false);});

test("Team AI exposes review-chain integrity as read-only audit",async()=>{const fs=await import('node:fs'),page=fs.readFileSync(new URL('../app/team/ai/page.tsx',import.meta.url),'utf8'),route=fs.readFileSync(new URL('../app/api/ai-intelligence/route.ts',import.meta.url),'utf8');assert.match(page,/review-chain integrity .* tamper-evident audit/i);assert.match(route,/reviewChainIntegrity=await buildAtlasReviewChainIntegrity/);});

test("Atlas precedent readiness blocks broken and legacy review chains for reviewed proposals",async()=>{const precedent=await import('../lib/intelligence/atlas-precedent-readiness.ts');const fresh=[{proposalId:'RCG',proposalType:'report.generate',status:'approved',riskClass:'low',recordedSnapshotHash:'h',currentSnapshotHash:'h',fresh:true,terminal:false,requiresReevaluation:false,requiresFounderReview:false,advisoryOnly:true,authorityMutationAllowed:false,reason:'proposal_snapshot_matches_current_canonical_snapshot'}],quality=[{proposalId:'RCG',proposalType:'report.generate',status:'approved',score:100,dimensions:{grounding:100,reviewTimeliness:100,resolution:100,stability:100},sourceCount:4,reviewLatencyMs:0,artifactStatus:'generated',outcome:{status:null,observedNetDelta:null,direction:'unknown',causalAttribution:false},advisoryOnly:true,authorityMutationAllowed:false,note:'x'}],consistency=[{snapshotHash:'h',proposalType:'report.generate',proposalCount:2,distinctActions:1,distinctStatuses:1,consistent:true,basisIds:['b'],proposalIds:['RCG','RCG2'],actions:[{}],statuses:['approved'],requiresFounderReview:false,advisoryOnly:true,authorityMutationAllowed:false,note:'x'}],policy=[{proposalId:'RCG',proposalPolicyVersion:'atlas-v2.2',currentPolicyVersion:'atlas-v2.2',compatible:true,requiresReevaluation:false,reason:'policy_current',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false}],input=[{proposalId:'RCG',recordedFingerprint:'x',recomputedFingerprint:'x',verifiable:true,matches:true,requiresReevaluation:false,reason:'decision_input_fingerprint_matches',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false,note:'x'}],broken=[{proposalId:'RCG',reviewCount:1,chainedReviewCount:1,chainVerifiable:true,chainValid:false,reason:'review_chain_broken',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false}],legacy=[{...broken[0],chainedReviewCount:0,chainVerifiable:false,reason:'legacy_unchained_reviews'}],rb=precedent.buildAtlasPrecedentReadiness(fresh,[],quality,consistency,policy,input,broken)[0],rl=precedent.buildAtlasPrecedentReadiness(fresh,[],quality,consistency,policy,input,legacy)[0];assert.equal(rb.readiness,'not_ready');assert.equal(rb.reviewChainValid,false);assert.ok(rb.blockers.includes('review_chain_broken'));assert.ok(rl.blockers.includes('legacy_unchained_reviews'));});

test("Atlas decision lineage marks invalid review-chain evidence incomplete",async()=>{const lineage=await import('../lib/intelligence/atlas-decision-lineage.ts'),row={id:'LRC',proposal_type:'report.generate',snapshot_hash:'h',basis_id:'b',source_ids_json:'["s"]',risk_class:'low',status:'approved',created_by:'atlas',created_at:1,reviewed_by:'founder',reviewed_at:2,executed_at:null,review_basis_code:'explicit_founder_approval',review_note:null,reviewed_decision_fingerprint:'fp',decision_fingerprint:'fp',policy_version:'atlas-v2.2'},chain=[{proposalId:'LRC',reviewCount:1,chainedReviewCount:1,chainVerifiable:true,chainValid:false,reason:'review_chain_broken',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false}],r=lineage.buildAtlasDecisionLineage([row],[],[],[],[],chain)[0];assert.equal(r.reviewChainValid,false);assert.equal(r.reviewChainReason,'review_chain_broken');assert.equal(r.lineageComplete,false);assert.ok(r.missingLineage.includes('review_chain_broken'));});

test("Team AI wires review-chain integrity into precedent readiness and lineage",async()=>{const fs=await import('node:fs'),route=fs.readFileSync(new URL('../app/api/ai-intelligence/route.ts',import.meta.url),'utf8'),page=fs.readFileSync(new URL('../app/team/ai/page.tsx',import.meta.url),'utf8');assert.match(route,/buildAtlasPrecedentReadiness\([^\n]*reviewChainIntegrity\)/);assert.match(route,/buildAtlasDecisionLineage\([^\n]*reviewChainIntegrity\)/);assert.match(page,/review-chain .*re-evaluate/i);});

test("Atlas v2 review hash preserves verified Founder authority provenance",async()=>{const{sqlite,db}=world(),snapshot=await atlas.buildAtlasBusinessSnapshot(db,{asOf:Date.now()}),p=await atlas.recordAtlasProposal(db,{proposalType:'report.generate',summary:'x',snapshot,basisId:'authority-proof',riskClass:'low',createdBy:'atlas'});await atlas.updateAtlasProposalStatus(db,{id:p.id,from:'proposed',to:'approved',actorId:'founder@pawspace.test',reviewBasisCode:'explicit_founder_approval',reviewerRoleCode:'founder',reviewerAuthoritySource:'authenticated_actor'});const review=sqlite.prepare("SELECT reviewer_role_code,reviewer_authority_source,review_hash_version,review_hash FROM atlas_proposal_reviews WHERE proposal_id=?").get(p.id);assert.equal(review.reviewer_role_code,'founder');assert.equal(review.reviewer_authority_source,'authenticated_actor');assert.equal(review.review_hash_version,2);assert.equal(typeof review.review_hash,'string');const integrity=await import('../lib/intelligence/atlas-review-chain-integrity.ts'),r=(await integrity.buildAtlasReviewChainIntegrity(db,[p.id]))[0];assert.equal(r.chainValid,true);assert.equal(r.reviewerAuthorityVerifiable,true);assert.equal(r.reviewerAuthorityValid,true);assert.equal(r.reviewerAuthorityReason,'reviewer_authority_valid');});

test("Atlas keeps v1 review hashes valid but blocks authority-unverified approvals from precedent",async()=>{const precedent=await import('../lib/intelligence/atlas-precedent-readiness.ts');const fresh=[{proposalId:'AUTH1',proposalType:'report.generate',status:'approved',riskClass:'low',recordedSnapshotHash:'h',currentSnapshotHash:'h',fresh:true,terminal:false,requiresReevaluation:false,requiresFounderReview:false,advisoryOnly:true,authorityMutationAllowed:false,reason:'proposal_snapshot_matches_current_canonical_snapshot'}],quality=[{proposalId:'AUTH1',proposalType:'report.generate',status:'approved',score:100,dimensions:{grounding:100,reviewTimeliness:100,resolution:100,stability:100},sourceCount:4,reviewLatencyMs:0,artifactStatus:'generated',outcome:{status:null,observedNetDelta:null,direction:'unknown',causalAttribution:false},advisoryOnly:true,authorityMutationAllowed:false,note:'x'}],consistency=[{snapshotHash:'h',proposalType:'report.generate',proposalCount:2,distinctActions:1,distinctStatuses:1,consistent:true,basisIds:['b'],proposalIds:['AUTH1','AUTH2'],actions:[{}],statuses:['approved'],requiresFounderReview:false,advisoryOnly:true,authorityMutationAllowed:false,note:'x'}],policy=[{proposalId:'AUTH1',proposalPolicyVersion:'atlas-v2.2',currentPolicyVersion:'atlas-v2.2',compatible:true,requiresReevaluation:false,reason:'policy_current',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false}],input=[{proposalId:'AUTH1',recordedFingerprint:'x',recomputedFingerprint:'x',verifiable:true,matches:true,requiresReevaluation:false,reason:'decision_input_fingerprint_matches',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false,note:'x'}],chain=[{proposalId:'AUTH1',reviewCount:1,chainedReviewCount:1,chainVerifiable:true,chainValid:true,reviewerAuthorityVerifiable:false,reviewerAuthorityValid:false,reviewerAuthorityReason:'reviewer_authority_unrecorded',reason:'review_chain_valid',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false}],r=precedent.buildAtlasPrecedentReadiness(fresh,[],quality,consistency,policy,input,chain)[0];assert.equal(r.readiness,'not_ready');assert.equal(r.reviewerAuthorityValid,false);assert.ok(r.blockers.includes('reviewer_authority_unrecorded'));});

test("Atlas decision lineage exposes reviewer role authority evidence",async()=>{const lineage=await import('../lib/intelligence/atlas-decision-lineage.ts'),row={id:'AUTHL',proposal_type:'report.generate',snapshot_hash:'h',basis_id:'b',source_ids_json:'["s"]',risk_class:'low',status:'approved',created_by:'atlas',created_at:1,reviewed_by:'founder',reviewed_at:2,executed_at:null,review_basis_code:'explicit_founder_approval',review_note:null,reviewed_decision_fingerprint:'fp',decision_fingerprint:'fp',reviewer_role_code:'founder',reviewer_authority_source:'authenticated_actor',policy_version:'atlas-v2.2'},chain=[{proposalId:'AUTHL',reviewCount:1,chainedReviewCount:1,chainVerifiable:true,chainValid:true,reviewerAuthorityVerifiable:true,reviewerAuthorityValid:true,reviewerAuthorityReason:'reviewer_authority_valid',reason:'review_chain_valid',requiresFounderReview:true,advisoryOnly:true,authorityMutationAllowed:false}],r=lineage.buildAtlasDecisionLineage([row],[],[],[],[],chain)[0];assert.equal(r.reviewerRoleCode,'founder');assert.equal(r.reviewerAuthoritySource,'authenticated_actor');assert.equal(r.reviewerAuthorityValid,true);assert.equal(r.lineageComplete,true);});

test("Founder approval entry points persist authenticated role authority",async()=>{const fs=await import('node:fs'),route=fs.readFileSync(new URL('../app/api/admin/atlas-chat/route.ts',import.meta.url),'utf8'),ws=fs.readFileSync(new URL('../lib/intelligence/atlas-websocket.ts',import.meta.url),'utf8'),data=fs.readFileSync(new URL('../lib/intelligence/atlas-data.ts',import.meta.url),'utf8');assert.match(route,/actorRoleCode:actor\.roleCode/);assert.match(ws,/actorRoleCode:actor\.roleCode/);assert.match(data,/reviewerAuthoritySource:input\.actorRoleCode\?"authenticated_actor":undefined/);});
