...
test("Cloudflare worker bypasses PawSpace user-session auth only for Meta's HMAC-authenticated webhook and observes the successful route response", () => {
  const worker = read("worker/index.ts");
  const route = read("app/api/whatsapp/meta-webhook/route.ts");
  assert.match(worker, /url\.pathname==="\/api\/whatsapp\/meta-webhook"/);
...
test("Elite runtime has real call-sites for CRM extraction, deal closer, churn and surge without direct financial execution", () => {
...
test("missing or revoked contact consent fails closed before autonomous deal outreach", () => {
  const runtime = read("lib/services/elite-runtime.ts");
...
test("surge route is preview-only and cannot mutate customer pricing", () => {
