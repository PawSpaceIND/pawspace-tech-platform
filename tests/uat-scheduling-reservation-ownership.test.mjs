...
test('reserve enforces customer ownership before reservation work and preserves auth status', () => {
  assert.match(route, /import \{authError,requireCustomerOwnership,resolveActor,securityAudit,type AuthenticatedActor\} from "\.\.\/\.\.\/\.\.\/lib\/server-auth";/);
  ...
  const resolveIndex = post.indexOf('const actor=await resolveActor(request);', post.indexOf('Missing scheduling fields'));
