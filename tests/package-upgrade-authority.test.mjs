...
test("A: the winner's claim token is what authorises the money, so a loser writes nothing", async () => {
  ...
  const appliedEvent = sqlite.prepare("SELECT detail_json FROM booking_operational_events WHERE booking_id='BK-1' AND event_type='package_upgrade.applied'").get();
  const audit = sqlite.prepare("SELECT detail_json FROM security_audit_events WHERE action='booking_operations.apply_package_upgrade'").get();
  assert.equal(JSON.parse(appliedEvent.detail_json).approvedAmount, winner, "the authoritative lifecycle event records only the winner");
  assert.equal(JSON.parse(audit.detail_json).approvedAmount, winner, "the authoritative audit records only the winner");
  assert.notEqual(money(sqlite).total, loser, "the losing amount never becomes the booking total");
  assert.notEqual(money(sqlite).payment, loser, "the losing amount never becomes the payment amount");
});
