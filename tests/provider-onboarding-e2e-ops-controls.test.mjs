...
test("provider Ops page exposes human-controlled E2E actions",()=>{for(const token of["create_verification","verification_result","clear_manual_verification","schedule_interview","complete_interview","record_human_decision","create_sla","activate_provider_uat"])assert.match(ops,new RegExp(token));assert.match(ops,/AI cannot approve or reject/);});
...
test("provider configuration page can prepare E2E fixtures through governed lifecycle",()=>{for(const token of["save_locale","create_policy_draft","create_content_draft","transition_policy","transition_content","submit_review","approve","activate"])assert.match(control,new RegExp(token));assert.match(control,/defaultQuestionCount:20/);assert.match(control,/durationMinutes:15/);assert.match(control,/legalUseApproved/);});
