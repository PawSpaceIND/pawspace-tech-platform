...
test("Atlas exposes all governed executive domains without autonomous execution",()=>{assert.deepEqual(ATLAS_EXECUTIVE_DOMAINS,["ceo","sales","marketing","operations","customer_success","finance","gst_tax","legal","hr","groomer","trainer","risk"]);assert.ok(atlasExecutiveRegistry().every(item=>item.mayExecuteAutonomously===false));});

test("fresh grounded draft stays advice-only",()=>{const result=evaluateAtlasProposal(proposal(),{now});assert.equal(result.risk,"low");assert.equal(result.disposition,"advice_only");assert.equal(result.autonomousExecution,false);assert.equal(result.evidenceFresh,true);});
