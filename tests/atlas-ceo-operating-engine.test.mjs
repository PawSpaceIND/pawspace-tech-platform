...
test("CEO brief ranks approval work, then confirmations, then advice without executing",()=>{const brief=buildAtlasCeoBrief([candidate("advice","sales","report.generate",100),candidate("confirm","operations","provider.assignment",50),candidate("approve","finance","refund.issue",10,{moneyMovement:true})],{now});assert.deepEqual(brief.items.map(item=>item.proposal.proposalId),["approve","confirm","advice"]);assert.equal(brief.approvalQueue.length,2);assert.equal(brief.autonomousExecution,false);});
...
test("CEO brief reports missing executive coverage instead of pretending completeness",()=>{const brief=buildAtlasCeoBrief([candidate("a","sales","report.generate",1)],{now});assert.deepEqual(brief.coverage.represented,["sales"]);assert.ok(brief.coverage.missing.includes("finance"));assert.ok(brief.coverage.missing.includes("risk"));});
