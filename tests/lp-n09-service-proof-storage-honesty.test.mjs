...
test("LP-N09 source contract: none of the three surfaces claims \"uploaded and verified\" or a ready state when objectStored is false", async () => {
  const [partner, control, trainer] = await Promise.all([
    "app/partner-app/page.tsx", "app/control/service-proof-review.tsx", "app/trainer/page.tsx",
  ...
  assert.match(trainer, /objectStored\?:boolean\|null/);
  assert.match(trainer, /evidence\.some\(item=>item\.objectStored===false\)/);
  assert.match(trainer, /file storage is not connected in this environment/i);
});
