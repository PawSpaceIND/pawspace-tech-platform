...
test("Training payment verification hydrates canonical booking truth before confirmation",()=>{const src=fs.readFileSync(new URL("../app/training/page.tsx",import.meta.url),"utf8");assert.match(src,/hydrateVerifiedBooking/);assert.match(src,/loadVerifiedTrainingConfirmation/);assert.match(src,/setBooking\(verified\.booking\)/);assert.doesNotMatch(src,/latest\.bookings\.find/);assert.match(src,/Refresh confirmation/);assert.doesNotMatch(src,/setBooking\(pendingCheckout\.booking\)/);});
