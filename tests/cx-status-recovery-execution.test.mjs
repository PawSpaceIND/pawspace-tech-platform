...
test('CX detail and queue attach canonical customer, booking and unified-case context without cross-customer links',async()=>{
 ...
 w.sqlite.exec("INSERT INTO unified_cases(id,idempotency_key,case_type,severity,title,description,customer_id,booking_id,source_type,source_id,owner_team,created_by,updated_by,created_at,updated_at,first_response_due_at,manager_escalation_due_at,resolution_due_at) VALUES ('CASE-CX','cx-key','customer_complaint','high','Delayed','Needs follow-up','CUS-CX','BOOK-CX','test','test','cx','qa','qa',1,1,1000,2000,3000)");
