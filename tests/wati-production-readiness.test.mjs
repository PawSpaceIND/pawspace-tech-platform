 ...
 *                   live delivery is Interakt-only while sandbox uses the allowlisted Meta UAT adapter
...
test("scheduled worker drains WhatsApp communication outbox through live Interakt or sandbox Meta UAT",()=>{const worker=read("worker/index.ts"),dispatcher=read("lib/communication-outbox-dispatcher.ts"),providerRuntime=read("lib/whatsapp-production-runtime.ts");assert.match(worker,/runCommunicationOutboxDispatcher/);assert.match(dispatcher,/communication_outbox/);assert.match(providerRuntime,/liveCommunicationEnabled/);assert.match(providerRuntime,/dispatchInteraktWhatsApp/);assert.match(providerRuntime,/dispatchMetaWhatsAppUat/);assert.match(providerRuntime,/metaWhatsAppCredentials/);assert.match(providerRuntime,/PAWSPACE_COMMUNICATION_ENV:"uat"/);assert.doesNotMatch(providerRuntime,/dispatchMetaCloudWhatsApp/);});
...
test("CX refresh source contract keeps stream notification and page polling without remount",()=>{const source=read("app/team/customer-experience/page.tsx"),shell=read("app/team/customer-experience/template.tsx");assert.match(source,/window\.setInterval\(refreshFromEvent, inboxRefreshMs\)/);assert.match(source,/subscribeConversationRefresh/);assert.match(source,/dispatchEvent\(new Event\("pawspace:conversation-refresh"\)\)/);assert.match(source,/addEventListener\("pawspace:conversation-refresh"/);assert.doesNotMatch(shell,/key=\{revision\}/);assert.doesNotMatch(shell,/^\s*(?:"use client"|'use client')/);assert.match(source,/loadThreads/);assert.match(source,/loadConversation/);assert.match(source,/loadControl/);assert.match(source,/human_reply/);});
...
// Handler declarations alone do not make an unconfigured deployment ready.
import { PRODUCTION_SERVICE_REGISTRY, assertProductionReadiness, ProductionConfigurationError } from "../lib/production-readiness-enforcement.mjs";
test("WhatsApp readiness refuses missing integration configuration despite implemented handlers", () => {
  const service = PRODUCTION_SERVICE_REGISTRY.find(service => service.id === "whatsapp_messaging");
  assert.ok(service);
  const env = { NODE_ENV: "production", PAWSPACE_TEST_API_POLICY_OVERRIDE: "false", PAWSPACE_PAYMENT_ENV: "sandbox", PAWSPACE_PAYMENT_LIVE_APPROVED: "false" };
  assert.throws(() => assertProductionReadiness(env, [service]), error => {
    assert.ok(error instanceof ProductionConfigurationError);
    for (const name of [...service.requiredSecrets, ...service.requiredConfig]) {
      assert.ok(error.problems.some(problem => problem.endsWith(` ${name}`)), `must refuse missing ${name}`);
    }
    return true;
  });
  const staging = assertProductionReadiness({ ...env, NODE_ENV: "staging" }, [service]);
  assert.equal(staging.productionReady, false);
  assert.equal(staging.releaseSignOffAllowed, false);
});
