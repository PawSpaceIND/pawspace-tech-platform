...
test("Meta webhook supports secure R2 media ingestion without dropping text or interactive routing",()=>{const webhook=read("lib/meta-whatsapp-webhook.ts"),media=read("lib/meta-whatsapp-media.ts"),inboundRoute=read("app/api/whatsapp/meta-webhook/route.ts"),inboundProcessing=read("lib/meta-whatsapp-inbound-processing.ts");for(const type of["image","audio","video","document"])assert.match(media,new RegExp(`\\"${type}\\"`));assert.match(webhook,/ingestMetaInboundMediaRetrySafe/);assert.match(inboundRoute,/processMetaEnvelope/);assert.match(inboundProcessing,/processMetaWhatsAppEvents\(env\.DB,messages,rawBody,env/);assert.match(webhook,/recordWhatsAppInteractiveSubmission/);assert.match(webhook,/routeInboundAutomation/);assert.match(media,/PAWSPACE_MEDIA_BUCKET/);assert.match(media,/SHA-256/);assert.match(media,/r2:\/\/PAWSPACE_MEDIA_BUCKET/);});
test("scheduled worker drains WhatsApp through the unified communication adapter boundary with no duplicate provider dispatcher",()=>{const worker=read("worker/index.ts"),dispatcher=read("lib/communication-outbox-dispatcher.ts"),providerRuntime=read("lib/whatsapp-production-runtime.ts"),boundary=read("lib/communication-provider-boundary.ts");assert.match(worker,/runCommunicationOutboxDispatcher/);assert.match(dispatcher,/communication_outbox/);assert.match(dispatcher,/"email","sms","whatsapp"/);assert.match(dispatcher,/dispatchExternalCommunication/);assert.match(providerRuntime,/export \{runWhatsAppOutboxDispatcher\} from "\.\/communication-outbox-dispatcher"/);assert.doesNotMatch(providerRuntime,/dispatchInteraktWhatsApp/);assert.doesNotMatch(providerRuntime,/dispatchMetaWhatsAppUat/);assert.match(boundary,/live_ready/);assert.match(boundary,/sandbox_ready/);});
