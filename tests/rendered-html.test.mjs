...
test("keeps payment timing, confidence meetings and delay recovery explicit", async () => {
  ...
  assert.match(grooming, /payment:\{method:pay==="online"\?"upi":"cash",mode:pay==="online"\?"prepaid":"pay_after_service",status:"created"/);
...
test("uses 60 minutes per training pet and one GPS policy for doorstep providers", async () => {
  ...
  assert.match(groomingLive, /map refreshes automatically/);
