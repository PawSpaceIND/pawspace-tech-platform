...
import { decideCustomerDataAccess, APPROVED_DATA_ACCESS } from "../lib/purpose-based-access.ts";
...
test("service_delivery purpose grants full address while assignment is active", () => {
  const decision = decideCustomerDataAccess(APPROVED_DATA_ACCESS, {
    actor: { email: "provider@test", roleCode: "service_provider", permissions: ["bookings.view"] },
    subject: {
      customerId: "c1",
      name: "Customer",
      address: { line1: "12 MG Road", area: "Indiranagar", city: "Bengaluru", pincode: "560038" },
    },
    purpose: "service_delivery",
    assignment: {
      type: "booking",
      id: "b1",
      assignedTo: "provider@test",
      status: "on_the_way",
  ...
  assert.equal(decision.address.line1, "12 MG Road");
});
